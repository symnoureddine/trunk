...
import OxParametersProvider from "@system/OxParametersProvider"
...
export default class OxParametersApi {
    ...
    public static tr (key: string): string {
        const trad = OxStoreCore.getters.tr(key)
        // if (!trad) {
        //     console.warn("Missing traduction for '" + key + "'")
        // }
        return trad
    }
     ...
     *
     * @param {string} key - Clef de traduction
     * @param {string} traduction - Valeur de traduction
     */
    public static setTr (key: string, traduction: string): void {
        OxStoreCore.commit("setLocale", { key: key, traduction: traduction })
    }

    /**
     * Renseigne un ensemble de traductions
     *
     * @param {Array<{key: string; traduction: string}>} traduction - Ensemble de traductions
     */
    public static setTrs (traductions: {key: string; traduction: string}[]): void {
        OxStoreCore.commit("setLocales", traductions)
    }
}
