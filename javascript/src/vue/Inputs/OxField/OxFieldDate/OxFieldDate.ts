...
import OxDatePicker from "@system/OxDatePicker"
 ...
 * OxFieldDate
 *
 * Composant de champ de date
 */
@Component({ components: { OxDatePicker, OxFieldString, OxButton } })
export default class OxFieldDate extends OxFieldString {
    @Prop({ default: "date" })
    ...
    protected updateMutatedValue (): void {
        this.setMutatedValue(this.value.toString())
        this.resetModalMode()
    }
    ...
    private setMutatedValue (value: string): void {
        if (this.format === "date" || this.format === "month") {
            ...
            this.mutatedValue = value
            if (!this.mutatedValue && this.defaultDate && this.defaultTime) {
