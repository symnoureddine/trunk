...
foreach ($long_request_whitelist as $module_action) {
  if (preg_match("/^(?P<module>[a-zA-Z0-9]+)\s(?P<action>[a-zA-Z0-9_]+)\s(?P<probability>\d+)/", trim($module_action), $matches)) {
...
if ($duration > $long_request_log_level || ($isWhitelisted && (mt_rand(1, $probability) === 1))) {
  ...
  $long_request_log->session_id       = CSessionHandler::getSessionId();
  ...
  $long_request_log->_query_report = array(
