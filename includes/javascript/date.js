...
var Calendar = {
  ...
  setNow: function(element) {
    ...
    var da = form.elements[element.name+"_da"];
    var now = new Date();
  ...
  getDateHolidaysCP: function(date, ref_pays, ref_cp) {
    ...
    switch(ref_pays) {
      ...
      case 2:
        ...
        var firstSundaySeptember  = firstDaySeptember.cloneDate();
        var fridayBeforeSeptember = firstDaySeptember.cloneDate();

        // Get first sunday of september
        for (var i = 0; i <= 6; i++) {
          var _next_day = firstDaySeptember.cloneDate().addDays(i);

          if (_next_day.getDay() == 0) {
            firstSundaySeptember = _next_day.cloneDate();
            break;
          }
        }

        // Get friday before september
        var temp_day = easter.cloneDate();
        ...
        var thirdSundaySeptember = firstSundaySeptember.cloneDate().addDays(14);
        ...
        switch (canton) {
          ...
          case '10':
            ...
            // Dynamic
            datesH.push(easter.cloneDate().addDays(-2).toDATE());              // Vendredi saint
            datesH.push(easter.cloneDate().addDays(1).toDATE());               // Lundi de Pâques
            datesH.push(easter.cloneDate().addDays(39).toDATE());              // Jeudi de l'Ascension
            datesH.push(easter.cloneDate().addDays(50).toDATE());              // Lundi de Pentecôte
            datesH.push(thirdSundaySeptember.cloneDate().addDays(1).toDATE()); // Lundi du jeûne fédéral
            break;
          ...
          case '12':
            ...
            datesH.push(easter.cloneDate().addDays(39).toDATE());              // Jeudi de l'Ascension
            datesH.push(easter.cloneDate().addDays(50).toDATE());              // Lundi de Pentecôte
            datesH.push(firstSundaySeptember.cloneDate().addDays(4).toDATE()); // Jeûne genevois

            // Static
            datesH.push(year + "-12-31");                                      // Restauration de la République
            break;
        }
        break;
    }
