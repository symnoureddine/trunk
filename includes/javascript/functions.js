...
document.observe("keydown", function (e) {
  ...
  if (e.altKey) {
    switch (key) {
      ...
      case 65: // a
        Event.stop(e);
        MediboardExt.toggleModuleSelecter();
        break;
    }
  }
});
...
function uploadFile(object_guid, file_category_id, _rename, named, onClose, ext_cabinet_id) {
  ...
  url.requestModal(760, 500, {onClose: onClose});
}
...
App.canMessageBackgroundTask = true;
App.initMessageBackgroundTask = function (data) {
  function canBackground() {
    return App.canMessageBackgroundTask;
  }
  ...
  function setTimer() {
    ...
    window.setInterval(function () {
      ...
      new Url('messagerie', 'ajax_get_messagerie_info').addParam('session_no_revive', '1').requestJSON(function (data) {
        App.processMessagerieData(data);
      });
    }, App.config.internal_mail_refresh_frequency);
  }
...
App.processMessagerieData = function (data) {
  if (data === null || Object.isUndefined(data)) {
    return App.canMessageBackgroundTask = false;
  }

  Object.keys(data).each(function(category) {
    if (category == 'external') {
      Object.keys(data[category]).each(function(key) {
        App.updateMessagerieCounter($('messagerie-' + category + '-' + key  + '-total-counter'), key, parseInt(data[category][key]));
      });
    }
    else {
      App.updateMessagerieCounter($('messagerie-' + category + '-total-counter'), 'total', parseInt(data[category]['total']));
    }
  });
};

App.updateMessagerieCounter = function(counter, index, value) {
  if (counter) {
...
//Porte documents
App.canPorteDocBackgroundTask = true;
App.initPorteDocBackgroundTask = function (data) {
  function canBackground() {
    return App.canPorteDocBackgroundTask;
  }
  ...
  function setTimer() {
    window.setInterval(function () {
      ...
      new Url('porteDocuments', 'ajax_get_folder_info').addParam('session_no_revive', '1').requestJSON(function (data) {
        App.processPorteDocData(data);
      });
    }, 300000);
  }
...
App.processPorteDocData = function (data) {
  if (data === null || Object.isUndefined(data)) {
    return App.canPorteDocBackgroundTask = false;
  }

  Object.keys(data).each(function (key) {
    var counter = $('porteDocument-total-counter');
    if (counter) {
      var value = parseInt(data['total']);
      counter.update(value);
      if (value) {
        counter.show();
        if (key == 'total') {
