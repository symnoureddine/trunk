...
var Reglette = Class.create({
   ...
   */
  initialize: function (reglette, format, default_time, before_range, after_range, width) {
    ...
    this.reglette_elt.down('.reglette_range-min').update(this.beautifyDate(this.min_time));
    this.reglette_elt.down('.reglette_range-max').update(this.beautifyDate(this.max_time));
    ...
    if(current_time) {
      this.updateRegletteByDatetime(this.strToDateTime(current_time), false)
          .displayTime(this.strToDateTime(current_time), false);
    } else {
