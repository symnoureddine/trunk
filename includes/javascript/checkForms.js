  ...
  check: {
    ...
    birthDate: function() {
      ...
      if (values) {
        ...
        if (values[2] === "00" || values[3] === "00") {
          this.addError("birthDate", printf("N'a pas un format de date correct. Format attendu : %s", 'YYYY-MM-dd'));
        }
    ...
    date: function() {
      ...
      if (!/^\d{4}-\d{1,2}-\d{1,2}$/.test(this.sValue))
        this.addError("date", printf("N'a pas un format de date correct. Format attendu : %s", 'YYYY-MM-dd'));
    },
