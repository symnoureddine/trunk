...
class CExternalDBImport {
  ...
  function getStats() {
    ...
    if (!$ds->hasTable($this->_table)) {
      ...
      mbTrace("Table missing : {$this->_table}");
      return false;
    }
    ...
    if ($this->_stats_total == '0') {
      mbTrace("Table empty : {$this->_table}");
      return false;
    }
  ...
  function storeMbObject($hash, $force = false) {
    ...
    if (self::$_debug) {
      mbTrace($hash, 'HASH');
      mbTrace(($this->_mb_object instanceof CMbObject) ? $this->_mb_object->getPlainFields() : $this->_mb_object, 'MB OBJECT');
    }
  ...
  static function isBadFileDate($file_date, $key = null, $correct_file = false, $module = null) {
    ...
    if ($file_date > self::getMinMaxDate('max', $mod) || $file_date < self::getMinMaxDate('min', $mod)) {
      CMbDebug::log($key . ' : Conflit de date : ' . $file_date);
      return true;
    }
