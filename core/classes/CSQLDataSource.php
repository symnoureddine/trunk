...
abstract class CSQLDataSource {
  ...
  static function get($dsn, $quiet = false, $connection_options = []) {
    ...
    $dataSource->init($dsn, $connection_options);
   ...
   * @return void
   */
  function init($dsn, $connection_options = []) {
    $this->dsn    = $dsn;
    ...
    $key = "ds_metadata-$dsn";
    if (null === $this->metadata = SHM::get($key)) {
      $links = array();
      $meta  = array();
      ...
      SHM::put($key, $datasource_meta, false, 300);
    }
  ...
  function exec($query) {
    ...
    if (self::$trace) {
      ...
      //$type    = floor(($pace + 3) / 2);
      CApp::queryTrace($message, $this->dsn, $step, $total, $query);
    }

    // Query trace in mbLog for request without dev toolbar (json)
    if (self::$trace_in_log) {
      CApp::log($query);
    }

    // Query log, durations in micro-seconds
    if (self::$log) {
      self::$log_entries[] = array($query, round($this->chrono->latestStep * 1000000));
    }
  ...
  static function displayReport($report_data = null, $inline = true) {
    ...
    foreach ($queries as $_index => $_query) {
      ...
      if ($inline) {
        ...
        $info = CAppUI::tr(
          "SQL Query %d: was called %d times [%d%%] for %01.3fms [%d%%]",
          $_index + 1,
          $_query["count"],
          $_query["count"] * 100 / $totals["count"],
          $_query["duration"] / 1000,
  ...
  function hasTable($table, $use_cache = true) {
    if ($use_cache) {
      $cache = new Cache(__METHOD__, array($this->dsn, $table), Cache::INNER_OUTER);
      if ($cache->exists()) {
        return $cache->get();
      }
  ...
  function hasField($table, $field, $use_cache = true) {
    $cache = new Cache(__METHOD__, array($this->dsn, $table, $field), Cache::INNER_OUTER);
   ...
   *
   */
  function fetchCSVFile($query, $csv = null, $write_head = true) {
