...
class CSmartyDP extends CSmartyMB {
  ...
  function __construct($dir = null) {
    ...
    $this->register_block("th_vertical"         , array($this, "th_vertical"));
    $this->register_block("th_rotate"           , array($this, "th_rotate"));
    $this->register_block("me_form_field"       , array($this, "me_form_field"));
    $this->register_block("me_form_bool"        , array($this, "me_form_bool"));
    $this->register_block("me_img_title"        , array($this, "me_img_content"));
    $this->register_block("me_scroller"         , array($this, "me_scroller"));
    ...
    $this->register_function("me_button"           , array($this,"me_button"));
    $this->register_function("me_dropdown_button"  , array($this,"me_dropdown_button"));
    $this->register_function("me_img"              , array($this,"me_img"));
  }
   ...
   *                      - cycle           : {optionnel} Cycle de répétition du séparateur (pour les enums en type radio) [default:
   *                      "1"]
   ...
   *                      - typeEnum    : {optionnel} Type d'affichage des enums à cibler (values : "select", "radio") [default:
   *                      "select"]
   * @param self  $smarty The Smarty object
   *
   * @return string
  ...
  function me_dropdown_button($params, &$smarty) {
    ...
    foreach ($buttons_list as $_button) {
      ...
      $_label     = $this->extractVar("button_" . $_button . "_" . $button_group . "_label",
        "", $params, $smarty);
      $_label_suf = $this->extractVar("button_" . $_button . "_" . $button_group . "_label_suf",
        "", $params, $smarty);
      $_callback  = $this->extractVar("button_" . $_button . "_" . $button_group . "_callback",
        "", $params, $smarty);
      $_link      = $this->extractVar("button_" . $_button . "_" . $button_group . "_link",
        "", $params, $smarty);
      $_attr      = $this->extractVar("button_" . $_button . "_" . $button_group . "_attr",
        "", $params, $smarty);
      $_icon      = $this->extractVar("button_" . $_button . "_" . $button_group . "_icon",
        "", $params, $smarty);
      $_old_class = $this->extractVar("button_" . $_button . "_" . $button_group . "_old_class",
        "", $params, $smarty);
      $_id        = $this->extractVar("button_" . $_button . "_" . $button_group . "_id",
        "", $params, $smarty);
      $_title     = $this->extractVar("button_" . $_button . "_" . $button_group . "_title",
        "", $params, $smarty);
      $_icon = str_replace("notext", "", $_icon);

      $html .= $this->me_button(array(
        "label"             => $_label,
  ...
  function me_img_content($params, $content, &$smarty) {
    ...
    if ($icon_name) {
      ...
      $icon .= $title ? " title='" . htmlspecialchars(CAppUI::tr($title), ENT_QUOTES) . "'" : ($content ? " title='"
        . ($auto_tr ? htmlspecialchars(CAppUI::tr($content), ENT_QUOTES) : htmlspecialchars($content, ENT_QUOTES)) . "'" : "");
    ...
    $img .= $title ? " title='" . htmlspecialchars(CAppUI::tr($title), ENT_QUOTES) . "'" : ($content ? " title='"
      . ($auto_tr ? htmlspecialchars(CAppUI::tr($content), ENT_QUOTES) : htmlspecialchars($content, ENT_QUOTES)) . "'" : "");
   ...
   *                        - {int}    nb_cells      : Taille de l'input dans le tableau (0 si hors d'un tableau)
   ...
   *                        - {bool}   animated      : Le label de l'input doit il être animé ?
   *                        - {string} null_chars    : Caractères considérés comme null (séparés par un | )
   *                        - {string} title_label   : Attribut title du label
   *                        - {string} field_class   : Classe(s) appliquées au champ
   *                        - {bool}   layout        : Génère seulement un template de me_form_field
   *                                                   sans modifier le style des inputs
   ...
   *                        - {string} label_prefix  : Prefix du label du champ
   *                        - {string} label_suffix  : Suffix du label du champ
   *                        - {int}    nb_cells      : Taille de l'input dans le tableau (0 si il n'est pas dans un tableau)
   *                        - {int}    rowspan       : Attribut rowspan du td contenant le champ
   *                        - {string} class         : Class du groupe
   *                        - {string} style_css     : Style initial du groupe
   *                        - {string} mb_class      : Classe PHP
  ...
  protected function getFormFieldLabel(
    ...
    elseif ($mb_class || $mb_object) {
      $params_mb_label = array("field"=>$mb_field);
      if ($mb_class) {
