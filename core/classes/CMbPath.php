...
abstract class CMbPath {
   ...
   */
  static function guessMimeType($file, $ext = null) {
    $ext = $ext ? $ext : strtolower(self::getExtension($file));
  ...
  static function remove($path, $log = true) {
    ...
    if (is_dir($path)) {
      if (self::emptyDir($path, $log)) {
        if ($log) {
          CApp::log(sprintf('CMbPath::remove - rmdir: %s', $path), null, CLogger::LEVEL_DEBUG);
        }
    ...
    if ($log) {
      CApp::log(sprintf('CMbPath::remove - unlink: %s', $path), null, CLogger::LEVEL_DEBUG);
    }
  ...
  static function getPathTreeUnder($dir, $ignored = array(), $extensions = null) {
    ...
    $tree  = array();
    $files = glob("$dir/*");
    usort($files, array(CMbPath::class, "cmpFiles"));
