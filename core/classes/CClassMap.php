...
class CClassMap
{
    ...
    public function buildClassRef(): string
    {
        ...
        $this->classref = [];
        $count_ref      = 0;
        $resource_names = []; // Check resource name unicity (api)
        ...
        foreach ($models as $full_name) {
            ...
            ) ? new $full_name : $reflection_class->newInstanceWithoutConstructor();
            ...
            foreach ($specs as $spec_name => $ref_spec) {
                ...
                if (!$ref_spec->back) {
                    // Allow undefined
                    if (($full_name === CExObject::class) || ($ref_spec->class === 'CExObject') || $instance->isModelObjectAbstract(
                        )) {
            ...
            if ($resource_name !== CModelObject::RESOURCE_NAME) {
                if (in_array($resource_name, $resource_names, true)) {
                    throw new CClassMapException(
                        "Duplicate resource name '{$resource_name}' in class '{$instance->_class}'"
                    );
                }
