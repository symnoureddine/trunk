...
class CSmartyMB extends Smarty {
  ...
  function __construct($dir = null) {
    global $version, $can, $m, $a, $tab, $g, $f, $action, $actionType, $dialog, $ajax, $suppressHeaders, $uistyle;
  ...
  function mb_ditto($params, &$smarty) {
    ...
    $replacement = CMbArray::extract($params, "replacement", "|", false);
    $old         = '';
    if (!$reset) {
   ...
   * @return string
   */
  function mb_path($params, &$smarty) {
    ...
    if ($this->_parse_resource_name($_params)) {
      ...
      switch ($_resource_type) {
        case 'file':
          ...
          $_return                      = isset($_is_cache_cache[$_resource_name]) || ($_is_cache_cache[$_resource_name] = is_file($_resource_name));
          //// ---- End cache
          break;
   ...
   * @return string|void
   */
  function tr($params, $content) {
    if (!isset($content)) {
      return;
    }

    $markdown = CMbArray::extract($params, 'markdown', null);
  ...
  function progressive_date($date) {
    ...
    if (intval($parts[1])) {
      return strftime("%m/%Y", strtotime($date));
    }
  ...
  function trace($value) {
    mbTrace($value instanceof CModelObject ? $value->getProperties() : $value);
  }
  ...
  function mb_script($params, &$smarty) {
    ...
    elseif ($register) {
      ...
      $version_build = CApp::getVersionKey();
  ...
  function display($resource_name, $cache_id = null, $compile_id = null) {
    ...
    $base_tpl = ["login.tpl", "common.tpl", "header.tpl", "footer.tpl", "tabbox.tpl", "ajax_errors.tpl", "inc_unlocalized_strings.tpl"];
   ...
   *
   * @param array $params Params to pass to the function (one of (document_id + document_class) or file_id or document is required):
   *                      - profile(required)     : Profile to use to get the size of the thumb (enum small|medium|large
   *                      default|medium)
   *                      - document_id(required) : file_id or compte_rendu_id to get thumb from
   *                      - document_class        : Class of the CDocumentItem (enum CFile|CCompteRendu default|CFile)
   ...
   *                      - extra                 : Any xml attributes to add to <img>. Fields like data-* must be data_*
   *
   * @return void
   */
  function thumbnail($params) {
    ...
    $profile_data = CMbArray::get(CThumbnail::$profiles, $profile, 'medium');
    // Get the width and height to display
    $width  = $profile_data['display_w'];
    $height = $profile_data['display_h'];
