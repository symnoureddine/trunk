...
class CMbObject extends CStoredObject {
  ...
  public $_ref_keychain_entries = [];

  /** @var CKeychainEntry[] Available entries */
  public $_ref_available_keychain_entries;
   ...
   */
  function loadRefsForms($where = array()) {
    if (!$this->_id) {
      return $this->_ref_forms = array();
    }
   ...
   * @return array()
   */
  function loadRefsDocItems($with_cancelled = true, $where = array()) {
    $this->_nb_files      = $this->loadRefsFiles($where);
    $this->_nb_docs       = $this->loadRefsDocs($where);
    ...
    foreach ($this->_ref_documents as $_key => $_document) {
      if (!$with_cancelled && $_document->annule) {
        unset($this->_ref_documents[$_key]);
        continue;
      }
    ...
    foreach ($this->_ref_files as $_key => $_file) {
      ...
      if (!isset($this->_ref_files_by_cat[$cat_name])) {
        $this->_ref_files_by_cat[$cat_name] = array();
      }
   ...
   */
  function countDocs() {
    $this->_nb_cancelled_docs = $this->countBackRefs("documents", array("annule" => "= '1'"), null, true, "docs_cancelled");
  ...
  function countFiles($where = array()) {
    $this->_nb_cancelled_files = $this->countBackRefs(
      "files", array_merge($where, array("annule" => "= '1'")), null, true, "files_cancelled"
    );
  ...
  static function massCountDocItems($objects) {
    self::massCountBackRefs($objects, "documents", array("annule" => "= '1'"), null, "docs_cancelled");
    self::massCountBackRefs($objects, "files", array("annule" => "= '1'"), null, "files_cancelled");
  ...
  static function countAlertDocs($objects) {
    ...
    self::massCountBackRefs($objects, "documents", array("alert_creation" => "= '1'"), null, "alert_docs");
   ...
   * @throws Exception
   */
  function loadAffectationsPersonnel($where = array()) {
    // Initialisation
   ...
   * @return void
   */
  function mapDocs($object, $params = array()) {
    ...
    $with_cancelled = CMbArray::extract($params, "with_cancelled");
    $tri            = CMbArray::extract($params, "tri", "date");
    $cat_ids        = CMbArray::extract($params, "cat_ids", array());
    ...
    if ($importance) {
      ...
      foreach ($cat_ids as $key => $_cat_id) {
        if (!isset($cats[$_cat_id])) {
          unset($cat_ids[$key]);
        }
      }
    ...
    $object->loadRefsDocItems($with_cancelled, $where);
    ...
    foreach ($object->_ref_documents as $_doc) {
      ...
      $this->_all_docs["docitems"][$prefixe][] = $_doc;
    ...
    foreach ($object->_ref_files as $_file) {
      ...
      $this->_all_docs["docitems"][$prefixe][] = $_file;
      $this->_all_docs["contexts"][$prefixe]   = $this->makeDateTime($object);
    ...
    if (!count($cat_ids)) {
      ...
      foreach ($ex_links as $_link) {
        ...
        $this->_all_docs["docitems"][$prefixe][] = $_link;
        $this->_all_docs["contexts"][$prefixe]   = $this->makeDateTime($object);
      }
    }
   ...
   */
  function loadKeychainEntries() {
    return $this->_ref_keychain_entries = $this->loadBackRefs('keychain_entries');
  }
   ...
   * @return CKeychainEntry[]
   */
  function loadAvailableKeychainEntries() {
    $this->loadKeychainEntries();

    $entries = array();
    CStoredObject::massLoadFwdRef($this->_ref_keychain_entries, 'keychain_id');
    foreach ($this->_ref_keychain_entries as $_entry) {
      $_keychain = $_entry->loadKeychain();

      if ($_keychain->isMyKeychain() || $_entry->public) {
  ...
  function storeDocItems() {
     ...
     * @throws Api\Exceptions\CApiException
     */
    public function getResourceFiles(): ?CCollection
    {
        if (!$this->loadRefsFiles()) {
            return null;
        }

        $items = new CCollection($this->_ref_files);
        $items->setName(CFile::RESOURCE_NAME);
