...
class CAppUI {
  ...
  const MESSAGERIE_ACCOUNTS_CACHE = __CLASS__ . '::getMessagerieAccounts';
  ...
  public $_authentication_factor_success = false;

  /** @var boolean Is the connected user LDAP related? */
  ...
  static function turnOffEchoStep(){
  ...
  static function login($force_login = false) {
    ...
    if ($loginas) {
      static::$instance->auth_method = 'substitution';

      if (static::$instance->user_type != 1 && !$force_login) {
        return static::authFailed("Auth-failed-loginas-admin", $loginas, static::$instance->user_id, static::$instance->auth_method);
      }
    ...
    elseif ($ldap_connection && $ldap_guid) {
      static::$instance->auth_method = 'ldap_guid';

      try {
        $user = CLDAP::getFromLDAPGuid($ldap_guid);
      }
      catch (Exception $e) {
        return static::authFailed(/*$e->getMessage()*/"Auth-failed-combination", $ldap_guid, null, static::$instance->auth_method);
      }
    }

    // -------------- Token sign-in
    elseif ($token_hash) {
      static::$instance->auth_method = 'token';
    ...
    elseif ($signature && $certificat_signature) {
      static::$instance->auth_method = 'card';
      ...
      if (CMbSecurity::verify($cipher, 'sha1', 2, $certificat_signature, 'mediboard', base64_decode($signature))) {
        $user->load($user_id);
      }
    } elseif (
        ...
        static::$instance->auth_method = 'sso';
    ...
    // -------------- Standard sign-in
      ...
      static::$instance->auth_method = 'standard';
    ...
    // User template case
    if ($user->template) {
      return static::authFailed("Auth-failed-combination", $user->user_username, $user->_id, static::$instance->auth_method);
    }
    ...
    if ($ldap_connection && $username) {
      ...
      if ($idex->_id) {
        ...
        try {
          ...
          if (!$user->_bound) {
            return static::authFailed("Auth-failed-combination", $username, $user_ldap->_id, static::$instance->auth_method);
          }
        }
    ...
    if (!$is_intranet && static::$instance->user_remote == 1 && $user->user_type != 1) {
      return static::authFailed("Auth-failed-user-noremoteaccess", $user->user_username, $user->_id, CAppUI::$instance->auth_method);
    }

    if ($user->mustHaveStrongAuthentication() && !$user->hasEnabledStrongAuthentication()) {
      return static::authFailed('Auth-failed-User must enable strong authentication', $user->user_username, $user->_id, CAppUI::$instance->auth_method);
    }
    ...
    if ($auth) {
      ...
      if ($disconnect_pref && $disconnect_pref['used']) {
        CUserAuthentication::disconnectSimilarOnes($auth);
      }
    }
  ...
  /**
   * Todo: Check if still necessay after v2 auth merging and/or move to CUser
   *
  ...
  static function checkPasswordAttempt(CUser $user) {
    ...
    if ($mediuser && $mediuser->_id) {
      if (!$mediuser->actif) {
        return static::authFailed("Auth-failed-combination", $sibling->user_username, $sibling->_id, static::$instance->auth_method);
      }
    ...
    if ($sibling->isLocked()) {
        ...
        if ($user->_id) {
            return static::authFailed("Auth-failed-user-locked", $sibling->user_username, $sibling->_id, static::$instance->auth_method);
        }
  ...
  {
      return self::$instance->auth_method === BasicAuthentication::METHOD_NAME;
  }
  ...
  static function updateUserAuthExpirationDatetime($session_id = null) {
    ...
    if ($user_auth
      ...
      self::setUserAuthExpirationDatetime(CMbDT::dateTime("+ $user_auth->session_lifetime second", $dtnow), $session_id);
    }
  }
  ...
  /**
   * Return the messagerie accounts (internal, external, apicrypt, mssante, medimail, mondialSante) of the connected users
   *
   * @return array
   */
  public static function getMessagerieAccounts() {
    ...
    $cache = new Cache(self::MESSAGERIE_ACCOUNTS_CACHE, "accounts-{$user->_guid}", Cache::OUTER, CUserMessageDest::getCacheLifetime());
  ...
  /**
   * Return the number of unread messages for the connected user
   *
   * @return array
   */
  public static function getPorteDocumentsInfo() {
    if (!CModule::getActive('porteDocuments') || !CModule::getCanDo('porteDocuments')->read) {
      return null;
    }

    return ['counters' => CFolder::getDocumentsUnread()];
  }

    /**
     * Return the number of order not treated for the connected user
     *
     * @return array|null
     */
    public static function getAppFineInfo(): ?array
    {
        if (
            !CModule::getActive('appFineClient') || !CModule::getActive('oxCabinet')
            || !CAppUI::gconf("appFineClient Sync allow_appfine_sync")
        ) {
            return null;
        }

        return ['counters' => CAppFineClient::countNotTreated()];
    }

    public static function getOxChatClientInfo(): ?array
    {
        if (!CModule::getActive('oxChatClient')) {
            return null;
        }
     ...
     * @return array
     */
    public static function getAssitance(): ?array
    {
        if (!CModule::getActive('supportClient') || !CModule::getCanDo('supportClient')->read) {
            return null;
        }
  ...
  static function reviveSession() {
     ...
     * @throws Exception
     */
  static public function forgeSessionName($root_dir = null) {
