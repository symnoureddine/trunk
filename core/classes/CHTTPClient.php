...
class CHTTPClient {
  ...
  function postFile($file_path, $mimetype = 'application/json', $postname = 'file', $close = true) {
    ...
    return $this->post(array('file' => new CURLFile(realpath($file_path), $mimetype, $postname)), $close);
  }
   ...
   * @return bool|int
   */
  static function checkUrl($url, $option = null, $return_body = false, $headers = array()) {
    try {
  ...
  function parseHeaders($headers) {
    ...
    foreach ($headers as $_header) {
      ...
      list($_key, $_value) = explode(":", $_header, 2);
