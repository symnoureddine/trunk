...
class CRequestBulk
{
     ...
     */
    public function __construct(CRequestApi $request_api)
    {
    ...
    public function createRequests(): array
    {
        ...
        foreach ($content_parsed['data'] as $operation) {
            ...
            // request API
            if (str_starts_with($path, '/' . CRequestFactory::PATH_API)) {
    ...
    private function parseContent($content_json): array
    {
        // Check content
        if ($content_json === '') {
            throw new CApiRequestException('Missing json content');
        }

        // Check json
        $content_decoded = json_decode($content_json, true);
        if ($content_decoded === null && json_last_error() !== JSON_ERROR_NONE) {
            throw new CApiRequestException('Invalid json content');
        }

        // Check json decoded
        if (!is_array($content_decoded)) {
            throw new CApiRequestException('Invalid json content (must be an array)');
        }

        if (!array_key_exists('data', $content_decoded)) {
            throw new CApiRequestException('Invalid json content (missing data key)');
        }

        if (!is_array($content_decoded['data'])) {
            throw new CApiRequestException('Invalid json content (data must be an array)');
        }

        // Check limit operations
        if (count($content_decoded['data']) > self::MAX_OPERATIONS) {
            throw new CApiRequestException('Max bulk operations limit exceeded');
        }
        ...
        foreach ($content_decoded['data'] as $request) {
            foreach (self::REQUIRED_KEYS as $key_required) {
                if (!array_key_exists($key_required, $request)) {
                    throw new CApiRequestException('Invalid request (missing required keys)');
                }
            }

            foreach ($request as $key_request => $value_request) {
                if (!array_key_exists($key_request, self::ALLOWED_KEYS)) {
                    throw new CApiRequestException('Invalid request (unknown schema key ' . $key_request . ')');
                }

                if (gettype($value_request) !== self::ALLOWED_KEYS[$key_request]) {
                    throw new CApiRequestException('Invalid request (wrong type key ' . $key_request . ')');
                }
            }

            if (in_array($request[self::KEY_ID], $request_ids, true)) {
                throw new CApiRequestException('Invalid request (duplicate id ' . $request[self::KEY_ID] . ')');
            }
