...
EOT;
  ...
  static function getPsStatus(){
    ...
    if (PHP_OS == "WINNT") {
      ...
      foreach ($out as $_line) {
        $result = preg_match('# *([0-9]{1,6}) +[?a-z0-9/.-_]+ +[0-9:]{8} ([a-zA-Z0-9-_:/. ]+)#', $_line, $matches);

        if ($result && array_key_exists(1, $matches) && array_key_exists(2, $matches) && array_key_exists($matches[1], $processes)
        ) {
          $_pid     = $matches[1];
          $_ps_name = $matches[2];
