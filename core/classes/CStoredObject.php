...
class CStoredObject extends CModelObject implements JsonSerializable
{
    ...
    const COMPRESS_MIN_LEN = 10;

    static $useObjectCache  = true;
    static $objectCounts    = [];
    static $cachableCounts  = [];
    static $deletedCounts   = [];
    static $min_search_size = null;
    ...
    static function isGuid($guid)
    {
        ...
        if (preg_match('/(?<object_class>\w+)-(?<object_id>\d+|none)/', $guid, $matches)) {
    ...
    function isCached($id)
    {
        ...
        $cache = new Cache(__CLASS__ . "::cache", "$this->_class-$id", Cache::INNER);
    ...
    public final function registerCache()
    {
        ...
        $cache = new Cache(__CLASS__ . "::cache", "$class-$this->_id", Cache::INNER);
     ...
     *
     * @return $this|null
     * @throws Api\Exceptions\CApiRequestException
     */
     ...
     * @throws Exception
     */
    function loadColumn($column, $where = null, $ljoin = null, $limit = null, $unique = true, bool $strict = true)
    {
        if (!$this->_ref_module) {
            return null;
        }
    ...
    * So that we can use the mechanism safely with probably no side effects */
    ...
    function merge($objects, $fast = false)
    {
        ...
        foreach ($objects as &$object) {
            if ($fast) {
                ...
                $msg = $this->transferBackRefsFrom($object);
            }
        ...
        if ($type === "store" || $type === "merge") {
            ...
            foreach ($fields as $_field) {
                ...
                if ($_spec instanceof CTextSpec || $_spec instanceof CHtmlSpec) {
                    ...
                    $opcodes     = (string)$diff->getOpcodes($form_text, $to_text);
    ...
    function transferBackRefsFrom(CMbObject &$object)
    {
        ...
        if ($object->_class !== $this->_class) {
            trigger_error(
                "An object from type '$object->_class' can't be merge with an object from type '$this->_class'",
                E_USER_ERROR
            );
        }
     ...
     * */
        ...
        $seekable_available = $this->canUseMatchAgainst($empty_keywords, $keywords);
                    ...
                    foreach ($seekables as $field => $spec) {
                        ...
                        if ($spec->seekable === true or $spec->seekable === 'order' or $index != 0) {
                            if ($spec instanceof CRefSpec && $spec->class !== $this->_class) {
                                ...
                                // Todo: BUG => Doesn't work on meta reference because we do not have the value of
        ...
        if ($order) {
            ...
            if ($seekable_available && ($seekables_order = $this->getSeekables(true))
            ) {
                $order_by_match = $this->prepareMatchAll($keywords, false, 'and', true) . ' DESC';
                if ($order_by_query) {
                    $order_by_query = $order_by_match . ', ' . $order_by_query;
                }
            }

            $query .= "\n $order_by_query";
        } else {
            if ($seekable_available && ($seekables = $this->getSeekables(true))) {
                $query .= "\n" . $this->prepareMatchAll($keywords, false, 'and', true) . ' DESC,';
            } else {
                foreach ($seekables as $field => $spec) {
                    $query .= "\n`$field`,";
                }
     ...
     * @throws Exception
     */
    private function canUseMatchAgainst(bool $empty_keywords, $values): bool {
        $seekable_available = (bool)(isset($this->_spec->seek) && $this->_spec->seek === 'match' && !$empty_keywords);

        if ($seekable_available) {
            if (!static::$min_search_size) {
                static::updateMinSearchSize();
            }
            ...
            foreach ($values as $_value) {
                if (strlen($_value) < static::$min_search_size) {
     ...
     * @return bool
     */
    function isExportable($prat_ids = [], $date_min = null, $date_max = null)
    {
        return true;
    }
    ...
    public static function prepareMatchOperatorValue(
        ...
        if ($mode === "boolean") {
            ...
            if (strlen($value) < static::$min_search_size) {
     ...
     *
     */
    public static function updateMinSearchSize()
    {
        $ds                      = CSQLDataSource::get('std');
        static::$min_search_size = $ds->loadResult('SELECT @@GLOBAL.ft_min_word_len');
    }
