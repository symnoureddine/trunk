...
class CDevtools
{
    ...
    /** @var int the header value of ox-devtools */
    static $level = 0;
    ...
    public static function start(Request $request)
    {
        ...
        // Devtools : active query trace, query report and tpl trace from header
        if (static::$level >= static::LEVEL_VERBOSE) {
     ...
     * @throws Exception
     */
    public static function stop(Response $reponse): void
    {
        if (!static::$start) {
            return;
        }
        ...
        // Build and display query_report
     ...
     * @throws Exception
     */
    public static function filePutContents($type, $data, $message = null): void
    {
        $dir = static::getDir();
    ...
    public static function addBlackfireMarker(string $label)
    {
    ...
    public static function dumpInConsole($var = null)
    {
    ...
    public static function getJson()
    {
        ...
        $dataSourceTime       = CApp::$performance["dataSourceTime"] ?? null;
        $nosqlTime            = CApp::$performance["nosqlTime"] ?? null;
        ...
        $dataSourcesCount     = CApp::$performance['dataSourceCount'] ?? null;
        $transport_tiers      = CApp::$performance['transportTiers'] ?? null;
        $transport_tiers_time = CApp::$performance['transportTiers']['total']['time'] ?? null;
        ...
        if (SHM::getEngine()) {
            $shm_infos = SHM::getEngine()->getEngineVersion();
            $shm       = [
                "Engine"  => isset($shm_infos[0]) ? $shm_infos[0] : null,
                "Version" => isset($shm_infos[1]) ? $shm_infos[1] : null,
            ];
        }

        if (DSHM::getEngine()) {
            $dshm_infos = DSHM::getEngine()->getEngineVersion();
            $dshm       = [
                "Engine"  => isset($dshm_infos[0]) ? $dshm_infos[0] : null,
                "Version" => isset($dshm_infos[1]) ? $dshm_infos[1] : null,
            ];
        }

        $opcache_infos = opcache_get_configuration();
        $opcache       = isset($opcache_infos["version"]) ? $opcache_infos["version"] : null;
        ...
        $status    = $constants[$status_code] ?? 'undefined';
     ...
     * @return false|string
     */
    private static function jsonEncode($datas)
    {
     ...
     * @return void
     */
    private static function deleteTmpFile($file): void
    {
    ...
    {
        if ($level > 0 && class_exists(Devtools::class)) {
