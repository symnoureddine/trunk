...
class AntiCsrfSharedMemoryRepository implements AntiCsrfRepositoryInterface
{
    ...
    public function __construct(ISharedMemory $driver)
    {
        $this->driver = $driver;
    }
    ...
    {
        try {
            if (!$this->driver->put($this->forgeSecretKey($identifier), $secret)) {
    ...
    {
        if (!$this->driver->put($this->forgeTokenKey($identifier, $token->getToken()), $token, $ttl)) {
            throw CouldNotUseCsrf::notInitialized();
        }
    }
    ...
    {
        return $this->driver->rem($this->forgeTokenKey($identifier, $token->getToken()));
    }
    ...
    {
        return $this->getPrefix() . self::POOL_KEY . '-' . $key;
    }
