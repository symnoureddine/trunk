...
abstract class CJSLoader extends CHTMLResourceLoader
{
    ...
    public static function registerFiles()
    {
        ...
        // Vue, Webpack
        $app_dist = "javascript/dist/app-dist.js";
        if (file_exists($app_dist)) {
            CJSLoader::$files[] = $app_dist;
        }

        $tamm_app_dist = "modules/oxCabinet/vue/_src/dist/app-dist.js";
        if (file_exists($tamm_app_dist) && CModule::getActive("oxCabinet")) {
            CJSLoader::$files[] = $tamm_app_dist;
        }
    }
