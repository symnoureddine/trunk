...
abstract class CObjectGenerator {
   ...
   * @throws Exception
   */
  protected function getRandomObject($max_count = null, $where = array()) {
   ...
   * @throws Exception
   */
  protected function getRandomNames($count = 1, $where = array()) {
   ...
   * @throws Exception
   */
  protected function getRandomCIM10Code($codes = array()) {
    $ds = CSQLDataSource::get('cim10');

    $type = CCodeCIM10::getVersion();
    switch ($type) {
      case 'atih':
        $select = array('code', 'libelle');
        $where  = array(
        ...
        break;
      case 'oms':
        $select = array('code' => 'abbrev', 'libelle' => 'abbrev');
        $where  = array(
          'code' => $ds->prepareNotIn($codes),
          'type' => "= 'S'",
        );
