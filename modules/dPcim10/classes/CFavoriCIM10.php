...
class CFavoriCIM10 extends CMbObject {
  const GET_LIST_FAVORIS_CACHE = __CLASS__ . '::getListFavoris';
  ...
  public static function findCodes(
      $users, $code = null, $keywords = null, $chapter = null, $category = null,
      $tag_id = null, $sejour_type = null, $field_type = null
  ) {
    if (is_array($users)) {
      $favoris = array();
  ...
  public static function getListFavoris($users, $tag_id = null) {
    ...
    $codes = array();
    $where = array(
      'favoris_user' => CSQLDataSource::prepareIn(array($users_id))
    );
    $ljoin = array();
  ...
  public static function resetListFavoris($users, $tag_id = null) {
    ...
    $cache = new Cache(self::GET_LIST_FAVORIS_CACHE, array($users_id, $tag_id), Cache::INNER_OUTER);
    if ($cache->exists()) {
      $cache->rem();
    }
  }
