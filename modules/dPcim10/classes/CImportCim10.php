...
class CImportCim10 implements IShortNameAutoloadable {
  ...
  function add($type, $entry) {
    switch ($type) {
      case 'chapter':
        list($master_entry, $chapter_entry) = $entry;
        ...
        if (strpos($code_excl, '-') !== false) {
          ...
          if (strpos($code_excl, '-') != strlen($code_excl) - 1) {
            ...
            if (strpos($code_excl, '.') === false) {
              ...
              if (!is_null($sid_excl)) {
                $this->insertExclusionNote($sid, $code_excl, $_note, $date, $entry['origine'], $sid_excl);
              }
              else {
                /* The exclusion is not on a group, but on an interval */
                $code_excl = str_replace(array('(', ')'), '', $code_excl);
                $letter = $code_excl[0];
              ...
              if ($begin_letter == $end_letter) {
                /* Add an exclusion note for each code in the interval */
                for ($i = $begin; $i <= $end; $i++) {
                  $this->insertExclusionNote($sid, substr($code_excl, 0, 4) . $i, $_note, $date, $entry['origine']);
                }
              }
            }
          }
          else {
            $query = "SELECT `SID`, `code` FROM `master` WHERE `code` LIKE '" . substr($code_excl, 0, strlen($code_excl) - 1) . "%';";
            $result = $this->ds->exec($query);
            while ($row = $this->ds->fetchAssoc($result)) {
              if (strpos($row['code'], 'X') == false) {
                $this->insertExclusionNote($sid, $row['code'], $_note, $date, $entry['origine'], $row['SID']);
              }
            }
          }
        }
  ...
  function addMemo($entry) {
    ...
    elseif (strpos($code, '-') !== false && strpos($code, '.') !== false) {
      ...
      list($group, $subdivision) = explode('.', $code);
      $letter = $group[0];
      list($begin, $end) = explode('-', $group);
    ...
    if (!is_null($sid)) {
      ...
      foreach ($matches[1] as $_match) {
        if (strpos($_match[0], '-') !== false) {
          $query = "SELECT `SID`, `code` FROM `master` WHERE `code` LIKE '" . substr($_match[0], 0, strlen($_match[0]) - 1) . "%';";
          $result = $this->ds->exec($query);
          while ($row = $this->ds->fetchAssoc($result)) {
  ...
  function getChapterAncestor($code) {
    ...
    $ancestor = null;
    $group_codes = explode('-', str_replace(array('(', ')'), '', $code));

    foreach ($this->chapters as $_chapter) {
      $_chapter_codes = explode('-', str_replace(array('(', ')'), '', $_chapter['code']));
      ...
      if ($group_codes[0] == $_chapter_codes[0] || $group_codes[1] == $_chapter_codes[1]) {
        ...
        $begin = array($group_codes[0], $_chapter_codes[0]);
        $end = array($group_codes[1], $_chapter_codes[1]);
        sort($begin);
        sort($end);
  ...
  function getGroupAncestor($code) {
    ...
    foreach ($groups as $_group) {
      $_group_codes = explode('-', str_replace(array('(', ')'), '', $code));
      ...
      if ($code == $_group_codes[0] || $code == $_group_codes[1]) {
        ...
        $begin = array($code, $_group_codes[0]);
        $end = array($code, $_group_codes[1]);
        sort($begin);
        sort($end);
  ...
  function getSubCategoryAncestor($code) {
    ...
    if (empty($ancestor)) {
      ...
      return array($ancestor, $ancestor_code);
    }
  }
