...
class CDRCConsultationResult extends CDRC {
  /** @var integer The id of the Consultation Result */
  public $result_id;
  ...
  /** @var integer The concerned genre for this result (1 => male, 2 => female, 3 => both) */
  public $sex;

  /** @var string The type of episode (C => chronicle, A => intense, I => intermediate, NC => not concerned) */
  public $episode_type;

  /** @var integer The version number */
  public $version;

  /** @var integer The state of the result (1 => valid, 2 => removed) */
  public $state;

  /** @var boolean Indicate whether this result can be used as a symptom */
  public $symptom;

  /** @var boolean Indicate whether this result can be used as a syndrome */
  public $syndrome;

  /** @var boolean Indicate whether this result can be used as a disease */
  public $disease;

  /** @var boolean Indicate whether this result can be used as a certified diagnosis */
  public $certified_diagnosis;

  /** @var boolean Indicate whether this result can be used as non pathological */
  public $unpathological;

  /** @var string The complete information about this result */
  public $details;

  /** @var integer The propable duration of the episode */
  public $dur_prob_epis;

  /** @var integer The minimum age for selecting this result */
  public $age_min;

  /** @var integer The maximum age for selecting this result */
  public $age_max;
  ...
  public $_critical_diagnoses = array();
  ...
  public $_transcodings = array();

  /** @var CDRCSynonym[] The synonyms of the result */
  public $_synonyms = array();

  /** @var array The list of the diagnosis positions */
  public static $diagnosis_positions = array('symptom', 'syndrome', 'disease', 'certified_diagnosis', 'unpathological');
  ...
  protected function formatDetails() {
    ...
    foreach ($titles as $i => $title) {
      ...
      if (!in_array($title, array('VOIR AUSSI', 'DIAGNOSTICS CRITIQUES', 'POSITIONS DIAGNOSTIQUES', 'CORRESPONDANCES CIM 10'))) {
  ...
  protected function loadCriticalDiagnoses() {
    $this->_critical_diagnoses = array();

    $query = new CRequest();
    $query->addTable('critical_diagnoses');
    $query->addSelect('critical_diagnoses.*');
    $query->addLJoinClause('results_to_diagnoses', 'results_to_diagnoses.diagnosis_id = critical_diagnoses.diagnosis_id');
  ...
  public static function get($result_id, $load = self::LOAD_LITE) {
    $cache = new Cache(__METHOD__, array($result_id), self::$cache_layers);
  ...
  public static function search(
      $keywords = '', $sex = null, $age = null, $class = null, $only_title = false, $start = 0, $limit = 100
  ) {
    self::getDatasource();
    ...
    if ($class && $class instanceof CDRCResultClass) {
      $query->addLJoinClause('results_to_classes', 'results_to_classes.result_id = consultation_results.result_id');
    ...
    /* Otherwise, we must make several requests for performance's sake */
      ...
      if ($prefilters) {
        ...
        if (count($results_id)) {
          $query = new CRequest();
          $query->addTable('consultation_results');
          $query->addSelect(array('DISTINCT result_id', 'title'));
