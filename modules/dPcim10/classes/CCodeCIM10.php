...
class CCodeCIM10 implements IShortNameAutoloadable {
    ...
    switch ($version) {
      case 'atih':
        $results = CCodeCIM10ATIH::findCodes(
          $code, $keys, $chapter, $category, $max_length, $where, null, $sejour_type, $field_type, $user_favorites
        );
        break;
      case 'gm':
  ...
  public static function getUsedCodesFor(
      $user, $code = null, $keywords = null, $chapter = null, $category = null, $sejour_type = null, $field_type = null
  ) {
    ...
    if (count($used_codes)) {
      ...
      $codes = CCodeCIM10::findCodes($code, $keywords, $chapter, $category, null, $where, null, $sejour_type, $field_type);
  ...
  public static function getLangCIM() {
    switch (CAppUI::pref("LOCALE")) {
      ...
      case "fr-be":
      case "nl_be":
      default: $lang = CCodeCIM10::LANG_FR;
    }
    return $lang;
  }
  ...
  public static function isCategory($code) {
    ...
    return strpos($code, '(') !== false || strpos($code, '-') !== false || preg_match('/[a-zA-Z]{1,5}[^0-9]/', $code) || (strlen($code) < 3 && $code != '');
  }
