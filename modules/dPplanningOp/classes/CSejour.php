...
use Ox\Mediboard\Anq\CAnq;
use Ox\Mediboard\Anq\CAnqHonos;
...
class CSejour extends CFacturable implements IPatientRelated, IGroupRelated, IRGPDEvent, ImportableInterface {
  ...
  private static $fields_etiq = array(
    "NDOS", "NRA", "DATE ENT", "HEURE ENT", "DATE SORTIE", "HEURE SORTIE", "TYPE HOSPITALISATION",
    "PRAT RESPONSABLE", "PRENOM PRAT RESPONSABLE", "NOM PRAT RESPONSABLE", "CODE BARRE NDOS", "CHAMBRE COURANTE", "SERVICE COURANT",
    "PRESTATIONS SOUHAITEES", "MEDICAMENTS DISPENSES",
    "NOM MAMAN", "PRENOM MAMAN",
  );

  static $destination_values = array("1", "2", "3", "4", "6", "7");
  static $provenances = array(
    "MCO_SSR" => array(
      0   => ["", 1, 2, 3, 4, "R"],
      6   => ["", 1, 2, 3, 4, 6],
      7   => ["", 1, 2, 3, 4, 6, "R"],
  ...
  static $destinations = array(
    "MCO_SSR" => array(
      "transfert_acte" => array("", 1, 2, 3, 4),
      "mutation"       => array("", 1, 2, 3, 4, 6),
      "transfert"      => array("", 1, 2, 3, 4, 6),
      "normal"         => array("", 0, 7),
      "deces"          => array("", 0),
    ),
    "psy"     => array(
      "transfert_acte" => array("", 0),
      "mutation"       => array("", 1, 2, 3, 4, 6),
      "transfert"      => array("", 1, 2, 3, 4, 6),
      "normal"         => array("", 0, 7),
      "deces"          => array("", 0),
  ...
  function getSpec() {
    ...
    $references_w_mandatory_fields['mandatory_fields'] = array('type', 'entree', 'sortie');
  ...
  public function getApiLink(): string {
    return CRouter::getInstance()
      ->generate('planning_sejour', ["sejour_id" => $this->_id]);
  }
  ...
  function getProps() {
    ...
    $props["uf_soins_id"]          = "ref class|CUniteFonctionnelle seekable back|sejours_soin";
    ...
    $props["chambre_seule"]        = "bool notNull show|0 default|" . (CGroups::loadCurrent()->chambre_particuliere ? 1 : 0) . " fieldset|placement";
  ...
  function check() {
    ...
    if (!$this->_id && $this->_check_bounds && !in_array($this->type, self::getTypesSejoursUrgence($this->praticien_id))) {
      ...
      elseif ($dhe_date_max && $this->entree_prevue > $dhe_date_max . ' 23:59:59') {
        return CAppUI::tr('CSejour-error-dhe_date_max-entree', CMbDT::format($dhe_date_max, CAppUI::conf("date")));
      }
    }
    ...
    if ($this->mode_sortie && (!$this->_id || $this->fieldModified("destination") || $this->fieldModified("mode_sortie"))) {
      if (!in_array($this->destination, self::$destinations[$this->type === "psy" ? "psy" : "MCO_SSR"][$this->mode_sortie])) {
        return CAppUI::tr("CSejour.destination_incoherente_mode_sortie");
      }
    }

    if ($this->mode_entree && (!$this->_id || $this->fieldModified("provenance") || $this->fieldModified("mode_entree"))) {
      if (!in_array($this->provenance, self::$provenances[$this->type === "psy" ? "psy" : "MCO_SSR"][$this->mode_entree])) {
  ...
  function getServiceFromSectorisationRules($quiet = false) {
    ...
    if (count($secondary_functions)) {
      $where["function_id"] = $ds->prepare("= ? OR `function_id` IS NULL OR `function_id` " . CSQLDataSource::prepareIn(array_keys($secondary_functions)), $praticien->function_id);
    }
   ...
   */
  function store() {
    $this->completeField("entree_reelle", "entree", "patient_id", "type_pec", "grossesse_id", "mode_sortie", "facturable");

    if ($msg = CBillingPeriod::checkStore($this)) {
      return $msg;
    }

    if (in_array($this->facturable, array("", null))) {
      $this->facturable = 1;
    }
    ...
    $form_handicap = (strpos($this->_handicap, ',') > -1) ? explode(',', $this->_handicap) : [$this->_handicap];
    if ($this->_handicap !== null) {
        ...
        $patient_handicap->patient_id = $patient_handicap->_id;
    ...
    if (CModule::getActive("hotellerie")) {
      if ($this->_ref_last_affectation
        ...
        $cleanup->lit_id    = $this->_ref_last_affectation->_ref_lit->_id;
   ...
   * @see parent::merge()
   */
  public function merge($objects, $fast = false) {
  ...
  function checkUpdateTimeAmbu() {
    ...
    if (count($this->_ref_operations) == 1) {
      ...
      if (isset($interv->presence_preop) && (isset($interv->horaire_voulu) || isset($interv->time_operation))) {
        ...
        if (CAppUI::conf('dPplanningOp CSejour heure_deb')) {
          $min_hour_sejour = $interv->date . ' ' . str_pad(CAppUI::conf('dPplanningOp CSejour heure_deb'), 2, '0', STR_PAD_LEFT) . ':00:00';
      ...
      if (isset($interv->presence_postop) && isset($interv->temp_operation) &&
  ...
  function updatePlainFields() {
    ...
    if ((!$this->mode_sortie || !in_array($this->mode_sortie, array("normal", "deces"))) && ($this->destination === "0" || $this->destination === 0)) {
    ...
    if (($this->fieldModified("type") || ($this->fieldModified("entree_prevue") && CMbDT::date($this->_old->entree_prevue) != CMbDT::date($this->entree_prevue)))
  ...
  function getDroitsCMU() {
    if ((!$this->_ref_patient->fin_amo || $this->_date_sortie_prevue <= $this->_ref_patient->fin_amo) && $this->_ref_patient->cmu) {
      $this->_couvert_cmu = 1;
    }
    else {
      $this->_couvert_cmu = 0;
  ...
  static function massLoadCurrAffectation(&$sejours = array(), $date = null, $service_id = null) {
    ...
    foreach ($sejours as $_sejour) {
      ...
      $_sejour->_ref_curr_affectation->loadRefLit()->loadRefChambre()->loadRefService();
    }
  }
  ...
  function loadRefsTransmissions($cible_importante = false, $important = false, $macro_cible = false, $limit = null, $date = null, $degre = null) {
    ...
    if (!$cible_importante && !$important) {
      $this->_ref_transmissions = $this->loadBackRefs("transmissions", null, null, null, null, null, "", array("cancellation_date IS NULL"));
    }
   ...
   * @return CRDVExterne[]
   */
  function loadRefsRDVExternes($where = array()) {
   ...
   * @throws Exception
   */
  function loadSuiviMedical($datetime_min = null, $cible_trans = null, &$cibles = array(), &$last_trans_cible = array(), $user_id = null, &$users = array(), $function_id = null, &$functions = array(), $print = 0, $datetime_max = null, $dietetique = "", $etiquette = null) {
    ...
    if (isset($this->_back["transmissions"])) {
      ...
      foreach ($list_trans as $_trans) {
        ...
        if (!isset($tmp_trans_update[$sort_key])) {
          $tmp_trans_update[$sort_key] = array("data" => array(), "action" => array(), "result" => array());
        }
      ...
      if ($see_old_trans) {
        ...
        foreach (array_keys($tmp_trans) as $sort_key) {
          ...
          foreach (preg_grep("/$cible_id$/", $tmp_trans_keys) as $other_sort_key) {
            ...
            foreach (array("data", "action") as $type) {
  ...
  function loadRefLastCPA() {
    ...
    $cpa      = array("date_cpa"     => null,
                      "consult_guid" => null);
    $last_cpa = end($consults_anesth);

    if (!empty($last_cpa)) {
      $last_cpa->loadRefConsultation()->loadRefPlageConsult();
      $cpa = array("date_cpa"     => $last_cpa->_ref_consultation->_datetime,
   ...
   */
  function loadRefsConsultations($order = "date DESC, heure DESC", $where = array()) {
    $this->_ref_consultations = $this->loadBackRefs("consultations", $order, null, null, array("plageconsult" => "plageconsult.plageconsult_id = consultation.plageconsult_id"), null, "", $where);
   ...
   * @return CPrescription[]
   */
  static function massLoadRefPrescriptionSejour($sejours = array()) {
  ...
  function loadComplete() {
    ...
    $fields_obs_entree = array(
      "motif", "histoire_maladie", "examen", "rques", "conclusion",
    );
  ...
  function loadView() {
    ...
    $this->_ref_patient->updateBMRBHReStatus();
  ...
  function loadListFromNDA($nda) {
    ...
    return $this->loadList(array("sejour_id" => CSQLDataSource::prepareIn($sejour_ids)), "entree ASC");
  }
  ...
  function fillLimitedTemplate(&$template) {
    ...
    $template->addProperty("$sejour_section - " . CAppUI::tr('CModeEntreeSejour'),
      $this->getFormattedValue($this->mode_entree_id ? "mode_entree_id" : "mode_entree"));
    $template->addProperty("$sejour_section - " . CAppUI::tr('CSejour-mode_sortie'),
    ...
    $template->addProperty("$sejour_section - " . CAppUI::tr('CSejour-Establishment of exit'), $this->getFormattedValue("etablissement_sortie_id"));
    ...
    $template->addProperty("$sejour_section - " . CAppUI::tr('CExamIgs-simplified_igs-court'), $last_exam_igs->_id ? $last_exam_igs->simplified_igs : "");
    ...
    $template->addProperty("{$sejour_section} - " . CAppUI::tr('CExamGir') . " (" . CAppUI::tr("common-date") . ")",
      $date_gir . " - " . $output);
    ...
    $template->addProperty("$hospi_section - " . CAppUI::tr('common-Practitioner') . " - " . CAppUI::tr('CMediusers-rpps'), $this->_ref_praticien->rpps);
    ...
    if ($curr_affectation && $curr_affectation->_id) {
      $curr_affectation_view = "$curr_affectation->_view du " . CMbDT::format($curr_affectation->entree, CAppUI::conf("datetime")) . " au " . CMbDT::format($curr_affectation->sortie, CAppUI::conf("datetime"));
    }
    ...
    $template->addProperty("$hospi_section - " . CAppUI::tr('CAffectation-Last assignment (Service)'), $last_affectation->loadRefService()->_view);
    $template->addProperty("$hospi_section - " . CAppUI::tr('CAffectation-Last assignment (Sector)'), $last_affectation->_ref_service->loadRefSecteur()->nom);
    ...
    $template->addListProperty("$sejour_section - $trans_subItem - " . CAppUI::tr('CTransmissionMedicale-High importance'), $transmissions_hautes);
    ...
    self::filllimitedTemplateObs(
      isset($this->_back["observations"]) ? $this->_back["observations"] : [], $template,
    ...
    foreach ($observation_tag->_specs["etiquette"]->_list as $_etiquette) {
      self::filllimitedTemplateObs(
        $this->loadRefsObservations(null, null, $_etiquette), $template,
    ...
    foreach ($observation_tag->_specs["type"]->_list as $_type) {
      self::filllimitedTemplateObs(
        $this->loadRefsObservations(null, $_type), $template,
    ...
    foreach ($observation_tag->_specs["type"]->_list as $_type) {
        foreach ($observation_tag->_specs["etiquette"]->_list as $_etiquette) {
            self::filllimitedTemplateObs(
                ...
                $template,
                "$sejour_section - " . CAppUI::tr("CSejour-back-observations") . " - ". CAppUI::tr("CObservationMedicale.type." . $_type) . " - " . CAppUI::tr("CObservationMedicale.etiquette." . $_etiquette)
            );
        }
    }
    ...
    self::filllimitedTemplateObs(
      $this->loadRefsObservations(null, "synthese"), $template,
      "$sejour_section - " . CAppUI::tr('CObservationMedicale-Synthesis observation|pl')
    );

    // Observations de communication
    self::filllimitedTemplateObs(
      $this->loadRefsObservations(null, "communication"), $template,
    ...
    $template->addMarkdown("$sejour_section - $obs_entree_subItem - " . CAppUI::tr('CConsultation-examen'), $obs_entree->getFormattedValue("examen"));
    $template->addMarkdown("$sejour_section - $obs_entree_subItem - " . CAppUI::tr('CSejour-rques-court'), $obs_entree->getFormattedValue("rques"));
    $template->addMarkdown("$sejour_section - $obs_entree_subItem - " . CAppUI::tr('CConsultation-traitement'), $obs_entree->getFormattedValue("traitement"));
    $template->addMarkdown("$sejour_section - $obs_entree_subItem - " . CAppUI::tr('CConsultAnesth-histoire_maladie'), $obs_entree->getFormattedValue("histoire_maladie"));
    $template->addMarkdown("$sejour_section - $obs_entree_subItem - " . CAppUI::tr('CConsultation-conclusion'), $obs_entree->getFormattedValue("conclusion"));
    ...
    foreach ($this->loadRefsConsultations() as $_consult) {
      ...
      if ($conclusion) {
        ...
        if (!isset($consults_by_function[$function->_view])) {
          $consults_by_function[$function->_view] = array();
        }
    ...
    if (CAppUI::gconf("dPhospi prestations systeme_prestations") == "expert") {
      $liaisons_j = $dates = $list_souhaits = $liaisons_by_id = array();

      self::getIntervallesPrestations($liaisons_j, $dates, $liaisons_by_id);
      $template->addProperty("$sejour_section - " . CAppUI::tr('CSejour-Prestations'),
        $this->printPrestations($liaisons_by_id, $dates), '', false);
    ...
    if (CModule::getActive("dPfacturation") && CAppUI::gconf("dPplanningOp CFactureEtablissement use_facture_etab")) {
      ...
      $template->addProperty("$sejour_section - " . CAppUI::tr('CSejour-_rques_assurance_maladie'), $this->getFormattedValue("_rques_assurance_maladie"));
    }
    $this->loadNRA();
    $template->addProperty("$sejour_section - " . CAppUI::tr('CSejour-Case number'), $this->_ref_NRA && $this->_ref_NRA->_id ? $this->_ref_NRA->id400 : "-");
    ...
    if (CModule::getActive("maternite")) {
      ...
      if ($this->grossesse_id && count($grossesse->loadRefsNaissances())) {
        ...
        $constantes        = $enfant->loadRefLatestConstantes(null, array("poids"));
      ...
      $template->addProperty("$sejour_section - $accouchement_subItem - " . CAppUI::tr('CNaissance-_heure-desc'), $naissance->getFormattedValue("_heure"));
      ...
      $template->addProperty("$sejour_section - $accouchement_subItem - " . CAppUI::tr('CNaissance-Child sex'), $enfant->getFormattedValue("sexe"));
      $template->addProperty("$sejour_section - $accouchement_subItem - " . CAppUI::tr('CExamenNouveauNe-Weight (kg)'), $constantes_enfant->poids . " kg");
      $template->addProperty("$sejour_section - $accouchement_subItem - " . CAppUI::tr('CExamenNouveauNe-Weight (g)'), $constantes_enfant->_poids_g . " g");
    ...
    $constantes       = CConstantesMedicales::getFirstFor($this->_ref_patient, null, null, $this, false);
    ...
    if ($dossier_medical->_ref_traitements && $dossier_medical->_count_traitements) {
      foreach ($dossier_medical->_ref_traitements as $curr_trmt) {
        if ($curr_trmt->fin) {
          $trmt_date = CAppUI::tr("common-Since") . " " . $curr_trmt->debut . " " . CAppUI::tr("CPrescriptionLine-until-court") . " " . $curr_trmt->fin;
        }
    ...
    $template->addProperty("$sejour_section - $autorisation_subItem - " . CAppUI::tr('common-Practitioner name'), $this->_ref_confirme_user->_view);
    ...
    $template->addProperty("$sejour_section - $autorisation_subItem - " . CAppUI::tr('CSejour-mode_sortie'), $this->getFormattedValue("mode_sortie"));
    ...
    if (CModule::getActive("addictologie")) {
      ...
      $template->addProperty("$sejour_section - $addictologie_subItem - " . CAppUI::tr('CDossierAddictologie-referent_user_id'), $dossier_addictologie->_ref_referent_user->_view);
      $template->addProperty("$sejour_section - $addictologie_subItem - " . CAppUI::tr('CDossierAddictologie-convention'), $dossier_addictologie->getFormattedValue("convention"));
      $template->addProperty("$sejour_section - $addictologie_subItem - " . CAppUI::tr('CDossierAddictologie-cas_particulier'), $dossier_addictologie->getFormattedValue("cas_particulier"));
      $template->addProperty("$sejour_section - $addictologie_subItem - " . CAppUI::tr('CDossierAddictologie-suivi_social'), $dossier_addictologie->getFormattedValue("suivi_social"));
      $template->addProperty("$sejour_section - $addictologie_subItem - " . CAppUI::tr('CDossierAddictologie-dispensation'), $dossier_addictologie->getFormattedValue("dispensation"));
    }
  ...
  function completeLabelFields(&$fields, $params) {
    ...
    if (isset($params["debut_dispensation"]) && isset($params["fin_dispensation"])) {
      ...
      foreach ($deliveries as $_delivery) {
        ...
        $meds_dispenses[] = "($_delivery->quantity) $product->code $product->name"
          . ($article->_unite_dispensation ? " ($article->_unite_dispensation)" : '');
      }
    }
    ...
    if (CAppUI::conf("ref_pays") == 2) {
      ...
      if ($facture) {
        $facture->loadRefAssurance();
        $fields["ASSUR"] = $facture->_ref_assurance_maladie->nom;
      }
    }
  ...
  function checkMerge($sejours = array()) {
    if (!static::getAllowMerge($this->group_id)) {
      return CAppUI::tr('CSejour-merge-warning-Not allowed');
    }
    ...
    // Les prescriptions ne bloquent pas la fusion car une interface gère cela dans le dossier de soins
    ...
    if (count(array_unique($sejour_ids)) > 1) {
      return CAppUI::tr("CSejour-merge-warning-Multiple affectations");
    }
  ...
  function getIntervallesPrestations(&$liaisons = array(), &$dates, &$liaisons_by_id, $only_souhait = false) {
    $where = array();
    ...
    foreach ($items_liaisons as $_item_liaison) {
      ...
      if ($_item_souhait->object_class == "CPrestationPonctuelle") {
        ...
        if ($_item_souhait->_ref_object->forfait) {
          $this->_ref_prestations[CMbDT::date($this->entree)][] = array(
            "quantite"          => 1,
            "item"              => $_item_souhait,
  ...
  function getPrestations() {
    ...
    $liaisons_j = $dates = $liaisons_by_id = array();
  ...
  function loadAllLiaisonsForDay($date = null, $prestation_id = null) {
    ...
    foreach ($results as $_result) {
      $prestas[$_result["object_id"]] = array("color" => $_result["color"], "nom" => $_result["nom"]);
    }
  ...
  static function massLoadLiaisonsForPrestation($sejours, $prestation_id, $date_min = null, $date_max = null) {
    ...
    foreach ($sejours as $_sejour) {
      $_sejour->_liaisons_for_prestation = array();
    }

    $item_liaison = new CItemLiaison();

    $temp_liaisons_by_sejour = array();
    ...
    foreach ($prestation_id as $_prestation_id) {
      ...
      foreach ($sejours as $_sejour) {
        ...
        if (!isset($temp_liaisons_by_sejour[$_sejour->_id])) {
          $temp_liaisons_by_sejour[$_sejour->_id] = array();
        }
        if (!isset($temp_liaisons_by_sejour[$_sejour->_id][$_prestation_id])) {
          $temp_liaisons_by_sejour[$_sejour->_id][$_prestation_id] = array();
        }
  ...
  function loadLiaisonsForDay($prestation_id, $date) {
    ...
    if ($prestation_id) {
      $where["item_liaison.prestation_id"] = " = '$prestation_id'";
    }
  ...
  function loadRefsUserSejour($userCourant = null, $date = null, $mode = null, $with_old = true) {
    ...
    if ($mode == "affectations" && $type_affectation == "segment") {
      ...
      foreach ($this->_ref_users_sejour as $key_user => $user_sejour) {
        if ((!$user_sejour->debut && !$user_sejour->fin) || (CMbDT::date($user_sejour->debut) != $date && $date != CMbDT::date($user_sejour->fin)) || isset($users[$user_sejour->user_id])) {
          if (!((!$user_sejour->debut && !$user_sejour->fin) || (CMbDT::date($user_sejour->debut) != $date && $date != CMbDT::date($user_sejour->fin)))) {
    ...
    $this->_ref_users_by_type = $this->_ref_group_users_by_type = array(
      "infirmiere" => array(),
      "AS"         => array(),
      "SF"         => array(),
      "kine"       => array(),
    ...
    foreach ($this->_ref_users_sejour as $_user_sejour) {
      ...
      $key                                                = $type_affectation == "segment" ? count($this->_ref_users_by_type[$name]) : $_user->_id;
    ...
    if ($userCourant && ($userCourant->isInfirmiere() || $userCourant->isAideSoignant() || $userCourant->isSageFemme() || $userCourant->isKine() || $userCourant->isPraticien())) {
  ...
  function getPrintDossierSoins($show_forms = 0) {
    $query = array(
      "m"                   => "soins",
  ...
  function loadRefsAppel($type = null, $all = false) {
    ...
    if ($type) {
      ...
      $where["type"] = " = '$type'";
      if (!$all) {
        $resultats = $this->loadBackRefs("appels", "appel_id DESC, datetime DESC", null, null, null, null, "", $where);
        if (!count($resultats)) {
  ...
  function loadCurrBrancardage() {
    ...
    $where[]                                   = "DATE(brancardage_item.demande_brancard) LIKE '$curr_date%' OR DATE(brancardage_item.patient_pret) LIKE '$curr_date%'";
    $where["brancardage_item.prise_en_charge"] = " IS NULL";
    $where[]                                   = "brancardage_item.demande_brancard IS NOT NULL OR brancardage_item.patient_pret IS NOT NULL";
    $brancardages                              = $this->loadBackRefs("sejour_brancard", "brancardage.brancardage_id DESC", "1", null, $ljoin, null, "", $where);
    if (!count($brancardages)) {
      $brancardages = array(new CBrancardage());
    }
  ...
  static function checkIncomingSejours($patients, $encours = false) {
    ...
    if ($encours) {
      ...
      $where["sortie"] = $ds->prepare("> ?", $now);
        ...
        $where [] = $ds->prepare("(entree_prevue >= ?) OR (entree_prevue <= ? AND entree_reelle IS NULL)", $now);
    }
  ...
  function getEventIcon() {
    ...
    if ($this->grossesse_id) {
      ...
      $icon['title'] = in_array($this->type, self::getTypesSejoursUrgence($this->praticien_id)) ? CAppUI::tr('CSejour-title-Emergency with pregnancy') : CAppUI::tr('CSejour-title-Stay with pregnancy');
    }
  ...
  function loadRefsMacrocible($type_load = "all") {
    ...
    $transmission                                     = new CTransmissionMedicale();
    if ($type_load != "all") {
      return $this->_ref_macrocibles = $transmission->loadIds($where, "date DESC, transmission_medicale_id DESC", null, "transmission_medicale_id", $ljoin);
    }
    $this->_ref_macrocibles = $transmission->loadList($where, "date DESC, transmission_medicale_id DESC", null, "transmission_medicale_id", $ljoin);
   ...
   */
  function loadRefsEvtsSSRSejour($where = array(), $ljoin = array()) {
   ...
   * @return array
   */
  static function getTypeSejourIdsPref($type_sejours_ids = array()) {
  ...
  function calculConstantesMedicales($date, $hour_min = "00:00:00", $hour_max = "23:59:59") {
    ...
    if ($hour_min > $hour_max) {
      [$hour_min, $hour_max] = array($hour_max, $hour_min);
    }
  ...
  public function getColorPrestations() {
    ...
    foreach ($liaisons as $_liaison) {
      $_liaison->loadRefItem()->loadRefObject();
      $_liaison->loadRefItemRealise()->loadRefObject();

      $item         = $_liaison->_ref_item;
      $item_realize = $_liaison->_ref_item_realise;
      ...
      elseif ($_liaison->item_souhait_id && !$_liaison->item_realise_id) {
        ...
        $this->_title_prestation = CAppUI::tr('CSejour-msg-Wish % s - Fulfilled %s', $item->nom, $item_realize->nom);
      }
    }
  }
  ...
  /**
   * Charge les ANQ du séjour
   *
   * @return CAnq[]
   */
  public function loadRefsAnqType($type) {
    if (!CModule::getActive("anq")) {
      return;
    }
    switch ($type) {
      case "CAnqHonos":
        return $this->_ref_honos = $this->loadBackRefs("anq_honos");
      case "CAnqBSCL":
        return $this->_ref_bscls = $this->loadBackRefs("anq_bscl");
      case "CAnqEFM":
        return $this->_ref_efms = $this->loadBackRefs("anq_efm");
      case "CAnqMIF":
        return $this->_ref_mifs = $this->loadBackRefs("anq_mif");
      case "CAnqCIRS":
        return $this->_ref_cirs = $this->loadBackRefs("anq_cirs");
      case "CAnqEBI":
        return $this->_ref_ebis = $this->loadBackRefs("anq_ebi");
      case "CAnqAO":
        return $this->_ref_aos = $this->loadBackRefs("anq_ao");
      case "CAnqOP":
        return $this->_ref_ops = $this->loadBackRefs("anq_op");
      default:
        break;
    }
  }
  ...
  /**
   * Charge les Anq du séjour par type
   *
   * @return array
   */
  public function loadRefsAnqsByType($type_anq) {
    if (!CModule::getActive("anq")) {
      return;
    }
    $anqs                    = $this->loadRefsAnqType($type_anq);
    $this->_ref_anqs_by_type = array("entree" => null, "sortie" => null, "other" => array());
    foreach ($anqs as $_anq) {
      if (in_array($_anq->_class, array("CAnqEFM", "CAnqOP", "CAnqAO")) || $_anq->type == "other") {
        $this->_ref_anqs_by_type["other"][] = $_anq;
      }
      else {
        $this->_ref_anqs_by_type[$_anq->type] = $_anq;
      }
    }

    return $this->_ref_anqs_by_type;
  }
  ...
  /**
   * Vérifie si le honos d'entrée est saisie dans les 72h de l'entrée du patient
   *
   * @return void|bool
   */
  public function checkAnqsEmpty() {
    if (!CModule::getActive("anq")) {
      return;
    }
    $this->_empty_anqs = array();
    $types_anq         = array();
    $now               = CMbDT::dateTime();
    if ($now > CMbDT::dateTime("+72 hours", $this->entree)) {
      $types_anq[] = "entree";
    }
    if ($this->sortie_reelle && $now > $this->sortie) {
      $types_anq[] = "sortie";
    }
    if (!count($types_anq)) {
      return;
    }
    foreach ($types_anq as $_type_es) {
      if ($this->type == "psy") {
        if (CAppUI::gconf("anq Category see_honos") && !$this->countBackRefs("anq_honos", array("type" => " = '$_type_es'"))) {
          $this->_empty_anqs["CAnqHonos"][$_type_es] = true;
        }
        if (CAppUI::gconf("anq Category see_bscl") && !$this->countBackRefs("anq_bscl", array("type" => " = '$_type_es'"))) {
          $this->_empty_anqs["CAnqBSCL"][$_type_es] = true;
        }
        if (CAppUI::gconf("anq Category see_efm") && $_type_es == "entree") {
          $efms_current = $this->loadBackRefs("anq_efm", null, null, null, null, null, null, array("(date_fin > '" . CMbDT::date() . "' OR date_fin IS NULL)"));
          if (count($efms_current)) {
            $this->_empty_anqs["CAnqEFM"][$_type_es] = true;
          }
        }
      }
      elseif ($this->type == "ssr") {
        if (CAppUI::gconf("anq Category see_cirs") && !$this->countBackRefs("anq_cirs", array("type" => " = '$_type_es'"))) {
          $this->_empty_anqs["CAnqCIRS"][$_type_es] = true;
        }
        if (CAppUI::gconf("anq Category see_mif") && !$this->countBackRefs("anq_mif", array("type" => " = '$_type_es'"))) {
          $this->_empty_anqs["CAnqMIF"][$_type_es] = true;
        }
        if (CAppUI::gconf("anq Category see_ebi") && !$this->countBackRefs("anq_ebi", array("type" => " = '$_type_es'"))) {
          $this->_empty_anqs["CAnqEBI"][$_type_es] = true;
        }
        if ($_type_es == "entree" && CAppUI::gconf("anq Category see_op") && !$this->countBackRefs("anq_op")) {
          $this->_empty_anqs["CAnqOP"][$_type_es] = true;
        }
        if ($_type_es == "sortie" && CAppUI::gconf("anq Category see_ao") && !$this->countBackRefs("anq_ao")) {
          $this->_empty_anqs["CAnqAO"][$_type_es] = true;
        }
      }
    }
  }
  ...
  function loadRefFolderLiaison($type = "pread") {
    $folders_liaison = $this->loadBackRefs("folder_liaison", null, null, null, null, null, "folder_liaison_$type", array("type" => " = '$type' "));

    if (count($folders_liaison) == 0) {
      $folders_liaison = array(new CAppFineClientFolderLiaison());
    }
   ...
   * @return void
   */
  function getColorCompletenessLastForm($event_name) {
    $color        = null;
    ...
    $this->_completeness_color_form = $color;
  }
  ...
  function isCoded() {
    ...
    if (($config == "oneday" && $this->sortie_reelle
        && CMbDT::daysRelative($this->sortie_reelle, CMbDT::date() . ' 00:00:00', $worked_days) >= 1)
      || ($config == '48h' && $this->sortie_reelle
        && CMbDT::daysRelative(CMbDT::dateTime('+48 hours', $this->sortie_reelle), CMbDT::dateTime(), $worked_days) >= 2)
    ) {
      $this->_coded         = true;
  ...
  public static function getTypesSejoursUrgence(int $user_id = null): array {
    ...
    return ((!$user_id || $is_urgentiste) && (CAppUI::gconf("dPurgences CRPU type_sejour") === "urg_consult")) ?
      ["urg", "consult"] : ["urg"];
  }
   ...
   * @throws CApiException
   */
  public function getResourcePraticien(): ?CItem {
    if (!$praticien = $this->loadRefPraticien()) {
      return null;
    }
    $res = new CItem($praticien);
    $res->setName(CMediusers::RESOURCE_NAME_PRATICIEN);
   ...
   * @throws \Exception
   */
  public function getResourceActesCcam(): ?CCollection {
    if (!$actes_ccam = $this->loadRefsActesCCAM()) {
      return null;
    }

    return new CCollection($actes_ccam);
  }
  ...
  public static function getFieldsEtiq() {
    ...
    if (CAppUI::conf("ref_pays") == 2) {
      $fields_etiq[] = "NATURE SEJOUR";
      $fields_etiq[] = "MODE TRT";
      $fields_etiq[] = "ASSUR MALADIE";
      $fields_etiq[] = "ASSUR ACCIDENT";
    }
     ...
     * @return string
     */
    public function getPassageBloc(): ?string
    {
