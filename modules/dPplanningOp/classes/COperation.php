...
{
    /** @var string */
    public const RESOURCE_NAME = 'intervention';
    ...
    function getProps()
    {
        ...
        $props["unite_rayons_x"]        = "enum list|mA|mGy|cGy.cm²|mGy.m²|mGy.cm²|Gy.cm² default|mA";
        $props['pds']                   = 'float';
        $props["unite_pds"]             = "enum list|uGycm²|mGycm² default|mGycm²";
        $props['kerma']                 = 'float';
   ...
   * @inheritdoc
   */
  function getExecutantId($code_activite = 1) {
    $this->loadRefChir();
    $this->loadRefPlageOp();
   ...
   * @inheritdoc
   */
  function getExtensionDocumentaire($executant_id = 1) {
  ...
  function check() {
    ...
    if ($this->_id && $this->sortie_salle && !$this->_old->sortie_salle
      ...
      && CDailyCheckList::getCountChecklistInterv($this) < 3
    ) {
          ...
          case "toggle":
            ...
            foreach ($operations as $_op) {
              ...
              if ($_op->rank == $old->rank) {
                ...
                // Décalage du début pour l'intervention suivante
                $new_time = CMbDT::addTime($this->temp_operation, $new_time);
                $new_time = CMbDT::addTime($plage->temps_inter_op, $new_time);
                $new_time = CMbDT::addTime($this->pause, $new_time);
                $new_time = CMbDT::addTime($this->duree_bio_nettoyage, $new_time);
                $new_time = CMbDT::addTime($this->duree_postop, $new_time);
              }
            }
            ...
            array_multisort(CMbArray::pluck($operations, "rank"), SORT_ASC, $operations, $keys);
  ...
  function store($reorder = true) {
    ...
    if ($this->salle_id && $this->_datetime) {
    ...
    if ($this->fieldModified("plageop_id") && $this->plageop_id) {
      $this->time_operation = "";
    }
    ...
    if (CAppUI::conf("dPsalleOp COperation use_sortie_sans_sspi") && $this->fieldModified("sortie_sans_sspi")) {
    ...
    if ($change_date_codage) {
      ...
      if ($this->_ref_brancardage && $this->_ref_brancardage->_id) {
        $this->_ref_brancardage->date = $this->date;
  ...
  function fillLimitedTemplate(&$template) {
    ...
    if (CModule::getActive("tarmed") && CAppUI::gconf("tarmed CCodeTarmed use_cotation_tarmed")) {
      $tarmed_subItem = CAppUI::tr('CTarmed-TARMED');
      $caisse_subItem = CAppUI::tr('CActeCaisse-caisse_maladie_id');
      $this->loadRefsActes();
      $template->addProperty("$operation_section - $tarmed_subItem - " . CAppUI::tr('common-code|pl'), CActeTarmed::actesHtml($this), '', false);
      $template->addProperty("$operation_section - $caisse_subItem - " . CAppUI::tr('common-code|pl'), CActeCaisse::actesHtml($this), '', false);
    }
    ...
    if (CModule::getActive("mvsante")) {
      $template->addDateTimeProperty("$operation_section - " . CAppUI::tr('COperation-remise_chir'), $this->remise_chir);
      ...
      $template->addProperty("$operation_section - " . CAppUI::tr('common-Label 1'), $libelles[0]);
      $template->addProperty("$operation_section - " . CAppUI::tr('common-Label 2'), $libelles[1]);
      $template->addProperty("$operation_section - " . CAppUI::tr('common-Label 3'), $libelles[2]);
      $template->addProperty("$operation_section - " . CAppUI::tr('common-Label 4'), $libelles[3]);
    }
    ...
    $template->addProperty("$rayons_x_section - " . CAppUI::tr('COperation-labo'),
    ...
    if (CModule::getActive("monitoringBloc") && CAppUI::gconf("monitoringBloc general active_graph_supervision")) {
      ...
      if ($template->valueMode && $this->_id) {
        if ($this->graph_pack_id) {
          ...
          [$list, $grid, $graphs, $labels, $list_obr] = CObservationResultSet::getChronological($this, $this->graph_pack_id, true);
        ...
        if ($this->graph_pack_sspi_id) {
          /** @var CObservationResultSet[] $list_obr */
          [$list, $grid, $graphs, $labels, $list_obr] = CObservationResultSet::getChronological($this, $this->graph_pack_sspi_id, true);
  ...
  function completeLabelFields(&$fields, $params) {
    ...
    /**
     * Chargement du brancardage de l'intervention
     *
    ...
    public function loadRefBrancardage(string $date = null): ?CBrancardage
    {
        ...
        // Legacy stretchers
        if (!$brancardage->_id) {
            $brancardage = $this->loadLegacyStretcher();
        }

        if (!$brancardage->_id) {
            $brancardage->date = $this->date;
        }
    ...
    private function loadLegacyStretcher(): CBrancardage
    {
        ...
        $ljoin = [
            "brancardage_item" => "brancardage_item.brancardage_id = brancardage.brancardage_id",
        ];
        $where = [
            "brancardage.sejour_id" => $ds->prepare("= ?", $this->sejour_id),
            "brancardage.date"      => $ds->prepare("= ?", $date),
        ];
        if ($this->salle_id) {
            $where[] = $ds->prepare(
                "(brancardage_item.destination_id = ? AND brancardage_item.destination_class = 'CBlocOperatoire') 
  ...
  public function getColorConstantsByStep() {
    ...
    if (CModule::getActive("brancardage") && CAppUI::gconf("brancardage General use_brancardage")) {
      ...
      $first_brancard = $this->_ref_brancardage->_ref_first_item;
      $condition      = ($this->entree_salle || $first_brancard->patient_pret);
    }
    else {
  ...
  public function getStatusPreop($class_delay, $delay_phase, $sejour) {
    if (CModule::getActive("brancardage") && CAppUI::gconf("brancardage General use_brancardage")) {
      $this->loadRefBrancardage();
      $first_brancardage = $this->_ref_brancardage->_ref_first_item;
      $entree_brancard   = !$this->entree_salle && !$first_brancardage->demande_brancard;
    }
    else {
    ...
    if (CModule::getActive("brancardage") && CAppUI::gconf("brancardage General use_brancardage")) {
      if ($first_brancardage->demande_brancard || $this->entree_salle) {
  ...
  public function getStatusBloc($class_delay, $delay_phase, $sejour) {
    if (CModule::getActive("brancardage") && CAppUI::gconf("brancardage General use_brancardage")) {
      $this->loadRefBrancardage();
      $first_brancardage = $this->_ref_brancardage->_ref_first_item;
      $delay             = $delay_phase["bloc"] && !$this->entree_salle && $sejour->pec_service && $first_brancardage->arrivee;
    }
    else {
  ...
  public function getStatusSspi($class_delay, $delay_phase, $sejour) {
    if (CModule::getActive("brancardage") && CAppUI::gconf("brancardage General use_brancardage")) {
      $this->loadRefBrancardage();
      $last_brancardage = $this->_ref_brancardage->_ref_last_item;
      $delay            = $delay_phase["sspi"] && !$this->entree_reveil && !$this->sortie_reveil_reel && !$last_brancardage->demande_brancard && $this->sortie_salle;
    }
    else {
    ...
    //*********** sspi ***********
    // - etape
    if (CModule::getActive("brancardage") && CAppUI::gconf("brancardage General see_round_trip_bloc") && CAppUI::gconf("brancardage General use_brancardage")) {
      $color_phase = $this->sortie_reveil_reel && $last_brancardage->demande_brancard;
    }
    else {
  ...
  public function getStatusPostop($class_delay, $delay_phase, $sejour) {
    if (CModule::getActive("brancardage") && CAppUI::gconf("brancardage General use_brancardage") && CAppUI::gconf("brancardage General see_round_trip_bloc")) {
      $this->loadRefBrancardage();
      $last_brancardage = $this->_ref_brancardage->_ref_last_item;
      $delay            = $delay_phase["postop"] && $last_brancardage->arrivee && !$sejour->confirme_user_id && !$sejour->sortie_reelle;
    }
    else {
   ...
   * @return array
   */
  public function getFormsEventByStep() {
    $sejour = $this->_ref_sejour;
   ...
   * @return void
   */
  public function computeStatusPanier() {
    $where = [];
    $ljoin = [];
    ...
    $this->_legend_panier = CAppUI::tr("CMaterielOperatoire-Status materiel", $count_materiels_ok, $count_materiels);
  }
