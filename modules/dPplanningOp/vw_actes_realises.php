...
$sejours_other = $bloc_id ? array() : $sejour->loadList($where, null, null, "sejour_id", $ljoin);
...
$sejours_other_ngap = $bloc_id ? array() : $sejour->loadList($where2, null, null, "sejour_id", $ljoin);
...
$distinct_executant = CAppUI::conf("ref_pays") != 2;
...
foreach ($sejours as $sejour) {
  ...
  if (!count($sejour->_ref_actes) && !count($sejour->_ref_operations) && !count($sejour->_ref_consultations)) {
    ...
    if (CModule::getActive("dPfacturation") && CAppUI::gconf("dPplanningOp CFactureEtablissement use_facture_etab") && CAppUI::conf("ref_pays") == 1) {
...
if (!$export_csv) {
  ...
  $titles = array(
    ...
    CAppUI::tr("CFactureEtablissement-patient_id"), "Total Séjour",
    CAppUI::tr("CActeCCAM-object_class"), CAppUI::tr("CActeCCAM-code_acte"), CAppUI::tr("CCodageCCAM-activite_anesth-court"),
    CAppUI::tr("CActeCCAM-code_phase"), 'Mod', 'ANP', CAppUI::tr("CActeCCAM-montant_base"),
    CAppUI::tr("CActeCCAM-montant_depassement"), CAppUI::tr("CActeCCAM-_montant_facture")
  );
  if ($use_facture_etab) {
