...
<table class="tbl">
  ...
  {{foreach from=$list_protocoles item=_protocole}}
    <tr {{if !"appFineClient"|module_active || !"appFineClient Sync allow_appfine_sync"|gconf || !$appFine_pack_id}}onclick="ProtocoleDHE.chooseProtocole({{$_protocole->_id}}); return false;" style="cursor: pointer;"{{/if}}>
      {{if $appFine_pack_id}}
        <td>
