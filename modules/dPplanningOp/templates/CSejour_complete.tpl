    ...
    <tr>
      <th class="title text" colspan="2" style="vertical-align:middle;">
        ...
        <div style="float: right;{{if $limit_prise_rdv}}display:none;{{/if}}">
          <button type="button" class="print" onclick="printDossierComplet('{{$sejour->_id}}');">{{tr}}CSejour-action-Folder care{{/tr}}</button>
          {{if $object->_ref_rpu && $object->_ref_rpu->_id}}
          <button type="button" class="print" onclick="printDossier({{$object->_ref_rpu->_id}})">{{tr}}CSejour-action-Folder emergency|pl{{/tr}}</button>
          {{/if}}
...
{{if !$limit_prise_rdv}}
  ...
  {{if $sejour->_canRead}}
    <table class="tbl print_sejour">
      ...
      <tr>
        <th class="title" colspan="2">{{tr}}CSejour-Medical follow-up{{/tr}}</th>
      </tr>
    </table>
