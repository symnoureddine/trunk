...
<tr id="{{$_materiel_operatoire->_guid}}">
  ...
  <td>
    ...
    {{if $advanced_prot || in_array($mode, array("validation", "consommation"))}}
    ...
    <td>
      {{if !$readonly}}
        ...
        <form name="addConsommation-{{$_materiel_operatoire->_id}}" {{if $manage_lot}}class="consommation_{{$dm->product_id}}"{{/if}} method="post"
              onsubmit="return onSubmitFormAjax(this, ProtocoleOp.refreshConsommations.curry('{{$_materiel_operatoire->_id}}'));">
          ...
          {{if $manage_lot && $dm->product_id}}
            ...
            <select name="lot_id">
              ...
              {{foreach from=$dm->_ref_product->_lots item=_lot name=lots}}
                <option value="{{$_lot->_id}}" {{if $smarty.foreach.lots.first && $class_notnull}}selected{{/if}}>
                  [{{$_lot->code}}]
