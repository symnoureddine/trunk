...
<span id="sejour-edit-patient_infos"{{if 'dPfacturation'|module_active && "dPplanningOp CSejour fields_display assurances"|gconf}} style="display: inline-block; width: 50%; vertical-align: top;"{{/if}}>
  <fieldset>
    <legend>Informations bénéficiaire</legend>
    <table class="form">
    <tr>
      <th>
        {{mb_label object=$patient field=cmu}}
      </th>
      <td>
        {{mb_field object=$patient field=cmu onchange="DHE.sejour.syncViewFlag(this); DHE.sejour.syncPatientField(this);"}}
      </td>
    </tr>
      {{if $conf.ref_pays != "2"}}
        <tr>
    ...
    <fieldset>
      ...
      <table class="form">
        ...
        <tr>
          ...
          <td>
            <input type="text" name="_assurance_maladie_view" value="{{if $sejour->_ref_factures|@count}}{{$sejour->_ref_facture->_ref_assurance_maladie->nom}}{{/if}}">
            ...
            <button type="button" class="cancel notext" onclick="DHE.emptyField(this.form._assurance_maladie);">{{tr}}Empty{{/tr}}</button>
          </td>
        </tr>
        <tr>
          <th>
