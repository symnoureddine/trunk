...
<table class="form">
  ...
  {{if $conf.ref_pays == '2'}}
    <tbody id="sejour-edit-reanimation-fields"{{if $sejour->reanimation != '1'}} style="display: none;"{{/if}}>
      <tr>
        <th class="halfPane">
          {{mb_label object=$sejour field=technique_reanimation_status}}
        </th>
        <td>
          {{mb_field object=$sejour field=technique_reanimation_status typeEnum='radio'}}
        </td>
      </tr>
      <tr>
        <th class="halfPane"></th>
        <td>
          {{mb_field object=$sejour field=technique_reanimation rows=3 form=sejourEdit aidesaisie="validateOnBlur: 0"}}
        </td>
      </tr>
    </tbody>
  {{/if}}
