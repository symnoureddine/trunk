    ...
    <table class="form">
      ...
      <tr>
        ...
        <td>
          {{if $conf.dPplanningOp.CSejour.use_custom_mode_entree && $modes_entree|@count}}
            ...
            <select name="mode_entree_id" class="{{$sejour->_props.mode_entree_id}}" style="width: 15em;" onchange="DHE.sejour.syncView(this, this.down('option:selected').innerHTML); $V(this.form.elements['mode_entree'], this.down('option:selected').get('mode'), true);">
      ...
      <tr>
        ...
        <td>
          ...
          <div id="_adresse_par_prat"{{if $sejour->adresse_par_prat_id && $patient->_ref_medecin_traitant && $sejour->_ref_adresse_par_prat == $patient->_ref_medecin_traitant->_id}} style="display: none;"{{/if}}>
            {{if $sejour->adresse_par_prat_id}}
              Autres : {{$sejour->_ref_adresse_par_prat}}
            {{/if}}
          </div>
        </td>
      </tr>
