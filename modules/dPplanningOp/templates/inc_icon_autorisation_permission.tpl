...
{{if $last_autorisation_permission && $last_autorisation_permission->_id}}
  ...
  {{if !$curr_affectation->_in_permission && ($last_autorisation_permission->debut < $dtnow) && ($last_autorisation_permission->_fin > $dtnow)}}
    {{assign var=perm_class  value="texticon-perm-progress"}}
  {{elseif $curr_affectation->_in_permission}}
