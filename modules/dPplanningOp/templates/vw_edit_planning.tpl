...
{{if "dPplanningOp COperation protocole_mandatory"|gconf && !$op->_id}}
  {{assign var=protocole_mandatory value=1}}
{{/if}}
...
<table class="main me-margin-bottom-40">
  ...
  >
    ...
    {{if $op->_id}}
      ...
      {{/if}}
    {{else}}
      <button id="didac_submit_interv" class="submit me-primary" type="button" onclick="createOperation();">{{tr}}Create{{/tr}}</button>
      {{assign var=types_forbidden value=","|explode:"Médecin"}}
      {{if $conf.dPplanningOp.CSejour.use_recuse && "reservation"|module_active && !$app->_ref_user->isFromType($types_forbidden)}}
        <button type="button" class="submit" onclick="createOperation('recuse');">{{tr}}Create{{/tr}} {{tr}}and{{/tr}} {{tr}}Validate{{/tr}}</button>
      {{/if}}
    {{/if}}
    </td>
  </tr>
