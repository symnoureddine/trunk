...
function changeModeEntree(mode_entree){
  ...
  Admissions.changeProvenance(mode_entree.form);
}
...
checkTypePec = function(field) {
  if (getForm('editSejour')._patient_sexe) {
    if ($V(field) == 'O' && $V(getForm('editSejour')._patient_sexe) == 'm') {
      field.up('td').insert(DOM.div({id: 'alert_type_pec', class: 'small-warning'}, $T('CSejour-msg-type_pec-O_for_male')));
    }
    else {
      if ($('alert_type_pec')) {
      ...
      for (var sejour_id in sejours_collision){
        ...
        if ((entree <= date_plage) && (sortie >= date_plage)) {
          if (sejour_courant_id != sejour_id){
            ...
            if ((operation_timing < datetime_entree) || (operation_timing > datetime_sortie)) {
...
createSubSejour = function() {
  ...
  if (window.ask_next_sejour) {
    askNDA(form, function() { form.submit(); })
  }
  else {
    form.submit();
  }
...
<tr>
  ...
  {{tr}}CSejour-annule{{/tr}}
    ...
    {{if $sejour->motif_annulation}}
      (<span title="{{$sejour->rques_annulation}}">{{mb_value object=$sejour field=motif_annulation}}</span>)
    {{/if}}
  </th>
</tr>
{{/if}}
...
<tr>
  ...
  <td colspan="3" id="selectSejours">
    <select name="sejour_id" style="width: 15em" onchange="reloadSejour()">
      ...
      <option value="{{$curr_sejour->_id}}" {{if $sejour->_id == $curr_sejour->_id}}selected{{/if}}>
        {{$curr_sejour->_view}}
...
<tr>
  ...
  <td colspan="3">
    ...
    {{/if}}
    <br />
    <input type="text" name="_seek_patient" style="width: 13em;" placeholder="{{tr}}fast-search{{/tr}} {{tr}}CPatient{{/tr}}" "autocomplete" onblur="$V(this, '')"/>
...
<tr>
  ...
  <td colspan="3">
    {{if $protocole_autocomplete}}
      <input type="text" name="libelle" value="{{$sejour->libelle}}" style="width: 20em;"{{if !$sejour->_id && !$mode_operation}} class="notNull"{{/if}} />

      <button class="search notext me-tertiary" type="button" onclick="ProtocoleSelector.init();">Choisir un protocole</button>

      {{if $mutation || $dhe_mater}}
        <button type="button" id="fast_operation_{{$sejour->_id}}" class="new me-tertiary" onclick="addFastOperation(this);">Ajouter une intervention hors plage</button>
      {{/if}}
    ...
    {{else}}
      {{mb_field object=$sejour field="libelle" form="editSejour" style="width: 20em" autocomplete="true,1,50,true,true"}}
    {{/if}}
  </td>
</tr>
...
<tr {{if !$conf.dPplanningOp.CSejour.easy_cim10 || $mutation}}class="modeExpert"{{/if}}>
  ...
  <td colspan="3">
    ...
    <input type="text" name="keywords_code" class="autocomplete str code cim10" value="{{$sejour->DP}}" onchange="Value.synchronize(this, 'editSejour{{if $mode_easy === "2col"}}Easy{{/if}}');" style="width: 12em" />
    <button type="button" class="cancel notext me-tertiary me-dark" onclick="$V(this.form.DP, '');"></button>
    <button type="button" class="search notext me-tertiary" onclick="CIM.viewSearch($V.curry(this.form.elements['DP']), $V(this.form.elements['praticien_id']){{if 'dPcim10 diagnostics restrict_code_usage'|gconf}}, null, null, null, $V.curry(getForm('editSejour').elements['type']), 'dp'{{/if}});">{{tr}}button-CCodeCIM10-choix{{/tr}}</button>
    <input type="hidden" name="DP" value="{{$sejour->DP}}" onchange="$V(this.form.keywords_code, this.value); Value.synchronize(this, 'editSejour{{if $mode_easy === "2col"}}Easy{{/if}}');"/>
  </td>
</tr>
...
<tr {{if !$conf.dPplanningOp.CSejour.easy_cim10 || $mutation}}class="modeExpert"{{/if}}>
  ...
  <td colspan="3">
    ...
    <input type="text" name="DR_keywords_code" class="autocomplete str code cim10" value="{{$sejour->DR}}" onchange="Value.synchronize(this, 'editSejour{{if $mode_easy === "2col"}}Easy{{/if}}');" style="width: 12em" />
    <button type="button" class="cancel notext me-tertiary me-dark" onclick="$V(this.form.DR, '');"></button>
    <button type="button" class="search notext me-tertiary" onclick="CIM.viewSearch($V.curry(this.form.elements['DR']), $V(this.form.elements['praticien_id']){{if 'dPcim10 diagnostics restrict_code_usage'|gconf}}, null, null, null, $V.curry(getForm('editSejour').elements['type']), 'dr'{{/if}});">{{tr}}button-CCodeCIM10-choix{{/tr}}</button>
    <input type="hidden" name="DR" value="{{$sejour->DR}}" onchange="$V(this.form.DR_keywords_code, this.value); Value.synchronize(this, 'editSejour{{if $mode_easy === "2col"}}Easy{{/if}}');"/>
  </td>
</tr>
...
<tbody id="ald_patient" {{if !$conf.dPplanningOp.CSejour.easy_ald_cmu || $mutation}}class="modeExpert"{{/if}} {{if !"dPplanningOp CSejour fields_display show_cmu_ald"|gconf}}style="display: none;"{{/if}}>
  ...
  <td colspan="3">
    ...
    {{else}}
      <select name="service_id" class="{{$sejour->_props.service_id}}" style="width: 15em"
        onchange="Value.synchronize(this, 'editSejourEasy', false); updateUFSoins(this);$V(this.form.keywords, '');">
        <option value="">&mdash; {{tr}}Choose{{/tr}}</option>
        {{foreach from=$listServices item=_service}}
...
<tr>
  {{if $conf.dPplanningOp.CSejour.show_only_charge_price_indicator && $use_charge_price_indicator != "no"}}
    ...
    <td>
      ...
      <select class="ref{{if $use_charge_price_indicator == "obl"}} notNull{{/if}}" name="charge_id" onchange="updateCPI(this)">
  ...
  <td colspan="2" rowspan="{{if $use_charge_price_indicator != "no" && !$conf.dPplanningOp.CSejour.show_only_charge_price_indicator}}2{{/if}}">
    <table>
...
<tr {{if !"dPplanningOp CSejour fields_display show_hospit_de_jour"|gconf || $mutation}}style="display: none;"{{/if}}>
  <th>{{mb_label object=$sejour field="hospit_de_jour"}} {{if $hdj_seance}}{{tr}}CSejour-Hdj / Seance{{/tr}}{{/if}}</th>
  <td>
...
{{if $count_etab_externe}}
  <tr id="listEtabTransfertEntree" {{if $sejour->mode_entree != 7}}style="display:none;"{{/if}}>
    ...
    <td colspan="3">
      ...
      <input type="text" name="etablissement_entree_id_view" value="{{$sejour->_ref_etablissement_provenance}}"/>
    </td>
  </tr>
{{/if}}
  ...
  <td>
    {{if $can->view}}
      ...
      <div id="listEtabExterne" {{if !$sejour->etablissement_sortie_id}}style="display:none"{{/if}}>
        ...
        <input type="text" name="etablissement_sortie_id_view" value="{{$sejour->_ref_etablissement_transfert}}" style="width: 12em;"/>
...
<tr id="correspondant_medical">
  ...
  {{mb_include module=planningOp template=inc_check_correspondant_medical use_meff=false}}
</tr>
<tr>
  <td></td>
  <td colspan="3">
  ...
  <tr {{if !$conf.dPplanningOp.CSejour.easy_chambre_simple}}class="modeExpert"{{/if}}>
    ...
    <td colspan="3">
      <select name="prestation_id" style="width: 15em;" onchange="checkPresta();">
        ...
        <option value="{{$_prestation->_id}}" {{if $sejour->prestation_id == $_prestation->_id}}selected{{/if}}>{{$_prestation}}</option>
        {{/foreach}}
      </select>
    </td>
...
<tr {{if $mutation}}style="display: none;"{{/if}}>
  ...
  {{if $conf.ref_pays == "2"}}
    <td class="text" colspan="2">
      {{mb_label object=$sejour field="technique_reanimation_status"}}
...
<tr {{if $mutation}}style="display: none;"{{/if}}>
  ...
  {{if $conf.ref_pays == "2"}}
    <td colspan="2" {{if $sejour->technique_reanimation}}style=""{{/if}}>
...
{{if "dPplanningOp CSejour fields_display assurances"|gconf && !$mutation && $conf.ref_pays == "2"}}
    ...
    {{if $sejour->_id}}
      {{if !$sejour->sortie_reelle || $can->admin}}
        ...
        {{if !$sejour->entree_preparee && $modules.dPadmissions->_can->edit}}
          <button class="tick me-secondary" type="submit" onclick="$V(this.form.entree_preparee, 1)">{{tr}}CSejour-entree_preparee{{/tr}}</button>
        {{/if}}

        {{if !$sejour->annule && $sejour->type === "seances" && "dPplanningOp CSejour hospit_seance"|gconf}}
          <button type="button" class="new me-secondary" onclick="Modal.open('hospitalize_modal')">{{tr}}CSejour-Hospitalize{{/tr}}</button>
        {{/if}}
        ...
        {{if !$sejour->annule && "reservation"|module_active && $conf.dPplanningOp.CSejour.use_recuse}}
          ...
          {{if !$current_user->isFromType($types_forbidden)}}
            {{if $sejour->recuse == "-1"}}
              <button type="button" class="tick me-tertiary" onclick="$V(this.form.recuse, 0); this.form.submit();">
                {{tr}}Validate{{/tr}}
              </button>
            {{elseif $sejour->recuse == "0"}}
              <button type="button" class="cancel me-tertiary" onclick="$V(this.form.recuse, -1); this.form.submit();">
        ...
        {{if !$conf.dPplanningOp.CSejour.delete_only_admin || $can->admin}}
          <button class="trash me-tertiary" type="button" onclick="confirmDeletion(this.form,{typeName:'le {{$sejour->_view|smarty:nodefaults|JSAttribute}}'});">
            {{tr}}Delete{{/tr}}
          </button>
        {{/if}}

        <button class="print me-tertiary me-dark" type="button" onclick="printFormSejour();">{{tr}}Print{{/tr}}</button>
      {{else}}
        <div class="big-info">
      ...
      {{/if}}
    {{else}}
      <button id="didac_button_create" class="submit me-primary" type="button"
      ...
      {{if $conf.dPplanningOp.CSejour.use_recuse}}
        ...
        {{if "reservation"|module_active && !$current_user->isFromType($types_forbidden)}}
          <button type="button" class="submit me-secondary" onclick="createSejour('recuse');">
