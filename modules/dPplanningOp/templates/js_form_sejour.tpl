  ...
  url.requestUpdate(SystemMessage.id,
    {
      insertion: function(receiver, text){
        {{if "dPplanningOp CSejour fields_display show_cmu_ald"|gconf}}
        ...
        {{/if}}
        {{if $conf.dPplanningOp.CSejour.easy_ald_cmu}}
...
changePat = function() {
  ...
  {{if "appFineClient"|module_active && "appFineClient Sync allow_appfine_sync"|gconf}}
    var oForm = getForm("editSejour");
    appFineClient.refreshButtonPackDemandeDHE($V(oForm.patient_id), "CSejour-"+$V(oForm.sejour_id));
    appFineClient.refresButtonAccountAppFine($V(oForm.patient_id), "CSejour-"+$V(oForm.sejour_id));
  {{/if}}
};
...
setTutelle = function(input) {
  ...
  if ($V(form.patient_id)) {
    var input = DOM.input({type: 'hidden', name: 'tutelle'});
    form.insert(input);
    $V(form.tutelle, $V(input));
    return onSubmitFormAjax(form, function() { input.remove(); });
  }
};
