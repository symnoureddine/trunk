...
<script>
  ...
  Main.add(function() {
    ...
    ProtocoleDHE.updateListCPI(form, '{{$conf.dPplanningOp.CSejour.show_only_charge_price_indicator}}');
    {{/if}}
  });
</script>
...
<form name="editProtocole" action="?m={{$m}}" method="post" class="{{$protocole->_spec}}"
  ...
  <table class="form">
    ...
    <tr>
      ...
      <td>{{mb_field object=$protocole field=facturation_rapide onchange="ProtocoleDHE.toggleCodageButton();"}}</td>
    </tr>
  </table>
