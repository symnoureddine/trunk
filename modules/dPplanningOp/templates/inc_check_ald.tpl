...
<tr>
  ...
  <td colspan="3" class="text me-ws-nowrap">
    <script>
      setAldCmu = function(curr_form, field) {
        ...
        return onSubmitFormAjax(pat_form, function() { input.remove(); });
      };
    </script>

    <!-- Patient sous CMU -->
   <span class="me-margin-right-4 {{if $circled}} circled{{/if}}">
     <label title="Patient bénéficiant du régime dde couverture maladie universelle">
       Patient sous CMU
       {{if $patient && $patient->_id}}
         {{mb_field object=$sejour field=_cmu_pat typeEnum=checkbox onchange="setAldCmu(this.form, 'cmu');" default=$patient->cmu}}
       {{else}}
         <input name="__cmu_pat" type="checkbox" disabled />
    ...
    {{if $conf.ref_pays != "2"}}
     <span class="me-margin-right-4 {{if $circled}}circled{{/if}}">
       ...
       {{if $patient && $patient->_id}}
         {{mb_field object=$sejour field=_ald_pat typeEnum=$type_enum_ald_pat onchange="setAldCmu(this.form, 'ald');" default=$patient->ald}}
       {{else}}
     ...
     <span class="me-margin-right-4 {{if $circled}}circled{{/if}}">
       ...
       <label for="ald" class="{{$class_sejour_ald}}" title="{{tr}}CSejour-ald-desc{{/tr}}">{{tr}}CSejour-ald{{/tr}}</label>
       ...
       {{if $ald_mandatory && $patient && $patient->_id}}
         <label>
           <input type="radio" name="ald" value="1" {{if $ald_sejour_disabled}}disabled{{/if}} class="bool {{$class_sejour_ald}}"
         ...
         <label>
           <input type="radio" name="ald" value="0" {{if $ald_sejour_disabled}}disabled{{/if}} class="bool {{$class_sejour_ald}}"
    ...
    <span class="me-margin-right-4 {{if $circled}}circled{{/if}}">
      <label title="{{tr}}CSejour-acs-msg-Patient receiving support for a complementary health{{/tr}}">
        {{tr}}CSejour-acs{{/tr}}
        {{if $patient && $patient->_id}}
          {{mb_field object=$sejour field=_acs_pat typeEnum=checkbox onchange="setAldCmu(this.form, 'acs');" default=$patient->acs}}
        {{else}}
          <input name="__acs_pat" onclick="setAldCmu(this.form, 'acs');" type="checkbox" disabled />
