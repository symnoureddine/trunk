...
<table class="form me-small-form">
    ...
    <td colspan="{{$colspan}}">
      <table class="layout" style="width: {{$easy_length_input_label}}%; box-sizing: border-box;">
        <tr>
          ...
          <td style="padding: 0;" class="narrow me-valign-middle">
  ...
  {{if $mode_easy === "1col" && $conf.dPplanningOp.CSejour.easy_cim10}}
    <tr>
      ...
      <td colspan="{{$colspan}}">
        ...
        <button type="button" class="cancel notext me-tertiary me-dark" onclick="$V(this.form.DP, '');">{{tr}}Cancel{{/tr}}</button>
        <button type="button" class="search notext me-tertiary" onclick="CIM.viewSearch($V.curry(this.form.elements['DP']), $V(this.form.elements['chir_id']){{if 'dPcim10 diagnostics restrict_code_usage'|gconf}}, null, null, null, $V.curry(getForm('editSejour').elements['type']), 'dp'{{/if}});">{{tr}}button-CCodeCIM10-choix{{/tr}}</button>
    ...
    <tr>
      ...
      <td colspan="{{$colspan}}">
        ...
        <button type="button" class="search notext me-tertiary" onclick="CIM.viewSearch($V.curry(this.form.elements['DR']), $V(this.form.elements['chir_id']){{if 'dPcim10 diagnostics restrict_code_usage'|gconf}}, null, null, null, $V.curry(getForm('editSejour').elements['type']), 'dr'{{/if}});">{{tr}}button-CCodeCIM10-choix{{/tr}}</button>
        ...
        <select name="type_anesth" style="width: 15em;" onchange="Value.synchronize(this)">
          ...
          {{foreach from=$listAnesthType item=curr_anesth}}
            {{if $curr_anesth->actif || $op->type_anesth == $curr_anesth->type_anesth_id}}
              <option value="{{$curr_anesth->type_anesth_id}}" {{if $op->type_anesth == $curr_anesth->type_anesth_id}}selected{{/if}}>
  ...
  {{else}}
    <tr>
      ...
      <td colspan="{{$colspan}}">
        ...
        <button id="didac_plage_op_select_button" type="button" class="search notext me-tertiary" onclick="PlageOpSelector.init()">Choisir une date</button>
  ...
  <tr>
    ...
    <td colspan="{{$colspan}}">
      ...
      />
      {{if $conf.dPplanningOp.CSejour.patient_id || !$sejour->_id || $app->user_type == 1}}
        <button id="didac_patient_select_button" type="button" class="search notext me-tertiary" onclick="PatSelector.init()">Choisir un patient</button>
  ...
  {{if "dPplanningOp CSejour show_tutelle"|gconf}}
    <tr{{if !$conf.dPplanningOp.CPatient.easy_tutelle}} style="display: none;"{{/if}}>
      <th><label for="_tutelle" title="{{tr}}CPatient-tutelle-desc{{/tr}}">{{tr}}CPatient-tutelle{{/tr}}</label></th>
      <td colspan="{{$colspan}}">
  ...
  {{if $conf.dPplanningOp.CPatient.easy_aide_organisee}}
    <tr>
      ...
      <td colspan="{{$colspan}}">{{mb_field object=$sejour field=aide_organisee onchange="Value.synchronize(this, 'editSejour');"}}</td>
    </tr>
  {{/if}}
  ...
  {{if $conf.dPplanningOp.CSejour.easy_mode_sortie}}
    <tr>
      ...
      <td colspan="{{$colspan}}">{{mb_field object=$sejour field=mode_sortie onchange="Value.synchronize(this, 'editSejour');"}}</td>
    </tr>
  {{/if}}
  ...
  {{if $mode_easy === "1col" && $conf.dPplanningOp.CSejour.easy_chambre_simple && "dPhospi prestations systeme_prestations"|gconf == "standard"}}
    <tr>
      ...
      <td colspan="{{$colspan}}">{{mb_field object=$sejour field="chambre_seule" onchange="checkChambreSejourEasy()"}}</td>
    </tr>
  {{/if}}
  ...
  {{if $conf.dPplanningOp.CSejour.easy_chambre_simple || $conf.dPplanningOp.COperation.easy_regime || "dPbloc CPlageOp systeme_materiel"|gconf == "expert" || "dPhospi prestations systeme_prestations"|gconf == "expert"}}
    <tr>
      ...
      {{if $conf.dPplanningOp.CSejour.easy_chambre_simple && "dPhospi prestations systeme_prestations"|gconf == "expert" && $sejour->_id}}
        <button type="button" class="search me-tertiary" onclick="Prestations.edit('{{$sejour->_id}}')">Prestations</button>
      {{/if}}
  ...
  <tr>
    ...
    <td colspan="{{$colspan}}">{{mb_field object=$sejour field=consult_accomp typeEnum=radio onchange="checkConsultAccompSejourEasy()"}}</td>
  </tr>
  {{/if}}
  ...
  <tr>
    ...
    <td colspan="{{$colspan}}">{{mb_field object=$sejour form="editOpEasy" field="date_accident" register=true onchange="checkAccidentEasy();"}}</td>
  </tr>
  <tr>
    <th>{{mb_label object=$sejour field="nature_accident"}}</th>
    <td colspan="{{$colspan}}">{{mb_field object=$sejour field="nature_accident" emptyLabel="Choose" style="width: 15em;" onchange="checkAccidentEasy();"}}</td>
  </tr>
  {{/if}}
  ...
  {{if "dPplanningOp CSejour fields_display assurances"|gconf && $conf.dPplanningOp.COperation.easy_assurances && $conf.ref_pays == "2"}}
