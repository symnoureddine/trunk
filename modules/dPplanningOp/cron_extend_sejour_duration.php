...
if ($limitation && !($time >= "$date 20:00:00" && $time <= "$date 23:59:59")) {
  CApp::log('Tâche exécutée hors de la plage de temps autorisée (20h-00h)', null, CLogger::LEVEL_WARNING);
}
else {
  $where = array(
    'group_id'      => " = $group->_id",
  ...
  foreach ($sejours as $_sejour) {
    ...
    $msg = $_sejour->store();
    if ($msg) {
      CApp::log("Séjour en erreur : '$_sejour->_guid', $msg", null, CLogger::LEVEL_WARNING);
    }
  }
}