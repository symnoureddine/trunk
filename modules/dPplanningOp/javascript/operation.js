...
Operation = {
  ...
  checkOperationInStay: function(oFormOperation, oFormSejour) {
    ...
    if (oFormOperation._locale_date) {
      if ($V(oFormOperation._locale_date).includes('h')) {
        operation_timing = Date.fromLocaleDateTime($V(oFormOperation._locale_date).replace(/h/, ':') + ':00').toDATETIME().replace('+', ' ');
      }
      else if (!$V(oFormOperation._locale_date).includes('h')) {
        var operation_timing_format =  $V(oFormOperation._locale_date).split('/');
        operation_timing =  operation_timing_format[2] + "-" + operation_timing_format[1] + "-" + operation_timing_format[0];
