...
<table class="tbl tooltip">
  ...
  {{if $mediuser->_is_praticien}}
    ...
    <tr>
      <td colspan="2">
        {{if $conf.ref_pays == 2}}
          {{mb_label class=CMediusersCompteCh field=rcc}} : {{if $mediuser->compte_ch_id}}{{mb_value object=$mediuser->loadRefCompteCh() field=rcc}}{{/if}}
        {{else}}
      ...
      <tr>
        <td colspan="3" class="button">
          <div>
            ...
            <a class="button search" href="?m=admin&tab=vw_edit_users&user_id={{$mediuser->_id}}&tab_name=edit_perms">
              {{tr}}common-Right|pl{{/tr}}
            </a>

            <a class="button search" href="?m=admin&tab=vw_edit_users&user_id={{$mediuser->_id}}&tab_name=edit_prefs">
