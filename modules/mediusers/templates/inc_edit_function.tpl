...
<script type="text/javascript">
  Main.add(function () {
    InseeFields.initCPVille("editFrm", "cp", "ville", null, "tel");
...
<form name="editFrm" method="post" onsubmit="return onSubmitFormAjax(this, {onComplete : Control.Modal.close});">
  ...
  <div id="CFunctions" style="display: none;">
    <table class="form">
      ...
      {{if $conf.ref_pays == 2}}
        <tr>
          <th>{{mb_label object=$function field=ean}}</th>
          <td>{{mb_field object=$function field=ean}}</td>
        </tr>
        <tr>
          <th>{{mb_label object=$function field=ean_xml_factu}}</th>
          <td>{{mb_field object=$function field=ean_xml_factu}}</td>
        </tr>
        <tr>
          <th>{{mb_label object=$function field=rcc}}</th>
          <td>{{mb_field object=$function field=rcc}}</td>
        </tr>
      {{/if}}
      <tr>
