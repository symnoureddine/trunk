...
<tr>
  ...
  <td>{{mb_field object=$object field="_user_sexe"}}</td>
</tr>
...
<tr>
  ...
  <td>{{mb_field object=$object field="activite"}}</td>
</tr>
  ...
  <td>
      ...
      {{if !$is_admin && $object->isAdmin()}} disabled{{/if}}>
        ...
        {{foreach from=$utypes key=curr_key item=type}}
          {{if $type !== "Patient" && ($type !== "ASSC" || $conf.ref_pays == 2)}}
            <option value="{{if $curr_key != 0}}{{$curr_key}}{{/if}}"
                    {{if $curr_key == $object->_user_type}}selected{{/if}}
              {{if (!$is_admin || !$is_admin_module) && $curr_key == 1}} disabled > {{$type}} ({{tr}}CMbFieldSpec.perm{{/tr}})
              {{else}} > {{$type}}{{/if}}
            </option>
          {{/if}}
        {{/foreach}}
...
<tr>
  ...
  <td>
    <select name="_profile_id" style="width: 150px;" {{if !$is_admin && !$is_admin_module && $object->user_id}}disabled{{/if}}>
      <option value="">&mdash; {{tr}}Choose{{/tr}}</option>
...
<tr>
  ...
  <td>
    {{if !$readOnlyLDAP}}
      {{mb_field object=$object field="_user_email" onkeyup='checkMailAndPassword(this);'}}
    {{else}}
