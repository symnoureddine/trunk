  ...
  {{foreach from=$import_users key=_file_name item=_infos}}
    <tr class="search-import-row">
      ...
      <td align="center" id="import-user-{{$_file_name}}">
        {{if $_infos.new}}
          <button class="search" type="button" onclick="ImportUsers.displayExistingUser('{{$_file_name}}', '{{$directory}}');">
            {{tr}}CUser-import-show-perms{{/tr}}
          </button>
          <button class="import" type="button" onclick="ImportUsers.importNewProfile('{{$_file_name}}', '{{$directory}}');">{{tr}}Import{{/tr}}</button>
        {{else}}
          <button class="search" type="button"
                  onclick="ImportUsers.displayExistingUser('{{$_file_name}}', '{{$directory}}');">{{tr}}CUser-import-show-compare{{/tr}}</button>
        {{/if}}
      </td>
    </tr>
  {{/foreach}}
