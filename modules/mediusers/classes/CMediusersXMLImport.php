...
class CMediusersXMLImport extends CMbXMLObjectImport {
  ...
  static $already_imported = array();

  static $_ignored_classes = array('CModule', 'CGroups');
   ...
   */
  function importFunction($element, $object) {
   ...
   */
  function importMediusers($element, $object) {
   ...
   */
  function importUF($element, $object) {
   ...
   */
  function importTarif($element, $object) {
    /** @var CTarif $_tarif */
   ...
   */
  function importPreferences($element, $object) {
    ...
    if ($_pref->_id && (($_pref->restricted && !$this->options['update_perms_functionnal'])
   ...
   */
  function importPermObject($element, $object) {
   ...
   */
  function importPermModule($element, $object) {
   ...
   */
  function importUser($element, $object) {
    /** @var CUser $_user */
   ...
   * @return CUser|null
   */
  function getProfileFromXML($load_matching = true) {
   ...
   * @return string
   */
  function getHashFromXML($hash_type) {
   ...
   * @return array
   */
  function compareProfileFromXML() {
    $compare = array(
   ...
   * @return array
   */
  function getObjectsFromXML($class, $user_field = null, $ignore_refs = false) {
  ...
  function comparePermsObject($user, $new_perms_object) {
    ...
    foreach ($new_perms_object as $_perm) {
      if (!array_key_exists($_perm->object_class, $perms_object)) {
        $perms_object[$_perm->object_class] = array(
  ...
  function comparePermsModule($user, $new_perms_module) {
    ...
    foreach ($new_perms_module as $_perm) {
      ...
      elseif (array_key_exists($_perm->mod_id, self::$modules) && !array_key_exists(self::$modules[$_perm->mod_id], $perms_module)) {
        $perms_module[self::$modules[$_perm->mod_id]] = array(
          'new' => array(
            'perm' => $_perm->permission,
            'view' => $_perm->view,
          ),
   ...
   * @return void
   */
  function updateProfile($permissions = array(), $options = array()) {
  ...
  function parsePermissions($permissions) {
    $objects = array(
      'perms_module'      => $this->sanitizePermsModule($permissions['perms_module'], $permissions['perms_module_view']),
      ...
      'perms_functionnal' => $this->sanitizeOthers($permissions['perms_functionnal'], 'use_perms_functionnal_', '_prefs'),
    );
  ...
  function sanitizeOthers($objects, $prefix, $suffix) {
    $objects_return = array();

    foreach ($objects as $_obj) {
      $_obj    = str_replace(array($prefix, $suffix), '', $_obj);
