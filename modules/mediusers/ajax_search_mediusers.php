...
if (($ldap_bound || $ldap_bound === '0') && !is_array($ldap_bound)) {
  ...
  if ($ldap_tag) {
    if ($ldap_bound === '0') {
      $ljoin['id_sante400']           = $ds->prepare('id_sante400.object_id = users.user_id AND id_sante400.object_class = ?1 AND id_sante400.tag = ?2', 'CUser', $ldap_tag);
      $where['id_sante400.object_id'] = 'IS NULL';
    }
    else {
      $ljoin["id_sante400"]              = "id_sante400.object_id = users.user_id";
      $where["id_sante400.object_class"] = " = 'CUser'";
      $where["id_sante400.tag"]          = $ds->prepare('= ?', $ldap_tag);
    }
  }
}
