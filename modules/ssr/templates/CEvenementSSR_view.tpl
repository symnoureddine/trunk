...
{{assign var=actes_tarmed        value=$evenement_ssr->_refs_tarmeds}}
{{assign var=counter_tarmeds     value=$evenement_ssr->_counter_tarmeds}}
...
<table class="tbl tooltip">
  {{if $evenement_ssr->sejour_id}}
    ...
    {{if $prestas_ssr|@count || $actes_tarmed|@count}}
      <!-- Prestations SSR -->
      ...
      {{/if}}
      <!-- Actes Tarmed -->
      {{if $actes_tarmed|@count}}
        <tr>
          <td class="text">
            <strong>{{tr}}CEvenementSSR-back-tarmeds{{/tr}}</strong> :
            {{foreach from=$counter_tarmeds key=code_tarmed item=_tarmed name=tarmeds}}
              {{$code_tarmed}} {{if $counter_tarmeds.$code_tarmed.quantity}}(x {{$counter_tarmeds.$code_tarmed.quantity}}){{/if}}{{if !$smarty.foreach.tarmeds.last}}, {{/if}}
            {{/foreach}}
          </td>
        </tr>
      {{/if}}
    {{elseif "ssr general use_acte_presta"|gconf == 'presta_tarmed' && (!$prestas_ssr|@count || !$actes_tarmed|@count)}}
      <tr>
        <td>
          <div class="small-warning">
