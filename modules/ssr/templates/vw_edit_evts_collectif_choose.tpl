      ...
      <td>
        ...
        <form name="editEvenementSSR-{{$unique_id_collectif}}" method="post" action="?">
          ...
          <script>
            Main.add(function(){
              ...
              {{if "tarmed"|module_active}}
                ...
                if ($('code_tarmed_autocomplete-{{$unique_id_collectif}}')) {
                  var url = new Url("tarmed", "ajax_do_tarmed_autocomplete");
                  url.autoComplete(formSSR.code_tarmed, "code_tarmed_autocomplete", {
                    dropdown: true,
                    minChars: 2,
                    select: "newcode",
                    width: "250px",
                    updateElement: function(selected) {
                      Planification.updateFieldCodesSSR(selected, 'tarmed', '{{$unique_id_collectif}}');
                    }
                  } );
                }
              {{/if}}
          ...
          <!-- prestation SSR -->
            ...
            {{if "tarmed"|module_active}}
              <!-- actes Tarmed -->
              <fieldset class="line_tarmed" style="width: 40%;float: left;">
                <legend>{{tr}}CTarmed{{/tr}}</legend>
                <div id="tarmeds-{{$line_element->_id}}" class="tarmeds">
                  {{foreach from=$line_element->_ref_element_prescription->_refs_tarmeds item=_tarmed}}
                    <label>
                      <input type="checkbox" class="checkbox-tarmeds nocheck" name="tarmeds[{{$_tarmed->code}}]" value="{{$_tarmed->code}}"/>
                      <span onmouseover="ObjectTooltip.createEx(this, '{{$_tarmed->_guid}}')">{{$_tarmed->code}}</span>
                    </label>

                    (<span title="{{tr}}CActePrestationSSR-Amount of code to add{{/tr}}">x</span>
                    <input type="text" id="tarmeds_quantity_{{$_tarmed->code}}"
                           name="tarmeds_quantity[{{$_tarmed->code}}]" value="{{$_tarmed->quantite}}" style="width: 17px;" />)

                    <script>
                      Main.add(function () {
                        $('tarmeds_quantity_{{$_tarmed->code}}').addSpinner({min: 0});
                      });
                    </script>
                  {{/foreach}}
                </div>
                ...
                <div id="div_other_tarmed">
                  <label>
                    <input type="checkbox" name="_tarmed" value="other" onclick="Planification.toggleOtherTarmed(this, '{{$unique_id_collectif}}');" /> {{tr}}Other{{/tr}}:
                  </label>
                  <span id="other_tarmed{{$unique_id_collectif}}" style="display: none;">
                       <input type="text" name="code_tarmed" class="autocomplete" canNull=true size="2" />
                       <div style="display: none;" class="autocomplete" id="code_tarmed_autocomplete-{{$unique_id_collectif}}"></div>
                    </span>
                </div>
              </fieldset>
            {{/if}}
