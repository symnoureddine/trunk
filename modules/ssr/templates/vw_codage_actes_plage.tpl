  ...
  Main.add(function(){
    ...
    if ($('code_tarmed_autocomplete')) {
      ...
      url.autoComplete(oFormEvenementSSR.code_tarmed, "code_tarmed_autocomplete", {
        ...
        width: "250px",
        updateElement: function(selected) {
          Planification.updateFieldCodesSSR(selected, 'tarmed', null, true);
        }
      } );
    }
    {{/if}}
    ...
    {{/if}}
    {{if $plage->_ref_actes_other.tarmed|@count}}
      $('Edit-'+'{{$plage->_guid}}'+'__tarmed').click();
    {{/if}}
  });
</script>
...
{{if $use_acte_presta == "presta_tarmed"}}
  ...
  {{if "tarmed"|module_active}}
    ...
    <tr id='tr-tarmed'>
      <th>{{tr}}CTarmed{{/tr}}</th>
      <td class="text">
        <div id="tarmeds-{{$plage->_guid}}" class="tarmeds">
          {{foreach from=$plage->_ref_element_prescription->_refs_tarmeds item=_tarmed}}
            {{assign var=code_acte value=$_tarmed->code}}
            <label>
              <input type="checkbox" class="checkbox-tarmeds nocheck" name="_tarmeds[{{$_tarmed->code}}-{{$_tarmed->_guid}}]" value="{{$_tarmed->code}}"
                     {{if $_tarmed->_checked}}checked="checked"{{/if}}/>
              <span onmouseover="ObjectTooltip.createEx(this, '{{$_tarmed->_guid}}')">{{$_tarmed->code}}</span>
              (<span title="{{tr}}CActePrestationSSR-Amount of code to add{{/tr}}">x</span>
              <input type="text" name="_prestas_quantity[{{$_tarmed->code}}-{{$_tarmed->_guid}}]"
                     value="{{$_tarmed->_quantite}}"
                     id="Edit-{{$plage->_guid}}__prestas_quantity[{{$_tarmed->code}}-{{$_tarmed->_guid}}]"
                     style="width: 17px;" />)
              <script>
                Main.add(function () {
                  $('Edit-{{$plage->_guid}}__prestas_quantity[{{$_tarmed->code}}-{{$_tarmed->_guid}}]').addSpinner({min: 0.1});
                });
              </script>
            </label>
          {{/foreach}}
        ...
        <!-- Autre actes Tarmed -->
        <div id="div_other_tarmed">
          <label>
            <input type="checkbox" name="_tarmed" value="other" onclick="Planification.toggleOtherTarmed(this);"
                   id="Edit-{{$plage->_guid}}__tarmed"/> {{tr}}Other{{/tr}}:
          </label>
          <span id="other_tarmed" style="display: none;">
           ...
           <div style="display: none;" class="autocomplete" id="code_tarmed_autocomplete"></div>
            {{foreach from=$plage->_ref_actes_other.tarmed item=_acte_tarmed}}
              <span>
                <input type="hidden" name="_tarmeds[{{$_acte_tarmed->code}}-{{$_acte_tarmed->_guid}}]" value="{{$_acte_tarmed->code}}" class="checkbox-other-tarmeds">
                <button class="cancel notext" type="button" onclick="Planification.deleteCode(this)">{{tr}}Delete{{/tr}}</button>
                <label>{{$_acte_tarmed->code}}</label>
                (<span title="{{tr}}CActePrestationSSR-Amount of code to add{{/tr}}">x</span>
                <input type="text" name="_prestas_quantity[{{$_acte_tarmed->code}}-{{$_acte_tarmed->_guid}}]"
                       value="{{$_acte_tarmed->quantite}}"
                       id="Edit-{{$plage->_guid}}__prestas_quantity[{{$_acte_tarmed->code}}-{{$_acte_tarmed->_guid}}]"
                       style="width: 17px;" />)
                <script>
                  Main.add(function () {
                    $('Edit-{{$plage->_guid}}__prestas_quantity[{{$_acte_tarmed->code}}-{{$_acte_tarmed->_guid}}]').addSpinner({min: 0.1});
                  });
                </script>
              </span>
            {{/foreach}}
