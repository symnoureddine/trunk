...
<table class="tbl" style="margin-right: 10px;" id="list-evenements-modal">
  ...
  {{foreach from=$evenements item=_evenements_by_sejour key=sejour_id}}
    ...
    <tr>
      ...
      {{elseif $use_acte_presta == 'presta_tarmed'}}
        <th class="section">{{tr}}CEvenementSSR-back-actes_prestas{{/tr}} / {{tr}}CEvenementSSR-back-actes_tarmeds{{/tr}}</th>
      {{else}}
      ...
      <tr>
        ...
        <td class="text codes_container" data-evenement_id="{{$_evenement->_id}}">
          ...
          {{if $_evenement->seance_collective_id && (($use_acte_presta == 'csarr' && ($_evenement->_ref_actes_cdarr || $_evenement->_ref_actes_csarr))
            || ($use_acte_presta == 'presta_tarmed' && ($_evenement->_refs_prestas_ssr || $_evenement->_refs_tarmeds)))}}
          ...
          {{if $use_acte_presta == 'csarr'}}
            ...
            {{/if}}
          {{elseif $use_acte_presta == 'presta_tarmed'}}
            {{assign var=actes_tarmed        value=$_evenement->_refs_tarmeds}}
            {{assign var=counter_tarmeds     value=$_evenement->_counter_tarmeds}}
            ...
            {{if "tarmed"|module_active}}
              <div style="clear: both;">
                <strong>{{tr}}CEvenementSSR-back-actes_tarmeds{{/tr}}</strong> :
                {{foreach from=$counter_tarmeds key=code_tarmed item=_tarmed name=tarmeds}}
                  {{assign var=code_tarmed_ssr value=$counter_tarmeds.$code_tarmed}}

                  <code onmouseover="ObjectTooltip.createEx(this, 'CActePrestationSSR-{{$code_tarmed_ssr.acte_prestation_id}}')">{{$code_tarmed}}</code>
                  {{if $code_tarmed_ssr.quantity}}(x {{$code_tarmed_ssr.quantity}}){{/if}}{{if !$smarty.foreach.tarmeds.last}}, {{/if}}
                {{foreachelse}}
                  <span class="empty">{{tr}}CEvenementSSR-back-actes_tarmeds.empty{{/tr}}</span>
                {{/foreach}}
              </div>
            {{/if}}
          {{/if}}
