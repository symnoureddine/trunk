...
{{assign var=tarmed_therapeute_id value=""}}
{{assign var=lines_presta value=$rhs->_prestas_ssr}}
{{assign var=lines_tarmed value=$rhs->_tarmeds}}
...
<table class="tbl">
  ...
  {{foreach from=$lines_tarmed key=rhs_key item=_line}}
    {{assign var=therapeute value=$_line->_ref_executant}}

    {{if $therapeute->_id != $tarmed_therapeute_id}}
      <tr>
        <th class="text section" colspan="18" style="text-align: left;">
          {{mb_include module="mediusers" template="inc_vw_mediuser" mediuser=$therapeute}}
          &mdash;
          {{tr}}CEvenementSSR-back-tarmeds{{/tr}}
        </th>
      </tr>
    {{/if}}

    {{assign var=tarmed_therapeute_id value=$therapeute->_id}}
    <tr>
      <td>
        {{$_line->_ref_tarmed->code}}
      </td>
      <td>
        {{$_line->_ref_tarmed->type}}
      </td>
      <td>
        {{$_line->_ref_tarmed->libelle}}
      </td>
  ...
  {{if !$lines_tarmed|@count && !$lines_presta|@count}}
    <tr>
