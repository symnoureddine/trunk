...
<div id="view_groupe_patients">
  <form name="gestion_groupe_patients" method="post">
    ...
    <table class="main tbl">
      ...
      {{foreach from=$sejours item=_sejour}}
        ...
        {{foreach from=$lines_element item=_line name=list_element}}
          {{assign var=element_prescription value=$_line->_ref_element_prescription}}
          {{assign var=index                value=$smarty.foreach.list_element.index}}
          ...
          {{if in_array($element_prescription->_id, array_keys($elements_prescription_plage))}}
            ...
            {{assign var=executants  value=$element_prescription->_ref_executants_by_cat}}
            ...
            <tr class="elements_line_{{$_sejour->_id}} element_prescription_{{$element_prescription->_id}}_{{$_sejour->_id}}" style="display: none;">
              ...
              <th class="category" style="width: 10%; text-align: left !important; padding-left: 5px;">
                {{if $actes_csarr && $actes_csarr|@count}}
                  <input name="check_all_actes_{{$_sejour->_id}}" type="checkbox" onchange="GroupePatient.selectCheckboxes(this.form, $V(this), 'acte_selected_{{$element_prescription->_id}}_{{$_sejour->_id}}_{{$index}}');"/>
                {{/if}}
            ...
            {{foreach from=$actes_csarr item=_acte name=list_actes}}
              ...
              <tr class="elements_line_{{$_sejour->_id}} element_prescription_{{$element_prescription->_id}}_{{$_sejour->_id}}"
                ...
                <td>
                  <div id="order_arrow_{{$_acte->_id}}_{{$_sejour->_id}}_{{$index}}" style="display: none;">
                    <span class="arrow_up" style="display: block;">
                        <i class="fas fa-sort-up graph-rank up-rank-graph"
                           ...
                           onclick="GroupePatient.changeRank(this, 'up', '{{$actes_csarr|@count}}', 'line_csarr_{{$element_prescription->_id}}_{{$_sejour->_id}}_{{$acte_index}}{{$index}}');"></i>
                    </span>
                    <span class="arrow_down" style="display: block;">
                        <i class="fas fa-sort-down graph-rank down-rank-graph"
                           ...
                           onclick="GroupePatient.changeRank(this, 'down', '{{$actes_csarr|@count}}', 'line_csarr_{{$element_prescription->_id}}_{{$_sejour->_id}}_{{$acte_index}}{{$index}}');"></i>
                    </span>
                  </div>
                </td>
                <td colspan="7">
