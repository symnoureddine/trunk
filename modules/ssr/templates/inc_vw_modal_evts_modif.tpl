  ...
  submitCdarrs = function(){
    ...
    if (qtes.length == 0) {
      ...
      qtes.each(function(input) {
        ...
        var added_codes = new TokenField(form.added_codes);
        var remed_codes = new TokenField(form.remed_codes);
        var type_presta = input.hasClassName('tarmeds') ? 'tarmed' : 'presta_ssr';
    ...
    {{foreach from=$types item=_type}}
      ...
      url.autoComplete(getForm("editCodes").code_{{$_type}}, "other_{{$_type}}_auto_complete", {
        ...
        updateElement: function(selected) {
          {{if $_type == 'prestas' || $_type == 'tarmeds'}}
    ...
    <tr>
      ...
      {{if $use_acte_presta == "presta_tarmed"}}
        ...
        {{if "tarmed"|module_active}}
          <th class="narrow">{{tr}}CActeTarmed{{/tr}}</th>
        {{/if}}
      {{/if}}
    </tr>
    {{foreach from=$evenements item=_evenement}}
      <tr>
        ...
        {{if $use_acte_presta == "presta_tarmed"}}
          ...
          {{if "tarmed"|module_active}}
            <td style="text-align: center;">{{$_evenement->_refs_tarmeds|@count|nozero}}</td>
          {{/if}}
        {{/if}}
      </tr>
    {{foreachelse}}
