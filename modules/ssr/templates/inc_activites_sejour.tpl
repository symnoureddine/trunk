...
{{assign var=tarmed_active   value="tarmed"|module_active}}
...
<script>
  selectActivite = function(activite, use_acte_presta) {
    ...
    if (use_acte_presta == 'csarr') {
    ...
    else if (use_acte_presta == 'presta_tarmed'){
      // Presta
      ...
      {{if "tarmed"|module_active}}
        // Tarmed
        $('div_other_tarmed').hide();
        $('other_tarmed').hide();
        $V(oFormEvenementSSR.code_tarmed, '');
        $$(".code_tarmed").invoke('hide');
      {{/if}}
    }
  ...
  selectElement = function(line_id, use_acte_presta){
    ...
    if (use_acte_presta == 'csarr') {
      ...
      {{if "tarmed"|module_active}}
        ...
        $$(".tarmeds").invoke('hide');
        if (line_id !== "") {
          $("tarmeds-"+line_id).show();
        }
        $('div_other_tarmed').show();
      {{/if}}
    }
  ...
  hideCodes = function(use_acte_presta) {
    ...
    if (use_acte_presta == 'csarr') {
      ...
      {{if "tarmed"|module_active}}
        // Tarmed
        $V(oFormEvenementSSR._tarmed, false);
        $$('#other_tarmed span').invoke('remove');
        $('other_tarmed').hide();
      {{/if}}
    }
  };
  ...
  submitSSR = function(use_acte_presta){
    ...
    if (use_acte_presta == 'csarr') {
      ...
      {{if "tarmed"|module_active}}
        // Tarmed
        var other_tarmed = oFormEvenementSSR.select('input.checkbox-other-tarmeds').length;
        tarmed_count = ($V(oFormEvenementSSR.code_tarmed) && other_tarmed) ? 1 : 0;
        tarmed_count += oFormEvenementSSR.select('input.checkbox-tarmeds:checked').length;
        tarmed_count += other_tarmed;
      {{/if}}

      if (presta_ssr_count == 0 && tarmed_count == 0 && !oFormEvenementSSR.no_code_to_evt.checked && !patient_missing) {
        alert($T('CPrestaSSR-msg-Please select at least one SSR service or one tarmed act'));
        return false;
      }
    }
  ...
  Main.add(function(){
    ...
    {{if "tarmed"|module_active}}
      // Tarmed other code autocomplete
      if ($('code_tarmed_autocomplete')) {
        var url = new Url("tarmed", "ajax_do_tarmed_autocomplete");
        url.autoComplete(oFormEvenementSSR.code_tarmed, "code_tarmed_autocomplete", {
          dropdown: true,
          minChars: 2,
          select: "newcode",
          width: "250px",
          updateElement: function(selected) {
            Planification.updateFieldCodesSSR(selected, 'tarmed');
          }
        } );
      }
    {{/if}}
...
<div id="add_ssr">
  ...
  <form name="editEvenementSSR" method="post" action="?" onsubmit="return submitSSR('{{$use_acte_presta}}');">
    ...
    <table class="form me-no-align me-no-box-shadow me-small-form">
      ...
      <!-- Affichage des prestations SSR  et actes Tarmed -->
      {{if $use_acte_presta == "presta_tarmed"}}
      <!-- prestation SSR -->
        <tr id='tr-presta-ssr'>
        ...
        {{if "tarmed"|module_active}}
          ...
          <tr id='tr-tarmed'>
            ...
            <td class="text">
              ...
              {{if $prescription}}
                <!-- Affichage des actes tarmed -->
                {{foreach from=$prescription->_ref_prescription_lines_element_by_cat item=_lines_by_chap}}
                  {{foreach from=$_lines_by_chap item=_lines_by_cat}}
                    {{foreach from=$_lines_by_cat.element item=_line}}
                      <div id="tarmeds-{{$_line->_id}}" style="display : none;" class="tarmeds">
                        {{foreach from=$_line->_ref_element_prescription->_refs_tarmeds item=_tarmed}}
                          {{unique_id var=acte_id}}
                          <label>
                            <input type="checkbox" class="checkbox-tarmeds nocheck" value="{{$_tarmed->code}}"
                                   name="tarmeds[{{$_tarmed->code}}-{{$acte_id}}]" />
                            <span onmouseover="ObjectTooltip.createEx(this, '{{$_tarmed->_guid}}')">
                              {{$_tarmed->code}}
                            </span>
                          </label>

                          (<span title="{{tr}}CActePrestationSSR-Amount of code to add{{/tr}}">x</span>
                          <input type="text" id="tarmeds_quantity_{{$_tarmed->code}}_{{$acte_id}}"
                                 name="tarmeds_quantity[{{$_tarmed->code}}-{{$acte_id}}]" value="{{$_tarmed->quantite}}" style="width: 17px;" />)

                          <script>
                            Main.add(function () {
                              $('tarmeds_quantity_{{$_tarmed->code}}_{{$acte_id}}').addSpinner({min: 1});
                            });
                          </script>
                        {{/foreach}}
                      </div>
              ...
              <!-- Autre actes Tarmed -->
              <div id="div_other_tarmed" style="display: none;">
                <label>
                  <input type="checkbox" name="_tarmed" value="other" onclick="Planification.toggleOtherTarmed(this);" /> {{tr}}Other{{/tr}}:
                </label>
                <span id="other_tarmed" style="display: block;">
                   <input type="text" name="code_tarmed" class="autocomplete" canNull=true size="2" />
                   <div style="display: none;" class="autocomplete" id="code_tarmed_autocomplete"></div>
                </span>
              </div>
            </td>
          </tr>
