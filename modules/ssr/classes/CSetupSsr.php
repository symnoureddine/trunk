...
class CSetupSsr extends CSetup
{
    ...
    function __construct()
    {
      ...
      SET `sejour`.`hospit_de_jour` = `bilan_ssr`.`hospit_de_jour`;";
        ...
        // Data source query
        $query = "SELECT * FROM `intervenant` WHERE `libelle` LIKE '%MASSEUR KINeSITHERAPEUTE%'";
        $this->addDatasource("cdarr", $query);

        if (CAppUI::gconf("ssr general use_acte_presta") == 'csarr') {
            $query = "SELECT * FROM `activite` WHERE `code` = 'ZZQ+085' AND LENGTH(libelle) = 131 /* DETECTION CSARR BASE V2020 */";
            $this->addDatasource("csarr", $query);
        } elseif (CAppUI::gconf("ssr general use_acte_presta") == 'presta_tarmed') {
