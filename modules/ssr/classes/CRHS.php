...
class CRHS extends CMbObject {
  ...
  function recalculate() {
    ...
    foreach ($evenements as $_evenement) {
      ...
      $presta_ch["tarmed"]     = $_evenement->loadRefsActesTarmed();
  ...
  function buildTotaux() {
    ...
    foreach ($lines_by_executant as $executant_guid => &$_lines) {
        ...
        elseif (!in_array(null, CMbArray::pluck($_lines, "code_activite_csarr"))) {
            array_multisort(CMbArray::pluck($_lines, "code_activite_csarr"), SORT_ASC, $_lines);
        }
    }
  ...
  /**
   * Charge les actes Tarmed des lignes d'activité qui composent le RHS
   *
   * @return CLigneActivitesRHS[]
   */
  function loadRefActesTarmed() {
    $where_presta   = array();
    $where_presta[] = "type_activite = 'tarmed'";
    $this->_tarmeds = $this->loadRefLignesActivites($where_presta);

    foreach ($this->_tarmeds as $_line) {
      $_line->loadRefExecutant();
      $_line->loadRefTarmed();
    }

    return $this->_tarmeds;
  }
}
