...
class CPlageSeanceCollective extends CMbObject {
  ...
  function store() {
    ...
    if (!$this->_readonly) {
      ...
      if ($this->_tarmeds) {
        foreach ($this->_tarmeds as $key_tarmed => $_tarmed) {
          $this->_update_actes["tarmed"][] = $_tarmed."-".$this->_prestas_quantity[$key_tarmed];
        }
      }
      if ($this->_id) {
    ...
    foreach ($this->_ref_actes as $_acte) {
      switch ($_acte->type) {
        ...
        case "presta":
          ...
          break;
        case "tarmed":
          $this->_ref_tarmeds[$_acte->_id] = $_acte;
          break;
        default:
      }
    }
  ...
  function rangeActesOther() {
    ...
    if ($this->element_prescription_id) {
      foreach (array("csarr" => "_ref_csarrs", "presta" => "_refs_presta_ssr","tarmed" => "_refs_tarmeds") as $type => $_prop) {
