...
class CActiviteCsARR extends CCsARRObject {
  const GET_USED_CODES_FOR_CACHE = __CLASS__ . '::getUsedCodesFor';
  ...
  function loadRefsModulateurs() {
    ...
    if (!$this->code) {
      return $this->_ref_modulateurs = array();
    }
   ...
   * @return CActiviteCsARR[]
   */
  public static function findCodes($keywords, $code = null, $hierarchy = null, $where = array(), $start = 0, $rows = 20) {
   ...
   * @return int
   */
  public static function countResults($keywords, $code = null, $hierarchy = null, $where = array()) {
    if (!self::$search) {
      self::findCodes($keywords, $code = null, $hierarchy = null, $where = array());
    }
  ...
  public static function getUsedCodesFor($user, $keyword = null) {
    ...
    if (!$keyword && $cache->exists()) {
      ...
      $query->addWhereClause('acte_csarr.code', CSQLDataSource::prepareNotIn(CMbArray::pluck(CFavoriCsARR::getFor($user), 'code')));
  ...
  public static function resetUsedCodesCache($user_id) {
    $cache = new Cache(self::GET_USED_CODES_FOR_CACHE, array("CMediusers-$user_id"), Cache::INNER_OUTER);
    if ($cache->exists()) {
      $cache->rem();
    }
  }
