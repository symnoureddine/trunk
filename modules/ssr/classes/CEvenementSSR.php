...
class CEvenementSSR extends CMbObject {
  ...
  function changeLignesRhs($create, $delete, $therapeute) {
    ...
    foreach (array($this->_refs_prestas_ssr, $this->_refs_tarmeds) as $_prestas) {
  ...
  /**
   * Charge les actes Tarmed
   *
   * @return CActePrestationSSR[]
   */
  function loadRefsActesTarmed() {
    $tarmeds = $this->loadBackRefs("prestas_ssr", "code ASC", null, null, null, null, "prestas_tarmed", array("type" => " = 'tarmed'"));

    $this->_counter_tarmeds = array();
    foreach ($tarmeds as $_tarmed) {
      if (!isset($this->_counter_tarmeds[$_tarmed->code]['quantity'])) {
        $this->_counter_tarmeds[$_tarmed->code]['quantity'] = 0;
      }
      $this->_counter_tarmeds[$_tarmed->code]['quantity'] += $_tarmed->quantite;
      $this->_counter_tarmeds[$_tarmed->code]['acte_prestation_id'] = $_tarmed->_id;
    }
    return $this->_refs_tarmeds = $tarmeds;
  }
