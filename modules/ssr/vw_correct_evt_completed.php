...
$list_evts                                   = $evenement->loadList($where, "debut desc", $clean ? null : "$page, 50", "evenement_ssr.evenement_ssr_id", $ljoin);
...
if ($clean) {
  ...
  $list_evts       = $evenement->loadList($where, "debut desc", "$page, 50", "evenement_ssr.evenement_ssr_id", $ljoin);
  $list_evts_empty = $evenement->loadList($where2, "debut desc", null, "evenement_ssr.evenement_ssr_id", $ljoin2);
  $list_evts       = array_merge($list_evts, $list_evts_empty);
}
foreach ($list_evts as $_evt_collectif) {
