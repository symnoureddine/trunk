...
Planification = window.Planification || {
  ...
  toggleOtherTarmed:          function (elem, uniq_id) {
    var form = elem.form;
    var id_tarmed = uniq_id ? uniq_id : '';
    $('other_tarmed' + id_tarmed).setVisible($V(elem));
    $V(form.code_tarmed, '');
    $(form.code_tarmed).tryFocus();
  },
  ...
  updateFieldCodesSSR:        function (selected, type, uniq_id, trame) {
    ...
    var acte_id = type === 'presta_ssr' || type === 'tarmed' ? '-' + Math.ceil(Math.random() * 100000) : '';

    var element_quantity = null;

    if (code_selected.className != 'empty') {
      if (type === 'presta_ssr' || type === 'tarmed') {
