...
GroupePatient = {
    ...
    if ($V(element)) {
      ...
      if (filter_element_id > 0) {
        ...
        if ($$('.elements_line_' + sejour_id + ':not([style="display: none;"])').length == 0) {
          $('gestion_groupe_patients_patient_' + sejour_id).up('tr').hide();
        }
      } else {
   ...
   * @param index
   */
  addValues: function (form, acte_id, sejour_id, name, index) {
    var input_name_modulateur = name + '_' + acte_id + '_' + sejour_id + '_' + index;
    var input_name_acte = 'acte_csarr_' + acte_id + '_' + sejour_id + '_' + index;
    var modulateur_values = [$V(form.elements[input_name_modulateur])].flatten().join('|');

    if (name == 'modulateurs') {
      if (!form.elements[input_name_modulateur].length) {
        modulateur_values = $(form.name + '_' + input_name_modulateur).checked ? $(form.name + '_' + input_name_modulateur).value : '';
      }
    }
  ...
  checkElementsEnable: function (element, element_name) {
    ...
    var index    = element_name.split('_')[3];
    ...
    var elements_arrow = $('order_arrow_' + acte_id + '_' + sejour_id + '_' + index);
  ...
  confirmValidation: function (form) {
    ...
    getForm('gestion_groupe_patients').select('.add_acte_csarr:checked').each(function (add_acte_csarr) {
      if (!$('gestion_groupe_patients_executant_' + add_acte_csarr.value).value) {
      ...
      Object.keys(manage_patients.sejours).each(function (sejour_id) {
        if (manage_patients.sejours[sejour_id]['checked']) {
          form.select('.add_acte_csarr:checked').each(function (elt) {
            ...
            var index = elt.value.split('_')[2];

            if (elt_sejour_id === sejour_id) {
              manage_patients.sejours[sejour_id][acte_id + "_" + index] = {
                code:                    elt.get('code'),
              ...
              Object.keys(line_element).each(function (event_id) {
                ...
                Object.keys(event).each(function (acte_csarr_id) {
                  ...
                  if (typeof acte_csarr === 'object') {
                    var acte_csarr_code = acte_csarr['code'].replace("+", ""); console.log('.acte_csarr_' + acte_csarr_code + '_' + sejour_id + '_' + index);
                    var acte_checkbox = $$('.acte_csarr_' + acte_csarr_code + '_' + sejour_id + '_' + index)[0];
                    ...
                    var modulateurs_checkbox = $$('.modulateurs_' + acte_csarr_code + '_' + sejour_id + '_' + index);
                    var duree_input = $$('.duree_' + acte_csarr_code + '_' + sejour_id + '_' + index)[0];
                    var extension_input = $$('.extension_' + acte_csarr_code + '_' + sejour_id + '_' + index)[0];
                    var type_seance_input = $$('.type_seance_' + acte_csarr_code + '_' + sejour_id + '_' + index)[0];
                    var executant_input = $$('.executant_' + acte_csarr_code + '_' + sejour_id + '_' + index)[0];
    ...
    if (confirm($T('CPlageGroupePatient-msg-Are you sure you want to apply the same values of this element to all other elements of the same name'))) {
      ...
      getForm('gestion_groupe_patients').select('.add_acte_csarr').each(function (elt) {
        ...
        if ((elt.get('element_prescription_id') === element_id) && (elt_sejour_id != sejour_id)) {
          ...
          var modulateurs_checkbox = $$('.modulateurs_' + acte_csarr_code + '_' + elt_sejour_id + '_' + index);
          var duree_input = $$('.duree_' + acte_csarr_code + '_' + elt_sejour_id + '_' + index)[0];
          var extension_input = $$('.extension_' + acte_csarr_code + '_' + elt_sejour_id + '_' + index)[0];
          var type_seance_input = $$('.type_seance_' + acte_csarr_code + '_' + elt_sejour_id + '_' + index)[0];
          var executant_input = $$('.executant_' + acte_csarr_code + '_' + elt_sejour_id + '_' + index)[0];
  ...
  uncheckAllPatients: function () {
    if ($('gestion_groupe_patients_check_all_patients').checked) {
      $('gestion_groupe_patients_check_all_patients').checked = false;
   ...
   * @param class_acte_code The label element
   * @param addValues       Add value
   * @param index           Index element
   */
  setNotNull: function (acte_id, sejour_id, class_acte_code, addValues, index) {
    var executant = $('gestion_groupe_patients_executant_' + acte_id + '_' + sejour_id + '_' + index).value || !$('gestion_groupe_patients_acte_csarr_' + acte_id + '_' + sejour_id + '_' + index).checked;
    var label = $('label_executant_' + class_acte_code + '_' + sejour_id + '_' + index);
    var buttonDuplicate = $('duplicateReeducateur_' + class_acte_code + '_' + sejour_id + '_' + index);
    ...
    if (addValues) {
      GroupePatient.addValues(label.up('form'), acte_id, sejour_id, 'executant', index);
    }
  },
  /**
   * Add 2 times
   ...
   * @param index
   */
  changeTimings: function (element, position, max_actes, container_name, index) {
    ...
    var index = index ? index : main_input.dataset.index;
    ...
    for (var i = position; i < parseInt(max_actes); i++) {
      if ($(container_name + '_' + i + '' + index)) {
        var next_input = $(container_name + '_' + i + '' + index).down('input');
  ...
  changeRank: function (element, sortable, max_actes, container_name) {
    ...
    // Get the first div
    var index = main_div_id.slice(-1);
    var first_div_id = main_div_id.slice(0, -2) + '0' + '' + index;
    ...
    this.changeTimings(first_input, 0, max_actes, container_name.slice(0, -3));
