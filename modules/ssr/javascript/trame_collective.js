...
TrameCollective = {
  ...
  onsubmit: function(form) {
    if (form.name  != 'select_patients_planning_collectif') {
      ...
      if (TrameCollective.use_acte_presta == 'csarr') {
      ...
      else if (TrameCollective.use_acte_presta == 'presta_tarmed'){
        // Presta
        ...
        if ($('div_other_tarmed')) {
          // Tarmed
          tarmed_count = $V(form.code_tarmed) ? 1 : 0;
          tarmed_count += form.select('input.checkbox-tarmeds:checked').length;
          tarmed_count += form.select('input.checkbox-other-tarmeds').length;
        }

        if (presta_ssr_count == 0 && tarmed_count == 0) {
