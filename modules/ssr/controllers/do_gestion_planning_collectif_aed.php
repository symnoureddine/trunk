...
foreach ($sejour_ids as $sejour_id => $_sejour) {
  ...
  if ($_sejour["checked"]) {
    ...
    $line_elt->loadObject($where, "debut asc", "prescription_line_element.prescription_line_element_id", $ljoin);
    if ($line_elt->_id) {
      ...
      foreach ($days as $_day) {
        ...
        $evts_prioritaire = $evt_prioritaire->getCollectivesCollisions(null, $plage->niveau, $sejour_id, "<", false);
        if (count($evts_prioritaire) > 0) {
          continue;
        }
        $evt->deleteCollectivesByPlage(array($_day), $plage->niveau);
            ...
            foreach ($_actes as $_acte_plage) {
              switch ($_type_acte) {
                ...
                default:
                  ...
                  $new_acte->type = $_type_acte == "presta" ? "presta_ssr" : "tarmed";
                  break;
              }
