...
<table class="tbl">
  ...
  {{foreach from=$listSejours item=_sejour}}
    <tr>
      ...
      {{if "atih"|module_active && "atih CGroupage use_fg"|gconf && !in_array($_sejour->type, 'Ox\Mediboard\PlanningOp\CSejour::getTypesSejoursUrgence'|static_call:$_sejour->praticien_id)}}
        ...
        {{else}}
          <td id="groupage_{{$_sejour->_guid}}">
            <label title="{{if !$rss_id}}{{tr}}CGroupage-Grouping impossible without RSS file{{/tr}}{{else}}{{tr}}CGroupage-action-Launch the grouping{{/tr}}{{/if}}">
      ...
      {{else}}
        <td>
          <div class="small-warning">{{tr}}CRSS-msg-Creation of the RSS file impossible because you are on urgence stay{{/tr}}</div>
        </td>
      {{/if}}
    </tr>
    {{foreachelse}}
    <tr>
      <td class="empty" style="text-align: center;" colspan="9">
