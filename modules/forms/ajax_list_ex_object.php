...
if ($search_mode) {
  ...
  elseif ($date_time_min || $date_time_max) {
    $where[] = "ex_link.datetime_create >= '" . ($date_time_min ? $date_time_min : $date_time_max) . "'";
  }
...
if ($reference && $reference->_id) {
  $alerts = CExObject::getThresholdAlerts(array($reference), $ex_class_id);
}
...
if ($detail == 2) {
  foreach ($ex_objects as $_ex_class_id => $_ex_objects) {
    ...
    if ($_ex_class->canPerm("c") && !$search_mode && $_ex_class->allow_create_in_column) {
      ...
      $ex_class_event = (isset($ex_classes_creation[$_ex_class_id])) ? reset($ex_classes_creation[$_ex_class_id]) : null;
      ...
      $all_fields = $_new_ex_object->getReportedValues($ex_class_event);
...
if (CModule::getActive("appFineClient") && ($detail == 0 || $detail == 0.5)) {
  $orders_item_form = $orders_item_form_waiting = array();
  ...
  if ($reference instanceof CSejour || $reference instanceof CConsultation) {
    $orders_item_form = $reference->loadRefsOrdersItem(
      array("received_datetime" => "IS NULL", "canceled" => " = '0' ", "context_class" => " = '$reference->_class' ",  "context_id" => " = '$reference->_id' ")
    );
