...
switch ($surveillance) {
  ...
  case 'sspi':
    ...
    $datetime_max = $operation->sortie_reveil_reel ? $operation->sortie_reveil_reel : $operation->sortie_reveil_possible;
    break;
  default:
    $datetime_max = null;
    $datetime_min = null;
...
$administrations_dm = array();
$prescription_id = null;
if (CModule::getActive("dPprescription")) {
  $prescription_id = $sejour->_ref_prescription_sejour->_id;
  if ($prescription_id) {
    $administrations = CAdministration::getPerop($prescription_id, false, $operation_id, $datetime_min, $datetime_max);

    if ($dmi_active) {
      $administrations_dm = CAdministrationDM::getPerop($prescription_id, false, $operation_id, $datetime_min, $datetime_max);
    }
  }
}
...
if ($prescription_id && CPrescription::isMPMActive()) {
  ...
  foreach ($mixes as $_mix) {
    ...
    $_mix->loadRefsLines();
    if ($_mix->date_pose && $_mix->time_pose && (!$surveillance || ($_mix->_pose >= $datetime_min && $_mix->_pose <= $datetime_max))) {
...
if (CModule::getActive("monitoringBloc") && CAppUI::gconf("monitoringBloc general active_graph_supervision")) {
  if (($surveillance === 'preop' || !$surveillance) && $operation->graph_pack_preop_id) {
    ...
    // filtrer
    [$list, $grid, $graphs, $labels, $list_obr] = CObservationResultSet::getChronological(
      $operation, $operation->graph_pack_preop_id, false, $datetime_min, $datetime_max
    );
  ...
  if (($surveillance === 'perop' || !$surveillance) && $operation->graph_pack_id) {
    /** @var CObservationResultSet[] $list_obr */
    [$list, $grid, $graphs, $labels, $list_obr] = CObservationResultSet::getChronological(
      $operation, $operation->graph_pack_id, false, $datetime_min, $datetime_max
    );
  ...
  if (($surveillance === 'sspi' || !$surveillance) && $operation->graph_pack_sspi_id) {
    /** @var CObservationResultSet[] $list_obr */
    [$list, $grid, $graphs, $labels, $list_obr] = CObservationResultSet::getChronological(
      $operation, $operation->graph_pack_sspi_id, false, $datetime_min, $datetime_max
    );
...
if (CAppUI::conf('dPsalleOp COperation use_garrot') && CAppUI::gconf('dPsalleOp COperation garrots_multiples')) {
