...
{{if $use_brancardage && $show_brancardage}}
  ...
  <table class="form me-no-box-shadow" style="float:left;min-width:120px;width:{{if $brancard_use_retour_sans_sspi}}12{{else}}15{{/if}}%;margin:0;">
    ...
    <tr>
      {{assign var=destination_guid value="CBlocOperatoire-`$bloc_brancard`"}}
      <td id="demande_brancard-{{$selOp->_guid}}--{{$destination_guid}}" rowspan="2">
        {{mb_include module=brancardage template=inc_exist_brancard colonne="demande_brancard" object=$selOp}}
      </td>
    </tr>
  </table>
{{/if}}
...
<table class="form me-no-box-shadow vw-timings"
  ...
  {{if $conf.dPsalleOp.COperation.use_entree_bloc || $conf.dPsalleOp.COperation.use_cleaning_timings
        || $conf.dPsalleOp.COperation.use_entree_sortie_salle || $use_tto || $see_pec_anesth
        || $use_fin_installation || $use_debut_installation || $conf.dPsalleOp.COperation.use_remise_chir}}
    {{assign var=see_cell_preaparation value=true}}
  {{/if}}
  {{assign var=see_cell_operation value=false}}
  {{if $conf.dPsalleOp.COperation.use_garrot || $conf.dPsalleOp.COperation.use_debut_fin_op
        || $conf.dPsalleOp.COperation.use_suture || $conf.dPsalleOp.COperation.use_prep_cutanee || $use_incision || $see_patient_stable}}
    {{assign var=see_cell_operation value=true}}
  {{/if}}
  {{assign var=see_cell_sortie value=false}}
  {{if $conf.dPsalleOp.COperation.use_entree_sortie_salle || $conf.dPsalleOp.COperation.use_sortie_sans_sspi
        || $conf.dPsalleOp.COperation.use_cleaning_timings || $see_entree_reveil
  ...
  <tr>
    {{if $see_cell_preaparation}}
      <td class="me-valign-top">
        ...
        {{if $conf.dPsalleOp.COperation.use_remise_chir && $place_remise_chir == "below_entree_salle"}}
        ...
        {{if $conf.dPsalleOp.COperation.use_remise_chir && $place_remise_chir == "under_entree_salle"}}
          {{mb_include module=salleOp template=inc_form_timing object=$selOp field=remise_chir}}
        {{/if}}
    ...
    {{if $see_cell_operation}}
      <td class="me-valign-top">
        {{if $conf.dPsalleOp.COperation.use_prep_cutanee}}
          {{mb_include module=salleOp template=inc_form_timing object=$selOp field=prep_cutanee}}
        {{/if}}

        {{if $conf.dPsalleOp.COperation.use_garrot && !$garrots_multiples}}
    ...
    {{if $see_cell_sortie}}
      <td style="text-align: center;" class="me-valign-top">
        ...
        {{if $conf.dPsalleOp.COperation.use_cleaning_timings}}
        ...
        <form name="timing{{$selOp->operation_id}}-sortie_sans_sspi" method="post">
          ...
          {{if $conf.dPsalleOp.COperation.use_sortie_sans_sspi && $modif_operation}}
...
{{if $use_brancardage && $show_brancardage && $brancard_use_retour_sans_sspi && $conf.dPsalleOp.COperation.use_sortie_sans_sspi}}
  <table class="form me-no-box-shadow" style="float:left;min-width:120px;width:13%;margin:0;">
    ...
    <tr>
      {{assign var=destination_guid value=null}}
      {{assign var=origine_guid value="CBlocOperatoire-`$bloc_brancard`"}}
      <td id="demande_brancard-{{$selOp->_guid}}-{{$origine_guid}}-" rowspan="2">
        {{mb_include module=brancardage template=inc_exist_brancard colonne="demande_brancard" object=$selOp}}
      </td>
    </tr>
  </table>
