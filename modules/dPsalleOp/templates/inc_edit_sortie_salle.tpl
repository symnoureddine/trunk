...
<script>
  Main.add(function () {
    ...
    Calendar.regField(getForm('edit_sortie_salle').sortie_salle, dates, options);
      ...
      if (SalleOp.checkTimingOperation(entree_sejour, sortie_sejour, form.sortie_salle, '{{$operation->_id}}', last_timing)) {
        ...
        if (stop_session) {
          App.loadJS({module: "patientMonitoring", script: "concentrator"}, function () {
            Concentrator.stopCurrentSession("{{$operation->_id}}", function () {
            });
          });
        }
          ...
          onComplete: function () {
            ...
            App.loadJS({module: "patientMonitoring", script: "concentrator"}, function () {
              Concentrator.askPosteConcentrator(
                ...
                form,
                function () {
                  Concentrator.importDataToConstants('{{$operation->_id}}', 'perop');
                },
                start_session,
                null,
                'sortie_salle'
