...
<script>
  ...
  setDents = function(form) {
    var url = new Url('ccam', 'ajax_set_dents');
    ...
    <th id="codages-title" class="title" style="border-bottom: none; border-spacing: 0px;">
      <div style="float: left">
        <form name="addActes-{{$subject->_guid}}" method="post" onsubmit="return false">
          ...
          {{elseif $subject|instanceof:'Ox\Mediboard\Ccam\CDevisCodage'}}
            <input type="hidden" name="m" value="ccam" />
            <input type="hidden" name="dosql" value="do_devis_codage_aed" />
          {{elseif $subject|instanceof:'Ox\Mediboard\Ccam\CModelCodage'}}
            <input type="hidden" name="m" value="ccam" />
            <input type="hidden" name="dosql" value="do_model_codage_aed" />
          ...
          {{foreach from=$codages item=_codage}}
            ...
            <li>
              <a href="#codage-{{$_codage->_id}}">
                ...
                {{if $subject->_class == 'COperation' && $subject->date != $_codage->date}}&nbsp;{{mb_value object=$_codage field=date}}{{/if}}&nbsp;&nbsp;
  ...
  <div style="text-align: center;">
    <form name="remboursementExceptionnel" action="?" method="post" onsubmit="return false;">
      <table class="form">
        <tr>
          <th>
            <label for="rembourse" title="{{tr}}CActeCCAM-rembourse-desc{{/tr}}">{{tr}}CActeCCAM-rembourse{{/tr}}</label>
          </th>
          <td>
