...
<script>
  ...
  lockCodages = function(praticien_id, codable_class, codable_id, export_acts) {
    ...
    {{else}}
      var url = new Url('ccam', 'ajax_lock_codage');
    {{/if}}
    url.addParam('praticien_id', praticien_id);
    url.addParam('codable_class', codable_class);
    url.addParam('codable_id', codable_id);
    ...
    {{else}}
      url.requestUpdate('systemMsg', {onComplete: ActesCCAM.notifyChange.curry(codable_id, praticien_id), method: 'post', getParameters: {m: 'ccam',a: 'ajax_lock_codage'}});
    {{/if}}
  };
...
<table class="main layout">
  ...
  <tr>
    ...
    <td>
      <fieldset id="didac_inc_manage_codes_fieldset_code" class="me-no-box-shadow">
...
<!-- Pas d'affichage de inc_manage_codes si la consultation est deja validée -->
 {{*if $subject instanceof CConsultation && !$subject->_coded*}}
  <table class="main layout">
    <tr>
      {{if !$subject|instanceof:'Ox\Mediboard\Cabinet\CConsultation' && !$subject|instanceof:'Ox\Mediboard\Ccam\CDevisCodage' && !$subject|instanceof:'Ox\Mediboard\Ccam\CModelCodage'}}
      <td class="halfPane">
        <fieldset>
          <legend>Validation du codage</legend>
          {{if $conf.dPsalleOp.CActeCCAM.envoi_actes_salle || $m == "dPpmsi" && $subject|instanceof:'Ox\Mediboard\PlanningOp\COperation'}}
