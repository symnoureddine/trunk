...
{{foreach from=$operations item=_operation name=ops}}
  ...
  {{if !$_operation->annulee || ($_operation->annulee && $app->user_prefs.planning_bloc_show_cancelled_operations)}}
    {{if $smarty.foreach.ops.index > 0}}
      ...
      {{assign var=intervalle_a value=$tempop_save}}
      {{if $postop_save}}
        {{assign var=intervalle_a value='Ox\Core\CMbDT::addTime'|static_call:$postop_save:$tempop_save}}
      {{/if}}
      {{assign var=intervalle_a value='Ox\Core\CMbDT::addTime'|static_call:$intervalle_a:$timeop_save}}

      {{assign var=intervalle_b value=$timeop_curr}}
      {{if $preop_curr}}
        {{assign var=intervalle_b value='Ox\Core\CMbDT::subTime'|static_call:$preop_curr:$timeop_curr}}
      {{/if}}

      {{if ($intervalle_a < $intervalle_b) && ($save_op->pause !== "00:00:00")}}
        <tr>
          <th colspan="5" class="section">
            {{* Retait du temps de pause entre les interventions défini sur la plage afin d'avoir le temps réel de pause défini sur l'intervention *}}
            {{if $_operation->_ref_plageop->temps_inter_op}}
              {{assign var=intervalle_b value='Ox\Core\CMbDT::subTime'|static_call:$_operation->_ref_plageop->temps_inter_op:$intervalle_b}}
            {{/if}}

            {{assign var=time_pause value='Ox\Core\CMbDT::timeRelative'|static_call:$intervalle_a:$intervalle_b:"%02dh%02d"}}
    ...
    <tbody class="hoverable">
      <tr {{if $_operation->_id == $operation_id}}class="selected"{{/if}}>
        ...
        {{else}}
          ...
          {{if $mode_presentation && @$modules.brancardage->_can->read && "brancardage General use_brancardage"|gconf}}
            {{assign var=destination_guid value="CBlocOperatoire-`$_operation->_ref_salle->bloc_id`"}}
            <td id="-{{$_operation->_guid}}--{{$destination_guid}}">
              {{mb_include module=brancardage template=inc_exist_brancard colonne="" object=$_operation mode_presentation=1}}
            </td>
          {{/if}}
          {{if !$vueReduite}}
