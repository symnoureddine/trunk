...
<script>
  ...
  submitFormAct = function(form) {
    ...
    {{/if}}
    return onSubmitFormAjax(form, {onComplete: function() {window.urlEditActe.modalObject.close();}});
  };
</script>

<form name="formEditFullActe-{{$view}}" method="post" enctype="multipart/form-data" onsubmit="return submitFormAct(this);">
  ...
  <table class="form" style="min-width: 400px;">
    ...
    {{if $phase->nb_dents}}
      ...
      <tr>
        <th>Dent(s) concernée(s) (<span id="checked_teeth-{{$view}}">{{$teeth_checked}}</span> / {{$phase->nb_dents}} sélectionnée(s))</th>
        <td class="text" colspan="10">
    ...
    <tr>
      <th>
        {{if $code->remboursement == 2 || $code->remboursement == 3}}
          ...
          <i class="fas fa-exclamation-triangle fa-lg" style="color: #ffa30c;" title="{{'Ox\Core\CAppUI::tr'|static_call:"CDatedCodeCCAM-msg-remboursement.$remb":$code->code}}"></i>
        {{/if}}
    ...
    <tr>
      <th>
        {{if $code->entente_prealable}}
          <i id="info_dep" class="fa fa-lg fa-exclamation-circle" style="color: #{{if $acte->accord_prealable && $acte->date_demande_accord && $acte->reponse_accord}}197837{{else}}ffa30c{{/if}};" title="{{tr}}CActeCCAM-msg-dep{{/tr}}"></i>
        {{/if}}
