        ...
        {{if (!$acte->_id || ($acte->executant_id == $codage->praticien_id && $acte->_id|@array_key_exists:$codage->_ref_actes_ccam)) &&
          ...
          <script type="application/javascript">
            Main.add(function() {
              ...
              {{if "pyxVital"|module_active && 'pyxVital General mode'|gconf != 'test'}}
                dates.limit = {
              ...
              <th class="section" colspan="15" style="text-align: left;">
                <span style="float: right;">
                  {{if is_array($_code->assos) && count($_code->assos) > 0}}
                    ...
                    <input type="text" size="8em" name="keywords" placeholder="{{$_code->assos|@count}} cmp./sup." onclick="$V(this, '');"/>
                  </form>
          ...
          <tr {{if !$acte->_id}}class="activite-{{$acte->code_activite}}"{{/if}}>
            ...
            {{if $subject->_class != 'CConsultation' && $subject->_class != 'CDevisCodage'}}
              <td class="narrow">
                {{if $_code->extensions|@count}}
                  <form name="codageActeExtensionPMSI-{{$view}}" action="?" method="post" onsubmit="return false;" class="prepared">
                    {{if 'dPccam codage pmsi_extension_mandatory'|gconf}}
                      <label for="codageActeExtensionPMSI-{{$view}}_code_extension" class="notNull"></label>
                      <script type="text/javascript">
                        Main.add(function() {
                    ...
                    {{/if}}
                    <select id="codageActeExtensionPMSI-{{$view}}_code_extension" name="code_extension" style="width: 4em;" onchange="syncCodageField(this, '{{$view}}');"{{if 'dPccam codage pmsi_extension_mandatory'|gconf}} class="notNull"{{/if}}{{if $acte->_billed}} disabled{{/if}}>
            ...
            <td>
              <form name="codageActeFacturable-{{$view}}" action="?" method="post" onsubmit="return false;" class="prepared">
            ...
            <td
              ...
              {{if $acte->_id}}
                <form name="codageActeCodeAssociation-{{$view}}" action="?" method="post" onsubmit="return false;" class="prepared">
            ...
            <td class="greedyPane{{if !$_phase->_modificateurs|@is_countable || !$_phase->_modificateurs|@count}} empty{{/if}}">
              ...
              {{foreach from=$_phase->_modificateurs item=_mod name=modificateurs}}
                <span class="circled {{if $_mod->_state == 'prechecked'}}ok{{elseif ($_mod->_checked && in_array($_mod->_state, array('not_recommended', 'forbidden')) && $_mod->code != 'K') || $_mod->_montant == '0'}}error{{elseif in_array($_mod->_state, array('not_recommended', 'forbidden'))}}warning{{/if}}"
                      title="{{$_mod->libelle}} ({{$_mod->_montant}})">
                  <label for="modificateur_{{$_mod->code}}{{$_mod->_double}}">
                    <input type="checkbox" name="modificateur_{{$_mod->code}}{{$_mod->_double}}" data-state="{{$_mod->_state}}"
                           ...
                           data-acte="{{$view}}" data-code="{{$_mod->code}}" data-price="{{$_mod->_montant}}" data-double="{{$_mod->_double}}" class="modificateur" onchange="syncCodageField(this, '{{$view}}');"{{if $acte->_billed}} data-billed="true"{{/if}}/>
                    {{$_mod->code}}
                  </label>
                </span>
            ...
            <td class="narrow">
              {{if $acte->code_activite == 4}}
                <form name="codageActeExtDoc-{{$view}}" action="?" method="post" onsubmit="return false;" class="prepared">
                  {{if 'dPccam codage doc_extension_mandatory'|gconf}}
                    <label for="codageActeExtDoc-{{$view}}_extension_documentaire" class="notNull"></label>
                    <script type="text/javascript">
                      Main.add(function() {
            ...
            {{if ($subject->_class == 'CConsultation' && $subject->concerne_ALD) || ($subject->_class == 'CSejour' && $subject->ald)}}
              <td class="narrow">{{mb_field object=$acte field=ald typeEnum='select' onchange="syncCodageField(this, '$view');"}}</td>
            {{/if}}
            ...
            {{if $codage->_show_depassement}}
              <td>
                <form name="codageActeMontantDepassement-{{$view}}" action="?" method="post" onsubmit="return false;" class="prepared">
                  {{if $acte->_billed}}
              ...
              <td>
                <form name="codageActeMotifDepassement-{{$view}}" action="?" method="post" onsubmit="return false;" class="prepared">
            ...
            {{/if}}
            <td>
              <form name="codageActe-{{$view}}"{{if !$acte->_id}} class="new-act-form-{{$codage->_id}}"{{/if}} action="?" method="post" data-view="{{$view}}" onsubmit="return submitFormAct(this);" class="prepared">
                ...
                {{/if}}
                  ...
                  {{/if}}
                {{foreach from=$_phase->_modificateurs item=_mod name=modificateurs}}
                  <input type="checkbox" name="modificateur_{{$_mod->code}}{{$_mod->_double}}" {{if $_mod->_checked}}checked="checked"{{/if}} style="display: none;"/>
                {{/foreach}}
                ...
                {{else}}
                  ...
                  {{/if}}
                  <button class="edit notext compact" type="button" onclick="ActesCCAM.edit({{$acte->_id}})"{{if $acte->_billed}} disabled{{/if}}>{{tr}}Edit{{/tr}}</button>
...
{{if $codage->locked != '1'}}
  <div style="text-align: center; margin-top: 10px;">
    <button type="button" class="tick"{{if !$codage->_ref_actes_ccam|@count && (!$codage->_codage_sibling || ($codage->_codage_sibling->_id && !$codage->_codage_sibling->_ref_actes_ccam|@count))}}disabled="disabled"{{/if}}
...
<ul id="rules-tab_codage-{{$codage->_id}}" class="control_tabs">
  ...
  <li style="float: right;" class="me-tabs-flex">
    {{* Formulaire ALD/CMU *}}
    {{if 'dPccam codage display_ald_cmu'|gconf && ($subject->_class == 'COperation' || $subject->_class == 'CSejour')}}
    ...
    <table class="tbl">
      ...
      {{if isset($codage->_possible_rules.EG6|smarty:nodefaults)}}
        <tr>
          ...
          <td class="text">
            Les forfaits de <strong>cardiologie</strong>, de <strong>réanimation</strong>, les actes de <strong>surveillance post-op</strong> d'un patient de chirurgie cardiaque avec CEC
            et les actes <strong>d'accouchements</strong> peuvent être associés à taux plein à un seul des actes introduits par la note "Facturation: éventuellement en supplément"
          </td>
        </tr>
      {{/if}}
      <tr>
