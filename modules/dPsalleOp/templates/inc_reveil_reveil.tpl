...
<table class="tbl me-no-align me-small">
  <tr>
    ...
    <th
      class="me-small-fields">{{mb_colonne class="COperation" field="_patient" order_col=$order_col order_way=$order_way function=orderTabreveil}}
        ...
        <td>
          {{if $salvage->_id}}
            <div style="float:left ; display:inline">
              <a href="#" title="Voir la procédure RSPO" onclick="viewRSPO({{$_operation_id}});">
      ...
      {{if @$modules.brancardage->_can->read && "brancardage General use_brancardage"|gconf}}
        ...
        <td>
          {{if $consult_anesth && $consult_anesth->_id}}
            ...
            <br/>
          {{/if}}
          {{assign var=origine_guid value="CBlocOperatoire-`$bloc_id`"}}
          <div id="demande_brancard-{{$_operation->_guid}}-{{$origine_guid}}-">
            ...
            {{if $modif_operation}}
              ...
              {{if !$_operation->sortie_reveil_reel}}
                <button class="tick notext me-tertiary me-small" type="button"
                        ...
                        submitReveilForm(this.form, true, '{{$concentrator_session}}', '{{$current_session->_id}}');">
                  {{tr}}Modify{{/tr}}
                </button>
              {{/if}}
            {{else}}
