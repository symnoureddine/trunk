...
<fieldset style="background-color: #fff;" id="surveillance-timeline-{{$uid_plot}}"
  ...
  <legend>
      ...
      {{if "patientMonitoring"|module_active && ($interv->graph_pack_id || $interv->graph_pack_sspi_id || $interv->graph_pack_preop_id) && $concentrators !== null}}
                        ...
                        <script>
                            ...
                            Main.add(function () {
                              ...
                              ph.bind("plotclick", function (event, pos, item) {
                                ...
                                SurveillancePerop.editObservationResultSet(data.set_id, '{{$pack->_id}}', data.result_id, null, element_main, '{{$type}}');
                              });
                                {{/if}}
                              ...
                              var graph = {
                                ...
                                options:        options,
                                isGraph:        true,
                                isConcentrator: {{if $_graph->automatic_protocol == 'Kheops-Concentrator'}}true{{else}}false{{/if}},
                          ...
                          Main.add(function () {
                            App.loadJS(['lib/visjs/vis'], (function (currentTL, vis) {
                              ...
                              stli.samplingFrequency = parseInt('{{$_graph->sampling_frequency}}');
                                {{if $_graph->automatic_protocol == 'Kheops-Concentrator'}}
      ...
      <td>
        ...
        {{if ($type == "perop" && $interv->graph_pack_id) || ($type == "sspi" && $interv->graph_pack_sspi_id) ||
          ...
          <button class="new not-printable me-primary"
                  onclick="SurveillancePerop.createObservationResultSet(null, '{{$interv->_guid}}', '{{$pack->_id}}', this.up('.timeline-container-{{$type}}'), true, '{{$type}}')" {{if $readonly}} disabled {{/if}}>
