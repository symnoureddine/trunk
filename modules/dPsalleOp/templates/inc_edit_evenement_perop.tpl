          ...
          <tr>
            ...
            <td>
              {{if $precisions|@count > 0}}
                <select name="geste_perop_precision_id" onchange="GestePerop.showPrecisionValues(this.value);" style="margin: 0;">
                  <option value="">
                ...
                {{if $valeurs|@count > 0}}
                  <select class="me-margin-0" name="precision_valeur_id" onchange="loadConstantesMedicales($V($('select_context')));">
                    ...
                    {{foreach from=$valeurs item=_valeur}}
                      <option value="{{$_valeur->_id}}"
                        {{if $_valeur->_id == $evenement->precision_valeur_id}} selected="selected"{{/if}}>
                        {{$_valeur->_view}}
                      </option>
                    {{/foreach}}
                  </select>
