...
<script>
  submitPrepaForm = function(oFormPrepa, askPoste, stop_session, operation_id) {
    ...
    {{if $use_concentrator}}
      if (stop_session) {
        App.loadJS({module: "patientMonitoring", script: "concentrator"}, function () {
          Concentrator.stopCurrentSession(operation_id, function () {
...
<table class="tbl me-no-align me-small">
  <tr>
    ...
    <th class="me-small-fields">{{mb_colonne class="COperation" field="_patient" order_col=$order_col order_way=$order_way function=orderTabpreop}} <input type="text" name="_seek_patient_preop" value="" class="seek_patient" onkeyup="seekPatient(this);" onchange="seekPatient(this);" /> </th>
  ...
  {{foreach from=$listOperations item=_operation}}
    ...
    <tr>
      ...
      {{/if}}
      <td>
        <button class="button soins notext me-tertiary me-small" onclick="showDossierSoins('{{$sejour_id}}','{{$_operation_id}}');">
          Dossier séjour
        </button>
        {{if $isImedsInstalled}}
          <button class="labo button notext me-tertiary me-small" onclick="showDossierSoins('{{$sejour_id}}','{{$_operation_id}}','Imeds');">Labo</button>
          {{mb_include module=Imeds template=inc_sejour_labo link="#1" sejour=$_operation->_ref_sejour float="none"}}
        {{/if}}
        <button type="button" class="injection notext me-tertiary me-small" onclick="Operation.dossierBloc('{{$_operation_id}}', true, 'surveillance_preop');">Dossier de bloc</button>
      ...
      {{if @$modules.brancardage->_can->read && "brancardage General use_brancardage"|gconf}}
        <td>
          {{assign var=destination_guid value="CBlocOperatoire-`$bloc_id`"}}
           <div id="demande_brancard-{{$_operation->_guid}}--{{$destination_guid}}">
              {{mb_include module=brancardage template=inc_exist_brancard colonne="demande_brancard" object=$_operation}}
           </div>
        </td>
      {{/if}}
