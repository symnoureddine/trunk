...
class CDailyCheckList extends CMbObject { // not a MetaObject, as there can be multiple objects for different dates
  ...
  static function getCheckLists(CMbObject $object, $date, $type = "ouverture_salle", $multi = false) {
    ...
    foreach ($daily_check_list_types as $_list_type) {
      ...
      if ($choose_moment_edit || $daily_check_list->_ref_list_type->lock_view || !$currUser->_is_praticien || $multi || $check_list_not_validated) {
   ...
   * @return array
   */
  static function getsCheckLists(CMbObject $object, $date, $type = "ouverture_salle", $multi = false, $sspi_id) {
   ...
   * @return string
   */
  static function getCountChecklistInterv(CMbObject $object) {
    $where = array(
    ...
    return $daily_check_list->countList($where);
  }
