...
foreach ($listOperations as $op) {
  ...
  $sejour->loadRefCurrAffectation($date . ($date === $now ? " " . CMbDT::time() : null));
...
if ($type === "reveil" || $type === "preop") {
  ...
  $date_close_checklist = CDailyCheckList::getDateLastChecklist($bloc, $type_close, true, $require_check_list_close);
  $date_open_checklist  = CDailyCheckList::getDateLastChecklist($bloc, $type_checklist, true, $require_check_list);

  // Prise en compte de la sspi pour la récupération de la dernière saisie de checklist
  if ((!$date_close_checklist || !$date_open_checklist) && $sspi_id) {
    $sspi = new CSSPI();
    $sspi->load($sspi_id);

    if (!$date_close_checklist) {
      $date_close_checklist = CDailyCheckList::getDateLastChecklist($sspi, $type_close, true, $require_check_list_close);
    }

    if (!$date_open_checklist) {
