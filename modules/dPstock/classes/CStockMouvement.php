...
class CStockMouvement extends CMbObject {
  ...
  public static $_types_mvt = array("administration", "destruction", "disparition", "apparition", "retour_pharma", "apport_patient",
    "delivrance", "apport_service", "retour_service", "transfert_patient", "vaccination", "delivre_patient");
  ...
  function changeStock($store = true) {
    ...
    if ($this->_ref_cible && $this->cible_class != "CAdministration") {
      ...
      if ($this->cible_class == "CStockSejour") {
        $this->_ref_cible->quantite_reelle += $quantite_cible;
        $this->_ref_cible->datetime        = CMbDT::dateTime();
      }
      elseif ($this->cible_class !== CClassMap::getSN(CInjection::class)) {
