...
class CProduct extends CMbObject {
  ...
  function getSpec() {
    ...
    $spec->table           = 'product';
    $spec->key             = 'product_id';
    $spec->uniques["code"] = array("code", "bdm");
    $spec->uniques["name"] = array("name", "bdm");
  ...
  function getProps() {
    ...
    $props["bdm"]            = "enum list|bcb|compendium|vidal|besco default|bcb";
  ...
  function updateFormFields() {
    ...
    if (CModule::getActive("dispensation") && CAppUI::gconf("dispensation general use_dispentation_ucd") && CMedicament::getBase() === "besco") {
   ...
   * @throws Exception
   */
  public function loadRefsSelection() {
   ...
   * @throws Exception
   */
  public function loadRefsStocksService() {
    $ljoin = [
      'service' => "service.service_id = product_stock_service.object_id AND product_stock_service.object_class = 'CService'"
    ];
   ...
   * @throws Exception
   */
  public function loadRefsStocksGroup() {
   ...
   * @throws \Exception
   */
  function loadRefsLotsAvailable($dm_id = null) {
    ...
    $lots = $reception->loadList($where, null, null, "product_order_item_reception.order_item_reception_id", $ljoin);

    CStoredObject::massCountBackRefs($lots, "consommations");
    foreach ($lots as $_lot) {
      if (($_lot->getUsedQuantity() >= $_lot->quantity) || $_lot->countBackRefs("consommations")) {
        unset($lots[$_lot->_id]);
      }
    }

    $order_items = CStoredObject::massLoadFwdRef($lots, "order_item_id");
    CStoredObject::massLoadFwdRef($order_items, "reference_id");
    foreach ($lots as $_lot) {
  ...
  static function getConsumptionMultipleProducts(
    ...
    if (CModule::getActive("dispensation")) {
      return Dispensation::getConsumptionMultipleProducts($products, $since, $date_max, $services, $include_loss, $detail);
    }
  ...
  private static function fillFlow(&$array, $products, $n, $start, $unit, $services) {
    ...
    $d = array_map_recursive(array(CProduct::class, "round2"), $d);
  ...
  static function computeBalance(array $products, array $services, $year, $month = null) {
    ...
    $balance = array_map_recursive(array(CProduct::class, "round2"), $balance);
  ...
  static function makeBalanceCSV($list_products, $services, $year, $month, $day) {
    ...
    foreach ($consumation as $_consum) {
      ...
      $prep    = $prep_id ? (isset($users[$prep_id]) ? $users[$prep_id] : $users[$prep_id] = CMediusers::get($prep_id)->_view) : '';

      $vali_id = $_consum["validateur_id"];
      $vali    = $vali_id ? (isset($users[$vali_id]) ? $users[$vali_id] : $users[$vali_id] = CMediusers::get($vali_id)->_view) : '';
  ...
  function loadMedicamentATC() {
    $this->_classe_atc = $this->loadMedicamentArticle()->_ref_ATC_5_code;
  }
   ...
   */
  function loadMedicamentArticle() {
    return $this->_med_article = CMedicamentArticle::get($this->code, $this->code_up_adm, $this->code_up_disp, $this->bdm);
  }
   ...
   * @return array
   */
  static function getCacheProduitsStock($cache_produits_stock, $_code_cip, $bdm) {
    if (!$bdm) {
    ...
    return array($cache_produits_stock, $_product_stock);
  }
   ...
   */
  function loadRefEmplacementGroup($codes_cip) {
    $this->completeField("product_id");
    $ljoin                                 = array();
    ...
    $emplacements = $emplacement->loadList($where, "name", null, "product_stock_location.stock_location_id", $ljoin);
   ...
   * @return array
   */
  static function getsFromCIS($codes_cip, $service_id, $bdm = null) {
    if (!$bdm) {
      $bdm = CMedicamentProduit::getBase();
    }
    if (!count($codes_cip)) {
    ...
    $stocks = array();
    $types  = array("groups" => CGroups::loadCurrent()->_id);
    foreach ($codes_cip as $_code_cip) {
      $stocks[$_code_cip] = array();
    }
    if ($service_id) {
      $types["service_id"]            = $service_id;
      $stocks["CService-$service_id"] = array();
    }
