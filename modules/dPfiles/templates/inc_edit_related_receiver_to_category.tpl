...
<form name="editRelatedReceiverToCategory-{{$related_receiver->_guid}}" method="post" onsubmit="return onSubmitFormAjax(this, Control.Modal.close);">
  ...
  <table class="form">
    ...
    <tr>
      ...
      <td>
        <select name="receiver_id" class="notNull"
              ...
              {{foreach from=$_available_receivers item=_available_receiver}}
                {{if $_available_receiver|@is_array}}
                  {{foreach from=$_available_receiver item=_available_receiver_sih_cabinet}}
                    <option data-receiver-class="{{$_available_receiver_sih_cabinet->_class}}" value="{{$_available_receiver_sih_cabinet->_id}}" {{if $related_receiver->_ref_receiver->_id == $_available_receiver_sih_cabinet->_id}}selected{{/if}}>
                      {{$_available_receiver_sih_cabinet->_view}}
                    </option>
                  {{/foreach}}
                {{else}}
                  <option data-receiver-class="{{$_available_receiver->_class}}" value="{{$_available_receiver->_id}}" {{if $related_receiver->_ref_receiver->_id == $_available_receiver->_id}}selected{{/if}}>
