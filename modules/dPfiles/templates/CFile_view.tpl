...
<table class="main">
  <tr>
    <td style="text-align: center; width: 66px;" {{* Ne pas utiliser la class narrow car déforme la tooltip (Chrome) *}}>
    ...
    <td style="vertical-align: top;" class="text">
      ...
      {{if $file->_count_deliveries > 0}}
        <br><strong>{{tr}}CCompteRendu.count_sending{{/tr}}</strong> : {{$file->_count_deliveries}}
        <span class="far fa-lg fa-caret-square-right" onclick="toggleDeliveries(this, '{{$file->_guid}}');" style="cursor: pointer;"></span>
        <ul id="{{$file->_guid}}-deliveries" style="display: none;">
          {{foreach from=$file->_ref_deliveries item=delivery}}
  ...
  <tr>
    <td colspan="2" class="button">
      {{if $file->_can->edit}}
        ...
        <button type="button" class="edit" onclick="renameFile('{{$file->_id}}')">{{tr}}CFile-_rename{{/tr}}</button>
        ...
        <form name="actionFile{{$file->_guid}}" method="post">
          ...
          {{if $file->annule}}
            <button type="button" class="undo" onclick="restoreFile(this.form);">{{tr}}Restore{{/tr}}</button>
          {{else}}
            <button type="button" class="cancel" onclick="archiveFile(this.form);">{{tr}}Cancel{{/tr}}</button>
          {{/if}}
          {{if $can->admin}}
        ...
        <button type="button" class="fa fa-share-alt" onclick="DocumentItem.viewRecipientsForSharing('{{$file->_guid}}')">{{tr}}Send{{/tr}}</button>
