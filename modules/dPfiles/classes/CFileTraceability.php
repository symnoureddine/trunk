...
class CFileTraceability extends CMbObject
{
    ...
    function loadRefPatient()
    {
        ...
        // Search by IPP
        if (CAppUI::gconf('hubDocumentaire search search_by_ipp') && $this->IPP) {
        ...
        switch (CAppUI::gconf('hubDocumentaire search automatic_patient_reconciliation')) {
            case "strict":
                if ($patient->_id) {
    ...
    function loadRefSejour()
    {
        ...
        // Search by NDA
        if (CAppUI::gconf('hubDocumentaire search search_by_nda') && $this->NDA) {
    ...
    function filterPatient(CPatient $filter_patient, $where = [], $order = null, $limit = null, $ljoin = null)
    {
        ...
        foreach ($files_traceabilities as $key => $_file_traceability) {
            ...
            // Si on n'a pas retrouvé le patient sur la trace mais que dans les filters on recherchait un patient en particulier
            if (!$patient->_id && ($filter_patient->_id || $filter_patient->nom || $filter_patient->prenom)) {
            ...
            // Patient filter
            if (($filter_patient->nom && $patient->_id && strtoupper($patient->nom) != strtoupper($filter_patient->nom))
                || ($filter_patient->prenom && $patient->_id && strtoupper($patient->prenom) != strtoupper($filter_patient->prenom))
     ...
     * @throws Exception
     */
    function getFiles($where = [], $order = null, $limit = null, CPatient $patient = null)
    {
        $ljoin                    = [];
    ...
    static function getSourceName(CInteropActor $actor, ?CDocumentItem $docItem = null)
    {
        ...
        if ($actor instanceof CInteropReceiver) {
            switch ($actor->_class) {
                case 'CReceiverHL7v2':
                    if (
                        CModule::getActive("appFineClient") && CMbArray::get($actor->_configs, 'send_evenement_to_mbdmp')
                    ) {
        ...
        if ($actor instanceof CInteropSender) {
            if ($sas_name = CMbArray::get($actor->_configs, 'sas_name')) {
                return $sas_name;
            }
        }
    ...
    /**
     * Check if trace exist for a docItem and a receiver
     *
     * @param CDocumentItem    $documentItem
     * @param CInteropReceiver $receiver
     *
     * @return bool|CFileTraceability
     * @throws Exception
     */
    static function getTrace(CDocumentItem $documentItem, CInteropReceiver $receiver, $status = 'pending')
    {
        if (!$documentItem->_id || !$receiver->_id) {
            return new CFileTraceability();
        }
        ...
        $file_traceability->status      = $status;
        $file_traceability->initiator   = "client";
        $type_request                   = CFileTraceability::findRequestType($documentItem, $receiver);
        if ($type_request) {
    ...
    function getObject()
    {
        ...
        if ($object instanceof CFileTraceability || !$object) {
            ...
            if (!CAppUI::gconf('hubDocumentaire search advanced_search_patient_duplicate')) {
                return $object_found;
            }
            ...
            if ($patient_ids && !$object_found || !$object_found->_id) {
    ...
    function searchSejour($dateTime, CPatient $patient, $praticien_id = null)
    {
        ...
        $search_min_admit = CAppUI::gconf('hubDocumentaire search_interval search_min_admit');
        $search_max_admit = CAppUI::gconf('hubDocumentaire search_interval search_max_admit');
     ...
     */
    function searchConsultation($dateTime, CPatient $patient, $praticien_id = null, $sejour)
    {
        // Recherche de la consutlation dans le séjour
        $date = CMbDT::date($dateTime);

        $search_min_appointment = CAppUI::gconf('hubDocumentaire search_interval search_min_appointment');
        $search_max_appointment = CAppUI::gconf('hubDocumentaire search_interval search_max_appointment');
     ...
     */
    function searchOperation($dateTime, CPatient $patient, $praticien_id = null, $sejour)
    {
        //Recherche de la consutlation dans le séjour
        $date = CMbDT::date($dateTime);

        $search_min_surgery = CAppUI::gconf('hubDocumentaire search_interval search_min_surgery');
        $search_max_surgery = CAppUI::gconf('hubDocumentaire search_interval search_max_surgery');
     ...
     * @throws Exception
     */
    public function deleteTrace(CDocumentItem $docItem, string $source_name = null): void
    {
