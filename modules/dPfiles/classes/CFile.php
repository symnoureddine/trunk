...
class CFile extends CDocumentItem implements IIndexableObject, ImportableInterface
{
    ...
    /** @var null ?string */
    public static $directory_private = null;
    ...
    function completeFilePath()
    {
        ...
        if ($this->conf('migration_started') && !$this->conf('migration_finished')) {
            ...
            $_old_absolute_dir = self::$directory . "/$_old_sub_dir/$this->object_id";
    ...
    function store()
    {
        ...
        if (($ext = CMbPath::getExtension($this->file_name)) && !self::isAllowedExtension($ext)) {
    ...
    function loadNbPages()
    {
        if (strpos($this->file_type, "pdf") !== false && file_exists($this->_file_path)) {
            ...
            if (strpos($dataFile, "%PDF-1.4") !== false && $nb_count >= 2) {
                        ...
                        $nombre_temp     = explode(
                            ...
                            trim(
                                substr(
                                    $splitval,
                                    $position_count,
                                    strlen($splitval) - $position_count
                                )
                            ),
                            2
                        );
            ...
            } elseif (strpos($dataFile, "%PDF-1.3") !== false || strpos(
                    $dataFile,
                    "%PDF-1.6"
                ) !== false || $nb_count == 1) {
                ...
                $nombre_temp     = explode(
                    " ",
                    trim(substr($dataFile, $position_count, strlen($dataFile) - $position_count)),
                    2
                );
                $this->_nb_pages = intval(trim($nombre_temp[0]));
            }
     ...
     * @return null|string
     */
    function getThumbnailDataURI($options = ["width" => 640, "base64" => false, "quality" => "high"])
    {
    ...
    {
        $dir = CAppUI::conf('dPfiles CFile thumbnails_directory');
        if (!$dir) {
            $dir = CAppUI::conf('root_dir') . '/tmp/phpthumb/';
        }
        $n = $count;
    ...
    static function migrateFiles($limit)
    {
        ...
        $directories = glob(CFile::$directory . '/C*', GLOB_ONLYDIR);
            ...
            if (!is_dir($pathname)) {
                ...
                if ($prefix_format) {
                    ...
                    if (!$file->_id) {
                        ...
                        $prefix                   = self::getPrefix($file->file_date);
                        $filename                 = $prefix . $file->file_real_filename;
                        $file->file_real_filename = $filename;
                        if (!$file->rawStore()) {
    ...
    function fastTransfer(CStoredObject $from, CStoredObject $to)
    {
        ...
        $files_from   = glob(self::$directory . "/$sub_dir_from/$from->_id/*");

        $sub_dir_to = "$to->_class/" . intval($to->_id / 1000);
        $dir_to     = self::$directory . "/$sub_dir_to/$to->_id/";
    ...
    {
        $cache = new Cache(__METHOD__, 'last-dir', Cache::INNER);
     ...
     * @return string
     */
    static function getPrivateDirectory()
    {
     ...
     * @throws Exception
     */
    public function getResourceContext(): ?CItem
    {
        $context = $this->loadTargetObject();
        ...
        $item = new CItem($context);
        $item->setName($context::RESOURCE_NAME);
     ...
     * @throws Exception
     */
    public function getResourceCategory(): ?CItem
    {
        ...
        $item = new CItem($category);
        $item->setName(CFilesCategory::RESOURCE_NAME);
