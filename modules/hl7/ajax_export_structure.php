...
switch (CValue::get('value_entity')) {
  ...
  case 'PL': case 'UNT_MDCL': case 'UAC': case 'BTMNT': case 'L': case 'ETG': case 'AL': case 'PNT_CLCT': case 'PNT_LVRSN': case 'SL_RVL':
  break;
...
if ($cn_receiver_guid = CValue::sessionAbs("cn_receiver_guid")) {
  ...
  $receiver_hl7v2           = CInteropReceiverFactory::makeHL7v2();
...
foreach ($receivers as $_receiver) {
  if (!$hl7_handler->isMessageSupported($message, $code, $_receiver)) {
    continue;
  }
  $entity->_receiver = $_receiver;

  $ack_data = $hl7_handler->sendEvent($profil, $message, $code, $entity);
}
