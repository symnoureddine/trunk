...
class CHL7v2RecordAdmit extends CHL7v2MessageXML
{
    ...
    function handle(CHL7Acknowledgment $ack = null, CMbObject $newPatient = null, $data = [])
    {
        ...
        if (CMbArray::get($configs, "handle_patient_ITI_31")) {
            ...
            if (CModule::getActive("appFine") && CMbArray::get($configs, "handle_portail_patient")
                && in_array($exchange_hl7v2->code, CHL7v2RecordAdmit::$event_codes)) {
    ...
    function handleA05(CHL7Acknowledgment $ack, CSejour $newVenue, $data)
    {
        ...
        if (!$NDA->_id) {
            ...
            if ($this->isAmbiguousNDA($newVenue, $data, $NDA, $error_code)) {
                return $exchange_hl7v2->setAckAR($ack, $error_code, CAppUI::tr("CHL7Event-AR-E234"), $newVenue);
            }
    ...
    function createObjectByVisitNumber(CSejour $newVenue, $data)
    {
        ...
        if (!$object_found_by_vn->_id && CAppUI::conf("smp create_object_by_vn")) {
            ...
            if ($object_found_by_vn instanceof CConsultation) {
                ...
                if ($msg = $object_found_by_vn->createByDatetime(
                    $datetime,
    ...
    function getAttendingDoctor(DOMNode $node, CSejour $newVenue)
    {
        ...
        foreach ($PV17 as $_PV17) {
            if ($doctor_id = $this->getDoctor($_PV17, $mediuser)) {
                continue 1;
            }
        }
    ...
    {
        $sender = $this->_ref_sender;
        $PV1_8  = $this->queryNode("PV1.8", $node);
        if (!$PV1_8) {
            return;
        }

        $medecin    = new CMedecin();
        $medecin_id = $this->getDoctor($PV1_8, $medecin);
     ...
     * @throws Exception
     */
    function getConsultingDoctor(DOMNode $node, CSejour $newVenue)
    {
        $sender = $this->_ref_sender;
        $PV1_9  = $this->queryNode("PV1.9", $node);
        if (!$PV1_9) {
            return;
        }
        ...
        switch ($sender->_configs["handle_PV1_9"]) {
            ...
            case 'famille':
                $medecin    = new CMedecin();
                $medecin_id = $this->getDoctor($PV1_9, $medecin);
                if (!$medecin_id) {
                    return;
                }
