...
class CHL7v2RecordPerson extends CHL7v2MessageXML
{
        ...
        if (!$patientPI || !$IPP->_id) {
            ...
            if (!$newPatient->_id) {
                ...
                if ($sender->_configs["create_duplicate_patient"]) {
                    $newPatient->_force_duplicate = "DOUA";
                ...
                // Autorisé la création d'un doublon (AppFine et config.)
                if (!isset($newPatient->_force_duplicate)) {
                    // Patient retrouvé
                    if (CMbArray::get($sender->_configs, 'search_patient')
    ...
    function secondaryMappingPatient($data, CPatient $newPatient)
    {
        ...
        // On store le patient
        return CEAIPatient::storePatient($newPatient, $sender, false, $data);
    }
     ...
     * @throws Exception
     */
    function getZFD(DOMNode $node, CPatient $newPatient)
    {
        // Date lunaire
