...
class CHL7v2SegmentOBX extends CHL7v2Segment
{
    ...
    function build(CHEvent $event, $name = null)
    {
        ...
        if ($receiver->_configs['build_OBX_3'] == 'DMP' && CModule::getActive('dmp')) {
            $data[] = $this->getValueTypeDMP($this->docItem);
        } else {
            $data[] = array(
                array(
                    $this->docItem->file_category_id,
                    $this->docItem->loadRefCategory()->nom,
                )
            );
        }
        ...
        if (CModule::getActive("appFine") && ($object instanceof CEvenementMedical || $object instanceof CPatient)) {
            $data[] = CAppFineServer::generateOBX15($this->docItem, $this->object, $receiver);
        } elseif (CModule::getActive("appFineClient") && $event->_receiver->_configs["send_evenement_to_mbdmp"]
    ...
    function initializeDatas(
        CMbObject $object, $set_id, $value_type, $observation_identifier, $observation_value, $unit, $observation_datetime
    ) {
        $this->object                 = $object;
        $this->set_id                 = $set_id;
