  ...
  function build(CHEvent $event, $name = null) {
    ...
    $data[] = array(
      array(
        ...
        ($receiver->_configs["build_fields_format"] == "uppercase") ? CMbString::upper($correspondant->ville) : $correspondant->ville,
        null,
        $correspondant->cp,
      )
    );
    ...
    if ($correspondant->relation == "autre") {
      $roles = array_merge($roles, array($correspondant->relation_autre));
    }
    $data[] = array(
      $roles
    );
