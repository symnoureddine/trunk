...
class CHL7v3EventPRPA extends CHL7v3Event implements CHL7EventPRPA {
  ...
  function addPersonalRelationship(DOMNode $elParent, CPatient $patient) {
    ...
    $values = CInteropResources::loadEntryJV(
      CMbArray::get(CDMPValueSet::$JDV, "qualiteRepresentantLegal"),
  ...
  function addRepresentedOrganizationGroup(DOMNode $elParent, CGroups $group) {
    ...
    if (CAppUI::gconf('dmp general information_certificat', $group->_id) == 'siret' && $group->siret) {
      $identifiant = "3$group->siret";
    }
    elseif (CAppUI::gconf('dmp general information_certificat', $group->_id) == 'finess' && $group->finess) {
