    ...
    <tr>
      ...
      <td>
        ...
        <form name="edit-{{$_examen->_guid}}" action="?m=dPcabinet" method="post"
          ...
          {{if !$_examen->fait}}
            <button class="tick" type="button" onclick="$V(this.form.fait, '1'); this.form.onsubmit();">{{tr}}Done{{/tr}}</button>
          {{else}}
            <button class="cancel notext" type="button" onclick="$V(this.form.fait, '0'); this.form.onsubmit();">{{tr}}Cancel{{/tr}}</button>
          {{/if}}
        </form>
      </td>
    </tr>
