...
abstract class CAPITiers implements IAPITiers, IShortNameAutoloadable {
  ...
  public function storeConstants($data) {
    ...
    return CConstantReleve::storeReleveAndConstants($to_send, $this->patient_api->patient_id, self::getCronID($this->_class));
  }
   ...
   * @throws CAPITiersException
   */
  public static function getCronID($class_name) {
    $class_name = substr($class_name, strrpos($class_name, '\\') + 1);
    $user_id = CAppUI::conf("api $class_name user_api_id");
    if ($user_id === "") {
      throw new CAPITiersException(CAPITiersException::USER_API_ID_NOT_FOUND, $class_name);
    }
  ...
  public function loadAccessTokenUserMb() {
    ...
    $where             = array(
      "user_id" => "= '" . self::getCronID($this->_class) . "'"
    );
