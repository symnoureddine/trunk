...
class CAPIDispatcher implements IShortNameAutoloadable {
  ...
  static public function dispatchNew($post = []) {
    ...
    if (!$api_version) {
      throw new CAppFineException('CAPI-error-No API version provided');
    }

    if (!$product_name) {
      throw new CAppFineException('CAPI-error-No product name provided');
    }
    ...
    if (!class_exists($product_api)) {
      throw new CAppFineException("CAPI-error-Invalid product name");
    }
