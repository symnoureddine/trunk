...
abstract class CAPITiersOAuth extends CAPITiers {
  ...
  function sendRequest($url_comp = null, $url_param = null, $header = null, $method = "GET", $mime_type = null, $otherSource = null) {
    ...
    $source = $this->getSourceAPI($method);
    ...
    $source->host           = "$source->host/$url_comp";
    $source->_authorization = $header;
    $source->_multipart     = false;
    if ($mime_type) {
      $source->_mimetype = $mime_type;
    }

    $this->_datetime_send = CMbDT::dateTime();
    $start                = microtime(true) * 1000;
    $source->send(null, $url_param);
    $this->_datetime_received = CMbDT::dateTime();
    $source->host             = $url;

    $this->_acquittement  = $source->_acquittement;
    $end                  = microtime(true) * 1000;
    $this->_time_response = intval($end - $start);
    $this->_response_code = intval($source->_response_http_code);

    return json_decode($source->_acquittement, true);
  }
