...
Tarif = {
  ...
  editActes: function(form) {
    ...
    if ($V(form.tarif_id)) {
      ...
      if (form.codes_tarmed) {
        url.addElement(form.codes_tarmed);
      }
      if (form.codes_caisse) {
        url.addElement(form.codes_caisse);
      }
    }
...
Code = {
  ...
  edit: function (tarif_id) {
    ...
    url.addParam('tarif_id'    , tarif_id);
    url.requestModal(700, '80%');
    this.modal = url.modalObject;
    Code.url = url;
  },
  addCode: function (form, code, quantite, type, code_reference) {
    $V(form._add_code , 1);
    $V(form._dell_code, 0);
    $V(form._code     , code);
    $V(form._quantite , quantite);
    $V(form._type_code , type);
    if (code_reference) {
      $V(form._code_ref , code_reference);
    }
    else {
      $V(form._code_ref , $V(form.code_ref));
    }
    if (type == 'tarmed') {
      $V(form._version_tarmed , $V(form.version_tarmed));
    }
    return onSubmitFormAjax(form, {
      onComplete : function() {
