...
Consultation = window.Consultation || {
    ...
    editRDVModal: function (consult_id, chir_id, plage_id, pat_id, type, grossesse_id, sejour_id, callback) {
        ...
        url.modal(
            {
                ...
                onClose: function () {
                    if (type) {
                        TimelineImplement.refreshResume(null, null);
                    }
                    if (window.TdBTamm) {
                        TdBTamm.refreshTimeline(pat_id);
                    }
                }
            }
        );
        ...
        if (document.location.href.indexOf('m=cabinet') > 0
            && document.location.href.indexOf('tab=vw_planning_new') > 0
    ...
    editModal: function (consult_id, fragment, dossier_anesth_id, callback) {
        ...
        url.modal(
            {
                ...
                onClose: function () {
                    ...
                    if (window.TdBTamm) {
                        ...
                        if (form) {
                            TdBTamm.initUpdateListConsults($V(form.date));
                        }
                    }
    ...
    openConsultImmediate: function (patient_id, sejour_id, operation_id, grossesse_id, callback, type) {
     ...
     */
    checkByDateAndPrat: function (form, container) {
        new Url('cabinet', 'ajax_consult_immediate_validation')
    ...
    /**
     * Open the teleconsultation modal from the consultation targeted
     * @param consult_id
     */
    openTeleconsultation: function(consult_id, start, title) {
        new Url('teleconsultation', 'vw_visioconference')
            .addParam('consult_id',consult_id)
            .addParam('start',start)
            .requestModal('100%','100%', {incrustable: true, title : title});
    }
};
