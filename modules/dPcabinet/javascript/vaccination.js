...
Vaccination = {
   ...
   */
  makeInjection: function (form, calendar, use_stock) {
    if (use_stock) {
      ...
      new Url('oxCabinet', 'ajax_have_stock')
        ...
        .requestJSON(
          function (json) {
            ...
            Vaccination.createInjection(form, vaccines, calendar, true);
          }
        );
    }
    else {
      Vaccination.createInjection(form, vaccines, calendar, false);
    }
  },
   ...
   * @param {boolean} use_stock
   */
  createInjection: function (form, vaccines, calendar, use_stock) {
    Control.Modal.close();
    ...
    new Url('cabinet', 'do_vaccination', 'dosql')
      ...
      .addParam('injection_date', form.injection_date.value)
  ...
  checkRecallCurrentDate: function () {
    $$('input[name=injection_date_da]')[0].observe(
      'focusout', function (event) {
        if (Vaccination.isDateCoherent($$('input[name=birthday]')[0].value, $$('input[name=injection_date]')[0].value, $$('input[name=recall_age]')[0].value)) {
  ...
  saveFields: function (form) {
    if (form.speciality.value.trim() !== '' && form.speciality.value !== 'N/A') {
