...
<table class="layout" style="width: 100%;">
            ...
            <ul>
              {{foreach from=$opened_factures item=_opened_facture}}
                <li>
                  ...
                  {{$_opened_facture}} - {{tr}}CFacture.Opened the{{/tr}} {{mb_value object=$_opened_facture field=ouverture}}
                </li>
              {{/foreach}}
            </ul>
          </div>
  ...
  <tr>
    ...
    <td class="halfPane me-valign-top" id="{{if $displayFSE && $app->user_prefs.LogicielFSE == "oxPyxvital" && 'oxPyxvital'|module_active}}fse" style="height: 450px;{{else}}accident_travail{{/if}}">
      ...
      <td colspan="2">
        {{foreach from=$consult->_ref_factures item=_facture}}
          {{if $_facture->numero != 1}}
            <button type="button" class="search" onclick="Facture.edit('{{$_facture->_id}}', '{{$_facture->_class}}');">{{tr var1=$_facture->numero}}CFactureEtablissement-Bill number %s-court{{/tr}}</button>
          {{/if}}
        {{/foreach}}
      </td>
    </tr>
