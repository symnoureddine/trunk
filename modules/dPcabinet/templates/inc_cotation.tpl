...
<fieldset>
  ...
  <div style="text-align: center; font-weight: bold;">
    ...
    <span id="patient_acs"{{if $patient->acs != '1'}} style="display: none;"{{/if}}>
      ...
      <span id="patient_acs_contrat"{{if $patient->acs_type == 'none'}} style="display: none;"{{/if}}>
          &mdash; {{tr}}CPatient-contrat{{/tr}} <span id="patient_acs_contrat_value">{{$patient->acs_type|strtoupper}}</span>
      </span>
    <span>
  </div>

  <!-- Formulaire de selection de tarif -->
  <form name="selectionTarif" action="?m={{$m}}" method="post" onsubmit="return Cotation.selectTarif(this, {{$ald_mandatory}});">
    ...
    <table class="form me-no-box-shadow">
      {{if $conf.ref_pays != "2"}}
        {{if !$consult->valide}}
          <tr id="consult_concern_ald"{{if $consult->_ref_patient->ald != '1'}} style="display: none;"{{/if}}>
            ...
            <td style="width: 85%;">
              <input id="selectionTarif_concerne_ALD_1" type="radio" class="bool{{$ald_class}}" name="concerne_ALD" value="1"
              ...
              <input id="selectionTarif_concerne_ALD_0" type="radio" class="bool{{$ald_class}}" name="concerne_ALD" value="0"
          ...
          {{if $displayFSE && ($app->user_prefs.LogicielFSE == 'pv' || $app->user_prefs.LogicielFSE == 'oxPyxvital') && $consult->_ref_patient->ald == '0'}}
            <tr id="consult_force_ald">
              <th style="width: 15%;"><label for="selectionTarif_concerne_ALD" title="{{tr}}CConsultation-forcer-ald-desc{{/tr}}">{{tr}}CConsultation-forcer-ald{{/tr}}</label></th>
        ...
        {{elseif $consult->_ref_patient->ald || $consult->concerne_ALD}}
          <tr>
            ...
            <td style="width: 85%;">{{if $consult->concerne_ALD != ''}}{{mb_value object=$consult field=concerne_ALD}}{{/if}}</td>
          </tr>
        {{/if}}
      {{/if}}
      ...
      {{if !$consult->valide}}
        <tr>
          <th style="width: 15%;"><label for="choix" title="{{tr}}CConsultation-cotation-desc{{/tr}}">{{tr}}CConsultation-cotation{{/tr}}</label></th>
          <td style="width: 85%;">
            <select name="_tarif_id"  class="str" style="width: 130px;" onchange="this.form.onsubmit();">
  ...
  <form name="editExecTarif" action="?m={{$m}}" method="post" onsubmit="return onSubmitFormAjax(this, Reglement.reload.curry());">
    {{mb_key object=$consult}}
  ...
  <table class="form me-no-box-shadow">
        ...
        <tr>
          ...
          <td colspan="2">{{mb_field object=$consult field="_tokens_ccam" readonly="readonly" hidden=1}}
            {{foreach from=$consult->_ref_actes_ccam item=acte_ccam}}
              {{if $can_delete_acte}}
                <form name="delActeCCAP-{{$acte_ccam->_guid}}" method="post" onsubmit="return onSubmitFormAjax(this, Reglement.reload)">
          ...
          <td rowspan="{{$rowspan}}" style="vertical-align: middle;">
            <button type="button" class="edit" id="edit_actes" style="float: right;" onclick="Cotation.viewActes('{{$consult->_id}}', {{$ald_mandatory}});">{{tr}}CConsultation-action-gerer-actes{{/tr}}</button>
          </td>
        </tr>
      {{/if}}
      <tr>
        ...
        <td colspan="2">{{mb_field object=$consult field="_tokens_ngap" readonly="readonly" hidden=1}}
          {{foreach from=$consult->_ref_actes_ngap item=acte_ngap}}
            {{if $can_delete_acte}}
              <form name="delActeNGAP-{{$acte_ngap->_guid}}" method="post" onsubmit="return onSubmitFormAjax(this, Reglement.reload)">
    ...
    {{if "tarmed validation use_validate"|gconf}}
      <script>
        Main.add(function() {
          var url = new Url("tarmed", "ajax_vw_actes_tarmed");
          url.addParam("object_id"   , '{{$consult->_id}}');
          url.addParam("object_class", '{{$consult->_class}}');
          url.addParam("only_alerte" , '1');
          url.requestUpdate("alertes_tarmed_factu");
        });
      </script>
      <tr>
        <td class="text" colspan="11" id="alertes_tarmed_factu"></td>
      </tr>
    {{/if}}
      <tr>
        <th style="width: 15%;">{{tr}}CConsultation-codes_tarmed{{/tr}}</th>
        <td colspan="2">{{mb_field object=$consult field="_tokens_tarmed" readonly="readonly" hidden=1}}
          {{foreach from=$consult->_ref_actes_tarmed item=acte_tarmed}}
            ...
            {{foreachelse}}
            <span class="empty">{{tr}}CActeTarmed.none{{/tr}}</span>
          {{/foreach}}
        </td>
        <td rowspan="2" style="vertical-align: middle;">
          <button type="button" class="edit" id="edit_actes" style="float: right;" onclick="Cotation.viewActes('{{$consult->_id}}');">{{tr}}CConsultation-action-gerer-actes{{/tr}}</button>
        </td>
      </tr>
      <tr>
        <th style="width: 15%;">{{tr}}CConsultation-codes_prestation{{/tr}}</th>
        <td colspan="2">{{mb_field object=$consult field="_tokens_caisse" readonly="readonly" hidden=1}}
          {{foreach from=$consult->_ref_actes_caisse item=acte_caisse}}
            ...
            {{foreachelse}}
            <span class="empty">{{tr}}CActeCaisse.none{{/tr}}</span>
          {{/foreach}}
        </td>
      </tr>
    {{/if}}
  ...
  <form name="tarifFrm" action="?m={{$m}}" method="post" onsubmit="return checkForm(this)">
    ...
    <table class="form me-no-box-shadow">
      ...
      {{if $use_category_bill != "hide"}}
        <tr>
          <td>
            <label for="tarifFrm__category_facturation" {{if $use_category_bill == "obligatory"}}class="notNull"{{/if}}
          ...
          <td>
            ...
            <select name="_category_facturation" {{if $consult->_ref_facture->_id}}disabled{{/if}}
              ...
              {{foreach from=$categorys_factu item=_category}}
                <option value="{{$_category->_id}}" {{if $consult->_ref_facture->category_id == $_category->_id}}selected="selected"{{/if}}>
                  {{$_category->_view}}
                </option>
              {{/foreach}}
            </select>
      ...
      {{if $consult->valide}}
        <tr>
          ...
          <td style="width: 35%;">
            {{if $conf.ref_pays == 2}}
              {{$consult->secteur1|string_format:"%.2f"}} Pts
            {{else}}
          ...
          <td style="width: 35%;">
            {{if !$cotation_full}}
              <button type="button" class="search notext" style="float: right;" title="{{tr}}CReglement.full_cotation{{/tr}}"
                      onclick="Reglement.cotationModal();"></button>
            {{/if}}
            {{if $conf.ref_pays == 2}}
              {{math equation="x+y+z+a" x=$consult->secteur1 y=$consult->secteur2 z=$consult->secteur3 a=$consult->du_tva assign=somme_ch}}
              {{$somme_ch|string_format:"%.2f"}} Pts
            {{else}}
        ...
        <tr {{if !$cotation_full}}style="display: none;"{{/if}}>
          ...
          <td>
            {{if $conf.ref_pays == 2}}
              {{$consult->secteur2|string_format:"%.2f"}} Pts
            {{else}}
          ...
          <td>
            {{if $conf.ref_pays == 2}}
              {{$consult->du_patient|string_format:"%.2f"}} Pts
            {{else}}
      ...
      {{else}}
        ...
        <tr>
          ...
          <td style="width: 35%;">
            {{if !$consult->_ref_actes|@count}}
              {{if $conf.ref_pays == 2}}
                <input type="text" name="secteur1" value="{{$consult->secteur1}}" class="currency min|0 show|0 styled-element" onchange="Cotation.modifTotal('{{$consult->valide}}')" size="6" /> Pts
              {{else}}
                {{mb_field object=$consult field="secteur1" onchange="Cotation.modifTotal('"|cat:$consult->valide|cat:"')"}}
              {{/if}}
            {{else}}
              {{if $conf.ref_pays == 2}}
                <input type="text" name="secteur1" value="{{$consult->secteur1}}" class="currency min|0 show|0 styled-element" readonly="readonly" size="6" /> Pts
              {{else}}
          ...
          <td style="width: 35%;">
            ...
            {{if $conf.ref_pays == 2}}
              <input type="text" name="_somme" value="{{$somme}}" class="currency styled-element" size="6" onchange="Cotation.modifSecteur2()" /> Pts
            {{else}}
        ...
        <tr {{if !$cotation_full}}style="display: none;"{{/if}}>
          ...
          <td>
            {{if $conf.ref_pays == 2}}
              <input type="text" name="secteur2" value="{{$consult->secteur2}}" class="currency show|0 styled-element" onchange="Cotation.modifTotal('{{$consult->valide}}')" size="6"/> Pts
            {{else}}
          ...
          {{if !$consult->_ref_facture->patient_date_reglement && !$consult->sejour_id}}
            ...
            <td>
              {{if $conf.ref_pays == 2}}
                <input type="text" name="du_patient" value="{{$consult->du_patient}}" class="currency show|0 styled-element" size="6" /> Pts
              {{else}}
        ...
        <tr {{if !$cotation_full || !"dPccam codage use_cotation_ccam"|gconf}}style="display: none;"{{/if}}>
          ...
          <td>
            ...
            {{if (!@$modules.tarmed->_can->read || !'tarmed CCodeTarmed use_cotation_tarmed'|gconf) && (!'fse'|module_active || ($app->user_prefs.LogicielFSE != 'pv' && $app->user_prefs.LogicielFSE != 'oxPyxvital') && $displayFSE)}}
              <button id="reglement_button_tiers_payant" type="button" class="tick" onclick="Cotation.tiersPayant();">
      ...
      <tr>
        <td colspan="4" class="button">
          ...
          {{if !$consult->_current_fse && !count($consult->_ref_facture->_ref_reglements)
            ...
            <button class="cancel" type="button" id="buttonCheckActe" onclick="Cotation.checkActe(this);">
              {{tr}}CConsultation-action-Reopen the quotation{{/tr}}
            </button>
          {{/if}}
          <button class="print" type="button" onclick="Cotation.printActes('{{$consult->_id}}')">{{tr}}CConsultation-action-print-actes{{/tr}}</button>
          {{if $frais_divers|@count}}
            <button class="add" type="button" onclick="Cotation.createSecondFacture();">{{tr}}CConsultation-action-add-invoice{{/tr}}</button>
          {{/if}}
        </td>
      </tr>
      {{elseif !$consult->_ref_facture->patient_date_reglement}}
        <tr>
          <td colspan="4" class="button">
            ...
            {{/if}}
            {{if $closing_rights}}
              <button id="reglements_button_cloturer_cotation" class="submit" type="button" onclick="Cotation.validTarif();">{{tr}}CConsultation-action-close-cotation{{/tr}}</button>
            {{/if}}
            <button class="cancel" type="button" onclick="Cotation.cancelTarif('delActes', null, '{{$app->user_prefs.autoCloseConsult}}')">{{tr}}CConsultation-action-empty-cotation{{/tr}}</button>
          </td>
        </tr>
      {{/if}}
    </table>
  ...
  {{if $frais_divers|@count}}
    <form name="addFactureDivers" action="" method="post" onsubmit="return onSubmitFormAjax(this, Reglement.reload.curry());">
  ...
  <div id="modal-concerne_ALD-mandatory" style="display: none;">
    <form name="editALDMandatory" method="" action="" onsubmit="return Cotation.submitALDMandatory(this);">
      ...
      <table class="form">
        ...
        <tr>
          <td class="button">
            <input id="editConsultALD-{{$consult->_id}}_concerne_ALD_1" type="radio" class="bool" name="concerne_ALD" value="1" onclick="this.form.onsubmit();"/>
            <label for="editConsultALD-{{$consult->_id}}_concerne_ALD_1">{{tr}}Yes{{/tr}}</label>
            <input id="editConsultALD-{{$consult->_id}}_concerne_ALD_0" type="radio" class="bool" name="concerne_ALD" value="0" onclick="this.form.onsubmit();"/>
