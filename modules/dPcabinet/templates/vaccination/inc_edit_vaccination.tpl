...
<script>
  Main.add(function () {
    ...
    edit_vaccination.speciality.addEventListener('change', function (e) {
      $V(edit_vaccination.quantity, 0);
  ...
  <table class="form me-no-box-shadow">
      ...
      {{me_form_field label=Vaccine nb_cells=4 layout=true}}
        {{if $label_read_only == 1 && $injection->_ref_vaccinations}}
          {{foreach from=$injection->_ref_vaccinations item=_vaccination}}
            <input id="edit-injection_vaccine-{{$_vaccination->type}}" type="checkbox" class="form-vaccines" disabled checked name="vaccines[]" value="{{$_vaccination->type}}">
            <label for="vaccine-{{$_vaccination->type}}">{{$_vaccination->type}}</label>
          {{/foreach}}
        {{else}}
          <input id="edit-injection_vaccine-other" type="checkbox" class="form-vaccines" disabled checked name="vaccines[]" value="Autre">
    ...
    <tr>
      {{me_form_field label=CVaccination nb_cells=4 layout=true}}
        <input id="edit-injection_vaccination_yes" type="radio" name="vaccination" value="1" {{if $vaccinated}}checked{{/if}}>
        <label for="vaccination_yes">{{tr}}Yes{{/tr}}</label>

        <input id="edit-injection_vaccination_no" type="radio" name="vaccination" value="0" {{if !$vaccinated}}checked{{/if}}>
    ...
    {{if $stock_active}}
      <tr class="stocks" style="display: none;">
        ...
        {{me_form_field mb_class=CProductStockGroup mb_field=quantity nb_cells=4 field_class="me-input-field-max-w25"}}
          ...
          <div style="display: inline-block; width: 50%; font-size: 1.2em; margin-left: 8px;" id="qte-text"> <span class="qte-left"></span> <span class="tr-qte-left"></span></div>
          <script>
            Main.add(function () {
    ...
    {{* Injection date *}}
    <tr>
      {{me_form_field mb_class=$injection mb_field=injection_date nb_cells=4}}
      {{mb_field object=$injection field=injection_date register=true form="edit-injection"}}
