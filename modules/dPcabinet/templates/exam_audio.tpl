  ...
  <table class="main" id="weber">
    ...
    <tr>
      <td style="text-align: right">
        <select name="consultation" onchange="ExamAudio.addOldExamaudio(this.value, '{{$consultation_id}}', '{{$_conduction}}', '{{$_oreille}}')">
    ...
    <tr>
      <td class="radiointeractive" colspan="2">
        ...
        <input type="radio" name="_conduction" value="pasrep" {{if $_conduction == "pasrep"}}checked{{/if}} />
        <label for="_conduction_pasrep" title="Pas de réponse pour la saisie intéractive">{{tr}}dPcabinet-no-response{{/tr}}</label>
      </td>
    </tr>
    <tr>
      <td colspan="2">
        <table class="form" id="allvalues">
          ...
          <tr>
            <th>{{tr}}dPcabinet-no-response{{/tr}}</th>
            {{foreach from=$frequences key=index item=frequence}}
              <td><input type="text" name="_droite_pasrep[{{$index}}]" class="num min|-10 max|120"
                         value="{{$exam_audio->_droite_pasrep.$index}}" tabindex="{{$index+150}}" size="4" maxlength="4"/></td>
            {{/foreach}}
          </tr>
          {{if $old_consultation_id}}
            <tr>
          ...
          <tr>
            <th>{{tr}}dPcabinet-no-response{{/tr}}</th>
            {{foreach from=$frequences key=index item=frequence}}
              <td><input type="text" name="_gauche_pasrep[{{$index}}]" class="num min|-10 max|120"
                         value="{{$exam_audio->_gauche_pasrep.$index}}" tabindex="{{$index+50}}" size="4" maxlength="4"/></td>
            {{/foreach}}
          </tr>
          {{if $old_consultation_id}}
            <tr>
          ...
          <tr>
            <td class="button" colspan="{{$colspan}}">
              <button class="submit" type="button" onclick="onSubmitFormAjax(this.form, function(){
    ...
    <tr>
      <td colspan="2" class="radiointeractive">
        <table class="form" id="allvocales">
          ...
          <tr>
            <td class="button" colspan="{{$colspan}}">
              <button type="button" class="submit" onclick="onSubmitFormAjax(this.form, function(){
                ExamAudio.updateAll({{if $old_consultation_id}}{{$old_exam_audio->_id}}{{/if}})})">
                {{tr}}Save{{/tr}}
              </button>
            </td>
          </tr>
