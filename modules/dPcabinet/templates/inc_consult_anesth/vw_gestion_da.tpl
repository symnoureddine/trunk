        ...
        <tr>
          <td class="button">
            ...
            {{else}}
              ...
              <button type="button" class="link" {{if $operation->annulee || !$perm_to_duplicate}}disabled="disabled"{{/if}}
      ...
      <tr>
        <td class="button">
          ...
          {{if $smarty.foreach.consults_anesth.last && $ops_sans_dossier_anesth|@count != 0}}
            <button class="down" id="didac_button_duplicate" onclick="createDA('{{$first_operation->_id}}','{{$consult_anesth->_id}}', 1, '{{$first_operation->sejour_id}}');" style="position:relative;top:15px;"
              {{if !$perm_to_duplicate}}disabled="disabled" title="{{tr var1=$consult->_ref_praticien->_ref_function}}CConsultAnesth-user_no_this_cabinet{{/tr}}"{{/if}}
