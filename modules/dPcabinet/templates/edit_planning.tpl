...
<script>
  ...
  createAndCloseRdv = function(form) {
    form.onsubmit = function() {return onSubmitFormAjax(this)};
    if (!form.callback) {
  ...
  rdvTriggerForms = function(form) {
    ...
    return onSubmitFormAjax(form, {onComplete: function() { $V(form.elements.ajax, ''); $V(form.elements.callback, ''); }});
  };
...
{{if !$dialog}}
  <div class="me-text-align-left">
    <a class="button new me-primary me-margin-2" id="add_edit_consult_button_new_consult" href="?m={{$m}}&tab={{$tab}}&consultation_id=">
...
<form name="editFrm" action="?m={{$m}}" class="watched" method="post" onsubmit="return {{if $modal}}onSubmitFormAjax(this);{{else}}checkFormRDV(this);{{/if}}">
  ...
  {{if $consult->_id && !$dialog}}
    <div class="me-text-align-left">
      <a class="button search me-secondary" href="?m={{$m}}&tab=edit_consultation&selConsult={{$consult->_id}}" style="float: right;">
    ...
    {{if $lots|@count}}
      ...
      <div class="small-{{if $pct_lot < "notifications general pct_alert_consult"|gconf}}info{{else}}warning{{/if}}">
        ...
        {{if $pct_lot >= "notifications general pct_alert_consult"|gconf}}
          <br/> {{tr}}CLotSms.pct_alert_consult{{/tr}} {{$pct_lot}}%
        {{/if}}
      </div>
          ...
          <table class="form me-no-box-shadow me-small-form">
            ...
            {{if $multi_ressources}}
              ...
              <tr>
                ...
                <td>
                    <input type="checkbox" name="__no_patient" onclick="$V(this.form.no_patient, $V(this)?1:0); $V(this.form.patient_id, null); $V(this.form._patient_view, ''); showPatient(this); changeTrads(this);" id="editFrm___no_patient">
            ...
            <tr>
              <th class="narrow">
                <label for="chir_id" title="{{tr}}CConsultation-Practitioner for consultation-desc{{/tr}}">{{tr}}common-Practitioner{{/tr}}</label>
              </th>
              <td>
                <select name="chir_id" {{if $plage_synchronized}}disabled{{/if}} style="width: 15em;" class="notNull"
            ...
            {{if !$consult->patient_id && !$consult->reunion_id}}
              <tr>
                <th>
                  <label for="_pause" title="{{tr}}CConsultation-Planning a break{{/tr}}">{{tr}}Pause{{/tr}}</label>
                </th>
                <td>
                  <input type="checkbox"
                         name="_pause"
            ...
            <tr id="viewPatient" {{if $consult->_id && $consult->patient_id==0}}style="display:none;"{{/if}}>
              ...
              {{if $plage_synchronized}}
                <input type="text" name="_patient_view" style="width: 15em;" value="{{$pat->_view}}"  disabled/>
              {{else}}
              ...
              {{/if}}
                ...
                {{if $can_edit_pat && !$plage_synchronized}}
                  <button class="search notext me-tertiary" id="add_edit_button_pat_selector" type="button" onclick="PatSelector.init()">{{tr}}Search{{/tr}}</button>
                  <button id="button-edit-patient" type="button" onclick="Patient.editModal(this.form.patient_id.value, 0, 'window.parent.afterEditPatient')"
                          class="edit notext me-tertiary me-dark" {{if !$pat->_id}}style="display: none;"{{/if}}>
                    {{tr}}Edit{{/tr}}
                  </button>
                {{else}}
                  {{if !$plage_synchronized}}
                ...
                {{/if}}
              {{if !$plage_synchronized}}
                <input type="text" class="me-placeholder" name="_seek_patient" style="width: 13em; {{if !$can_edit_pat}}display:none;{{/if}}"
                       placeholder="{{tr}}fast-search{{/tr}}" "autocomplete" onfocus="window._tmp_seek_patient='';"
              ...
              <td>
                <table class="form me-no-box-shadow me-small-form">
                  <tr>
                    ...
                    <td>
                      ...
                      {{else}}
                        ...
                        {{if !$plage_synchronized}}
                          <button class="search notext me-tertiary" id="addedit_planning_button_select_date" type="button" onclick="PlageConsultSelector.init(0,0)">{{tr}}common-action-Choice of schedule{{/tr}}</button>
                        {{/if}}
                      {{/if}}

                      <input type="hidden" name="_date_planning" value="{{$consult->_date}}">
                      ...
                      {{if !$multi_ressources && !$consult->reunion_id}}
                        {{if $following_consultations|@count}}
                          {{if !$plage_synchronized}}
                            <button class="agenda button me-tertiary" id="buttonMultiple" type="button" onclick="PlageConsultSelector.init(1, 1);" id="buttonMultiple">
                        ...
                        {{else}}
                          {{if !$plage_synchronized}}
                            <button class="agenda notext me-tertiary" id="buttonMultiple" type="button" onclick="PlageConsultSelector.init(1,0)">{{tr}}CConsultation-action-Multiple consultation{{/tr}}</button>
                          {{/if}}
                        {{/if}}

                        {{if !$plage_synchronized}}
                          <button class="me-tertiary" title="{{tr}}CPlageconsult-action-Next available time slot|pl{{/tr}}" style="padding-right: 0px;" type="button" onclick="CreneauConsultation.modalPriseRDVTimeSlot($V(this.form.chir_id), $V(this.form._function_id) , 1, null, 1);">
                            <i class="far fa-calendar-plus" style="vertical-align: top;" aria-hidden="true"></i>
                          </button>
                        {{/if}}
                      {{/if}}
                      </td>
                    ...
                    <tr>
                      ...
                      <td>
                        {{* this.blur to void infinie alert message *}}
                      {{if $plage_synchronized}}
                        <input type="text" name="heure" value="{{$consult->heure}}" style="width: 15em;" disabled readonly="readonly" />
                      {{else}}
                        <input type="text" name="heure" value="{{$consult->heure}}" style="width: 15em;"
                        {{if !$multi_ressources}}onfocus="this.blur();PlageConsultSelector.init(0,0)"{{/if}} readonly="readonly" />
                      {{/if}}
                          ...
                          {{if $consult->_id}}
                            <span>
                              ...
                              {{if $consult->_count_matching_sejours}}
                                <button type="button" class="link me-tertiary" onclick="linkSejour()"
                                        {{if $consult->valide}}
                                          disabled
                              ...
                              {{if ("dPcabinet CConsultation attach_consult_sejour"|gconf && "dPcabinet CConsultation create_consult_sejour"|gconf && $consult->_ref_praticien->activite === "salarie") || $create_sejour_consult}}
                                <button type="button" class="link me-tertiary" onclick="$V(this.form._force_create_sejour, 1); $V(this.form.sejour_id, ''); $('addedit_planning_button_save').click();">{{tr}}CConsultation-_link_new_sejour{{/tr}}</button>
                              {{/if}}
                            </span>
                          {{/if}}
                          <br />
                          <a class="button new me-secondary" id="addedit_button_new_rdv_same_patient" href="?m=cabinet&a=edit_planning&dialog={{$dialog}}&pat_id={{$consult->patient_id}}&consultation_id=&date_planning={{$consult->_date}}&chir_id={{$chir->_id}}">
                  ...
                  <tr>
                    ...
                    <td>
                      <input type="checkbox" {{if $plage_synchronized}}disabled{{/if}} name="_check_adresse" value="1"
                    ...
                    {{if $create_consult_sejour && (!$consult->_id || $consult->sejour_id)}}
                      ...
                      {{if $required_uf_soins != "no"}}
                        <tr>
                          ...
                          <td colspan="3">
                            <select {{if $plage_synchronized}}disabled{{/if}}name="_uf_soins_id" class="ref {{if $required_uf_soins == "obl" && !$consult->_id}}notNull{{/if}}" style="width: 15em" {{if $consult->_id}}disabled{{/if}}>
                              <option value="">&mdash; {{tr}}Choose{{/tr}}</option>
                              {{foreach from=$ufs.soins item=_uf}}
                                <option value="{{$_uf->_id}}" {{if $consult->_uf_soins_id == $_uf->_id}}selected{{/if}}>
                      ...
                      {{if $required_uf_med != "no"}}
                        <tr>
                          ...
                          <td colspan="3">
                            ...
                            <select {{if $plage_synchronized}}disabled{{/if}} name="_uf_medicale_id" class="ref {{if $required_uf_med == "obl" && !$consult->_id}}notNull{{/if}}" style="width: 15em" {{if $consult->_id}}disabled{{/if}}>
                              <option value="">&mdash; {{tr}}Choose{{/tr}}</option>
                              {{foreach from=$ufs.medicale item=_uf}}
                                <option value="{{$_uf->_id}}" {{if $consult->_uf_medicale_id == $_uf->_id}}selected{{/if}}>
                                  {{mb_value object=$_uf field=libelle}}
                                </option>
                              {{/foreach}}
                            </select>
                      ...
                      {{if $use_charge_price_indicator != "no"}}
                        <tr>
                          ...
                          <td colspan="3">
                            ...
                            {{/if}}
                            <select {{if $plage_synchronized}}disabled{{/if}} class="ref{{if $use_charge_price_indicator == "obl" && (!$consult->_id || $same_year_charge_id)}} notNull{{/if}}" name="_charge_id" {{if $consult->_id && !$same_year_charge_id}}disabled{{/if}}>
                              ...
                              {{foreach from='Ox\Mediboard\PlanningOp\CChargePriceIndicator::getList'|static_call:$type_charge item=_cpi name=cpi}}
                                <option value="{{$_cpi->_id}}" {{if $consult->_charge_id == $_cpi->_id}}selected{{/if}}>
                      ...
                      <select {{if $plage_synchronized}}disabled{{/if}} name="duree">
                        ...
                        {{if $consult->duree > 15}}
                          ...
                          {{if $plageConsult->_id}}
                            ...
                            {{math equation=(x-y*60) x=$duree_min y=$duree_hour assign=duree_min}}
                          {{/if}}
                          <option value="{{$i}}" selected>x{{$i}} {{if $plageConsult->_id}}({{if $duree_hour}}{{$duree_hour}}{{tr}}common-hour-court{{/tr}}{{/if}}{{if $duree_min}}{{$duree_min}}{{tr}}common-noun-minutes-court{{/tr}}{{/if}}){{/if}}</option>
                        {{/if}}
                      </select>
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td>
                      <select {{if $plage_synchronized}}disabled{{/if}} name="_function_id" style="width: 15em;" onchange="refreshListCategorie(null, null, this.value);
                  ...
                  {{if $display_elt}}
                    <tr>
                      ...
                      <td>
                        <input {{if $plage_synchronized}}disabled{{/if}} type="text" name="libelle" class="autocomplete"
                        ...
                        <button type="button" class="cancel notext me-tertiary me-dark" onclick="$V(this.form.element_prescription_id, ''); $V(this.form.libelle, '');"></button>
                      </td>
                    </tr>
                  {{/if}}
                </table>
              ...
              <td id="multiplePlaces">
                {{foreach from=1|range:$app->user_prefs.NbConsultMultiple-1 item=j}}
                  ...
                  <fieldset id="place_reca_{{$j}}" style="display: none;">
                    <legend>{{tr}}common-Rendez-vous{{/tr}} {{$j+1}} <button class="button cleanup notext" type="button" onclick="resetPlage('{{$j}}')">{{tr}}Delete{{/tr}}</button></legend>
                    <form>
                    ...
                    <p><textarea name="rques_{{$j}}" placeholder="{{tr}}CConsultation-Note rdv{{/tr}} {{$j+1}}..." style="width: 30em;"></textarea></p>
                    <p><textarea name="docs_necessaires_{{$j}}" placeholder="{{tr}}CConsultation-docs_necessaires rdv{{/tr}} {{$j+1}}..." style="width: 30em;"></textarea></p>
      ...
      <td colspan="2">
        <table class="form me-no-box-shadow">
          <tr>
            <td class="button">
              {{if $consult->_id}}
                  <button class="modify me-primary" id="addedit_planning_button_save" type="submit" onclick="return submitRDV();">
                    {{tr}}Save{{/tr}}
                  </button>
                {{if !$consult->_locks || $can->admin}}
                  ...
                  {{/if}}
                {{/if}}
                <button class="print me-tertiary me-dark" id="print_fiche_consult" type="button" onclick="printForm();"
              ...
              {{else}}
                <button style="display: none;" class="submit me-primary" id="addedit_planning_button_submitRDV_final" type="submit" onclick="return submitRDV();">
                  {{tr}}Create{{/tr}}
                </button>
                <button class="submit me-primary" id="addedit_planning_button_submitRDV" type="button" onclick="createRdv(this.form);">
                  {{tr}}Create{{/tr}}
                </button>

                {{if $ex_class_events}}
                  <button type="button" class="forms oneclick me-tertiary" onclick="rdvTriggerForms(this.form);">
                    {{tr}}CConsultation-action-Create and open appointment form{{/tr}}
                  </button>
                {{/if}}

                {{if $dialog}}
                  <button class="save me-secondary" type="button" onclick="createAndCloseRdv(this.form);">
