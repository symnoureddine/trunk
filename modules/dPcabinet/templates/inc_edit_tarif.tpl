...
<form name="editFrm" action="#" method="post" onsubmit="return Tarif.submit(this);">
  ...
  <table class="main form">
      ...
      <tr>
        ...
        <td>
          ...
          <select name="_type" onchange="Tarif.updateOwner();">
            <option value="chir"     {{if $tarif->chir_id}}     selected="selected" {{/if}}>{{tr}}CConsultation-Personal price{{/tr}}</option>
            <option value="function" {{if $tarif->function_id}} selected="selected" {{/if}}>{{tr}}CConsultation-Office price{{/tr}}</option>
            {{if "dPcabinet Tarifs show_tarifs_etab"|gconf}}
              <option value="group" {{if $tarif->group_id}}    selected="selected" {{/if}}>{{tr}}CConsultation-Etablishment price{{/tr}}</option>
            {{/if}}
          </select>
        </td>
      </tr>
    ...
    {{else}}
      <tr>
        ...
        <td>
          ...
          <select name="chir_id" onchange="getForm('editFrm').function_id.value = (this.value != '') ? '' : '{{$prat->function_id}}';">
    ...
    <tr {{if "dPccam codage use_cotation_ccam"|gconf != "1"}}style="display:none;"{{/if}}>
      ...
      <td>
        <button id="edit_actes_tarif" type="button" class="edit notext" onclick="Tarif.editActes(this.form);" {{if $tarif->_type == 'group'}} disabled="1"{{/if}}>{{tr}}CTarif-action-Enter the act|pl{{/tr}}</button>
      </td>
    </tr>
    {{if "dPccam codage use_cotation_ccam"|gconf == "1"}}
      {{if ($prat->_id && $prat->isExecutantCCAM()) || (!$prat->_id)}}
        <tr>
          ...
          <td>
            ...
            <div class="codes">
              {{foreach from=$tarif->_codes_lpp item=_code_lpp}}
                <span onmouseover="ObjectTooltip.createDOM(this, 'DetailNGAP-{{$_code_lpp}}');">{{$_code_lpp}}</span>
                <br/>
    ...
    {{elseif @$modules.tarmed->_can->read && 'tarmed CCodeTarmed use_cotation_tarmed'|gconf}}
      <tr>
      ...
      <tr>
        ...
        <td>
          ...
          {{else}}
            {{$tarif->secteur1}} {{tr}}CFactureItem.pts{{/tr}}
          {{/if}}
        </td>
      </tr>
    {{/if}}
    ...
    <tr>
      <td class="button" colspan="2">
        ...
        {{if $tarif->_id}}
          ...
          {{if $conf.ref_pays == "2" && "tarmed"|module_active && 'tarmed CCodeTarmed use_cotation_tarmed'|gconf}}
            <button class="edit" type="button" onclick="Code.edit('{{$tarif->_id}}');"> {{tr}}CTarif-action-Management codes{{/tr}} </button>
          {{/if}}
    ...
    <td colspan="2">
      ...
      {{else}}
        <div class="big-info">
          ...
          {{else}}
            {{tr}}CTarif-msg-To create a tariff containing Tarmed and delivery codes, perform a real quotation during a three-step consultation{{/tr}}:
          {{/if}}
          <ul>
            <li>{{tr}}CTarif-action-Add acts in the pane{{/tr}} <strong>{{tr}}CCodable-actes{{/tr}}</strong></li>
