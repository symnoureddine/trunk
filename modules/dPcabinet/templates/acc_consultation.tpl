...
<ul id="tab-consult" class="control_tabs me-align-auto" {{if $synthese_rpu}}style="display: none;"{{/if}}>
  ...
  {{if $rpu && $app->_ref_user->isPraticien()}}
    <li onmousedown="Soins.loadObservations('{{$sejour->_id}}');"><a href="#obs">{{tr}}soins.tab.obs{{/tr}}</a></li>
  {{/if}}
...
{{if $show_grossesse}}
  <div id="dossierGrossesse" class="me-align-auto me-padding-2 me-no-border-bottom">
    {{mb_include module=maternite template=inc_vw_tdb_grossesse grossesse=$consult->_ref_grossesse creation_mode=0 is_tdb_maternite=false is_consultation=true}}
  </div>
