                ...
                <select name="date" class="{{$plageSel->_props.date}}" style="width: 15em;">
                  ...
                  {{foreach from=$listDaysSelect item=curr_day}}
                    <option value="{{$curr_day}}"
                      ...
                      {{if !$plageSel->_id && array_key_exists($curr_day, $holidays) && !$app->user_prefs.allow_plage_holiday}}disabled{{/if}}
                    >
  ...
  <table class="form">
    <tr>
      ...
      <td class="button" colspan="4">
        ...
        <button type="button" class="button add me-secondary" onclick="extendPlage('{{$plageSel->_id}}', $V(this.form._type_repeat), $V(this.form._repeat) );">
          {{tr}}Extend{{/tr}}
        </button>
