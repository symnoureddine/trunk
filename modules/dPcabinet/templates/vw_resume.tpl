        ...
        <table class="tbl me-margin-bottom--3 me-no-border-bottom me-no-border-radius-bottom ">
          <tr>
            <th colspan="10" class="title">
              {{$patient->_view}} <br />
              {{tr var1=$patient->naissance|date_format:$conf.date }}CPatient-Born on %s{{/tr}} ({{mb_value object=$patient field=_age}}) <br />
    ...
    <td>
      <table class="tbl">
          ...
          <th colspan="4" class="title">
            <div style="float:right">
              <input type="checkbox" id="toggle_compta" onchange="Compta.toggle();"
                ...
                />
              <label for="toggle_compta" title="{{tr}}CPatient-action-Show / hide accounting data|pl-desc{{/tr}}">{{tr}}CPatient-Accounting-court{{/tr}}</label>
        ...
        <tr>
          ...
          <th class="compta">
            <label title="{{tr}}CReglement-Patient and third party regulations, in red if not fully adjusted-desc{{/tr}}">
            ...
            <tr>
              <td class="text top">
                ...
                {{if isset($_consult->_latest_constantes|smarty:nodefaults)}}
                  ...
                  <strong>{{tr}}CConstantesMedicales-poids{{/tr}} :</strong>  {{if $_latest_constantes->poids}} {{$_latest_constantes->poids}}  {{tr}}CPatient-unit Kg-court{{/tr}}{{else}}-{{/if}}
                  <strong>{{tr}}CConstantesMedicales-taille{{/tr}} :</strong> {{if $_latest_constantes->taille}}{{$_latest_constantes->taille}} {{tr}}CPatient-unit cm-court{{/tr}}{{else}}-{{/if}}
              ...
              <td class="top text patient-forms {{if !$app->user_prefs.dPpatients_show_forms_resume}} forms-hide{{/if}}">
                {{if $_consult->_ref_forms}}
                  <ul>
                    {{foreach from=$_consult->_ref_forms item=_ex_link}}
                      <li>
                        <a href="#1" onclick="ExObject.display('{{$_ex_link->ex_object_id}}', '{{$_ex_link->ex_class_id}}', '{{$_consult->_guid}}')">
                          [{{mb_value object=$_ex_link field=datetime_create}}] {{$_ex_link->loadRefExClass()}}
                        </a>
                      </li>
                    {{/foreach}}
                  </ul>
                ...
                {{if $_sejour->_ref_forms}}
                  <ul>
                    {{foreach from=$_sejour->_ref_forms item=_ex_link}}
                      <li>
                        <a href="#1" onclick="ExObject.display('{{$_ex_link->ex_object_id}}', '{{$_ex_link->ex_class_id}}', '{{$_sejour->_guid}}')">
                          [{{mb_value object=$_ex_link field=datetime_create}}] {{$_ex_link->loadRefExClass()}}
                        </a>
                      </li>
                    {{/foreach}}
                    ...
                    {{if $_op->_ref_forms}}
                      <ul>
                        {{foreach from=$_op->_ref_forms item=_ex_link}}
                          <li>
                            <a href="#1" onclick="ExObject.display('{{$_ex_link->ex_object_id}}', '{{$_ex_link->ex_class_id}}', '{{$_op->_guid}}')">
                              [{{mb_value object=$_ex_link field=datetime_create}}] {{$_ex_link->loadRefExClass()}}
                            </a>
                          </li>
                        {{/foreach}}
