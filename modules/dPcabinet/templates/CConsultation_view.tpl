...
<table class="tbl">
  ...
  {{if $can_edit}}
    {{if $consultation->motif && !$app->user_prefs.limit_prise_rdv}}
      <tr>
        <td colspan="2" class="text">
          {{mb_value object=$consultation field=motif}}
        </td>
      </tr>
    {{/if}}
    ...
    <tr>
      <td colspan="2" class="text">
        {{mb_value object=$consultation field=rques}}
      </td>
    </tr>
    {{/if}}
  {{/if}}
        ...
        <td class="button" colspan="2">
         ...
         {{if $patient->_id}}
            ...
            {{if @$modules.brancardage->_can->read && "brancardage General use_brancardage"|gconf && $consultation->sejour_id}}
              <div id="demande_brancard-{{$consultation->_guid}}--"
                   style="float: right;" class="me-display-flex me-float-initial">
                {{mb_include module=brancardage template=inc_exist_brancard colonne="demande_brancard" object=$consultation}}
              </div>
            {{/if}}
