...
{{mb_ternary var=view test=$acte->_id value="-"|cat:$acte->_id other=""}}
  ...
  Main.add(function() {
    ...
    url.autoComplete(form._executant_view, null, {
      ...
      afterUpdateElement: function(field, selected) {
        ...
        $V(getForm('editActeNGAP')._executant_spec_cpam, selected.down('.view').getAttribute('data-spec_cpam'));
      }
    });
    ...
    {{if !$acte->_id}}
      ...
      url = new Url("cabinet", "httpreq_do_ngap_autocomplete");
      ...
      url.autoComplete(form.code, 'code_auto_complete', {
        ...
        updateElement: function(selected) {
          var form = getForm('editActeNGAP-code');
          $V(form.code, selected.down('.code').innerHTML, false);
          ActesNGAP.syncCodageField(form.code, '');
          ActesNGAP.refreshTarif('');
        },
    ...
    {{else}}
      {{assign var=date_min value='Ox\Core\CMbDT::date'|static_call:'-27 months'}}
    ...
    {{if $acte->_id}}
      Calendar.regField(getForm('editActeNGAP-accord_prealable{{$view}}').elements['date_demande_accord'], {limit: {stop: '{{'Ox\Core\CMbDT::date'|static_call:$acte->execution}}'}});
    {{/if}}
  });
</script>
...
<tr {{if $acte->_id}}class="acteNGAP-line" data-view="{{$view}}"{{/if}}>
  ...
  <td>
    ...
    {{else}}
      ...
      {{else}}
        <form name="editActeNGAP-code{{$view}}" action="?" method="post" onsubmit="return false;">
  ...
  {{if $conf.ref_pays != "2" && $object->_ref_patient->ald}}
    <td>
      {{if $readonly}}
  ...
  <td>
    ...
    <button id="button_edit_dep{{$view}}"type="button" class="edit notext" onclick="ActesNGAP.editDEP('{{$view}}');"{{if !$acte->_dep}} style="display: none;"{{/if}}>{{tr}}CActeNGAP-accord_prealable{{/tr}}</button>
    <div id="modal_dep{{$view}}" style="display:none;">
      <form name="editActeNGAP-accord_prealable{{$view}}" action="?" method="post" onsubmit="return false;">
        <table class="form">
          <tr>
            <th class="title" colspan="2">Accord préalable</th>
          </tr>
          <tr>
            <th>
            ...
            <td>
              {{if $readonly}}
                {{mb_value object=$acte field=accord_prealable}}
              {{else}}
                {{mb_field object=$acte field=accord_prealable onchange="ActesNGAP.toggleDateDEP(this, '$view'); ActesNGAP.checkDEP('$view');"}}
              {{/if}}
            </td>
          </tr>
          <tbody id="accord_infos{{$view}}"{{if !$acte->accord_prealable}} style="display: none;"{{/if}}>
            <tr>
              <th>
              ...
              <td>
                {{if $readonly}}
                  {{mb_value object=$acte field=date_demande_accord}}
                {{else}}
                  {{mb_field object=$acte field=date_demande_accord onchange="ActesNGAP.checkDEP('$view');"}}
                {{/if}}
              </td>
            </tr>
            <tr>
              ...
              <td>
                {{if $readonly}}
                  {{mb_value object=$acte field=reponse_accord}}
                {{else}}
                  {{mb_field object=$acte field=reponse_accord onchange="ActesNGAP.checkDEP('$view');" emptyLabel='Select'}}
                {{/if}}
              </td>
            </tr>
          </tbody>
          <tr>
            <td class="buttons" style="text-align: center;" colspan="2">
              <button type="button" class="tick" onclick="ActesNGAP.syncDEPFields(this.form, '{{$view}}');">{{tr}}Validate{{/tr}}</button>
  ...
  <td>
    ...
    {{else}}
      <form name="editActeNGAP-executant_id{{$view}}" action="?" method="post" onsubmit="return false;">
        ...
        <input type="text" name="_executant_view" class="autocomplete me-w100px" value="{{if $acte->_ref_executant}}{{$acte->_ref_executant}}{{/if}}"/>
    ...
    <td class="narrow">
      <form name="editActeNGAP{{$view}}" action="?" method="post" onsubmit="return ActesNGAP.submit(this, '{{$target}}');">
        ...
        {{if !$acte->_id}}
          ...
          <button type="button" id="inc_codage_ngap_button_create" class="add notext" style="float: right;"
