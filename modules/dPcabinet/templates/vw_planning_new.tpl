        ...
        <td class="narrow" style="vertical-align: top;">
          <select name="function_id" style="width: 11em; float:left;" onchange="this.form.submit();">
            ...
            {{foreach from=$listFnc item=_fnc}}
              <option value="{{$_fnc->_id}}" {{if $_fnc->_id == $function_id}}selected{{/if}}>{{$_fnc}}</option>
            {{/foreach}}
          </select>
        </td>
        <td colspan="3" class="me-valign-bottom">
          {{if $multiple}}
    ...
    <table class="main me-no-align me-bg-white me-border-top">
      <tr>
        <th style="width: 25%; text-align: left;" class="me-padding-top-8">
          <select name="chirSel" style="width: 11em;float:left; {{if $multiple}}display:none;{{/if}}" onchange="refreshPlanning();">
            <option value="-1" {{if $chirSel == -1}}selected{{/if}}>&mdash; {{tr}}CMediusers-select-professionnel{{/tr}}</option>
        ...
        <th style="width: 15%; text-align: right;" class="me-padding-8">
          {{if $function_id}}
            <button class="new me-tertiary me-dark" type="button" onclick="CreneauConsultation.modalPriseRDVTimeSlot('', '{{$function_id}}', 0, 1);">
