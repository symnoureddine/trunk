        ...
        <tr>
          <td class="button">
            ...
            {{if $patient->_ref_family_patient && ($patient->_ref_family_patient->parent_id_1 || $patient->_ref_family_patient->parent_id_2)}}
              ...
              <button type="button" onclick="Patient.getAntecedentParents('{{$patient->_id}}', '{{$context_class}}', '{{$context_id}}');">
                <i class="far fa-eye"></i> {{tr}}CAntecedent-action-See the antecedent of the parents{{/tr}}
              </button>
            {{/if}}
          </td>
        </tr>
