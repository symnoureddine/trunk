...
if ($readonly || ($patient->_ref_dossier_medical && $patient->_ref_dossier_medical->_id)) {
  ...
  if ($readonly || !$patient->_ref_dossier_medical->_canEdit) {
...
if (CModule::getActive("appFineClient")) {
  ...
  $patient->loadRefsDossierTiers(array("name" => " = '$name_dossier_tiers_appFine'"));
  $dossier_tiers_appFine = reset($patient->_ref_dossiers_tiers);

// Si le patient n'a pas de dossier tiers AppFine, on lui en crée un
...
/* Verification de l'existance de la base DRC */
$smarty->assign('drc'                    , array_key_exists('drc', CAppUI::conf('db')));
/* Verification de l'existance de la base CISP */
$smarty->assign('cisp'                   , array_key_exists('cisp', CAppUI::conf('db')));

if (CModule::getActive("appFineClient")) {
  $smarty->assign("count_new_antecedents", $count_new_antecedents);
}
