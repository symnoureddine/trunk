  ...
  foreach ($plages as $_plage) {
    ...
    foreach ($_plage->_ref_consultations as $_consult) {
      ...
      if ($_consult->patient_id) {
        ...
        $event = new CPlanningEvent($_consult->_guid, $debute, $_consult->duree * $_plage->_freq, $motif, $_consult->_color_planning, true, null, null);
      }
      $event->type        = "rdvfull";
    ...
    foreach ($utilisation as $_timing => $_nb) {
      if (!$_nb) {
        $debute = "$jour $_timing";
        $event = new CPlanningEvent($debute, $debute, $_plage->_freq, "", $_plage->_color_planning, true, null, null);
        $event->type        = "rdvfree";
        $event->plage["id"] = $_plage->_id;
        if ($_plage->locked == 1) {
          $event->disabled = true;
