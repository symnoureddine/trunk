...
class CSearch implements IShortNameAutoloadable
{
    ...
    {
        $this->createClient();
        $this->_index = (CAppUI::conf("search index_name")) ?: CAppUI::conf("db std dbname");
        $this->_type  = '_doc';
    }
     ...
     *
     * @param array $hosts like array('mydomain.org:12345')
     *
     * @return void
     */
    protected function createClient($hosts = null)
    {
        if (!$hosts) {
            $hosts = $this->getServerAddresses();
        }
        ...
        $this->_client = ClientBuilder::create()->setHosts($hosts)->setConnectionParams($params)->build();
    }
    ...
    static function adaptStep($count, $time)
    {
        ...
        if ($new_step) {
            SHM::put("search_indexing_step", $new_step);
        }
    }
    ...
    {
        $step = SHM::get("search_indexing_step");
