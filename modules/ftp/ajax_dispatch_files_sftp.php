...
try {
    ...
    CApp::log($e->getMessage(), null, CLogger::LEVEL_ERROR);
...
foreach ($files as $_filepath) {
    ...
    try {
        $message = $source->getData($basename, true);
        if (!$message) {
            continue;
        }
    } catch (CMbException $e) {
    ...
    try {
        ...
        if ($sender->_delete_file) {
            $source->delFile($_filepath, null, true);
            if ($fileextension_write_end) {
                $source->delFile("$_filepath_no_ext.$fileextension_write_end", null, true);
            }
        }
