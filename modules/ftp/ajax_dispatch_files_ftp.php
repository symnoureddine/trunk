...
foreach ($files as $_filepath) {
    ...
    $source->renameFile($_old_basename, $basename);

    try {
        $message = $source->getData($basename);
        if (!$message) {
            continue;
        }
    }
    catch (CMbException $e) {
        $source->renameFile($basename, $_old_basename);
    ...
    if ($acq = CEAIDispatcher::dispatch($message, $sender, null, $to_treatment)) {
        try {
            ...
            if ($sender->_delete_file !== false) {
                $source->delFile($basename);

                if ($fileextension_write_end) {
                    $source->delFile("$filename.$fileextension_write_end");
                }
            }
            else {
