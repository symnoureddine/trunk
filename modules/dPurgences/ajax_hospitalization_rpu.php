...
if ($count_collision == 1) {
  ...
  $sejour_rpu->loadRefsAffectations();
  $sejour_collision->loadRefsAffectations();
  if (!$group_id || $group_id == $current_group->_id
    && count($sejour_rpu->_ref_affectations) && count($sejour_collision->_ref_affectations)
  ) {
    $affectations = array_merge($sejour_collision->_ref_affectations, $sejour_rpu->_ref_affectations);
  }
}
else {
  if (!$count_collision) {
...
if ($group_id && $group_id != CGroups::get()->_id) {
  ...
  $services = $service->loadList(array("group_id" => "= '$group_id'"), "nom");
...
$required_uf_soins = CAppUI::conf("dPplanningOp CSejour required_uf_soins", "CGroups-" . ($group_id ?: $current_group->_id));
$required_uf_med   = CAppUI::conf("dPplanningOp CSejour required_uf_med", "CGroups-" . ($group_id ?: $current_group->_id));

$ufs = CUniteFonctionnelle::getUFs(
  $sejour_rpu,
  $group_id ?: null,
  array("type_sejour" => "IS NULL OR type_sejour " . CSQLDataSource::prepareIn(array("comp", "ambu")))
);
