...
{{/if}}
  <div style="margin-bottom: 3px;">
    ...
    <a href="#1" onclick="
      {{if $app->_ref_user->isPraticien() && $app->_ref_user->isUrgentiste() && $rpu->_ref_sejour->_ref_consult_atu->_id}}
        Urgences.pecMed('{{$rpu->_ref_sejour->_ref_consult_atu->_id}}', 'rpuConsult');
      {{else}}
        Urgences.pecInf('{{$sejour_id}}', '{{$rpu_id}}');
      {{/if}}"
      >
      {{assign var=couleur_symbole value="gray"}}
      {{if ($rpu->_ref_sejour && $rpu->_ref_sejour->_ref_consult_atu->_id) || $rpu->mutation_sejour_id}}
    ...
    <div class="libelle compact me-display-flex me-justify-content-space-between" {{if $background_color_ccmu}}style="color:#000;"{{/if}}>
      <div onmouseover="ObjectTooltip.createEx(this, '{{$rpu->_guid}}');">
