...
{{if $conf.ref_pays == 2}}
  {{mb_include module=urgences template=CRPU_view_ch}}
  ...
  <tr>
    <th class="title text" colspan="3">
      ...
      {{if $app->_ref_user->isAdmin() && ('admin CBrisDeGlace enable_bris_de_glace'|gconf || 'admin CLogAccessMedicalData enable_log_access'|gconf)}}
        <a href="#" onclick="guid_access_medical('{{$sejour->_guid}}')" style="float:right;" ><img src="images/icons/planning.png" alt="" /></a>
      {{/if}}
    </th>
  </tr>
    ...
    <td class="button" colspan="3">
      ...
      {{if $object->canEdit()}}
        ...
        {{if $object->pec_inf || (!$is_inf && !$is_as)}}
          <button type="button" class="search" onclick="Urgences.pecInf('{{$sejour->_id}}', '{{$object->_id}}');">
        ...
        <button type="button" class="search" onclick="Urgences.actions('{{$object->_id}}', '{{$object->sejour_id}}');">{{tr}}Actions{{/tr}}</button>

        {{if $consult->_id && !$object->mutation_sejour_id}}
          <button type="button" class="search" onclick="Urgences.synthese('{{$consult->_id}}', '{{$object->sejour_id}}');">{{tr}}CRPU.synthese{{/tr}}</button>
        {{elseif !$consult->_id}}
          <button type="button" class="search" onclick="Urgences.syntheseRPU('{{$object->_id}}');">{{tr}}CRPU.synthese{{/tr}}</button>
        {{/if}}
      {{/if}}
      ...
      <button type="button" class="print" onclick="Document.printSelDocs('{{$sejour->_id}}', '{{$sejour->_class}}');">
      ...
      <button class="tick" type="button" onclick="Admissions.validerSortie('{{$sejour->_id}}', false, function() { window.location.reload(); });">
