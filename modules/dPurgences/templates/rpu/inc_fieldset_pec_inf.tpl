...
{{/if}}
  ...
  <table class="form me-no-align me-no-box-shadow me-small-form">
    ...
    <tr>
      ...
      <td colspan="3">
        <select name="_responsable_id" style="width: 15em;" class="{{$rpu->_props._responsable_id}}" onchange="checkIsRemplacant(this.value, $V(this.form._entree)); {{$submit_ajax}}">
      ...
      <tr>
        ...
        <td colspan="3">
          {{if "dPurgences Display see_ide_ref"|gconf || "dPurgences CRPU impose_ide_referent"|gconf}}
            ...
            <input type="text" name="ide_responsable_id_view" class="autocomplete" value="{{$rpu->_ref_ide_responsable}}"
    ...
    {{if "dPurgences Display display_cimu"|gconf && !"dPurgences CRPU french_triage"|gconf}}
      ...
      <tr>
        ...
        <td colspan="3">{{mb_field object=$rpu field="cimu" form="editRPU" emptyLabel="Choose" class=$notnull onchange=$submit_ajax}}</td>
      </tr>
    {{/if}}
    ...
    {{if "dPurgences CRPU french_triage"|gconf}}
      <tr>
          ...
          <td colspan="3">{{mb_field object=$rpu field=french_triage form="editRPU" emptyLabel="Choose" onchange=$submit_ajax}}</td>
      </tr>
    {{/if}}
    ...
    {{if $can->edit}}
      ...
      {{if "dPurgences CRPU display_motif_sfmu"|gconf}}
        ...
        <tr>
          ...
          <td colspan="3">{{mb_field object=$rpu field="motif_sfmu" autocomplete="true,1,10,true,true" form=editRPU size=50 class=$notnull onchange="changeMotifSfmu(this.form); $submit_ajax"}}
      ...
      {{if !"dPurgences CRPU french_triage"|gconf}}
          {{assign var=notnull value=""}}
          {{if $rpu->_id && "dPurgences CRPU gestion_motif_sfmu"|gconf}}
            {{assign var=notnull value="notNull"}}
          {{/if}}
          <tr>
    ...
    <tr>
      ...
      <td colspan="3">{{mb_field object=$rpu field="date_at" form="editRPU" register=true onchange=$submit_ajax}}</td>
    </tr>
  </table>
