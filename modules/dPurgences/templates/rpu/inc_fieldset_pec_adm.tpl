...
<script>
  ...
  loadTransfert = function(mode_entree) {
    ...
    {{if "dPplanningOp CSejour required_from_when_transfert"|gconf}}
  ...
  changePecTransport = function(transport) {
    ...
    def_pec_transport = {{$pec_transport|html_entity_decode}};
...
{{/if}}
  ...
  <table class="form me-no-align me-no-box-shadow me-small-form">
      ...
      {{if $list_mode_entree|@count}}
        ...
        <td>
          ...
          <input type="hidden" name="_mode_entree_id" value="{{$rpu->_mode_entree_id}}" onchange="{{$submit_ajax}}"
                 class="autocomplete {{if $pays != 2}}notNull{{/if}}" size="50"/>
    ...
    <tr>
      ...
      <td>{{mb_field object=$rpu field="pec_transport" emptyLabel="Choose" style="width: 15em;" onchange=$submit_ajax}}</td>
    </tr>
