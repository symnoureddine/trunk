...
{{assign var=required_from_when_transfert     value="dPplanningOp CSejour required_from_when_transfert"|gconf}}
              ...
              <td>
                ...
                <select name="mode_entree_id" style="width: 20em;" onchange="updateModeEntree(this);">
                  ...
                  {{foreach from=$list_mode_entree item=_mode}}
                    <option value="{{$_mode->_id}}" data-mode="{{$_mode->mode}}" {{if $protocole_rpu->mode_entree_id == $_mode->_id}}selected{{/if}}>
                      {{$_mode}}
                    </option>
                  {{/foreach}}
                </select>
