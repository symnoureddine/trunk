  ...
  return onSubmitFormAjax(getForm('editSejour'), function(){
    ...
    {{if $conf.dPurgences.valid_cotation_sortie_reelle}}
      return onSubmitFormAjax(getForm('ValidCotation'), function(){
        submitSejRpuConsult({{$consult|const:'TERMINE'}});
        $('button_reconvoc').disabled = null;
      });
    {{else}}
      $('button_reconvoc').disabled = null;
      return submitSejRpuConsult({{$consult|const:'TERMINE'}});
    {{/if}}
  });
}
</script>
  ...
  {{else}}    
    ...
    {{else}}
      <button class="tick singleclick" type="button"
              onclick="Urgences.modalSortie(ContraintesRPU.checkObligatory.curry('{{$rpu->_id}}', getForm('editSejour'), autoriserSortie.curry(1)));">
      ...
      {{if !$sejour->sortie_reelle}}
        {{if $rpu->sejour_id != $rpu->mutation_sejour_id && "dPurgences CRPU display_aut_eff_sortie_button"|gconf}}
          ...
          <button class="tick singleclick" type="button"
                  onclick="Urgences.modalSortie(ContraintesRPU.checkObligatory.curry('{{$rpu->_id}}', getForm('editSejour'), autoriserEffectuerSortie.curry()));">
