  ...
  <table class="form">
    {{if "dPurgences CRPU change_group"|gconf}}
      <tr>
        ...
        <td>
          <select name="group_id" onchange="Control.Modal.close(); Urgences.hospitalize('{{$rpu->_id}}', this.value)">
            {{foreach from=$etablissements item=curr_etab}}
              <option value="{{$curr_etab->group_id}}" {{if $sejour->group_id === $curr_etab->group_id}}selected{{/if}}>{{$curr_etab}}</option>
            {{/foreach}}
          </select>
        </td>
      </tr>
