...
{{if $conf.ref_pays == 2}}
  ...
  <div id="admission" class="me-padding-0" style="display: none;">
...
{{if !$rpu->_id || ($can->read && !$can->edit) || ($can->edit && $rpu->mutation_sejour_id)}}
  ...
  <script>
    Main.add(function() {
      showDossierSoins('{{$rpu->mutation_sejour_id}}', null, '{{$fragment}}');
    });
  </script>
  {{/if}}
...
{{if $can->edit}}
  ...
  <div id="header-rpu" style="position: relative;">
    <div id="patient-banner">
      {{mb_include module=soins template=inc_patient_banner object=$sejour}}
    </div>
