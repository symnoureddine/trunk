...
class COldCodeCCAM implements IShortNameAutoloadable {
  ...
  function getTarification() {
    $ds = $this->_spec->ds;
    $query = $ds->prepare("SELECT * FROM p_acte_infotarif WHERE CODEACTE = % ORDER BY DATEEFFET DESC", $this->code);
    $result = $ds->exec($query);
    $row = $ds->fetchArray($result);
  ...
  function getActesAsso($code = null, $limit = null) {
    ...
    while ($row = $ds->fetchArray($result)) {
      ...
      $query2 = $ds->prepare("SELECT * FROM p_acte WHERE CODE = % AND DATEFIN = '00000000'", trim($row["ACTEASSO"]));
      $result2 = $ds->exec($query2);
      $row2 = $ds->fetchArray($result2);
  ...
  function getActesIncomp() {
    ...
    while ($row = $ds->fetchArray($result)) {
      ...
      $query2 = $ds->prepare("SELECT * FROM p_acte WHERE CODE = % AND DATEFIN = '00000000'", trim($row["INCOMPATIBLE"]));
      $result2 = $ds->exec($query2);
      $row2 = $ds->fetchArray($result2);
  ...
  function getProcedure() {
    $ds = $this->_spec->ds;
    $query = $ds->prepare("SELECT * FROM p_acte_procedure WHERE CODEACTE = % GROUP BY CODEACTE ORDER BY DATEEFFET DESC", $this->code);
    $result = $ds->exec($query);
    if ($ds->numRows($result) > 0) {
      $row = $ds->fetchArray($result);
      $this->procedure["code"] = $row["CODEPROCEDURE"];
      $query2 = $ds->prepare("SELECT LIBELLELONG FROM p_acte WHERE CODE = % AND DATEFIN = '00000000'", $this->procedure["code"]);
      $result2 = $ds->exec($query2);
      $row2 = $ds->fetchArray($result2);
  ...
  function getForfait($modificateur) {
    $ds = $this->_spec->ds;
    $query = $ds->prepare("SELECT * FROM t_modificateurforfait WHERE CODE = % AND DATEFIN = '00000000'", $modificateur);
    $result = $ds->exec($query);
    $row = $ds->fetchArray($result);
    $valeur = array();
