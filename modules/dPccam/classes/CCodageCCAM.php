...
class CCodageCCAM extends CMbObject {
  ...
  public function getProps() {
    ...
    $props['association_rule'] = 'enum list|G1|EA|EB|EC|ED|EE|EF|EG1|EG2|EG3|EG4|EG5|EG6|EG7|EH|EI|GA|GB|G2|M default|M';
   ...
   * @return CCodageCCAM
   */
  public static function get($codable, $praticien_id, $activite = 1, $date = null, $creation = true) {
    if (!$date) {
  ...
  public function check() {
    ...
    if ($codable && $codable->_class == 'CConsultation') {
      $codable->loadRefPlageConsult();
      if ($this->date != $codable->_date && CAppUI::gconf('pyxVital General mode') != 'test') {
        return 'Impossible de créer un codage CCAM a une date différente de celle de la consultation';
      }
    }
    elseif ($codable && ($codable->_class == 'COperation' || $codable->_class == 'CModelCodage')) {
      if ($this->date < CMbDT::date('-1 day', $codable->date) || $this->date > CMbDT::date('+1 day', $codable->date)) {
   ...
   * @return void
   */
  protected function getComplementaryActs() {
    $this->loadActesCCAM();

    if (!isset($this->_complementary_acts)) {
      $this->_complementary_codes = array();
    }
    ...
    foreach ($this->_ref_actes_ccam as $act) {
      ...
      foreach ($act->_ref_code_ccam->activites as $activite) {
        if ($act->code_activite == $activite->numero) {
          $this->_complementary_codes = array_merge($this->_complementary_codes, CMbArray::pluck($activite->assos, 'code'));
        }
      }
    }
  }
   ...
   * @return bool
   */
  protected function isComplementaryAct($act) {
  ...
  protected function checkFacturableActs() {
    ...
    foreach ($this->_ref_actes_ccam as $_acte) {
      if ((!$_acte->facturable && !$_acte->facturable_auto)
  ...
  /**
    ...
    * @return void
    */
  public static function precodeModifiers(&$modifiers, &$act, $codable, bool $force_check = false) {
    ...
    $patient->evalAge();
    $checked = false;
    $spec_chir_gyneco = array(4, 7, 10, 11, 15, 16, 18, 41, 43, 44, 45, 46, 47, 48, 49, 70, 77, 79);
    $spe_gyneco = $spe_gyneco = array(7, 70, 77, 79);
    $spe_gen_pediatre = array(1, 12, 21, 22, 23);
      ...
      switch ($_modifier->code) {
        ...
        case 'K':
          ...
          $checked = CAppUI::gconf('dPccam codage precheck_modifiers_k_t')
            ...
            || (($patient->cmu || $patient->acs) && !$act->montant_depassement && $act->_ref_executant->pratique_tarifaire == 'optam'));
        ...
        case 'S':
          $checked = (
              in_array($spec_cpam, $spe_gen_pediatre) || in_array($spec_cpam, $spec_chir_gyneco) || in_array($spec_cpam, $spe_gyneco)
              || $spec_cpam == 2 || ($codable->_class == "COperation" && $codable->_lu_type_anesth))
        ...
        case 'T':
          $checked = CAppUI::gconf('dPccam codage precheck_modifiers_k_t')
            ...
            && ((!$patient->acs && !$patient->cmu) || (($patient->acs || $patient->cmu) && $act->montant_depassement));
  ...
  protected function applyRuleGA($act) {
    ...
    /* Exception for the acts AHQJ021 and YYYY041: the association codes must be set to 1, even if those acts are supplements */
    if ((strpos($this->_ref_codable->codes_ccam, 'AHQJ021') !== false || strpos($this->_ref_codable->codes_ccam, 'YYYY041') !== false)
        && $this->activite_anesth == '1'
    ) {
      $act->_guess_facturable = '1';
    ...
    elseif ($act->_position == 0 || $act->_position == -1 || $act->_ref_code_ccam->isSupplement()
        || $act->_ref_code_ccam->isComplement() || $act->_ref_code_ccam->isRadioCardioInterv() || $this->isComplementaryAct($act)
    ) {
      $act->_guess_facturable = '1';
      $act->_guess_association = '';
      $act->_guess_regle_asso = 'GA';
  ...
  protected function applyRuleGB($act) {
    ...
    if ($act->_position == 0 || $act->_ref_code_ccam->isSupplement()
  ...
  protected function checkRuleEG2() {
    ...
    foreach ($this->_ref_actes_ccam_facturables as $_act) {
      ...
      if ((isset($chapters[2]) && in_array($chapters[2]['rang'], $chapters_anapath))
  ...
  protected function checkRuleEG3() {
    ...
    $this->_check_rules['EG3'] = array('nb_electromyo' => $nb_electromyo);
  ...
  /**
   * ### Exception : forfait de cardiologie, de réanimation, actes de surveillance post-opératoire, actes d'acocuchements ###
   * * Nombre d'actes : 2
  ...
  protected function checkRuleEG6() {
    ...
    foreach ($this->_ref_actes_ccam_facturables as $_act) {
      $chapters = $_act->_ref_code_ccam->chapitres;
      if (in_array($_act->code_acte, array('YYYY001', 'YYYY002', 'YYYY015', 'YYYY020', 'YYYY108', 'YYYY118'))
  ...
  protected function checkRuleEG7() {
    ...
    foreach ($this->_ref_actes_ccam_facturables as $_act) {
      $chapters = $_act->_ref_code_ccam->chapitres;
      if (!in_array($_act->code_acte, $exclude_codes) && ((isset($chapters[2]) && in_array($chapters[2]['rang'], $chapters_dentaire))
          || (isset($chapters[3]) && in_array($chapters[3]['rang'], $chapters_dentaire))
          || in_array($_act->code_acte, $includes_codes))
      ) {
        $nb_bucco_dentaires++;
      }
      elseif ($_act->_ref_code_ccam->isSupplement()
          || $_act->_ref_code_ccam->isComplement()
          || $_act->_ref_code_ccam->isRadioCardioInterv()
      ) {
        $nb_supplements++;
    ...
    if (!$nb_bucco_dentaires || ($nb_bucco_dentaires == 1 && $nb_supplements == 1 && count($this->_ref_actes_ccam_facturables) == 2)) {
      return false;
    }
   ...
   * @return bool
   */
  protected function checkRuleEH() {
    $moments = [];
  ...
  protected function applyRuleEH($act) {
    ...
    foreach ($ordered_acts_eh as $_act) {
      ...
      if ($_act->_ref_code_ccam->isSupplement() || $_act->_ref_code_ccam->isComplement()
   ...
   * @return bool
   */
  public static function getVisibiliteDepassement($executant) {
