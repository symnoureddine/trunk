...
class CContexteTarifaireCCAM implements IShortNameAutoloadable {
  ...
  public static $patient_context = array(
  ...
  /** @var array The list of patient context */
  public static $practitioner_context = array(
    /*
     * Contexts used before the 2018-01-01
     * The first thirteenth context are normally used before the 2018-01-01, but since there is only 2 price grid, we only need 2
     */
  ...
  public static $contexts_to_grid = array(
   ...
   * @return string
   */
  public static function getPriceGrid($practitioner, $patient, $date = null) {
    ...
    $grid = null;
    if (array_key_exists($practitioner_context, self::$contexts_to_grid)
    ...
    /* If no grid is found, we used the grid 14,
     * which is for the other specialities, for the practitioners with no convention with the CPAM
    */
    else {
      $grid = '14';
    }
   ...
   * @return integer
   */
  protected static function getContextPatient($patient, $date = null) {
    $date = CMbDT::date($date);
   ...
   * @return integer
   */
  protected static function getPratictionerContext($practitioner, $date = null) {
    $date = CMbDT::date($date);

    if ($date < '2018-01-01') {
      if ($practitioner->secteur == '1' || ($practitioner->secteur == '2' && $practitioner->pratique_tarifaire != 'none')) {
        $context = 1;
      }
      else {
        $context = 2;
    ...
    elseif ($practitioner->ccam_context) {
      ...
      if (in_array($practitioner->spec_cpam_id, array(4, 7, 10, 11, 15, 16, 18, 41, 43, 44, 45, 46, 47, 48, 49, 70, 77, 79))) {
      ...
      /* Généralistes */
      elseif (in_array($practitioner->spec_cpam_id, array(1, 22, 23))) {
        $context = self::getContextGeneralists($practitioner);
      }
      /* Chirurgiens dentistes */
      elseif (in_array($practitioner->spec_cpam_id, array(19, 36, 53, 54))) {
   ...
   * @return int
   */
  protected static function getContextsSurgeons($practitioner) {
   ...
   * @return int
   */
  protected static function getContextAnesthesists($practitioner) {
    switch ($practitioner->secteur) {
      case '1':
   ...
   * @return int
   */
  protected static function getContextPediatrists($practitioner) {
    switch ($practitioner->secteur) {
      case '1':
        if ($practitioner->pratique_tarifaire == 'none' || !$practitioner->pratique_tarifaire) {
          $context = 34;
   ...
   * @return int
   */
  protected static function getContextGeneralists($practitioner) {
    switch ($practitioner->secteur) {
      case '1':
        if ($practitioner->pratique_tarifaire == 'none' || !$practitioner->pratique_tarifaire) {
          $context = 35;
   ...
   * @return int
   */
  protected static function getContextDentists($practitioner) {
   ...
   * @return int
   */
  protected static function getContextMidwives($practitioner) {
    if (!$practitioner->secteur || $practitioner->secteur == 'nc') {
      $context = 13;
    }
    else {
   ...
   * @return int
   */
  protected static function getContextOtherSpecialities($practitioner) {
    switch ($practitioner->secteur) {
      case '1':
        if ($practitioner->pratique_tarifaire == 'none' || !$practitioner->pratique_tarifaire) {
          $context = 36;
   ...
   * @return int
   */
  public static function getDOMCode($user) {
    $dom = 0;
