...
class CFilterCotation implements IShortNameAutoloadable
{
    ...
    public function getCotationDetails(string $period = null): array
    {
        ...
        foreach ($objects as $_class => &$_codables) {
            ...
            foreach ($_codables as $_key => $_codable) {
                ...
                $_codable->loadRefPatient()->updateBMRBHReStatus();
    ...
    public static function countUncodedCodable(
        ...
        $subquery->addWhere(
            [
                ...
                "acte_ccam.executant_id = '$executioner->_id'",
            ]
        );
        ...
        if ($class != 'COperation') {
            ...
            $subquery->addWhere(
                [
                    ...
                    "acte_ngap.executant_id = '$executioner->_id'",
                ]
            );
        }
        ...
        $codables_id = $ds->loadColumn($query->makeSelect());

        return count($codables_id);
    }
    ...
    public function getPeriods(bool $display = true): array
    {
        ...
        foreach (self::$periods as $_days => $_period) {
            ...
            if ($days >= $_period_days[0] && ($days_from_end_date <= $_period_days[1] || $_days == 203)) {
                if ($display) {
