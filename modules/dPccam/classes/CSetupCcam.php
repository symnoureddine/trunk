...
class CSetupCcam extends CSetup {
  ...
  function __construct() {
    ...
    $this->addDefaultConfig("dPccam frais_divers use_frais_divers_CConsultation", "dPccam CCodable use_frais_divers CConsultation");
    $this->addDefaultConfig("dPccam frais_divers use_frais_divers_COperation", "dPccam CCodable use_frais_divers COperation");
    $this->addDefaultConfig("dPccam frais_divers use_frais_divers_CSejour", "dPccam CCodable use_frais_divers CSejour");
    $this->addDefaultConfig("dPccam frais_divers use_frais_divers_CEvenementPatient", "dPccam CCodable use_frais_divers CEvenementPatient");
    ...
    if (array_key_exists('ccamV2', CAppUI::conf('db'))) {
      ...
      if ($dsn->fetchRow($dsn->exec('SHOW TABLES LIKE \'forfaits\';'))) {
        $query = "-- Forfaits SRC --
          SELECT * 
            FROM `forfaits` 
            WHERE `forfait` = 'SRC';";
    ...
    /* Tarifs NGAP : Ajout de l'acte INJ */
    if (array_key_exists('ccamV2', CAppUI::conf('db'))) {
      $dsn = CSQLDataSource::get('ccamV2');
      if ($dsn->fetchRow($dsn->exec('SHOW TABLES LIKE \'tarif_ngap\';'))) {
        $query = "-- NGAP : Ajout de l'acte INJ pour la vaccination contre le COVID19  --
