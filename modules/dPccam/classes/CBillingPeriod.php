...
class CBillingPeriod extends CMbObject
{
   ...
   * @return mixed
   */
  public static function checkStore($sejour, $context = null) {
    if (!$context) {
      $context = $sejour;
    }
    ...
    if (!$sejour->sortie_reelle || !$old->sortie_reelle
    ...
    switch ($context->_class) {
      default:
      case "CSejour":
        if ($sejour->fieldModified("uf_medicale_id")
        ...
        if (($entree_prevue && CMbDT::date($sejour->entree_prevue) !== $entree_prevue)
