...
<script>
  Main.add(function(){
    var form = getForm("edit-{{$integration->_guid}}");
    ContextualIntegration.toggleIconURL(form);
  });
</script>
  ...
  <table class="form">
    ...
    <tr>
      <td colspan="2">
        ...
        <fieldset>
          <legend>Insertion de valeurs</legend>
          <ul>
    ...
    <tr>
      <th>{{mb_label object=$integration field=icon_url}}</th>
      <td>
        <select name="_icon_url_fa" style="font-family: FontAwesome,Tahoma; font-size: 14px; width: 10em;"
                onchange="ContextualIntegration.toggleIconURL(this.form,true)">
          <option value=""> &mdash; URL </option>
          {{foreach from=$icons item=_code key=_icon}}
            <option value="fa-{{$_icon}}" {{if $integration->icon_url == "fa-$_icon"}} selected {{/if}}>
              &#x{{$_code}}; &nbsp; {{$_icon}}
            </option>
          {{/foreach}}
        </select>

        <span id="icon-url-container">
