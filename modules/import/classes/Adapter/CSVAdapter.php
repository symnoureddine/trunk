...
class CSVAdapter implements AdapterInterface, ConfigurableInterface
{
     ...
     * @inheritDoc
     */
    public function retrieve(string $collection, string $identifier, $id, array $conditions = [], array $select = []): ?array
    {
    ...
    private function sanitizeLine(array $line): array
    {
        ...
        $sanitize = ($this->configuration) ? $this->configuration->offsetGet('sanitize_functions') : null;
        if ($sanitize) {
