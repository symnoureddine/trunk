...
{{if $can_create_docs}}
  ...
  <form name="DocumentAdd-{{$unique_id}}-{{$object->_guid}}" action="?m={{$m}}" method="post" class="prepared not-printable">
    {{if $can_create_docs}}
      <input type="text" placeholder="&mdash; {{tr}}CModeleToPack-modele_id{{/tr}}" name="keywords_modele" class="autocomplete str" autocomplete="off" style="width: 5em;" />
      <input type="text" placeholder="&mdash; {{tr}}CPack{{/tr}}" name="keywords_pack" class="autocomplete str" autocomplete="off" style="width: 4em;"/>
...
{{if "ameli"|module_active}}
  {{mb_include module=ameli template=inc_chose_cerfa object=$object}}
{{/if}}

{{if $object->_nb_cancelled_docs && $mode != "hide"}}
  <button class="hslip not-printable me-tertiary me-dark" style="float: right;" data-show="" onclick="Document.showCancelled(this)">
    {{tr var1=$object->_nb_cancelled_docs}}CCompteRendu-toggle_display_cancelled_docs{{/tr}}
  </button>
{{/if}}

{{if $can_create_docs && $can->admin}}
  <form name="DeleteAll-{{$object->_guid}}" action="?m={{$m}}" method="post" onsubmit="return checkForm(this)" class="not-printable">
    ...
    <button class="trash me-tertiary me-dark" type="button" style="float: right;" onclick="Document.removeAll(this, '{{$object->_guid}}')">
        {{tr}}Delete-all{{/tr}}
    </button>
  </form>
{{/if}}
...
<div id="DocsEffect-{{$object->_guid}}-{{$unique_id}}" {{if $mode == "collapse" && $doc_count}}style="display: none;"{{/if}}>
  <table class="form" id="docs_{{$object_class}}{{$object_id}}">
    {{if $mode != "hide"}}
      <tr>
        <td class="text me-padding-0" colspan="3">
          {{if $affichageDocs}}
            <ul id="tabs-documents-{{$unique_id}}-{{$object->_class}}" class="control_tabs small me-border-only-bottom">
