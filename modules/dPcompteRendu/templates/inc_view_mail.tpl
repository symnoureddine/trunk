...
<script>
  ...
  addAddressLine = function(field) {
    if (!field || ($V(field) && !field.hasClassName('row_added'))) {
      ...
      row.insert(DOM.td({class: 'checkbox'}, DOM.input({type: 'checkbox', name: 'destinataire', 'data-object_guid': 'receiver_input'})));
...
<form name="formSendMail" method="get">
  <p>
    <label>{{tr}}CCompteRendu.mail_subject{{/tr}} :
      <input type="text" name="subject" value="{{$app->user_prefs.send_document_subject}}" style="width: 500px;"/>
    </label>
  </p>
  <p class="me-w75">
    <label>{{tr}}CCompteRendu.mail_body{{/tr}} :
      <textarea name="body">{{$app->user_prefs.send_document_body}}</textarea>
    </label>
  </p>
  ...
  <div style="padding-top: 10px; width: 100%; text-align: center;">
    ...
    <button id="btn_send_mail" class="tick me-primary" type="button" onclick="sendMail();">{{tr}}CCompteRendu.send_mail{{/tr}}</button>
  </div>
</form>
