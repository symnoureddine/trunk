...
<script>
  ...
  document.title = "{{$compte_rendu->_ref_object->_view|smarty:nodefaults}} - {{$compte_rendu->nom|smarty:nodefaults}}";
  ...
  function openWindowApicrypt() {
    ...
    url.addParam("doc_guid", '{{$compte_rendu->_guid}}');
    url.requestModal(750, 600);
  }

  function openWindowMSSante() {
    var url = new Url('mssante', 'ajax_view_send_document');
    url.addParam("object_guid", '{{$compte_rendu->object_class}}-{{$compte_rendu->object_id}}');
    url.addParam("document_guid", '{{$compte_rendu->_guid}}');
    url.requestModal(700, 600);
  }

  function openWindowMedimail() {
    var form = getForm('editFrm');
    var url = new Url('medimail', 'ajax_edit_message');
  ...
  function refreshListDocs(uniqueIdToRefresh) {
    var form = getForm("editFrm");
    if (window.opener.Document && window.opener.Document.refreshList) {
      window.opener.Document.refreshList($V(form.file_category_id), $V(form.object_class), $V(form.object_id), null, uniqueIdToRefresh);
    }
  ...
  <table class="form">
    <tr>
      <th class="category narrow" style="vertical-align: top;">
        {{if $compte_rendu->_id}}
          <a class="button left {{if !$prevnext.prev}}disabled{{/if}}"
            {{if $prevnext.prev}}
            href="?m=compteRendu&dialog=edit_compte_rendu&compte_rendu_id={{$prevnext.prev}}"
            {{/if}}>
            Préc.
          </a>
        {{/if}}
      ...
      <th class="category">
        ...
        <select name="file_category_id" style="width: 8em;" {{if $read_only}}disabled{{/if}} onchange="FilesCategory.checkTypeDocDmpSisra(this.form, this.value);">
          <option value="" {{if !$compte_rendu->file_category_id}}selected{{/if}}>&mdash; {{tr}}None|f{{/tr}}</option>
        ...
        {{if !$read_only}}
          &mdash;
          <button type="submit" class="save notext singleclick">{{tr}}Save{{/tr}}</button>
        {{/if}}
      ...
      {{if $user_opener->_id}}
        <th class="category narrow warning me-opener-base">
          ...
          <div id="opener-tooltip" class="me-opener-tooltip">
            ...
            <button type="button" class="me-tertiary me-margin-top-8" onclick="$('opener-tooltip').removeClassName('displayed');">
              OK
            </button>
          </div>
        </th>
      ...
      <th class="category narrow" style="vertical-align: top; width: 85px;">
        {{if $compte_rendu->_id}}
          <a style="float: right;" class="button right {{if !$prevnext.next}}disabled{{/if}}"
            {{if $prevnext.next}}
            href="?m=compteRendu&dialog=edit_compte_rendu&compte_rendu_id={{$prevnext.next}}"
            {{/if}}>
            Suiv.
          </a>
    ...
    <tr>
      ...
      {{if $pdf_and_thumbs}}
        <td id="thumbs_button" class="narrow me-valign-top">
          ...
          <div id="thumbs" style="overflow: auto; overflow-x: hidden; width: 300px; text-align: center; white-space: normal;"></div>
        </td>
      {{/if}}
    </tr>
  </table>
