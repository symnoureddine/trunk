...
{
    /** @var string */
    public const RESOURCE_NAME = 'medicalReports';
     ...
     * @return string
     */
    function getBinaryContent($load_content = true, $auto_print = true)
    {
    ...
    function store()
    {
        $this->completeField("content_id", "_source", "language", "version", "factory", "object_id");
        ...
        if ((!$this->_id) || ($this->_id && $content_modified)) {
            $this->version++;
        }
        ...
        if ($object instanceof CEvenementPatient && CModule::getActive("oxCabinet")) {
    ...
    function makePDFpreview($force_generating = false, $auto_print = true, $load_content = true)
    {
        ...
        //mbLog($this->_source, "ccompterendu");
        ...
        $pdf_content     = $htmltopdf->generatePDF($content, 0, $this, $file);
        $this->_ref_file = $file;
     ...
     * @return void
     */
    static function streamDocForObject($compte_rendu, $object, $factory = null, $auto_print = true)
    {
        ob_clean();
        ...
        $htmltopdf->generatePDF($template->document, 1, $compte_rendu, new CFile(), $auto_print);
    ...
    static function importModele($_modele, $user_id, $function_id, $group_id, &$modeles_ids)
    {
        ...
        if ($listes->length) {
            ...
            foreach ($listes as $_liste) {
                $nom   = html_entity_decode(utf8_decode($_liste->getAttribute("nom")));
                ...
                if (!$liste->loadObject($where)) {
                    ...
                    foreach ($_liste->childNodes as $_choix) {
                        $choix[] = html_entity_decode(utf8_decode($_choix->nodeValue));
                    }
    ...
    static function regenerateSomePDF($limit = null)
    {
        ...
        $last_id = @reset($file->loadIds(null, "file_id DESC", "1"));
