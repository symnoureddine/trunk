...
function playField(element, class_name, editor_element, name) {
  ...
  modal.down('button.tick').onclick = function() { replaceField(element, class_name); };
  ...
  modal.down('button.trash').onclick = function() { replaceField(element, class_name, 1); };
  ...
  if (!window.modal_mode_play || !window.modal_mode_play.isOpen) {
    window.modal_mode_play = Modal.open(modal, {draggable: modal.down('th.title'), overlayOpacity: 0.3, width: "500", height: "350"});
