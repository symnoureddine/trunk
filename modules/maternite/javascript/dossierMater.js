...
DossierMater = {
  ...
  refreshArea: function (grossesse_id, page, operation_id, child_page) {
    new Url("maternite", "dossier_mater_" + page)
      ...
      .requestUpdate("dossier_mater_" + page, {onComplete: function () {
  ...
  /**
   * Check if the field is empty or 0
   */
  checkFieldMandatory: function (form, element) {
    var element_rea_par_id = form.rea_par_id;

    if($V(element) == 0) {
      element_rea_par_id.removeClassName("notNull");
    }
    else {
      if (!element_rea_par_id.hasClassName('notNull')) {
        element_rea_par_id.addClassName("notNull");
      }
    }
  },
  /**
  ...
  deleteAntecedent: function (form_perinatal, form, atcd_rques, atcd_textarea, type) {
    ...
    url.requestJSON(function (atcd) {
      if (atcd[0] > 0) {
        ...
        onSubmitFormAjax(form_perinatal, function () {onSubmitFormAjax(form, DossierMater.refreshAtcd.curry($V(form._patient_id), type))});
      }
    });
  },
  /**
  ...
  manageAntecedents: function (element, prefix, atcd_textarea) {
    ...
    if ($V(element) == 1 || (element.tagName == 'TEXTAREA' && element.value)) {
      ...
      onSubmitFormAjax(element.form, function () {onSubmitFormAjax(form_atcd, DossierMater.refreshAtcd.curry(patient_id, type))});
    }
    else {
  ...
  updateSemaines: function () {
    ...
    var terme_prevu = new Date($V(form.terme_prevu));
   ...
   */
  ShowElements: function (element, container, class_containers) {
    var element_name = element.name;
    ...
    if (bool_element_names.includes(element_name) && container) {
      ...
      if (element_value == 'neg') {
        $$(class_containers).invoke('show');
      }
      else {
        $$(class_containers).invoke('hide');
      }
    }
