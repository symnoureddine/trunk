  ...
  {{foreach from=$services_selected key=_nom_serice item=naissances}}
    ...
    {{foreach from=$naissances item=_naissance}}
      ...
      <tr>
        ...
        <td>
          {{if $oea!=null && $oea|@count > 0}}
            <span onmouseover="ObjectTooltip.createDOM(this, 'oea_{{$_naissance->_id}}');">{{tr}}common-Yes{{/tr}}</span>
            <div id="oea_{{$_naissance->_id}}" style="display: none;">
              <table class="tbl">
                <tr>
                  <th colspan="4"><span title="{{tr}}CNaissance-oea-desc{{/tr}}">{{tr}}CNaissance-oea{{/tr}}</span></th>
                </tr>
                <tr>
                  <th>{{tr}}common-Date{{/tr}}</th>
                  <th>{{tr}}Who{{/tr}}</th>
                ...
                {{foreach from=$oea item=_administration}}
                  {{assign var=administrateur_oea value=$_administration.administrateur}}
                  <tr>
                    <td>{{$_administration.datetime|date_format:$conf.datetime}}</td>
                    <td>
                      <span
                        onmouseover="ObjectTooltip.createEx(this, '{{$administrateur_oea.guid}}');">{{$administrateur_oea.view}}</span>
                    </td>
                    <td>{{tr}}CExamenNouveauNe.oreille_droite.{{$_administration.oreille_droite}}{{/tr}}</td>
                    <td>{{tr}}CExamenNouveauNe.oreille_gauche.{{$_administration.oreille_gauche}}{{/tr}}</td>
                  </tr>
                {{/foreach}}
              </table>
            </div>
