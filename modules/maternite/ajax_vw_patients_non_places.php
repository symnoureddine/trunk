...
$consultations = CStoredObject::massLoadBackRefs($listNotAff["Non placés"], "consultations", "date DESC, heure DESC", null, $ljoin_consult);
...
foreach ($listNotAff["Non placés"] as $key => $_sejour) {
  ...
  if (($last_affectation && $last_affectation->lit_id) || ($_sejour->service_id && !in_array($_sejour->service_id, array_keys($services)))) {
