...
class CNaissance extends CMbObject {
  ...
  function getProps() {
    ...
    $props["rea_par"]                        = "enum list|sf|ped|anesth|samu|obst";
    $props["rea_par_id"]                     = "ref class|CMediusers back|naissances_rea";
  ...
  function fillLimitedTemplate(&$template) {
    $this->loadRefSejourMaman()->loadRefPatient()->fillLimitedTemplate($template, CAppUI::tr('CNaissance-Mother'), false);
  }
  ...
  function fillLiteLimitedTemplate(&$template, $champ) {
    ...
    $bebe->loadRefLatestConstantes($bebe->naissance, array("poids", "taille", "perimetre_cranien"));
    $constantes = CConstantesMedicales::getLatestFor($bebe, null, array("taille", "perimetre_cranien"), $this->_ref_sejour_enfant, false, $bebe->naissance);
    $first_constantes = CConstantesMedicales::getFirstFor($bebe, null, array("poids"), $this->_ref_sejour_enfant, false, $bebe->naissance);

    $template->addProperty("$champ - " . CAppUI::tr('CConstantesMedicales-weight'), "{$first_constantes[0]->_poids_g} g");
    $template->addProperty("$champ - " . CAppUI::tr('CConstantesMedicales-size'), "{$constantes[0]->taille} cm");
    $template->addProperty("$champ - " . CAppUI::tr('CConstantesMedicales-cranial perimeter'), "{$constantes[0]->perimetre_cranien} cm");
