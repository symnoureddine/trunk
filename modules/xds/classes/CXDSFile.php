...
class CXDSFile extends CMbObject {
  ...
  static function getTypeDocument() {
    return CInteropResources::loadJV(CMbArray::get(CXDSValueSet::$JDV, "typeCode"), CXDSValueSet::$type, true);
  }

  /**
   * Map the field of CDA into object
   *
   * @param String $xml_cda CDA
   *
   * @return void
   * @throws Exception
   */
  function getFromCDA($xml_cda) {
    $dom_cda = new CMbXMLDocument("UTF-8");
    $dom_cda->loadXMLSafe($xml_cda);

    $xpath = new CCDAXPath($dom_cda);

    $this->title = $xpath->getTitle();
    $code = $xpath->queryUniqueNode("/cda:code");
    $this->type_document = $xpath->getCodeCE($code);

    $this->create_date = $xpath->getCreateDate();

    $confidentiality = $xpath->queryUniqueNode("/cda:confidentialityCode");
    $this->confidentiality = $xpath->getCodeCE($confidentiality);

    $this->language = $xpath->queryAttributNode("/cda:languageCode", null, "code");

    $node_authors = $xpath->query("/cda:author");
    $authors = array();
    foreach ($node_authors as $_author) {
      $speciality = $xpath->queryUniqueNode("./cda:assignedAuthor/cda:code", $_author);
      $authors[] = array(
        "speciality" => $xpath->getCodeCE($speciality),
        "telecom"    => $xpath->getTelecom($_author),
        "name"       => $xpath->getName($xpath->queryUniqueNode("./cda:assignedAuthor/cda:assignedPerson/cda:name")),
      );
    }
