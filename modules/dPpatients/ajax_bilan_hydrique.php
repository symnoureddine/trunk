...
$bilan = CPrescription::getStructureBilanHydrique(
  "$date_bh 00:00:00", "$date_bh 23:59:59",
  $granularite
);
...
foreach ($bilan as $_datetime => $_bilan) {
  $series[0]["data"][] = array(count($series[0]["data"]), round($_bilan, 2));
  $ticks[]             = array(count($ticks), CMbDT::dateToLocale(implode("<br />", explode(" ", $_datetime))));
}
