...
foreach (array_keys(CCompteRendu::$templated_classes) as $_template_classe) {
  if (($object_class
    ...
    || !in_array($_template_classe, array("CConsultation", "CConsultAnesth", "COperation", "CPatient", "CSejour"))
  ) {
    continue;
  }
  ...
  $request->addWhere(array("compte_rendu.object_class" => "= '$_template_classe'"));
  $request->addWhere(array("compte_rendu.creation_date BETWEEN '$date_min 00:00:00' AND '$date_max 23:59:59'"));
  $request->addWhere(array("compte_rendu.object_id" => "IS NOT NULL"));
    if ($lock !== "all") {
        if ($lock === "0") {
    ...
    if ($nom_doc) {
    $request->addWhere(array("compte_rendu.nom" => "LIKE '%$nom_doc%'"));
  }

  // Contraites sur le séjour
  if ($object_class === "CSejour" && ($praticien_id || $service_id || $type)) {
    $request->addLJoin(array("sejour" => "sejour.sejour_id = compte_rendu.object_id"));
    if ($praticien_id) {
      $request->addWhere(array("sejour.praticien_id" => "= '$praticien_id'"));
    }
    if ($service_id) {
      $request->addWhere(array("sejour.service_id" => "= '$service_id'"));
    }
    if ($type) {
      $request->addWhere(array("sejour.type" => "= '$type'"));
    }
  }

  if (in_array($status_doc, array("signe", "non_signe"))) {
    $request->addWhere(
      array(
  ...
  $request->addWhere(array("compte_rendu.author_id" => $user_id ? "= '$user_id'" : CSQLDataSource::prepareIn($users_ids)));


  switch ($_template_classe) {
    case "CConsultAnesth":
      $request->addLJoin(array("consultation_anesth" => "consultation_anesth.consultation_anesth_id = compte_rendu.object_id"));
      $request->addLJoin(array("consultation" => "consultation.consultation_id = consultation_anesth.consultation_id"));

      if ($praticien_id) {
        $request->addLJoin(array("plageconsult" => "consultation.plageconsult_id = plageconsult.plageconsult_id"));
        $request->addWhere(array("consultation_anesth.chir_id = '$praticien_id' OR plageconsult.chir_id = '$praticien_id'"));
      }

      if ($patient_id_search) {
        $request->addWhere(array("consultation.patient_id" => "= '$patient_id_search'"));
      }
      break;

    case "CConsultation":
      $request->addLJoin(array("consultation" => "consultation.consultation_id = compte_rendu.object_id"));

      if ($praticien_id) {
        $request->addLJoin(array("plageconsult" => "consultation.plageconsult_id = plageconsult.plageconsult_id"));
        $request->addWhere(array("plageconsult.chir_id" => "= '$praticien_id'"));
      }
    ...
    case "COperation":
      $request->addLJoin(array("operations" => "operations.operation_id = compte_rendu.object_id"));
      $request->addLJoin(array("sejour" => "sejour.sejour_id = operations.sejour_id"));

      if ($praticien_id) {
        $request->addWhere(array("operations.chir_id" => "= '$praticien_id'"));
      }

      if ($patient_id_search) {
        $request->addWhere(array("sejour.patient_id" => "= '$patient_id_search'"));
      }
      break;

    case "CPatient":
      $request->addLJoin(array("patients" => "patients.patient_id = compte_rendu.object_id"));

      if ($patient_id_search) {
        $request->addWhere(array("patients.patient_id" => "= '$patient_id_search'"));
      }
      break;
...
if ($documents_ids) {
  array_multisort(CMbArray::pluck($documents_ids, "creation_date"), SORT_DESC, $documents_ids);

  $documents_ids = array_slice($documents_ids, $page, 30);

  $documents = $document->loadList(array("compte_rendu_id" => CSQLDataSource::prepareIn(CMbArray::pluck($documents_ids, "compte_rendu_id"))), "creation_date DESC");
