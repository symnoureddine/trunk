...
if (!$constantName) {
    ...
    $sexe        = $patient->sexe;
    $age         = ($patient->_mois < 36 && $graph_name != "_imc") ? 3 : 18;
    $number_aged = ($patient->_mois < 36 && $graph_name != "_imc") ? 0.0328767 : 0.00273973; // 1 Day = 0,00273973 year

    $patient->loadRefNaissance()->loadRefGrossesse();
    $patient->_ref_naissance;
    ...
    if ($graph_name == "bilirubine_transcutanee") {
        foreach ($constantes as $_constante) {
            $calculHour[] = CMbDT::hoursRelative($patient->_ref_naissance->date_time, $_constante->datetime);
...
if ($constantName) {
    ...
    $smarty->display('inc_select_courbe_reference_graph.tpl');
} else {
    ...
    $smarty->display('inc_courbe_reference_graph.tpl');
}
