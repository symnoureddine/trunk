...
$list_constantes           = CConstantesMedicales::getConvertUnitGlycemie($list_constantes);
...
if ($context && $context->_class == "CSejour") {
  $context->checkAnqsEmpty();
}

// Make blood sugar report
if ($context instanceof CSejour) {
  // Get the prescription of the stay
  $prescription = $context->loadRefPrescriptionSejour();
  $prescription->loadRefsLinesMed();

  // Get constants
  $context->loadRefConstantes();
  $constantes_medicales = $context->_ref_suivi_medical;
  $constantes_medicales = CConstantesMedicales::getConvertUnitGlycemie($constantes_medicales);

  // Select days of the report
  ...
  $midnight_enter = (new DateTimeImmutable($context->entree))->setTime(0, 0);
  ...
  for ($i = 0; $i < 5; $i++) {
    ...
    foreach ($A10A_lines as $_line) {
      if ($day < new DateTimeImmutable($_line->_debut_reel)
      ...
      foreach ($_line->_ref_administrations as $_administration) {
        ...
        $qte_ui = ($ratio_ui && $ratio_ui > 0) ? $_administration->quantite / $ratio_ui : '-';
...
$last_unit_glycemie = CConstantesMedicales::getlastUnitGlycemie($list_constantes, true);

if ($last_unit_glycemie) {
    $latest_constantes[0]->unite_glycemie = $last_unit_glycemie;
    $conv = CConstantesMedicales::getConv("glycemie", $last_unit_glycemie);
    ...
    $constantes->unite_glycemie = $last_unit_glycemie;
}
...
$smarty->assign("last_unit_glycemie", $last_unit_glycemie);
