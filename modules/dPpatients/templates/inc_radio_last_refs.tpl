...
{{if $patient->_id}}
  <form method="get" action="?" id="list_Relations">
    <ul style="text-align: left;">
      <li>
        <input type="checkbox" name="object" onclick="attach.setObject('{{$patient->_guid}}'); checkrelation();"
    ...
    {{foreach from=$patient->_ref_sejours item=_sejour}}
      <li>
        <input type="checkbox" name="object" onclick="attach.setObject('{{$_sejour->_guid}}');checkrelation();"
               {{if $_sejour->_id == $dossier_id}}checked{{/if}} />
        {{$_sejour}}
      </li>
      {{foreach from=$_sejour->_ref_operations item=_op}}
        <li style="margin-left: 15px; padding-left: 15px; border-left: solid 1px grey;">
          <input type="checkbox" name="object" onclick="attach.setObject('{{$_op->_guid}}'); checkrelation();" />
      ...
      {{foreach from=$_sejour->_ref_consultations item=_consult}}
        <li style="margin-left:15px; padding-left: 15px; border-left: solid 1px grey;">
          <input type="checkbox" name="object" onclick="attach.setObject('{{$_consult->_guid}}'); checkrelation(); "/>
