...
<form name="find" action="?" method="get" onsubmit="return Url.update(this, 'search_result_patient');" class="me-no-align">
  ...
  <table class="form me-align-auto me-form-search-patient">
    ...
    <tr>
      <td class="button me-text-align-right" colspan="4" style="white-space: normal;">
        ...
        {{/if}}
        <button type="button" class="erase me-tertiary" onclick="emptyForm();$('vw_idx_patient_button_create').hide();"
                title="{{tr}}CPatient-action-Clear the form fields-desc{{/tr}}">
          {{tr}}Empty{{/tr}}
        </button>
        {{if $can->edit}}
          <button id="vw_idx_patient_button_create" class="new" type="button" tabindex="15" style="display:none;"
