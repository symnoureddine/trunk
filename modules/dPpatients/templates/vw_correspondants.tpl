  ...
  <form name="find_medecin" action="?" method="get" onsubmit="return onSubmitFormAjax(this, null, 'medicaux_result')">
    ...
    <table class="main form me-no-box-shadow me-margin-top-0">
      ...
      <tr>
        ...
        <th></th>
        <td></td>
        {{me_form_bool nb_cells=2 mb_object=$medecin mb_field=actif}}
          {{mb_field object=$medecin field=actif}}
        {{/me_form_bool}}
      </tr>
      ...
      <tr>
        <td class="button" colspan="6">
          {{if !$dialog}}
            <button class="search me-primary" type="submit" onclick="$V(this.form.annuaire, 0);">{{tr}}Search{{/tr}}</button>
          {{else}}
          ...
          {{if $conf.dPpatients.CPatient.function_distinct}}
            <button class="search me-secondary" type="button" onclick="$V(this.form.annuaire, 1); this.form.onsubmit()">{{tr}}Search{{/tr}} dans
              l'annuaire
            </button>
          {{/if}}
          {{if $can->admin || !$edit_for_admin}}
            <button class="new" type="button"
                    onclick="Medecin.editMedecin('0', refreshPageMedecin.curry('0'), $V(this.form.type));">{{tr}}Create{{/tr}}</button>
            {{if $is_admin}}
              <button type="button" class="download me-tertiary" onclick="popupExportCorrespondants();">{{tr}}Export-CSV{{/tr}}</button>
  ...
  <div id="autres" style="display: none;">
    <form name="find_correspondant" action="?" method="get" onsubmit="return onSubmitFormAjax(this, null, 'correspondants_result')">
      ...
      <table class="form me-no-box-shadow me-margin-top-0">
        <tr>
          <th class="title me-no-title me-no-bg" colspan="6">{{tr}}CCorrespondantPatient.search{{/tr}} (Modèles)</th>
        </tr>
