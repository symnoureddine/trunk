...
{{else}}
  ...
  {{else}}
    {{assign var=src value="style/mediboard_ext/images/pictures/user.png"}}
  {{/if}}
{{/if}}
...
{{if $mode == "edit"}}
  <br />
  {{if !$file->_id}}
    <button type="button" class="search me-tertiary notext" title="{{tr}}Browse{{/tr}}" onclick="uploadFile('{{$patient->_guid}}', null, 'identite.jpg', 1)">
      {{tr}}Browse{{/tr}}
    </button>
