...
<script>
  InseeFields.initCPVille("editCorrespondant", "cp", "ville", null,  "tel");
...
<form name="editCorrespondant" method="post" action="?" onsubmit="return Correspondant.onSubmit(this);">
  ...
  <table class="form">
    ...
    <tr {{if $correspondant->relation != "confiance" && $correspondant->relation != "representant_th" &&
          $correspondant->relation != "parent_proche"}}style="display: none;"{{/if}} id="nom_jeune_fille">
    ...
    <tr {{if $correspondant->relation != "confiance" && $correspondant->relation != "representant_th" &&
          $correspondant->relation != "parent_proche"}}style="display: none;"{{/if}}
        id="naissance">
    ...
    {{if $conf.ref_pays == 2}}
      <tr
        id="ean" {{if $correspondant->relation != "assurance" && $correspondant->relation != "employeur"}} style="display: none;"{{/if}}>
        {{me_form_field nb_cells=2 mb_object=$correspondant mb_field="ean"}}
          {{mb_field object=$correspondant field="ean"}}
        {{/me_form_field}}
      </tr>
      <tr id="ean_base" {{if $correspondant->relation != "assurance"}} style="display: none;"{{/if}}>
        {{me_form_field nb_cells=2 mb_object=$correspondant mb_field="ean_base"}}
          {{mb_field object=$correspondant field="ean_base"}}
        {{/me_form_field}}
      </tr>
      <tr id="type_pec" {{if $correspondant->relation != "assurance"}} style="display: none;"{{/if}}>
        {{me_form_field nb_cells=2 mb_object=$correspondant mb_field="type_pec"}}
          {{mb_field object=$correspondant field="type_pec" emptyLabel="Choose"}}
        {{/me_form_field}}
      </tr>
      <tr id="ean_id" {{if $correspondant->relation != "assurance"}} style="display: none;"{{/if}}>
        {{me_form_field nb_cells=2 mb_object=$correspondant mb_field="ean_id"}}
          {{mb_field object=$correspondant field="ean_id"}}
        {{/me_form_field}}
      </tr>
      <tr id="assure_id" {{if $correspondant->relation != "assurance"}} style="display: none;"{{/if}}>
        {{me_form_field nb_cells=2 mb_object=$correspondant mb_field="assure_id"}}
          {{mb_field object=$correspondant field="assure_id"}}
        {{/me_form_field}}
      </tr>
    {{/if}}
    ...
    <tr>
      <td colspan="2" class="button">
        ...
        {{if $correspondant->_id}}
          <button type="button" onclick="Correspondant.confirmDeletion(this.form);" class="cancel me-secondary">{{tr}}Delete{{/tr}}</button>
        {{/if}}
      </td>
    </tr>
  </table>
