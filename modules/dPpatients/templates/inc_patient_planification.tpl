    ...
    {{if $m != "dPurgences"}}
      ...
      <div id="sejour-buttons" style="display: none;">
        <button class="big me-margin-4 me-primary me-width-auto" type="button"
                onclick="Control.Modal.close(); newOperation('{{$praticien_id}}', '{{$patient_id}}', '{{$consult->_id}}');"
                style="width: 20em;">
            {{tr}}COperation-title-create{{/tr}}
        </button>
        <br />
        <button class="big me-margin-4 me-big me-primary me-width-auto" type="button"
                onclick="Control.Modal.close(); newHorsPlage('{{$praticien_id}}', '{{$patient_id}}', '{{$consult->_id}}');"
                style="width: 20em;">
            {{tr}}COperation-title-create-horsplage{{/tr}}
        </button>
        <br />
        <button class="big me-margin-4 me-big me-primary me-width-auto" type="button"
                onclick="Control.Modal.close(); newSejour('{{$praticien_id}}', '{{$patient_id}}', '{{$consult->_id}}');"
                style="width: 20em;">
            {{tr}}CSejour-title-create{{/tr}}
        </button>
        {{me_button icon=new old_class="big" mediboard_ext_only="1" label="COperation-title-create" onclick="Control.Modal.close(); newOperation('$praticien_id', '$patient_id', '`$consult->_id`');"}}
        {{me_button icon=new old_class="big" mediboard_ext_only="1" label="COperation-title-create-horsplage" onclick="Control.Modal.close(); newHorsPlage('$praticien_id', '$patient_id', '`$consult->_id`');"}}
        {{me_button icon=new old_class="big" mediboard_ext_only="1" label="CSejour-title-create" onclick="Control.Modal.close(); newSejour('$praticien_id', '$patient_id', '`$consult->_id`');"}}
      </div>
    {{/if}}
  {{/if}}
{{/if}}
...
{{if !$sejour || !in_array($sejour->type, 'Ox\Mediboard\PlanningOp\CSejour::getTypesSejoursUrgence'|static_call:$sejour->praticien_id)}}
  ...
  {{/if}}
  {{mb_include module="cabinet" template="inc_button_consult_immediate" patient_id=$patient->_id type="" me_primary="false" callback=$callback}}
{{/if}}
