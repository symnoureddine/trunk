...
<table class="tbl me-patient-complete print_patient me-no-hover" style="{{if !$not_printable}}page-break-after: always;{{/if}}">
  {{if $offline_sejour}}
  ...
  <tr>
    <td class="text">
      <strong>{{mb_label object=$object field="prenom"}}</strong>
        {{mb_value object=$object field="prenom"}}{{if $object->_prenom_2}},
        {{mb_value object=$object field="_prenom_2"}}{{/if}}{{if $object->_prenom_3}},
        {{mb_value object=$object field="_prenom_3"}}{{/if}}{{if $object->_prenom_4}},
        {{mb_value object=$object field="_prenom_4"}} {{/if}}
    </td>
  ...
  <tr>
    <td class="text">
      ...
      {{mb_value object=$object field="naissance"}} <em>({{mb_value object=$object field=_age}})</em>
    </td>
  ...
  <tr>
    <td class="text">
      <strong>{{mb_label object=$object field=pays_naissance_insee}}</strong>
  ...
  {{foreach from=$object->_ref_cp_by_relation.prevenir item=prevenir name=foreach_prevenir}}
    <tr>
      <th class="category" colspan="3">Personne à prévenir</th>
    </tr>
    <tr>
      <td class="text">
  ...
  <tr>
    <th class="category" colspan="3">Bénéficiaire de soins</th>
  </tr>
  ...
  <!-- Dossier Médical -->
  {{if $object->_ref_dossier_medical->_canRead && $object->_ref_dossier_medical->_id && !$app->user_prefs.limit_prise_rdv}}
    <tr>
      <td colspan="3">
