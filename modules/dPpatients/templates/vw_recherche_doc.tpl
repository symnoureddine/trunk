    ...
    <table class="form me-no-box-shadow">
      <tr>
        ...
        {{me_form_field nb_cells=2 label="common-Context"}}
          ...
          <select name="object_class" onchange="RechercheDoc.filterByContext(this.value);">
            ...
            {{foreach from=$_spec->_locales item=_locale key=_object_class}}
              {{if in_array($_object_class, array("CConsultation", "CConsultAnesth", "COperation", "CPatient", "CSejour"))}}
                <option value="{{$_object_class}}" {{if $_object_class === $object_class}}selected{{/if}}>{{$_locale}}</option>
              {{/if}}
            {{/foreach}}
          </select>
        {{/me_form_field}}
        ...
        {{me_form_field nb_cells=2 label="Owner"}}
          ...
          <button type="button" class="cancel notext me-tertiary me-dark" onclick="$V(this.form.user_id, ''); $V(this.form.user_id_view, '');"></button>
        {{/me_form_field}}
      </tr>
      <tr>
      ...
      <tr>
        ...
        {{me_form_field nb_cells=2 mb_class=CConsultation mb_field=patient_id}}
          <input type="hidden" name="patient_id_search" value="{{$patient->_id}}" ondblclick="PatSelector.init();" />
          <input type="text" name="_pat_name" style="width: 15em;" value="{{$patient}}" readonly onfocus="PatSelector.init()" />
      ...
      <tr>
        <td colspan="8" class="button">
          <button type="button" class="search me-primary" onclick="RechercheDoc.listDocs(0);">{{tr}}Filter{{/tr}}</button>
        </td>
      </tr>
    </table>
  </fieldset>

  <fieldset id="filtre_CSejour" class="filtres_context" {{if $object_class !== "CSejour"}}style="display: none;"{{/if}}>
    <legend>
...
<fieldset class="me-align-auto me-no-border-radius-top">
  ...
  {{if $can->admin}}
    <button type="button" class="trash me-tertiary" onclick="RechercheDoc.deleteDocs();">{{tr}}Delete{{/tr}}</button>
  {{/if}}
  <button type="button" class="mail me-tertiary" onclick="RechercheDoc.sendDocs();">{{tr}}CCompteRendu.send_mail{{/tr}}</button>
