...
<tr class="{{$trClass}} {{if $_patient->deces}}hatching{{/if}}">
  ...
  {{else}}
    ...
    <td class="button" rowspan="{{$rowspan}}" style="white-space: nowrap;">
      {{if $can->edit}}
        <button class="edit compact me-tertiary" type="button" onclick="PatientFromSelector.edit({{$_patient->_id}})">
          {{tr}}Edit{{/tr}}
        </button>
      {{/if}}
      ...
      <button class="tick compact" id="inc_pat_selector_select_pat"
              type="button"
              onclick="PatientFromSelector.select({{$_patient->getProperties()|JSAttribute|@json:true|escape:"html"}});">
        {{tr}}Select{{/tr}}
      </button>
    </td>
  {{/if}}
