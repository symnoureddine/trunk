...
<script>
  Main.add(function() {
    ...
    {{foreach from=$regle->_ext_diagnostics item=_cim}}
      RegleEvt.createSpanCIM('{{$_cim->code}}');
    {{/foreach}}

    CIM.autocomplete(getForm('edit_program').keywords_code, null, {
      afterUpdateElement: function(input) {
        $V(getForm('edit_program')._added_code_cim, input.value);
      }
    });
  ...
  <table class="main form">
    ...
    <tr>
      ...
      <td>
        ...
        <input type="hidden" name="_added_code_cim" onchange="RegleEvt.createSpanCIM($V(this));$V(this, '');" />
        <button class="search notext" type="button"
                onclick="CIM.viewSearch($V.curry(this.form.elements['_added_code_cim']), this.form.elements['_praticien_id']);">
          {{tr}}Search{{/tr}}
        </button>
        <div id="codes_cim_regle_alerte"></div>
      </td>
    </tr>
    <tr>
