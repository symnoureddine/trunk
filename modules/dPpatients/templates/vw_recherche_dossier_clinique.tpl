...
<script>
  ...
  selectCriteria = function (mod_tamm) {
    ...
    url.requestJSON(function (list_criterion) {
      ...
      if (list_criterion['search_criteria_id']) {
        ...
        // patient
        if (mod_tamm == 1) {
          $V(form.date_min, list_criterion['date_min']);
          $V(form.date_min_da, Date.fromDATETIME(list_criterion['date_min']).toLocaleDateTime());
          $V(form.date_max, list_criterion['date_max']);
          $V(form.date_max_da, Date.fromDATETIME(list_criterion['date_max']).toLocaleDateTime());
        } else {
  ...
  Main.add(function () {
    ...
    new Url("ccam", "httpreq_do_ccam_autocomplete")
        ...
        <td colspan="2">
          ...
          <table class="main form me-no-box-shadow me-no-bg">
            ...
            <tr {{if $mod_tamm}}style="display: none"{{/if}}>
              {{me_form_field field_class="me-label-bg-light" nb_cells=2 mb_object=$sejour mb_field=entree}}
                {{mb_field object=$sejour field=entree register=true form="rechercheDossierClinique"}}
              {{/me_form_field}}
            </tr>
            <tr {{if $mod_tamm}}style="display: none"{{/if}}>
              {{me_form_field field_class="me-label-bg-light" nb_cells=2 mb_object=$sejour mb_field="sortie"}}
            ...
            <tr {{if !$mod_tamm}}style="display: none"{{/if}}>
              {{me_form_field field_class="me-label-bg-light" nb_cells=2 label="CSearchCriteria-date_min"}}
                <input type="text" class="dateTime" name="date_min_da" readonly value="{{$date_min|date_format:$conf.datetime}}" />
                <input class="dateTime" type="hidden" name="date_min" value="{{$date_min}}" />
              {{/me_form_field}}
            </tr>
            <tr {{if !$mod_tamm}}style="display: none"{{/if}}>
              {{me_form_field field_class="me-label-bg-light" nb_cells=2 label="CSearchCriteria-date_max"}}
                <input type="text" class="dateTime" name="date_max_da" readonly value="{{$date_max|date_format:$conf.datetime}}" />
                <input class="dateTime" type="hidden" name="date_max" value="{{$date_max}}" />
              {{/me_form_field}}
            </tr>
