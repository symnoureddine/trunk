...
<script>
  ...
  editCerfa = function (name, object_class, object_id) {
    var url = new Url("ameli", "vw_cerfa_edit");
  ...
  Main.add(function () {
    ...
    var url = new Url("ameli", "ajax_cerfa_autocomplete");
...
<fieldset class="case_add_doc">
  ...
  <form name="addDoc-{{$context->_guid}}" method="post">
    ...
    <table class="main">
      ...
      <tr>
        <td>
          <input type="text" placeholder="&mdash; {{tr}}CPack{{/tr}}" name="keywords_pack" class="autocomplete str" autocomplete="off"
                 style="font-size: 13pt; background-position: 100% 4px;" />
        </td>
      </tr>
    </table>
...
{{if "ameli"|module_active}}
  <fieldset class="case_add_doc">
    <legend>{{tr}}mod-ameli-tab-show_cerfa{{/tr}}</legend>
    <form name="form-{{$cerfa_guid}}-{{$context->_guid}}" method="get" onsubmit="return false">
      <table class="main">
        <tr>
          <td class="halfpane">
            <input type="text" placeholder="&mdash; {{tr}}CCerfa{{/tr}}" name="keywords" size="8" class="autocomplete str listCerfa"
