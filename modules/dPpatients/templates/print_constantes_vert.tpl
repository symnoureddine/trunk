...
{{mb_default var=last_unit_glycemie value=""}}
...
{{if $fixed_header}}
  ...
  <div style="display: inline-block; top: 0px;">
    <table id="constant_grid_vert_header" class="tbl constantes" style="width: auto; border-right: none; box-sizing: border-box;">
      <tr>
        ...
        <tr>
          <th style="text-align: left; height: 15px;">
            ...
            {{if $list_constantes.$_cste_name.unit}}
              {{if $_cste_name == "glycemie" && $last_unit_glycemie}}
                ({{$last_unit_glycemie}})
              {{else}}
    ...
    <table id="constant_grid_vert" class="tbl constantes" style="border-left: none;">
      <tr class="header">
        {{foreach from=$cste_grid.grid item=_constante key=_datetime}}
          <th class="text" style="width: 58px; text-align: center; vertical-align: top; font-size: 1em; cursor: pointer;">
          ...
          {{if isset($patient|smarty:nodefaults) && $view == 'display_all_constantes_patient'}}
            <button type="button" class="edit notext" onclick="editConstant('{{$_constante.object.id}}', '{{$patient->_id}}', 1);" title="{{tr}}Edit{{/tr}}"></button>
          {{/if}}
          {{if $_constante.comment}}
            ...
            {{else}}
              <div class="me-constante-comment" style="min-width: 120px; font-weight: normal; background: #eee; background: rgba(255,255,255,0.6); white-space: normal; text-align: left; padding: 2px; border: 1px solid #ddd;">
                {{$_constante.comment}}
              </div>
            {{/if}}
          {{/if}}
        ...
        {{foreach from=$cste_grid.grid item=_constante key=_datetime}}
          ...
          {{if is_array($_value)}}
            {{if $_value.value === null}}
              <td style="height: 15px; border-left: 1px solid #999; {{if $_value.color}}background-color: {{$_value.color}};{{/if}} cursor: pointer;" {{if $_value.span > 0}} colspan="{{$_value.span}}" {{/if}}{{if $view == 'view_constantes'}} onclick="editConstants('{{$_constante.object.id}}', '{{$_constante.object.context}}');"{{/if}}></td>
            {{else}}
              <td style="height: 15px; text-align: center; border-top: 1px solid {{if $_value.pair == "odd"}} #36c {{else}} #3c9 {{/if}}; border-left: 1px solid #999; {{if $_value.color}}background-color: {{$_value.color}};{{/if}} cursor: pointer;"
          ...
          {{elseif $_value != "__empty__"}}
            <td class="text" style="height: 15px; text-align: center; cursor: pointer;"{{if $view == 'view_constantes'}} onclick="editConstants('{{$_constante.object.id}}', '{{$_constante.object.context}}');"{{/if}}>
              {{if $_value !== ""}}
                {{$_value}}
                ...
                {{if array_key_exists('alerts', $_constante) && array_key_exists($_cste_name, $_constante.alerts)}}
                  ...
                  <i class="fa fa-exclamation-circle" style="color: firebrick; cursor: help;" onmouseover="ObjectTooltip.createDOM(this, 'alert_{{$alert_uid}}');"></i>
...
{{else}}
  ...
  <table class="tbl me-no-max-width constantes" style="{{if !$full_size}}width: 1%; {{/if}}font-size: inherit;">
    <tr>
      <th></th>
      {{foreach from=$cste_grid.grid item=_constante key=_datetime}}
        <th class="text" style="text-align: center; vertical-align: top; font-size: 0.9em; cursor: pointer;{{if !$print}} max-width: 80px;{{/if}}">
          <span{{if $view == 'view_constantes'}} onclick="editConstants('{{$_constante.object.id}}', '{{$_constante.object.context}}');"{{/if}}>
            {{$_datetime|substr:0:18|date_format:$conf.datetime}}
            ...
            {{if $_constante.comment}}
              {{if $app->user_prefs.constantes_show_comments_tooltip && !$print}}
                <i class="me-icon comment me-primary" title="{{$_constante.comment}}"></i>
              {{else}}
                <div class="me-constante-comment" style="font-weight: normal; background: #eee; background: rgba(255,255,255,0.6); white-space: normal; text-align: left; padding: 2px; border: 1px solid #ddd;{{if !$print}} width: 80px; box-sizing: border-box;{{else}}min-width: 120px;{{/if}}">
                  {{$_constante.comment}}
                </div>
              {{/if}}
            {{/if}}
          ...
          {{if isset($patient|smarty:nodefaults) && $view == 'display_all_constantes_patient'}}
            <button type="button" class="edit notext not-printable"
                    onclick="editConstant('{{$_constante.object.id}}', '{{$patient->_id}}', 1);" title="{{tr}}Edit{{/tr}}"></button>
          {{/if}}
        </th>
      {{/foreach}}
    </tr>
      ...
      <tr>
        <th style="text-align: right;">
          ...
          {{if $_cste_name|in_array:$noticed_constant && !$print}}
            <span onmouseover="ObjectTooltip.createDOM(this, '{{$_cste_name}}-notice');" style="float: right; color: #2946c9; border-bottom: none;">
              <i class="fa fa-lg fa-info-circle"></i>
            </span>
            <div id="{{$_cste_name}}-notice" class="me-color-black-high-emphasis" style="display: none;">
              {{tr}}CConstantesMedicales-notice-{{$_cste_name}}{{/tr}}
        ...
        {{foreach from=$cste_grid.grid item=_constante key=_datetime}}
          ...
          {{if is_array($_value)}}
            {{if $_value.value === null}}
              <td style="border-left: 1px solid #999; {{if $_value.color}}background-color: {{$_value.color}};{{/if}} cursor: pointer;" {{if $_value.span > 0}} colspan="{{$_value.span}}" {{/if}}{{if $view == 'view_constantes'}} onclick="editConstants('{{$_constante.object.id}}', '{{$_constante.object.context}}');"{{/if}}></td>
            {{else}}
              <td style="text-align: center; border-top: 1px solid {{if $_value.pair == "odd"}} #36c {{else}} #3c9 {{/if}}; border-left: 1px solid #999; {{if $_value.color}}background-color: {{$_value.color}};{{/if}} cursor: pointer;"
                {{if $_value.span > 0}} colspan="{{$_value.span}}" {{/if}}{{if $view == 'view_constantes'}} onclick="editConstants('{{$_constante.object.id}}', '{{$_constante.object.context}}');"{{/if}}>
                <strong>{{$_value.value}}</strong> -
                <small>{{$_value.day}}</small>
              </td>
            {{/if}}
          {{elseif $_value != "__empty__"}}
            <td class="text" style="text-align: center; cursor: pointer;"{{if $view == 'view_constantes'}} onclick="editConstants('{{$_constante.object.id}}', '{{$_constante.object.context}}');"{{/if}}>
              {{if $_value !== ""}}
                {{$_value}}
                {{if array_key_exists('comments', $_constante) && array_key_exists($_cste_name, $_constante.comments)}}
                  {{if !$print}}
