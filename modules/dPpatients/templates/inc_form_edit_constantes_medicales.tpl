...
<script>
  ...
  callbackSubmitConstantes = function(form) {
    ...
    {{elseif $conf.ref_pays == 2}}
      EchelleTri.refreshConstantesMedicalesTri($V(form.context_class) + '-' + $V(form.context_id));
    {{/if}}
  };
  ...
  showRedons = function() {
    ...
    .addParam('sejour_id', '{{$constantes->context_id}}')
    .requestModal('80%', '80%', {onClose: callbackSubmitConstantes.curry(form)});
  };
  ...
  Main.add(function () {
    ...
    Calendar.regField(getForm('edit-constantes-medicales{{$unique_id}}').datetime, dates);
...
<div id="constant_form{{$unique_id}}" style="position: absolute; top: 0; left: 0; bottom: 0; min-height: 290px; width: 100%;">
  ...
  {{if $use_redon}}
    <button type="button" class="search me-margin-top-4" style="float: right;" onclick="showRedons();">{{tr}}CRedon{{/tr}}</button>
  {{/if}}
  ...
  <form name="edit-constantes-medicales{{$unique_id}}" action="?" method="post"
    ...
    {{if $show_cat_tabs}}
      <ul id="constantes-by-type{{$unique_id}}" class="control_tabs small me-dossier-soin-tabs-cst me-no-border-radius" style="min-width: 200px;">
      ...
      <table class="main form constantes me-dossier-soin-constantes me-small" style="margin-right:20px;">
        <tr>
          <th class="category narrow">
            <button class="stats notext me-tertiary me-dark me-btn-small" type="button" onclick="displayConstantGraph();">
            ...
            {{foreach from=$_constants item=_constant}}
              <tr {{if $_rank == "hidden" && ($const->$_constant == "" || !$display_graph)}}
                ...
                <th style="text-align: left;" class="text constant_name">
                  <button type="button" class="stats notext me-tertiary me-dark me-btn-small" style="float: right; display: none;"
                          onclick="displayConstantGraph('{{$_constant}}');">
                    {{tr}}CConstantGraph-msg-display{{/tr}}
                  </button>
                  {{assign var=constantName value=""}}
                  {{if $constantes->_ref_patient->_annees <= "18"}}
                    {{if $constantes->_ref_patient->_annees <= "3" && $constantes->_ref_patient->_mois <= 36}}
                  ...
                  {{if ($constantes->_ref_patient->_annees <= "18" && in_array($_constant, array("poids", "taille", "_imc", $constantName, "bilirubine_transcutanee", "bilirubine_totale_sanguine")))}}
                    ...
                    <button type="button" class="stats notext me-tertiary me-dark me-btn-small" style="float: right; display: block;"
                  ...
                  <label for="{{$_constant}}" title="{{tr}}CConstantesMedicales-{{$_constant}}-desc{{/tr}}" onmouseover="">
                    ...
                    {{tr}}CConstantesMedicales-{{$_constant}}-court{{/tr}}
                      ...
                      {{if $_params.unit && ($_constant != "glycemie")}}
                  ...
                  {{if $_constant|in_array:$noticed_constant}}
                    ...
                    <div id="{{$_constant}}-notice" class="me-color-black-high-emphasis" style="display: none;">
                  ...
                  {{if $_constant|array_key_exists:$constantes->_refs_comments}}
                    ...
                    <i class="me-icon comment me-primary" style="float: right;" title="{{$comment->comment}}"></i>
                  {{/if}}
                </th>
                  ...
                  {{if $can_edit}}
                    <td>
                      {{foreach from=$_params.formfields item=_formfield_name key=_key name=_formfield}}
                        ...
                        {{if $_formfield_name == "_glycemie"}}
                          {{mb_field object=$constantes field=unite_glycemie class="me-small"}}
                        {{/if}}
                      {{/foreach}}
                    </td>
                    <td class="narrow">
                      <button id="edit_comment_{{$_constant}}{{$unique_id}}" type="button" style="float: right;" class="comment notext me-tertiary me-dark me-btn-small"
                  ...
                  <td style="text-align: center" title="{{$dates.$_constant|date_format:$conf.datetime}}">
                    {{if $const->$_constant}}
                      {{foreach from=$_params.formfields item=_formfield_name key=_key name=_formfield}}
                        ...
                        {{if $_formfield_name != "glycemie"}}{{mb_value object=$const field=unite_glycemie}}{{/if}}
                      {{/foreach}}
                    {{/if}}
                  </td>
                {{else}}
                ...
                {{if $display_graph}}
                  <td class="narrow">
                    {{if $_constant.0 != "_" || !empty($_params.plot|smarty:nodefaults)}}
                      <input type="checkbox" class="checkbox-constant" name="checkbox-constantes-medicales-{{$_constant}}"
    ...
    <div class="me-patient-constantes_footer" style="position: absolute; bottom:0; text-align:center; height:100px; width: 100%;" id="buttons_form_const-{{$unique_id}}">
      ...
      {{if !$hide_save_button}}
        <button class="modify me-primary me-small" id="btn_save_constant" onclick="return submitConstantesMedicales(this.form);">
          {{tr}}Save{{/tr}}
        </button>
      {{/if}}
      {{elseif $can_create}}
