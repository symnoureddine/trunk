    ...
    <table class="form me-box-shadow-table">
      ...
      <tr>
        <td colspan="2" class="button">
          {{if $can->edit}}
            <button class="new me-primary" type="button" onclick="PatientFromSelector.create({{$useVitale}});">
              {{tr}}CPatient.create_with_vitale{{/tr}}
            </button>
          {{/if}}
        </td>
        <td colspan="2" class="button">
          <button class="cancel me-tertiary me-dark" type="button" onclick="Control.Modal.close()">{{tr}}Cancel{{/tr}}</button>
        </td>
      </tr>
    </table>
  </form>
...
<form name="patientSearch" method="get" onsubmit="return Url.update(this, 'patients-list')">
  ...
  <table class="form me-box-shadow-table">
    ...
    <tr>
      ...
      <td class="button">
        <button class="search me-primary" id="pat_selector_search_pat_button" type="submit">{{tr}}Search{{/tr}}</button>
        <button class="erase me-tertiary" type="reset" title="{{tr}}Empty{{/tr}}">{{tr}}Empty{{/tr}}</button>
      </td>
      <td class="button">
        {{if $can->edit}}
