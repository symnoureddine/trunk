        ...
        <table class="form">
          ...
          <tr>
            {{me_form_field mb_object=$releve mb_field=qte_observee nb_cells=2}}
              {{mb_field object=$releve field=qte_observee form=$form_name increment=true onchange="Redon.updateDiff(this, `$redon->_id`);"}} ml
            {{/me_form_field}}

            {{me_form_field mb_object=$releve mb_field=_qte_diff nb_cells=2}}
              {{mb_field object=$releve field=_qte_diff readonly=true}} ml
            {{/me_form_field}}
          </tr>
      ...
      <table class="form">
        ...
        <tr>
          {{me_form_field mb_object=$last_releve mb_field=qte_observee nb_cells=2 class="halfPane"}}
            <span id="qte_observee_{{$redon->_id}}" data-vidange_apres_observation="{{$last_releve->vidange_apres_observation}}">{{mb_value object=$last_releve field=qte_observee}}</span> ml
          {{/me_form_field}}
        </tr>
        <tr>
          {{me_form_field mb_object=$redon mb_field=_qte_cumul nb_cells=2}}
            {{mb_value object=$redon field=_qte_cumul}} ml
