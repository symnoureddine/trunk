...
<script>
  ...
  sortBy = function (order_col, order_way) {
    ...
    new Url("patients", "ajax_all_docs")
      ...
      .requestUpdate("area_docs_{{$unique_all_docs}}");
  };

  loadAllDocs = function () {
    window.all_docs_guids.each(function (all_docs_guid) {
      window["loadAllDocs" + all_docs_guid]();
    });
  };

  loadAllDocs{{$unique_all_docs}} = function () {
    var form = getForm("filterDisplay{{$unique_all_docs}}");
    ...
    <tr>
      <th class="title" style="vertical-align: middle;" colspan="5">
        {{me_form_field field_class="me-form-icon search me-form-group-inline me-valign-middle"}}
          <input type="text" style="float: left;" class="search" onkeyup="filterResults{{$unique_all_docs}}(this.value)" />
        {{/me_form_field}}
        ...
        <span style="float: right;">
          ...
          {{if $explode_object.0 === "CConsultation"}}
            <button type="button" class="mail me-tertiary" onclick="Document.sendDocuments('{{$explode_object.0}}', '{{$explode_object.1}}');">
              {{tr}}CDocumentItem-action-send{{/tr}}
            </button>
            {{if 'apicrypt'|module_active}}
              {{mb_script module=apicrypt script=Apicrypt ajax=true}}
              <button type="button" class="mail-apicrypt me-tertiary" onclick="Apicrypt.sendDocuments('{{$context_guid}}');">{{tr}}CCompteRendu.send_mail_apicrypt{{/tr}}</button>
            {{/if}}
            {{if 'mssante'|module_active}}
              {{mb_script module=mssante script=MSSante ajax=true}}
              <button type="button" class="mail-mssante me-tertiary" onclick="MSSante.viewSendDocuments('{{$context_guid}}');">{{tr}}CCompteRendu.send_mail_mssante{{/tr}}</button>
            {{/if}}
          {{/if}}
        ...
        {{me_form_field field_class="me-form-group-inline me-valign-middle"}}
          <select name="tri" onchange="loadAllDocs{{$unique_all_docs}}()">
            <option value="date" {{if $tri === "date"}} selected{{/if}}>{{tr}}CPatient-status.date{{/tr}}     </option>
            {{if preg_match("/^CPatient/", $context_guid)}}
            <option value="context" {{if $tri === "context"}}selected{{/if}}>{{tr}}CPatient-status.context{{/tr}} </option>
            {{/if}})
            <option value="cat" {{if $tri === "cat"}}selected{{/if}}>{{tr}}CPatient-status.category{{/tr}}</option>
          </select>
        {{/me_form_field}}
        ...
        <label style="font-family: FontAwesome; font-size: 13pt; font-weight: normal; margin-left: 10px; margin-right: 5px;"
          ...
          <input type="radio" name="display" value="icon" {{if $display === "icon"}}checked{{/if}}
                 onclick="toggleLabels{{$unique_all_docs}}(); loadAllDocs{{$unique_all_docs}}()"
    ...
    <tr>
      <td colspan="2">
        <label class="me-margin-right-8">
          <input type="radio" name="importance" value="high" {{if $vue_globale_importance === "high"}}checked{{/if}}
                 onchange="savePref('vue_globale_importance', this.value, loadAllDocs)" />
          Importants
        </label>
        <label class="me-margin-right-8">
          <input type="radio" name="importance" value="medical" {{if $vue_globale_importance === "medical"}}checked{{/if}}
                 onchange="savePref('vue_globale_importance', this.value, loadAllDocs)" />
          Médicaux
        </label>
        <label>
      ...
      <td>
        <button type="button" class="user notext" onclick="toggleFilterOwner(this, this.form.user_id, User.id);"></button>
        <input type="hidden" name="user_id" />
      </td>
      <td class="narrow">
        <button class="hslip not-printable me-tertiary me-dark" style="float: right;" data-show="" onclick="$$('.doc_canceled').invoke('toggle');">
