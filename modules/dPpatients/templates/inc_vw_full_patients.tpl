...
<form name="fusion" method="get" onsubmit="return false;">
  ...
  <table class="tbl me-table-patient-event" style="vertical-align: middle;">
    ...
    {{mb_include module=dPpatients template=inc_vw_patient_events th_colspan=5 compact=true show_actions=false show_merge_chkbx=true show_files_btn=true}}
...
{{if "doctolib"|module_active && "doctolib staple_authentification client_access_key_id"|gconf}}
  <table class="tbl">
    ...
    <tr>
      <td class="button">
        {{mb_include module=doctolib template=buttons/inc_book patient_id=$patient->_id}}
        {{mb_include module=doctolib template=buttons/inc_patient_historic patient_id=$patient->_id}}
      </td>
    </tr>
  </table>
{{/if}}
