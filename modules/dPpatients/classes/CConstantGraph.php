...
class CConstantGraph implements IShortNameAutoloadable
{
     ...
     * @return void
     */
    public function formatGraphDatas(array $constants, array $constants_by_graph = []): void
    {
        $datas = [];
        ...
        $last_unit_glycemie = CConstantesMedicales::getlastUnitGlycemie($constants, true);
        ...
        foreach ($this->structure as $_rank => $_graphs) {
            foreach ($_graphs as $_graph_constants) {
                ...
                foreach ($_graph_constants as $_constant) {
                    ...
                    if ($last_unit_glycemie && $_constant == "glycemie") {
                        $label .= " ($last_unit_glycemie)";
                    }
                    ...
                    $series = $this->addSerie($series, $values, $_constant, $constant_type, $axis_id, $label, $last_unit_glycemie);
            ...
            if (!empty($constant_values)) {
                ...
                foreach ($periods as $_period) {
                    ...
                    $period_datas = [
                        $x,
                        $_period['value'],
                        'date'     => strftime('%d/%m/%y %H:%M', $_period['start_time']) . ' au ' . strftime(
                            '%d/%m/%y %H:%M',
                            $_period['end_time']
                        ),
                        'barWidth' => $bar_width,
