...
class CSetupPatients extends CSetup {
   ...
   * @return void
   */
  protected function checkOldCommunes() {
   ...
   * @return void
   */
  protected function checkNewCommunes() {
    $query = "SELECT * 
                  FROM `communes_france_new`, `communes_cp` 
                  WHERE `code_postal` = 71000 AND `commune` = 'MACON' AND `commune_id` = `commune_france_id`;";
    $this->addDatasource("INSEE", $query);

    $query = "SELECT * 
                  FROM `communes_france_new`, `communes_cp` 
                  WHERE `code_postal` = 01300 AND `commune` = 'MASSIGNIEU-DE-RIVES' AND `commune_id` = `commune_france_id`;";
    $this->addDatasource("INSEE", $query);

    $query = "SELECT * 
                  FROM `communes_france_new`, `communes_cp` 
                  WHERE `code_postal` = 50130 AND `commune` = 'CHERBOURG-OCTEVILLE' 
                  AND `commune_id` = `commune_france_id`;";
    $this->addDatasource("INSEE", $query);
  }
  ...
  function __construct() {
    ...
    $this->addQuery(CSetupCompteRendu::renameTemplateFieldQuery("Patient - antécédents", "Patient - Antécédents -- tous"));
    $this->addQuery(CSetupCompteRendu::renameTemplateFieldQuery("Patient - traitements", "Patient - Traitements"));
    $this->addQuery(CSetupCompteRendu::renameTemplateFieldQuery("Patient - addictions", "Patient - Addictions -- toutes"));
    $this->addQuery(CSetupCompteRendu::renameTemplateFieldQuery("Patient - diagnostics", "Patient - Diagnotics"));

    $this->addQuery(CSetupCompteRendu::renameTemplateFieldQuery("Sejour - antécédents", "Sejour - Antécédents -- tous"));
    $this->addQuery(CSetupCompteRendu::renameTemplateFieldQuery("Sejour - traitements", "Sejour - Traitements"));
    $this->addQuery(CSetupCompteRendu::renameTemplateFieldQuery("Sejour - addictions", "Sejour - Addictions -- toutes"));
...
ALTER TABLE `releve_comment` 
   ...
   $this->makeRevision('4.47');
    ...
    if ($dsn instanceof CSQLDataSource && array_key_exists('INSEE', CAppUI::conf('db'))) {
      $table = ($dsn->hasTable('communes_france_new') && $dsn->countRows('SELECT * FROM `communes_france_new`') > 0)
        ? 'communes_france_new' : 'communes_france';
      if ($dsn->fetchRow($dsn->exec("SHOW TABLES LIKE '$table';"))) {
        if ($table === 'communes_france_new') {
          $this->checkNewCommunes();
        }
        else {
          $this->checkOldCommunes();
        }
      }
    }
  }
