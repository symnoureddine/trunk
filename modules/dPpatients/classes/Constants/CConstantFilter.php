...
class CConstantFilter
{
    ...
    public function getResults(): array
    {
        ...
        foreach ($objects as $object) {
            ...
            $results[$object->_class] = $object->loadList($where, $order, $limit, $group, $lj);
        }
    ...
    protected function makeLeftJoin(CAbstractConstant $constant): ?array
    {
        ...
        $constant_primary_id = $constant->getPrimaryKey();

        $lj = [
            "$releve_table.$releve_primary_id = $constant_table.$constant_primary_id",
    ...
    {
        $orders = [];
        foreach ($this->orders as $prefix => $order) {
     ...
     * @return array|null
     */
    protected function makeGroup(): ?array
    {
     ...
     * @return self
     */
    public function addGroupClause($groups): self
    {
        if (is_array(!$groups)) {
            $groups = [$groups];
        }
    ...
    public function countResultsDetails(): array
    {
        ...
        foreach ($objects as $object) {
            ...
            $counts[$object->_class] = $object->countList($where, $group, $lj);
        }
