...
class CDossierMedical extends CMbObject {
  /** @var string  */
  const RESOURCE_NAME = 'dossierMedical';
  ...
  function merge($objects = array(), $fast = false) {
    ...
    if (count($prescriptions) > 1) {
      ...
      return $prescription->merge($prescriptions, $fast);
    }
  ...
  function loadRefsAntecedents(
    ...
    $this->_ref_antecedents_by_type_appareil = array_fill_keys(
      ...
      array_fill_keys($atcd->_specs["appareil"]->_list, array())
    );
    foreach ($this->_all_antecedents as $_atcd) {
      if ($_atcd->absence == 0) {
   ...
   */
  function loadRefsEvenementsPatient($where = array()) {
    $order = "date DESC";

    return $this->_ref_evenements_patient = $this->loadBackRefs("evenements_patient", $order, null, null, null, null, "", $where);
  }
  ...
  function fillTemplate(&$template, $champ = "Patient") {
    ...
    if (is_array($this->_ref_traitements)) {
      ...
      if ($prescription && $prescription->_id) {
        ...
        foreach ($prescription->_ref_prescription_lines as $_line) {
          ...
          if ($_line->debut && (!$_line->fin || $_line->fin >= CMbDT::date())) {
            if ($_line->fin) {
              $duree = " (" . CAppUI::tr('common-From %s to %s', $_line->getFormattedValue("debut"), $_line->getFormattedValue("fin")) . ")";
            }
            else {
      ...
      $template->addListProperty("$champ - $traitements_subItem", $list);
    }
    else {
      $template->addProperty(
        "$champ - $traitements_subItem",
        $this->absence_traitement ? $this->getLocale("absence_traitement") : ""
      );
    }
    $template->addProperty(
      "$champ - $traitements_subItem - " . CAppUI::tr('CDossierMedical-absence_traitement'),
      $this->absence_traitement ? $this->getLocale("absence_traitement") : ""
    );
  ...
  public function getValuesListForAntecedents($champ, &$template, $see_absence = false, $same_func = false) {
    ...
    foreach ($this->_all_antecedents as $_antecedent) {
      ...
      if (!array_key_exists($_antecedent->type, $lists_par_type_appareil)) {
        $lists_par_type_appareil[$_antecedent->type] = array();
      }
      if (!array_key_exists($_antecedent->appareil, $lists_par_type_appareil[$_antecedent->type])) {
        $lists_par_type_appareil[$_antecedent->type][$_antecedent->appareil] = array();
      }
    ...
    $template->addProperty("$champ - $atcd_section - " . CAppUI::tr('CAntecedent-all (without CIM code)|pl'), implode($separator, $parts), null, false);
