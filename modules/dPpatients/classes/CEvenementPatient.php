 ...
 */
class CEvenementPatient extends CFacturable implements ImportableInterface {
  // DB Table key
  ...
  {
      return ($this->_id ? $this->loadRefPraticien()->getPerm($permType) : null) && parent::getPerm($permType);
  }
  ...
  function fillLimitedTemplate(&$template) {
    ...
    $template->addLongDateProperty("Evenement - Date longue", $this->date);
    $template->addDateProperty("Evenement - Date", $this->date);
  ...
  function fillTemplate(&$template) {
    ...
    if (CModule::getActive('oxCabinet')) {
        (new TAMMSIHFields($this->parent_id ? $this->loadRefParent() : $this))->fillFields($template);
    }
  }
   ...
   */
  public function getExecutantId($code_activite = null) {
  ...
  function loadRefsUsers($sort = true) {
    ...
    if ($sort) {
      array_multisort(CMbArray::pluck($this->_ref_users, "_view"), SORT_ASC, $this->_ref_users);
    }
