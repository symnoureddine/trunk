...
use Ox\Interop\Mvsante\CMVSante;
 ...
 * The CPatient Class
 */
class CPatient extends CPerson implements IGeocodable, ImportableInterface {
    /** @var string */
  ...
  private static $fields_etiq = array(
    "DATE NAISS", "AGE", "IPP", "LIEU NAISSANCE", "PAYS_NAISSANCE",
    "NOM", "NOM JF", "FORMULE NOM JF", "PRENOM", "SEXE",
    "CIVILITE", "CIVILITE LONGUE",
    "ACCORD GENRE", "CODE BARRE IPP",
    "ADRESSE", "ADRESSE SEULE", "CODE POSTAL", "VILLE", "MED. TRAITANT",
  ...
  static function massLoadRefsAffectations($patients, $date = null) {
    ...
    $sejours_ids_by_patients_ids = array();
    $sejours_ids                 = array();
   ...
   * Return an array with the id of the object for key and new for value if the object have already changed his patient_field value
   *
  ...
  static function checkBackRefsOwner($back_name, $patient) {
    ...
    $where       = array(
      ...
      'date'         => $ds->prepare('BETWEEN ?1 AND ?2', CMbDT::dateTime("-10 MIN", $date), CMbDT::dateTime("+10 MIN", $date)),
  ...
  public static function transformRelativesPatient($relatives_array) {
    $patients = array("bros" => null, "children" => null, "parent_1" => null, "parent_2" => null);

    if (isset($relatives_array["bros"])) {
      foreach ($relatives_array["bros"] as $_bro) {
        $patients["bros"][] = array(CPatient::findOrFail($_bro[0]), $_bro[1]);
      }
    }
    if (isset($relatives_array["children"])) {
      foreach ($relatives_array["children"] as $_child) {
        $patients["children"][] = array(CPatient::findOrFail($_child[0]), $_child[1]);
      }
    }
    if (isset($relatives_array["parent_1"]) && $relatives_array["parent_1"][0]) {
      $patients["parent_1"] = array(CPatient::findOrFail($relatives_array["parent_1"][0]), $relatives_array["parent_1"][1]);
    }
    if (isset($relatives_array["parent_2"]) && $relatives_array["parent_2"][0]) {
      $patients["parent_2"] = array(CPatient::findOrFail($relatives_array["parent_2"][0]), $relatives_array["parent_2"][1]);
    }
  ...
  public static function countBloodRelatives($array) {
    ...
    $total += (isset($array["parent_1"]) && $array["parent_1"][0]) ? 1 : 0;
    $total += (isset($array["parent_2"]) && $array["parent_2"][0]) ? 1 : 0;
  ...
  public function getApiLink(): string {
    return CRouter::getInstance()
      ->generate('dossierpatient_patients',["patient_id" => $this->_id]);
  }
  ...
  function getProps() {
    ...
    $props['source_identite_id']       = 'ref class|CSourceIdentite back|patients';
    $props["nom"]                      = "str notNull confidential seekable|begin index fieldset|default";
    ...
    $props["sexe"]                     = "enum list|m|f fieldset|default";
    $props["civilite"]                 = "enum list|m|mme|mlle|enf|dr|pr|me|vve fieldset|default";
    $addr_mandatory                    = CAppUI::gconf("dPpatients CPatient addr_patient_mandatory") ? " notNull" : "";
    ...
    $cp_mandatory                      = CAppUI::gconf("dPpatients CPatient cp_patient_mandatory") ? " notNull" : "";
    ...
    $props['phone_area_code']       = 'numchar length|2 default|' . CAppUI::conf('system phone_area_code') . ' fieldset|contact';
    ...
    $props["cmu"]                           = "bool fieldset|assurance";
    ...
    $props["allow_email"]          = "bool default|1 fieldset|contact";
    ...
    $props["_age"]              = "str";
    $props["_vip"]              = "bool";
    $props["_prenom_2"]         = "str fieldset|default show|1";
    $props["_prenom_3"]         = "str fieldset|default show|1";
    $props["_prenom_4"]         = "str fieldset|default show|1";
    ...
    $props['_type_justificatif'] = 'enum list||passeport|carte_identite|acte_naissance|livret_famille|carte_sejour|doc_asile|carte_identite_electronique';
    ...
    $props['_assurance_assure_id'] = 'str';
    $props['_invalid_assurance']   = 'bool default|0';
    ...
    $props['_source_nom']              = 'str notNull confidential fieldset|default';
    $props['_source_prenom']           = 'str notNull confidential fieldset|default';
    $props['_source_prenom_usuel']     = 'str';
    $props['_source_prenoms']          = 'str fieldset|default';
    $props['_source_nom_jeune_fille']  = 'str confidential fieldset|default';
    $props['_source_sexe']             = 'enum list|m|f fieldset|default';
    $props['_source_civilite']         = 'enum list|m|mme|mlle|enf|dr|pr|me|vve fieldset|default';
    $props['_source_naissance']        = 'birthDate notNull fieldset|default';
    $props['_source_naissance_corrigee'] = 'bool';
    $props['_source_lieu_naissance']   = 'str fieldset|extra';
    $props['_source_commune_naissance_insee'] = 'str length|5 fieldset|extra';
    $props['_source_cp_naissance']     = "str minLength|$min_cp maxLength|$max_cp confidential fieldset|extra";
    $props['_source__pays_naissance_insee'] = 'str';
    $props['_source__date_fin_validite'] = 'date';
    $props['_map_source_form_fields']  = 'bool';
    $props['_force_manual_source']     = 'bool';
      ...
      foreach ($objects as $object) {
        ...
        if (CModule::getActive('appFineClient')) {
            ...
            $status_patient_users = $status_patient_user->loadList(
                array('patient_id' => CSQLDataSource::prepareIn(CMbArray::pluck($merged_objects, 'patient_id')))
            );
  ...
  function store() {
    ...
    foreach (CSourceIdentite::TRAITS_STRICTS_REFERENCE as $_trait) {
        ...
        // Ne pas autoriser la modification des traits stricts en dehors du formulaire d'ajout de justificatif
        if ($this->_id && !$this->_source_nom && in_array($this->status, ['VALI', 'QUAL'])) {
    ...
    $this->completeField('prenoms');
    if (!$this->prenoms) {
        $this->prenoms = $this->prenom;
    }

    if ($this->_prenom_2 || $this->_prenom_3 || $this->_prenom_4) {
      $this->prenoms = $this->prenom . ' ' . trim(implode(' ', [$this->_prenom_2, $this->_prenom_3, $this->_prenom_4]));
    }

    if ($this->fieldModified('source_identite_id')) {
    ...
    if ($msg = CConsentPatient::storeConsent($this)) {
      return $msg;
    }

    if ($msg = CSourceIdentite::manageSource($this)) {
        return $msg;
    }
    ...
    if ($this->_id) {
      CApp::doProbably(100, array($this, 'checkBmrBhreStatus'));
    }
  ...
  function checkAnonymous() {
    return (CMbString::upper($this->nom) === "ANONYME" || is_numeric($this->nom)) && CMbString::upper($this->prenom) === "ANONYME";
  }
  ...
  function check() {
    ...
    if ($this->status === "VALI" && !$this->fieldModified("status") && !CAppUI::pref("allowed_modify_identity_status")) {
      if ($this->fieldModified("nom") || $this->fieldModified("prenom")
        || $this->fieldModified("naissance") || $this->fieldModified("nom_jeune_fille")
      ) {
        return "CPatient-msg-You do not have the rights to modify the strict features of the patient";
      }
    }
   ...
   * @return null|String[]
   */
    public function getWhereDoubloon($patient, $other = false, $additionnal_fields = array(), $trim = false, $group_id = null) {
        $ds    = $patient->_spec->ds;
        $where = array();
  ...
  function updateFormFields() {
    ...
    $this->_prenoms       = array($this->prenom, $this->_prenom_2, $this->_prenom_3, $this->_prenom_4);
    ...
    $group           = CGroups::loadCurrent();
    $nom_naissance   = $this->nom_jeune_fille && ($this->nom_jeune_fille != $this->nom || CAppUI::conf("dPpatients CPatient nom_jeune_fille_always_present", $group)) ? " ($this->nom_jeune_fille)" : "";
    ...
    elseif (CAppUI::isGroup() && $this->group_id && CMediusers::get()->loadRefFunction()->group_id != $this->group_id) {
    ...
    $first_name_first_particule = ($first_name_first_particule) ? reset($first_name_first_particule) : $this->prenom;
    $this->_shortview           = CMbString::makeInitials("{$first_name_first_particule} {$last_name_last_particule}");
  ...
  static function applyModeIdentitoVigilance($string, $firstname = false, $mode_identito_vigilance = null, $anonyme = false) {
    $mode = $mode_identito_vigilance ?
      $mode_identito_vigilance : CAppUI::gconf("dPpatients CPatient mode_identito_vigilance");
  ...
  function loadRefsPraticiens() {
    ...
    $prat                  = new CMediusers();
    $this->_ref_praticiens = $prat->loadList(array("user_id" => CSQLDataSource::prepareIn($praticiens_ids)));
  ...
  function getRestAge(DateTimeImmutable $to = null) {
    ...
    if ($this->naissance) {
      ...
      $rest_months = ($achieved["year"] > 2) ? $diff->m : $achieved["month"];
      // Don't transform days to week until the 3rd week / 0.14: days to weeks
      $rest_weeks = ($achieved["day"] > 30) ? floor((int)$diff->d * 0.142857) : 0;
      $rest_days  = (int)$diff->d - $rest_weeks * 7;
    }
    ...
    if ($achieved["year"] < 15) {
      if ($achieved["month"] > 0 && $rest_months > 0) {
        $locale .= ($locale) ? ", " : "";
        $locale .= $rest_months . " ";
        $locale .= ($rest_months > 1) ? CAppUI::tr("months") : CAppUI::tr("month");
      }
      if ($achieved["year"] < 2 && $achieved["day"] > 30 && $rest_weeks > 0) {
        $locale .= ", ";
        $locale .= $rest_weeks . " ";
        $locale .= ($rest_weeks > 1) ? CAppUI::tr("weeks") : CAppUI::tr("week");
      }
      if ($achieved["year"] < 2 && $achieved["month"] == 0 && $achieved["week"] < 2 && $rest_days > 0) {
        $locale .= $rest_days . " ";
        $locale .= ($rest_days > 1) ? CAppUI::tr("days") : CAppUI::tr("day");
      }
    }

    return ["rest_months" => (int)$rest_months, "rest_weeks" => (int)$rest_weeks, "rest_days" => (int)$rest_days, "locale" => $locale];
  }
  ...
  function updatePlainFields() {
    ...
    if ($this->prenoms) {
        ...
        foreach ($prenoms as $_key => $_prenom) {
            $prenoms[$_key] = self::applyModeIdentitoVigilance($_prenom, true, null, $anonyme);
        }
   ...
   */
  function loadRefsSejours($where = array(), $limit = null) {
    if (!$this->_id) {
   ...
   * @return array;
   */
  function getNextSejourAndOperation($date = null, $withOperation = true, $consult_id = null, $with_consult_anesth = true) {
    $sejour = new CSejour();
    $op     = new COperation();
    if (!$date) {
      $date = CMbDT::date();
   ...
   *
   * @return array Sejour ID => array("entree_prevue" => DATE, "sortie_prevue" => DATE, "datetime_entree" => DATETIME, "datetime_sortie" => DATETIME)
   */
  ...
  function getPhoning($date = null) {
    ...
    elseif (CAppUI::isGroup()) {
      $group_id                   = CMediusers::get()->loadRefFunction()->group_id;
      $where["patients.group_id"] = "= '$group_id'";
    }
   ...
   */
  function loadRefsDossierTiers($where = array(), $backNameAlt = null) {
    return $this->_ref_dossiers_tiers = $this->loadBackRefs("dossiers_tiers", null, null, null, null, null, $backNameAlt, $where);
  }
   ...
   */
  function loadRefPatientUserAPI($where = array()) {
    return $this->_ref_patient_user_api = $this->loadUniqueBackRef(
      "patient_user_api", null, null, null, null, null, $where
    );
  }
  ...
  function loadRefsDocs($where = array()) {
    $docs_valid = parent::loadRefsDocs($where);
    if ($docs_valid) {
  ...
  function loadRefsConsultations($where = null) {
    ...
    if (!$curr_user->isAdmin() || $multi_group == "hidden" || count($where)) {
      ...
      if (!$curr_user->isAdmin()) {
        $where[] = "functions_mediboard.consults_partagees = '1' || functions_mediboard.function_id = '$curr_user->function_id'";
      }
    ...
    $this->_ref_consultations = $consultation->loadList($where, $order, null, null, $ljoin);
  ...
  function loadRefsAffectations($date = null) {
    ...
    $group   = CGroups::loadCurrent();
    $sejours = $this->loadBackIds("sejours", null, null, null, null, array("group_id" => "= '$group->_id'", "annule" => " = '0'"));
  ...
  function updateBMRBHReStatus() {
    $bmr_bhre = $this->loadRefBMRBHRe();

    if (!$bmr_bhre->_id
      /*|| (!$bmr_bhre->bmr && !$bmr_bhre->bhre && !$bmr_bhre->hospi_etranger && !$bmr_bhre->rapatriement_sanitaire
          && !$bmr_bhre->ancien_bhre && !$bmr_bhre->bhre_contact)*/
    ) {
      return;
    }

    $this->_bmr_bhre_status = array();

    if ($bmr_bhre->bmr === "1" || $bmr_bhre->bhre === "1") {
      $this->_bmr_bhre_status[$bmr_bhre->bmr ? "BMR+" : "BHReP"] = "red";
    }

    if ($bmr_bhre->hospi_etranger === "1" || $bmr_bhre->rapatriement_sanitaire === "1" || $bmr_bhre->ancien_bhre === "1") {
  ...
  function loadDossierComplet($permType = null, $hide_consult_sejour = true, $limit = null) {
    ...
    CStoredObject::massCountBackRefs($this->_ref_consultations, "files");
    CStoredObject::massCountBackRefs($this->_ref_consultations, "documents");
    ...
    foreach ($this->_ref_consultations as $consult) {
      ...
      if ($maternite_active && $consult->grossesse_id && isset($this->_ref_grossesses[$consult->grossesse_id])) {
        $result                                                                           = $this->_ref_grossesses[$consult->grossesse_id]->getAgeGestationnel($consult->_date);
    ...
    CStoredObject::massLoadBackRefs($this->_ref_sejours, "operations", "date DESC");
    CStoredObject::massCountBackRefs($this->_ref_sejours, "files");
    CStoredObject::massCountBackRefs($this->_ref_sejours, "documents");
    ...
    foreach ($this->_ref_sejours as $_sejour) {
      ...
      // RPU
      // Flog de chargement lite sinon écrasement de la consultation et du nombre d'items documentaires
      CRPU::$_load_lite = true;
      $rpu              = $_sejour->loadRefRPU();
      CRPU::$_load_lite = false;
      if ($rpu && $rpu->_id) {
  ...
  function loadRefPatientINSNIR() {
    return $this->_ref_patient_ins_nir = $this->loadUniqueBackRef("patient_ins_nir");
  }
   ...
   * @throws Exception
   */
  function loadRefsPatientUsers($where = array()) {
    return $this->_ref_patient_users = $this->loadBackRefs("patient_user", null, null, null, null, null, "", $where);
  }
  ...
  function fillLimitedTemplate(&$template, $champ = "Patient", $with_corresp = true) {
    ...
    $template->addProperty("$champ - " . CAppUI::tr('CPatient-long article (lowercase)'), strtolower($this->_civilite_long));
    $template->addProperty("$champ - " . CAppUI::tr('CPatient-last name'), $this->nom);
    $template->addProperty("$champ - " . CAppUI::tr('CPatient-birth name'), $this->nom_jeune_fille);
    $template->addProperty("$champ - " . CAppUI::tr('CPatient-first name'), $this->prenom);
    ...
    $template->addProperty("$champ - " . CAppUI::tr('CPatient-short sex'), substr(strtolower($this->getFormattedValue("sexe")), 0, 1));
    $template->addProperty("$champ - " . CAppUI::tr('CPatient-insured number'), $this->getFormattedValue("matricule"));
    $template->addProperty("$champ - " . CAppUI::tr('CPatient-phone'), $this->getFormattedValue("tel"));
    $template->addProperty("$champ - " . CAppUI::tr('CPatient-professional phone'), $this->getFormattedValue("tel_pro"));
    ...
    if (CAppUI::conf("ref_pays") == 2) {
      $template->addProperty("$champ - " . CAppUI::tr('CPatient-AVS'), $this->getFormattedValue("avs"));
    }
    ...
    $template->addProperty("$champ - " . CAppUI::tr('CPatient-Social Security number'), $this->getFormattedValue("matricule"));
    ...
    $template->addBarcode("$champ - " . CAppUI::tr('CPatient-INS barcode'), $this->_ref_last_ins ? $this->_ref_last_ins->ins : "");
    ...
    $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CMedecin-Last name First name'), $medecin_nom_prenom);
    $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CMedecin-name'), $medecin->nom);
    $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CMedecin-first name'), $medecin->prenom);
    $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CMedecin-address'), "$medecin->adresse \n $medecin->cp $medecin->ville");
    $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CMedecin-way'), $medecin->adresse);
    $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CMedecin-ZIP code-court'), $medecin->cp);
    $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CMedecin-city'), $medecin->ville);
    $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CMedecin-brotherhood'), $medecin->_confraternite);
    $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CMedecin-phone-court'), $medecin->getFormattedValue("tel"));
    $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CMedecin-fax'), $medecin->getFormattedValue("fax"));
    $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CMedecin-email'), $medecin->email);
    $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CMedecin-email_apicrypt'), $medecin->email_apicrypt);
    $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CMedecin-mssante_address'), $medecin->mssante_address);

    if ($medecin->sexe == "f") {
      $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CPatient-gender agreement'), "e");
      $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CPatient-long article'), "Mme le docteur");
    }
    elseif ($medecin->sexe == "m") {
      $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CPatient-gender agreement'), "");
      $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CPatient-long article'), "Mr le docteur");
    }
    else {
      $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CPatient-gender agreement'), "");
      $template->addProperty("$champ - $med_traitant_subItem - " . CAppUI::tr('CPatient-long article'), "le docteur");
    }
    ...
    foreach ($correspondants as $relation => $_correspondants) {
      ...
      switch ($relation) {
        case "employeur":
          $template->addProperty("$champ - $employeur_subItem - " . CAppUI::tr('common-name'), $_correspondant->nom);
          $template->addProperty("$champ - $employeur_subItem - " . CAppUI::tr('CCorrespondantPatient-address'), $_correspondant->adresse);
          $template->addProperty("$champ - $employeur_subItem - " . CAppUI::tr('CCorrespondantPatient-ZIP code-court'), $_correspondant->cp);
          $template->addProperty("$champ - $employeur_subItem - " . CAppUI::tr('CCorrespondantPatient-city'), $_correspondant->ville);
          $template->addProperty("$champ - $employeur_subItem - " . CAppUI::tr('CCorrespondantPatient-phone-court'), $_correspondant->getFormattedValue("tel"));
          $template->addProperty("$champ - $employeur_subItem - " . CAppUI::tr('CCorrespondantPatient-mobile phone'), $_correspondant->getFormattedValue("mob"));
          $template->addProperty("$champ - $employeur_subItem - " . CAppUI::tr('CCorrespondantPatient-foreign phone-court'), $_correspondant->getFormattedValue("tel_autre"));
          $template->addProperty("$champ - $employeur_subItem - " . CAppUI::tr('CCorrespondantPatient-urssaf number'), $_correspondant->urssaf);
          $template->addProperty("$champ - $employeur_subItem - " . CAppUI::tr('CCorrespondantPatient-e-mail'), $_correspondant->email);
          if (CAppUI::conf("ref_pays") == 2) {
            $template->addProperty("$champ - $employeur_subItem - " . CAppUI::tr('CCorrespondantPatient-ean'), $_correspondant->getFormattedValue("ean"));
          }
          break;
        case "prevenir":
        case "assurance":
          ...
          for ($i = 1; $i < 4; $i++) {
            ...
            if ($section === "prévenir") {
              $template->addProperty("$champ - $section$key- " . CAppUI::tr('common-name'), $_correspondant->nom);
              $template->addProperty("$champ - $section$key- " . CAppUI::tr('CPatient-first name'), $_correspondant->prenom);
              $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-address'), $_correspondant->adresse);
              $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-ZIP code-court'), $_correspondant->cp);
              $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-city'), $_correspondant->ville);
              $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-phone-court'), $_correspondant->getFormattedValue("tel"));
              $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-mobile phone'), $_correspondant->getFormattedValue("mob"));
              $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-foreign phone-court'), $_correspondant->getFormattedValue("tel_autre"));
              $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-relationship'), $_correspondant->parente);
              $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-e-mail'), $_correspondant->email);
            }

            if ($section == "assurance") {
              $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-begin'), $_correspondant->getFormattedValue("date_debut"));
              $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-end'), $_correspondant->getFormattedValue("date_fin"));
              if (CAppUI::conf("ref_pays") == 2) {
                $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-ean'), $_correspondant->getFormattedValue("ean"));
                $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-ean_base'), $_correspondant->getFormattedValue("ean_base"));
                $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-type of support'), $_correspondant->getFormattedValue("type_pec"));
                $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-insurance number'), $_correspondant->getFormattedValue("ean_id"));
                $template->addProperty("$champ - $section$key- " . CAppUI::tr('CCorrespondantPatient-identifier of the insured'), $_correspondant->getFormattedValue("assure_id"));
              }
            }
          }
          break;
        ...
        case "representant_th":
          ...
          $name_relation = $relation == "confiance" ? $relation : CAppUI::tr('CCorrespondantPatient.relation.representant_th');
          $template->addProperty("$champ - $name_relation - " . CAppUI::tr('common-name'), $_correspondant->nom);
          $template->addProperty("$champ - $name_relation - " . CAppUI::tr('CCorrespondantPatient-maiden name'), $_correspondant->nom_jeune_fille);
          $template->addProperty("$champ - $name_relation - " . CAppUI::tr('CPatient-first name'), $_correspondant->prenom);
          $template->addProperty("$champ - $name_relation - " . CAppUI::tr('CCorrespondantPatient-birth date'), $_correspondant->getFormattedValue("naissance"));
          $template->addProperty("$champ - $name_relation - " . CAppUI::tr('CCorrespondantPatient-address'), $_correspondant->adresse);
          $template->addProperty("$champ - $name_relation - " . CAppUI::tr('CCorrespondantPatient-ZIP code-court'), $_correspondant->cp);
          $template->addProperty("$champ - $name_relation - " . CAppUI::tr('CCorrespondantPatient-city'), $_correspondant->ville);
          $template->addProperty("$champ - $name_relation - " . CAppUI::tr('CCorrespondantPatient-phone-court'), $_correspondant->getFormattedValue("tel"));
          $template->addProperty("$champ - $name_relation - " . CAppUI::tr('CCorrespondantPatient-mobile phone'), $_correspondant->getFormattedValue("mob"));
          $template->addProperty("$champ - $name_relation - " . CAppUI::tr('CCorrespondantPatient-foreign phone-court'), $_correspondant->getFormattedValue("tel_autre"));
          $template->addProperty("$champ - $name_relation - " . CAppUI::tr('CCorrespondantPatient-relationship'), $_correspondant->parente);
          $template->addProperty("$champ - $name_relation - " . CAppUI::tr('CCorrespondantPatient-e-mail'), $_correspondant->email);
      }
    }
    ...
    $fields = array(CAppUI::tr('CMedecin-Last name First name'), CAppUI::tr('CMedecin-address'), CAppUI::tr('CMedecin-way'),
      CAppUI::tr('CMedecin-ZIP code-court'), CAppUI::tr('CMedecin-city'), CAppUI::tr('CMedecin-brotherhood'),
      CAppUI::tr('CMedecin-speciality'), CAppUI::tr('CMedecin-phone-court'), CAppUI::tr('CMedecin-fax'), CAppUI::tr('CCorrespondantPatient-foreign phone-court'),
      CAppUI::tr('CCorrespondantPatient-e-mail'), CAppUI::tr('CMedecin-type praticioner-court'), CAppUI::tr('CPatient-dear'), CAppUI::tr('CPatient-Dear (uppercase)'),
      CAppUI::tr('CMedecin-email_apicrypt'), CAppUI::tr('CMedecin-mssante_address'),
    );
    ...
    foreach ($this->_ref_medecins_correspondants as $corresp) {
      ...
      if ($medecin->sexe == "m" || $medecin->sexe == "u") {
        ...
        $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CPatient-Dear (uppercase)'), "Cher");
      }
      elseif ($medecin->sexe == "f") {
        $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CPatient-dear'), "chère");
        $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CPatient-Dear (uppercase)'), "Chère");
      }

      $template->addProperty("$champ - $med_corresp_subItem $i", $nom);
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CMedecin-Last name First name'), "$medecin->nom $medecin->prenom");
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CMedecin-address'), "{$medecin->adresse}\n{$medecin->cp} {$medecin->ville}");
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CMedecin-way'), $medecin->adresse);
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CMedecin-ZIP code-court'), $medecin->cp);
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CMedecin-city'), $medecin->ville);
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CMedecin-brotherhood'), $medecin->_confraternite);
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CMedecin-speciality'), $medecin->disciplines);
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CMedecin-phone-court'), $medecin->getFormattedValue("tel"));
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CMedecin-fax'), $medecin->getFormattedValue("fax"));
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CCorrespondantPatient-foreign phone-court'), $medecin->getFormattedValue("tel_autre"));
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CCorrespondantPatient-e-mail'), $medecin->email);
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CMedecin-email_apicrypt'), $medecin->email_apicrypt);
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CMedecin-mssante_address'), $medecin->mssante_address);
      $template->addProperty("$champ - $med_corresp_subItem $i - " . CAppUI::tr('CMedecin-type praticioner-court'), $medecin->getFormattedValue("type"));

      if ($with_corresp) {
        $specialite = strtolower(trim($medecin->_ref_spec_cpam->text));
        $specialite = str_replace("-", " ", $specialite);

        if ($medecin->sexe === "m" || $medecin->sexe == "u") {
          $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CPatient-dear'), "cher");
          $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CPatient-Dear (uppercase)'), "Cher");
        }
        elseif ($medecin->sexe == "f") {
          $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CPatient-dear'), "chère");
          $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CPatient-Dear (uppercase)'), "Chère");
        }

        $template->addProperty("$medCorresp_subItem - $specialite", $nom);
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CMedecin-Last name First name'), "$medecin->nom $medecin->prenom");
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CMedecin-address'), "{$medecin->adresse}\n{$medecin->cp} {$medecin->ville}");
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CMedecin-way'), $medecin->adresse);
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CMedecin-ZIP code-court'), $medecin->cp);
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CMedecin-city'), $medecin->ville);
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CMedecin-brotherhood'), $medecin->_confraternite);
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CMedecin-speciality'), $medecin->disciplines);
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CMedecin-phone-court'), $medecin->getFormattedValue("tel"));
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CMedecin-fax'), $medecin->getFormattedValue("fax"));
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CCorrespondantPatient-foreign phone-court'), $medecin->getFormattedValue("tel_autre"));
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CCorrespondantPatient-e-mail'), $medecin->email);
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CMedecin-email_apicrypt'), $medecin->email_apicrypt);
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CMedecin-mssante_address'), $medecin->mssante_address);
        $template->addProperty("$medCorresp_subItem - $specialite - " . CAppUI::tr('CMedecin-type praticioner-court'), $medecin->getFormattedValue("type"));
      }
    }

    $template->addProperty("$champ - " . CAppUI::tr('CMedecin-corresponding doctor|pl'), implode(" - ", $noms));

    $template->addListProperty("$champ - " . CAppUI::tr('CMedecin-corresponding doctor (with address)|pl'), $noms_addresses, false);

    if (CAppUI::conf("ref_pays") == 2) {
      //Date de la première consultation du patient
      $ljoin                            = array();
      $ljoin["consultation"]            = "plageconsult.plageconsult_id = consultation.plageconsult_id";
      $where                            = array();
      $where["consultation.patient_id"] = " = '$this->_id'";
      $plage                            = new CPlageconsult();
      $plage->loadObject($where, "plageconsult.date ASC", "plageconsult_id", $ljoin);
      $template->addDateProperty("$champ - " . CAppUI::tr('CPatient-date of the first consultation'), $plage->date);
    }
    ...
    $template->addProperty("$champ - " . CAppUI::tr('CConstantesMedicales-constant temperature'), $const_med->temperature . "°");
    $template->addProperty("$champ - " . CAppUI::tr('CConstantesMedicales-ta-court'), ($const_med->ta ? "$const_med->_ta_systole / $const_med->_ta_diastole" : ""));
    ...
    if ($this->_id) {
      $last_cste->patient_id = $this->_id;
      $last_cste->loadMatchingObject("datetime DESC");

      // Ne pas utiliser le getLastConstantes qui va charger l'intégralité des constantes du patient... (dépassement mémoire)
      //$this->getLastConstantes();
    }

    $consts       = array($last_cste);
    ...
    CConstantesMedicales::addConstantesTemplate($template, $grid_complet, $grid_minimal, $grid_valued, $champ, CAppUI::tr('CConstantesMedicales-Constant (last statement)|pl'));
    ...
    $template->addImageProperty("$champ - " . CAppUI::tr('CMediusers-ID photo'), $identite->_id, array("title" => "$champ - " . CAppUI::tr('CMediusers-ID photo')));
    ...
    $template->addProperty("$champ - $ass_social_subItem - " . CAppUI::tr('CCorrespondantPatient-maiden name-court'), $this->assure_nom_jeune_fille);
    $template->addProperty("$champ - $ass_social_subItem - " . CAppUI::tr('CPatient-first name'), $this->assure_prenom);
    $template->addProperty("$champ - $ass_social_subItem - " . CAppUI::tr('CPatient-birth date'), $this->getFormattedValue("assure_naissance"));
    $template->addProperty("$champ - $ass_social_subItem - " . CAppUI::tr('CPatient-article'), $this->_assure_civilite);
    $template->addProperty("$champ - $ass_social_subItem - " . CAppUI::tr('CPatient-long article'), $this->_assure_civilite_long);
    $template->addProperty("$champ - $ass_social_subItem - " . CAppUI::tr('CPatient-address'), $this->assure_adresse);
    $template->addProperty("$champ - $ass_social_subItem - " . CAppUI::tr('CPatient-city'), $this->assure_ville);
    $template->addProperty("$champ - $ass_social_subItem - " . CAppUI::tr('CPatient-ZIP code-court'), $this->assure_cp);
    ...
    $template->addProperty("$champ - $ass_social_subItem - " . CAppUI::tr('CPatient-mobile phone'), $this->assure_tel2);
    $template->addProperty("$champ - $ass_social_subItem - " . CAppUI::tr('CPatient-Birth ZIP code-court'), $this->assure_cp_naissance);
    $template->addProperty("$champ - $ass_social_subItem - " . CAppUI::tr('CPatient-place of birth'), $this->assure_lieu_naissance);
    $template->addProperty("$champ - $ass_social_subItem - " . CAppUI::tr('CPatient-profession'), $this->assure_profession);

    // Bénéficiaire de soins
    $benef_soin_subItem = CAppUI::tr('CPatient-Beneficiary of care');
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-code regime'), $this->code_regime);
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-manager office-court'), $this->caisse_gest);
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-manager center-court'), $this->centre_gest);
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-manager code-court'), $this->code_gestion);
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-health diet'), $this->regime_sante);
    $template->addDateProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-early period'), $this->deb_amo);
    $template->addDateProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-end of period'), $this->fin_amo);
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-am diet'), $this->getFormattedValue("regime_am"));
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-long term affection-court'), $this->getFormattedValue("ald"));
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-incapable adult'), $this->getFormattedValue("incapable_majeur"));
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-universal health coverage-court'), $this->getFormattedValue("cmu"));
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-ATNC'), $this->getFormattedValue("ATNC"));
    $template->addDateProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-vitale validity'), $this->fin_validite_vitale);
    ...
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-mutual contract type|pl'), addslashes($this->mutuelle_types_contrat));
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-amo note|pl'), addslashes($this->notes_amo));
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-exo label'), addslashes($this->libelle_exo));
    $template->addProperty("$champ - $benef_soin_subItem - " . CAppUI::tr('CPatient-amc note|pl'), addslashes($this->notes_amc));

    if (CModule::getActive("maternite")) {
      $allaitement_subItem = CAppUI::tr('CAllaitement');
      $allaitement         = $this->loadLastAllaitement();
      $template->addDateProperty("$champ - $allaitement_subItem - " . CAppUI::tr('common-Start'), $allaitement->date_debut);
      ...
      if ($grossesse && $grossesse->_id) {
        $sa_reste_jour = $grossesse->_semaine_grossesse . " " . CAppUI::tr('CDepistageGrossesse-_sa') . " + " . $grossesse->_reste_semaine_grossesse . " J";
      }

      $template->addDateProperty("$champ - $grossesse_subItem - " . CAppUI::tr('CGrossesse-terme_prevu'), $grossesse->terme_prevu);
      $template->addDateProperty("$champ - $grossesse_subItem - " . CAppUI::tr('CGrossesse-Date of the last rule|pl'), $grossesse->date_dernieres_regles);
      $template->addProperty("$champ - $grossesse_subItem - " . CAppUI::tr('CGrossesse-active'), $grossesse->getFormattedValue("active"));
      $template->addProperty("$champ - $grossesse_subItem - " . CAppUI::tr('CGrossesse-multiple'), $grossesse->getFormattedValue("multiple"));
      $template->addProperty("$champ - $grossesse_subItem - " . CAppUI::tr('CGrossesse-allaitement_maternel'), $grossesse->getFormattedValue("allaitement_maternel"));
      $template->addProperty("$champ - $grossesse_subItem - " . CAppUI::tr('CGrossesse-num_semaines'), $grossesse->getFormattedValue('num_semaines'), null, false);
      $template->addProperty("$champ - $grossesse_subItem - " . CAppUI::tr('CGrossesse-lieu_accouchement'), $grossesse->getFormattedValue("lieu_accouchement"));
      $template->addProperty("$champ - $grossesse_subItem - " . CAppUI::tr('CGrossesse-rques'), $grossesse->rques);
      $template->addProperty("$champ - $grossesse_subItem - " . CAppUI::tr('CGrossesse-_semaine_grossesse'), $sa_reste_jour);
  ...
  /**
   * Chargement de la fdernière grossesse active
   *
   * @return CGrossesse|null
   */
  ...
  function loadRefAssuranceCurrent() {
    ...
    $assurances        = $this->loadBackRefs("correspondants_patient", "date_debut ASC", 1, null, null, null, "", $where);
    if (count($assurances)) {
  ...
  function loadLastDossierAddictologie() {
    ...
    $dossiers_addictologie = $dossier_addictologie->loadList($where, "dossier_addictologie_id DESC", null, null, $ljoin);
  ...
  function completeLabelFields(&$fields, $params) {
    ...
    if (CModule::getActive("barcodeDoc") && CAppUI::gconf("barcodeDoc general module_actif")) {
      $fields = array_merge(
        $fields,
        array(
          "CODE BARRE IPP AVEC PREFIXE" => "@BARCODE_" . CAppUI::gconf("barcodeDoc general prefix_IPP") . $this->_IPP . "@",
        )
      );
    }
  }
  ...
  function getSpecialIdex(CIdSante400 $idex) {
    ...
    if (CModule::getActive("mvsante")) {
      if ($idex_type = CMVSante::getSpecialIdex($idex)) {
        return $idex_type;
      }
    }
  ...
  function loadAllDocs($params = array()) {
    ...
    foreach ($this->loadRefsConsultations(array("sejour_id" => "IS NULL")) as $_consult) {
      $this->mapDocs($_consult, $params);
    }
  ...
  function getSharingList() {
    ...
    foreach ($groups as $_group) {
      $patient_groups[$_group->_id] = array(
        'label' => $_group->_view,
  ...
  function getTimeline() {
    ...
    foreach ($this->_ref_sejours as $_sejour) {
      ...
      if (!isset($events_by_date[$_date])) {
        $events_by_date[$_date] = array();
      }

      if (!isset($events_by_date[$_date][$_sejour->entree])) {
        $events_by_date[$_date][$_sejour->entree] = array();
      }

      $_sejour_related = array();
      foreach ($_sejour->_ref_operations as $_operation) {
        if (!isset($_sejour_related[$_operation->_datetime_best])) {
          $_sejour_related[$_operation->_datetime_best] = array();
        }
      ...
      foreach ($_sejour->_ref_consultations as $_consult) {
        if (!isset($_sejour_related[$_consult->_datetime])) {
          $_sejour_related[$_consult->_datetime] = array();
        }
    ...
    foreach ($this->_ref_consultations as $_consult) {
      ...
      if (!isset($events_by_date[$_date][$_consult->_datetime])) {
        $events_by_date[$_date][$_consult->_datetime] = array();
      }
  ...
  function getSurpoids() {
    ...
    $this->loadRefLatestConstantes(null, array("poids"));
  ...
  */
  ...
  function loadRefLastDirectiveAnticipee() {
    ...
    return $this->_ref_last_directive_anticipee = $directives_patient ? end($directives_patient) : $directive_patient;
  }
  ...
  function getByNumSS($num_ss) {
    $field_card = CAppUI::conf("ref_pays") == 2 ? "avs" : "matricule";
  ...
  public function getBloodRelatives() {
    ...
    $total             = array();
    $total["bros"]     = array();
    $total["children"] = array();

    $total["bros"]     = array_merge(array_map(function ($obj) {
      return array($obj->patient_id, $obj->type);
    }, $bro_sis_list), $total["bros"]);
    $total["children"] = array_merge(array_map(function ($obj) {
      return array($obj->patient_id, $obj->type);
    }, $children), $total["children"]);

    $total["parent_1"] = array($this->_ref_family_patient->parent_id_1, $this->_ref_family_patient->type);
    $total["parent_2"] = array($this->_ref_family_patient->parent_id_2, $this->_ref_family_patient->type);
  ...
  public function loadRefInjections() {
    return $this->_ref_injections = $this->loadBackRefs("injections");
  }
  ...
  public function loadRefSourceIdentite(bool $cache = true): CSourceIdentite
  {
      ...
      $this->_mode_obtention = $this->_ref_source_identite->_id ?
          $this->_ref_source_identite->mode_obtention : 'manuel';
  ...
  public function loadRefsSourcesIdentite(bool $only_actives = true, bool $update_fields = true): array
  {
      ...
      $this->_ref_sources_identite = $this->loadBackRefs('sources_identite', null, null, null, null, null, null, $where);
  ...
  public function isConsent() {
    $consent        = $this->loadUniqueBackRef("patient_consents");
    $this->_consent = intval($consent->status) === CRGPDConsent::STATUS_ACCEPTED ? 1 : 0;
  }
  ...
  function loadRefNaissance() {
    ...
    $where                     = array();
    $where['sejour_enfant_id'] = CSQLDataSource::prepareIn(array_keys($sejours));

    $naissance  = new CNaissance();
    $naissances = $naissance->loadList($where);

    return $this->_ref_naissance = reset($naissances);
  }
   ...
   */
  function getResourceSejours():? CCollection {
    if (!$sejours = $this->loadRefsSejours()) {
      return null;
    }

    return new CCollection($sejours);
  }

  public function getResourceAvatar():? CItem {
    if (!$fichier = $this->loadRefPhotoIdentite()) {
      return null;
    }

    return new CItem($fichier);
  }
   ...
   * @throws Exception
   */
  public function getResourceDoctor(): ?CItem {
    $medecin = $this->loadRefMedecinTraitant();
    if (!$medecin || !$medecin->_id) {
      return null;
    }

    $res = new CItem($medecin);
    $res->setName(CMedecin::RESOURCE_NAME);
     ...
     * @throws Exception
     */
    public function getResourceCorrespondantsPatient(): ?CCollection
    {
        ...
        $res = new CCollection($correspondants);
        $res->setName(CCorrespondantPatient::RESOURCE_NAME);
     ...
     * @throws Exception
     */
    public function getResourceMedicalRecord(): ?CItem
    {
        ...
        $res = new CItem($dossier);
        $res->setName(CDossierMedical::RESOURCE_NAME);
     ...
     * @throws Exception
     */
    public function getResourceRgpd(): ?CCollection
    {
        $patient_users = $this->loadRefsPatientUsers();
        if (!$patient_users) {
            return null;
   ...
   * @throws \Ox\Core\Api\Exceptions\CApiException
   */
  public function getResourceAllergies(): ?CCollection {
    ...
    return new CCollection($allergies);
  }
