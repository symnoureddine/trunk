...
<td colspan="{{if $print_global}}1{{else}}2{{/if}}" class="text">
  ...
  <span class="CPatient-view me-patient-view">
    {{if "maternite"|module_active && $_sejour->_ref_first_affectation->_ref_parent_affectation->_id}}
      {{assign var=maman value=$_sejour->_ref_first_affectation->_ref_parent_affectation->_ref_sejour->_ref_patient}}
      <img src="style/mediboard_ext/images/icons/grossesse.png" onmouseover="ObjectTooltip.createEx(this, '{{$maman->_guid}}');"
...
{{* Appel contextuel *}}
{{if $canPlanningOp->read}}
  <td style="white-space: normal; {{if $print_global}}display: none;{{/if}}">
    ...
    {{if 'notifications'|module_active}}
      {{if 'Ox\Mediboard\Notifications\CNotification::isDayBeforeNotificationSent'|static_call:$_sejour->_guid}}
        <span class="circled" style="color: #007f00; border-color: #007f00; background-color: white;" title="{{tr}}CNotification-msg-sent-CSejour-day_before{{/tr}}">SMS</span>
      {{/if}}
    {{/if}}
...
<td>
  {{if $canAdmissions->edit}}
    ...
    {{if $_sejour->entree_reelle}}
      {{'Ox\Core\CMbDT::format'|static_call:"":$_sejour->entree_reelle|date_format:"%Hh%M"}}<br/>
...
{{if $canPlanningOp->read && "dmp"|module_active}}
  <td {{if $print_global}}style="display: none;"{{/if}}>
...
{{if $canPlanningOp->read}}
  <td {{if $print_global}}style="display: none;"{{/if}}>
    <a class="action me-planif-icon me-button actionPat notext me-margin-2" title="Modifier le séjour" href="#editDHE"
    ...
    {{if $number_op == 1}}
      {{foreach from=$_sejour->_ref_operations item=_op}}
        <button class="print notext me-margin-2" title="{{tr}}admissions-action-Print the DHE from the intervention{{/tr}}"
...
<td {{if $print_global}}style="display: none;"{{/if}} class="me-ws-wrap">
  ...
  {{if $canPlanningOp->read}}
    <a href="#showDocs" title="{{tr}}admissions-action-Display document and file|pl{{/tr}}" class="button" onclick="Admissions.showDocs('{{$_sejour->_id}}')">
...
<td class="button">
  ...
  {{/if}}
  {{if $conf.ref_pays != "2" && $_sejour->_couvert_ald}}
