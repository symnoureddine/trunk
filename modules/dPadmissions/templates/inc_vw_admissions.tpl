...
<table class="tbl me-no-align me-small-tbl" id="admissions" {{if $print_global}}style="border-collapse: separate;"{{/if}}>
  <tbody>
  {{foreach from=$sejours item=_sejour}}
    <tr class="sejour sejour-type-default sejour-type-{{$_sejour->type}} {{if !$_sejour->facturable}} non-facturable {{/if}}"
        id="{{$_sejour->_guid}}">
      ...
      <th colspan="14">
        <h1>
          <a href="#" onclick="window.print()">
            {{tr var1=$date|date_format:$conf.longdate var2=$sejours|@count}}admissions-Admission of %s (%s admission)|pl{{/tr}}
          </a>
        </h1>
      </th>
    </tr>
    ...
    <tr>
      <th class="title" colspan="18">
        ...
        <div>
          ...
          <br>
          <button id="printSelectionButton" type="button" onclick="Admissions.choosePrintForSelection()" disabled class="button print me-primary disabled">
  ...
  <tr>
    <th class="narrow" {{if $print_global}}style="display: none;"{{/if}}>
      <input type="checkbox" style="float: left;" onclick="Admissions.togglePrint(this.checked); Admissions.updatePrintSelectionButtonDisplay();" />
    </th>
    <th>
