...
$where = [
  "sejour.type" => (count($type_pref) > 0) ? CSQLDataSource::prepareIn($type_pref) : CSQLDataSource::prepareNotIn(array_merge(CSejour::getTypesSejoursUrgence(), ["seances"]))
];
$ljoin = [];
...
foreach ($days as $_date => $num) {
  ...
  // Get data and count unique ids
  $total_list      = $sejour->loadList($where, null, null, "sejour.sejour_id", $ljoin);
  $size_unique_ids = count(array_unique(CMbArray::pluck($total_list, "_id")));

  // Add it to the specific day of the month
  $days[$_date] = $size_unique_ids;
}
