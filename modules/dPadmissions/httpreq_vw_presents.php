...
/** @var CSejour[] $sejours */
$sejours = $use_perms ?
  $sejour->loadListWithPerms(PERM_READ, $where, $order, null, "sejour.sejour_id", $ljoin, false) :
  $sejour->loadList($where, $order, null, "sejour.sejour_id", $ljoin, null, null, false);
...
$affectation_order = (!$show_curr_affectation) ? 'sortie DESC' : null;
$affectations = CStoredObject::massLoadBackRefs($sejours, "affectations", $affectation_order);
...
foreach ($sejours as $sejour_id => $_sejour) {
  ...
  if ($show_curr_affectation) {
    ...
    $affectation = $_sejour->loadRefCurrAffectation($datetime->format('Y-m-d H:i:s'));
  }
  else {
