...
<span>
  ...
  <span style="float:right; text-align: right">
    ...
    {{if $conf.ref_pays == 2}}
      {{if $liaison_sejour}}
        <strong onclick="Prestations.edit('{{$sejour->_id}}', '{{$operation->date}}')">{{$liaison_sejour|smarty:nodefaults}}</strong>
      {{/if}}

      {{if "dPplanningOp CSejour use_charge_price_indicator"|gconf != "no" && $charge->_id}}
  ...
  <span {{if $show_tooltip}}onmouseover="ObjectTooltip.createEx(this, '{{$patient->_guid}}');"{{/if}}>
    <span class="{{if !$sejour->entree_reelle && $conf.reservation.use_color_patient}}patient-not-arrived{{/if}} {{if $sejour->septique}}septique{{/if}}"
  ...
  {{if "mvsante"|module_active && $modules.mvsante->_can->read}}
  ...
  Sejour: <span {{if $show_tooltip}}onmouseover="ObjectTooltip.createEx(this, '{{$sejour->_guid}}');"{{/if}}>{{mb_value object=$sejour field=entree}}</span>
  ...
  {{if $chir_2->_id}}
    <br/>{{mb_include module=mediusers template=inc_vw_mediuser mediuser=$chir_2}}
  {{/if}}

  {{if $chir_3->_id}}
    <br/>{{mb_include module=mediusers template=inc_vw_mediuser mediuser=$chir_3}}
  {{/if}}

  {{if $chir_4->_id}}
    <br/>{{mb_include module=mediusers template=inc_vw_mediuser mediuser=$chir_4}}
  {{/if}}

  {{if $anesth->_id}}
    <img src="images/icons/anesth.png" alt="WRN" />{{mb_include module=mediusers template=inc_vw_mediuser mediuser=$anesth}}
  {{/if}}
