...
<form name="listFilterFunction" action="?" method="get" class="prepared">
  ...
  <input type="hidden" name="tab" value="vw_all_perms" />

  <table class="main layout me-margin-10">
    <tr>
      <td colspan="2">
        <button type="submit" class="tick" style="float: left;">
          {{if is_array($users_ids) && $users_ids|@count == 0 && is_array($profiles_ids) && $profiles_ids|@count == 0}}
            {{tr}}common-action-Display selection{{/tr}}
          {{else}}
            {{tr}}common-action-Display all{{/tr}}
          {{/if}}
        </button>
        <button type="button" class="search" onclick="Perm.legend();" style="float: left;">{{tr}}Legend{{/tr}}</button>
      </td>
    </tr>
  </table>
  ...
  <table class="tbl me-margin-10 me-table-col-separated" style="font-size: 0.9em;">
    {{if !$only_profil}}
      <tr>
        ...
        <th colspan="3" class="text" style="background-color: gray; color: #000;">Profil</th>
      ...
      {{foreach from=$profiles item=_profile}}
        ...
        <tr>
          ...
          {{foreach from=$listModules item=curr_mod}}
            ...
            <td
              style="font-size: 1px; text-align: center; {{if $matriceProfil.$user_id.$mod_id.type == "spécifique"}} background-color: #faa;{{/if}}"
              title="{{$matriceProfil.$user_id.$mod_id.text}} ({{$matriceProfil.$user_id.$mod_id.type}})">
              <div class="bullet {{$matriceProfil.$user_id.$mod_id.permIcon}}"></div>
              <div class="bullet {{$matriceProfil.$user_id.$mod_id.viewIcon}}"></div>
            </td>
          {{/foreach}}
        </tr>
      {{/foreach}}
    ...
    {{if !$only_user}}
      {{foreach from=$listFunctions item=curr_func}}
        <tr>
          ...
          {{th_vertical class="text" style="background-color: #$color; color: #000;"}}Accès distant{{/th_vertical}}
