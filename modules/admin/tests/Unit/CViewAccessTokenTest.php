 ...
 */
class CViewAccessTokenTest extends UnitTestMediboard {
  /**
   * @return array|int[]
   */
  ...
  private function getRandomUser(): CUser {
    ...
    return $user = $this->getRandomObjects(CUser::class, 1);
  }
   ...
   * @throws TestsException
   */
  public function test_token_computes_hash_length_according_to_validity_duration(int $hash_length, ?int $validity_duration = null): void {
    $user = $this->getRandomUser();
   ...
   * @throws TestsException
   */
  public function test_token_computes_validity_duration_according_to_hash_length(int $hash_length, ?int $validity_duration = null): void {
    $user = $this->getRandomUser();
    ...
    if ($hash_length < $token::MINIMUM_HASH_LENGTH) {
      ...
      if ($validity_duration !== null) {
        $this->assertEquals(CMbDT::dateTime("+{$validity_duration} seconds", $token->datetime_start), $token->datetime_end);
      }
      else {
