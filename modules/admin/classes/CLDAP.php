 ...
 */
class CLDAP implements IShortNameAutoloadable {
  /**
   * Log a user in
   *
  ...
  static function login(CUser $user, $ldap_guid) {
    ...
    if (!$ldap_guid || $user->_user_password) {
      $source_ldap = CLDAP::poolConnect($user->_id);

      if (!$source_ldap || !$source_ldap->_ldapconn) {
        //CAppUI::setMsg('CSourceLDAP_all-unreachable', UI_MSG_WARNING);
      ...
      try {
        ...
        $user->_bound = $source_ldap->ldap_bind($source_ldap->_ldapconn, $user->user_username, stripcslashes($user->_user_password));
      }
        ...
        $source_ldap->ldap_bind($source_ldap->_ldapconn, $source_ldap->user, $source_ldap->password);
      }
    }
    else {
      ...
      $user->_bound = $source_ldap->ldap_bind($source_ldap->_ldapconn, $source_ldap->user, $source_ldap->password);
  ...
  static function poolConnect($user_id, $group_id = null) {
    if (!$user_id && !$group_id) {
      return false;
    }
   ...
   *
   * @param int $group_id CGroups ID
   *
   * @return CSourceLDAP[]
   */
  static public function getAvailablePool($group_id) {
   ...
   * @return CUser
   */
  static function searchAndMap(CUser $user, CSourceLDAP $source_ldap, $ldapconn, $person = null, $filter = null, $force_create = false, $check_password_expiration = true) {
    if (!$person) {
    ...
    $idex->id400 = self::getObjectGUID($results, $source_ldap);
    ...
    if (!$idex->_id) {
      ...
      if ($msg = $idex->store()) {
        ...
        if ($check_password_expiration && $user->_id == CMediusers::get()->_id) {
   ...
   * @param CSourceLDAP $source_ldap Source LDAP
   *
   * @return CUser
   */
  static function mapTo(CUser $user, $values, CSourceLDAP $source_ldap) {
    $user->user_username   = $source_ldap->isAlternativeBinding() ? self::getValue($values, 'cn') : self::getValue($values, "samaccountname");
  ...
  static function getObjectGUID($values, CSourceLDAP $source_ldap) {
    if ($source_ldap->isAlternativeBinding()) {
      return self::getValue($values, 'cn');
    }
  ...
  static function getMaxPasswordAge($values, $source_ldap) {
    ...
    $results = $source_ldap->ldap_search($ldapconn, '(objectclass=domain)', array('maxpwdage'), true);
