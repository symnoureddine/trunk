...
<script>
  selectChapter = function(select) {
    ...
    if (chapter_id) {
      new Url('lpp', 'ajax_get_descendant_chapters')
  ...
  searchCode = function(form, start) {
    var url = new Url('lpp', 'ajax_search_codes');
  ...
  <table class="form">
    ...
    <tr>
      <td class="buttons" style="text-align: center;" colspan="4">
        <button id="search_codes" type="button" class="search" onclick="this.form.onsubmit();">{{tr}}Search{{/tr}}</button>
      </td>
    </tr>
  </table>
</form>
