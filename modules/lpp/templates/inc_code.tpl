        ...
        <td>
          <select name="selectPricing" onchange="selectPricing($V(this));">
            {{foreach from=$code->_pricings item=_pricing}}
              <option value="{{$_pricing->begin_date}}" {{if $_pricing->begin_date == $code->_last_pricing->begin_date}}selected="selected"{{/if}}>
                {{mb_value object=$_pricing field=act_date}} &mdash; {{mb_value object=$_pricing field=jo_date}}
              </option>
            {{/foreach}}
          </select>
        </td>
  ...
  <table class="layout" style="width: 100%;">
    <tr>
      ...
      <td id="incompatibilities" class="halfPane" style="vertical-align: top; text-align: right;">
        <fieldset style="width: 95%; display: inline;">
          <legend>{{tr}}CLPPCode-title-incompatibilities{{/tr}} ({{$code->_incompatibilities|@count}})</legend>
          <ul style="list-style: none">
            {{foreach from=$code->_incompatibilities item=_code}}
              <li style="text-align: left;"><strong>{{$_code->code}}</strong><span class="text compact" style="padding-left: 5px;">{{$_code->name}}</span></li>
            {{foreachelse}}
              <li class="empty" style="text-align: left;">{{tr}}CLPPCode-_incompatibilities.none{{/tr}}</li>
            {{/foreach}}
          </ul>
        </fieldset>
      </td>
