...
<ul>
  {{foreach from=$codes item=_code}}
    <li data-code="{{$_code->code}}" data-code_prestation="{{$_code->_last_pricing->prestation_code}}"
        ...
        data-dep="{{$_code->_last_pricing->dep}}" data-qualif_depense="{{'|'|implode:$_code->_unauthorized_expense_qualifying}}">
      ...
      <div class="text compact" style="width: 100%; overflow: hidden; text-overflow: ellipsis;">{{$_code->name}}</div>
    </li>
  {{foreachelse}}
