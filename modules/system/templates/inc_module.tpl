...
{{else}}
  <td class="narrow">
    {{mb_include module=system template=module_icon mod_name=$_mb_module->mod_name size="18px" mbd_size="24px" img_style="float: right"}}
  </td>
  <td>
      ...
      <form name="formOrderModule-{{$module_id}}" method="post" onsubmit="return onSubmitFormAjax(this)">
        ...
        <map name="map-{{$_mb_module->_id}}">
          <area coords="0,0,10,7" href="#1"
                onclick="$V(this.up('form').cmd, 'moveup'); Module.moveRowUp(this.up('tr'));   this.up('form').onsubmit();" />
          <area coords="0,8,10,14" href="#1"
                onclick="$V(this.up('form').cmd, 'movedn'); Module.moveRowDown(this.up('tr')); this.up('form').onsubmit();" />
        </map>
      </form>
    {{/if}}
  </td>
