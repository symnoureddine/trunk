...
<form name="form-merge" action="?m={{$m}}&{{$actionType}}={{$action}}&dialog={{$dialog}}" method="post"
      onsubmit="return {{if $checkMerge}}false;{{else}}checkForm(this);{{/if}}">
        ...
        <span onmouseover="ObjectTooltip.createEx(this, '{{$object->_guid}}')">
          ...
          <label style="font-weight: normal;">
            <input type="radio" name="_base_object_id" value="{{$object->_id}}"
              ...
              {{if $object->_disabled}}disabled{{/if}}
      ...
      <td colspan="100" class="text">
        {{if !$dialog}}
          <div class="big-warning">
            ...
            <br />
            <strong>Cette opération est irréversible, il est donc impératif d'utiliser cette fonction avec une extrême prudence
              !</strong>
            <br />
    ...
    <tr>
      <td colspan="100" class="button">
        {{if $checkMerge}}
          ...
          <div class="big-error">
            <p>La fusion de ces deux objets <strong>n'est pas permise</strong> à cause des problèmes suivants :</p>
            <ul>
              <li>{{$checkMerge}}</li>
            </ul>
        ...
        {{else}}
          ...
          {{if !$merge_type || $merge_type == "check"}}
            <button type="submit" class="merge" onclick="return ObjectMerger.confirm('0')" {{if $mode == 'fast'}}disabled{{/if}}>
              {{tr}}Merge{{/tr}}
            </button>
          {{/if}}
          {{if (!$merge_type || $merge_type == "fast") && $user->isAdmin()}}
            <button type="submit" class="merge" onclick="return ObjectMerger.confirm('1');" {{if $mode == 'check'}}disabled{{/if}}>
