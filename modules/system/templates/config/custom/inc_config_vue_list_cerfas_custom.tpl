...
{{if $is_last}}
  ...
  <script>
    createLi = function(cerfa, target) {
      ...
      var _span = DOM.span({"type": "text", "className":"title notext", "style": "margin-left: 5px;"}, $T("CCerfa-" + cerfa));
  ...
  <select name="components" {{if $is_inherited}}disabled{{/if}} onchange="createLi($V(this), $$('ul.list_tabs_cerfas')[0]); this.selectedIndex = 0;" style="width: 350px;">
    <option value="">&mdash; {{tr}}CCerfa-action-Choose a Cerfa{{/tr}}</option>
  {{foreach from=$components item=_component}}
    <option value="{{$_component}}">{{tr}}CCerfa-{{$_component}}{{/tr}}</option>
  {{/foreach}}
  </select>
...
{{else}}
  {{if $value}}
    <ul>
      ...
      <li class="cerfa_custom">{{tr}}CCerfa-{{$_elt}}{{/tr}}</li>
      {{/foreach}}
    </ul>
  {{/if}}
{{/if}}
