...
{{mb_default var=show_keychain_entries value=false}}
...
<tr>
  {{if $object->_id}}
    <th class="title modify {{$css_class}}" colspan="{{$colspan}}">
      ...
      {{if $show_keychain_entries}}
        {{mb_include module=passwordKeeper template=inc_object_keychain_entries object=$site}}
      {{/if}}
