...
class CExObject extends CMbObject implements IPatientRelated, IIndexableObject
{
    ...
    static function checkLocales()
    {
        ...
        $static_cache = new Cache(__METHOD__, "status", Cache::INNER_OUTER, 600);
        if ($static_cache->exists()) {
            return;
        }

        if (!$engine = DSHM::getEngine()) {
            return;
        }

        $last_check = null;
        // Cache to store the last engine used for ExObjects locales
        $cache = new Cache(__METHOD__, "check", Cache::INNER_OUTER);
        if ($cache->exists()) {
        ...
        if (!$last_check || $engine_class != $last_check) {
    ...
    function store()
    {
        ...
        if ($new_object) {
            ...
            if ($object instanceof IGroupRelated) {
                $group_id = $object->loadRelGroup()->_id;
            } else {
