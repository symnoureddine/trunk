...
class CExClassField extends CExListItemsOwner {
  static $_props_exclude = [
    ...
    "loggable", "refDate", "flag", "monitored",
    "numChars", "alphaChars", "alphaLowChars",
    "specialChars", "alphaUpChars", 'hideDate',
    'group', 'back',
  ];
  ...
  static $_formula_valid_types = [
    "float", "num", "numchar", "pct", "currency", "date", "dateTime", "time", "bool",
  ];
  ...
  function store() {
    ...
    if ($locale || $locale_desc || $locale_court) {
      ...
      if ($msg = $trans->store()) {
        mbTrace($msg, CClassMap::getInstance()->getShortName($this), true);
      }
    }
