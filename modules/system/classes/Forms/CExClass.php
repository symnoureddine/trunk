...
class CExClass extends CMbObject {
  ...
  public $_duplication_mapping = array();
  ...
  function loadEditView() {
    ...
    if ($this->pixel_positionning) {
      ...
      list($grid, $out_of_grid) = $this->getGrid(4, 60, false, true);
    }
    ...
    foreach ($classes as $_class) {
      ...
      if (isset($instances[$_class]['_view'])) {
        ...
        $instances[$_class] = array_merge(array('_view' => $_view), $instances[$_class]);
      }
    }
    ...
    foreach ($_fields as $_field) {
      if ($_field->concept_id) {
        if (!array_key_exists($_field->concept_id, $same_concepts)) {
          $same_concepts[$_field->concept_id] = array('count' => 1, 'where' => array());
        }
        else {
      ...
      foreach ($search[$_field->concept_id] as $_val) {
        ...
        if ($same_concepts[$_field->concept_id]['count'] > 1) {
          if (isset($comp_map[$_comp])) {
            $same_concepts[$_field->concept_id]['where'][] = "$_col " . $_ds->prepare($comp_map[$_comp] . "%", $_val_a);
          }
          else {
            switch ($_comp) {
              case "contains":
                $same_concepts[$_field->concept_id]['where'][] = "$_col " . $_ds->prepareLike("%$_val_a%");
                break;
              case "begins":
                $same_concepts[$_field->concept_id]['where'][] = "$_col " . $_ds->prepareLike("$_val_a%");
                break;
              ...
              case "between":
                $_val_b                                        = $_val['b'];
                $same_concepts[$_field->concept_id]['where'][] = "$_col " . $_ds->prepare("BETWEEN ?1 AND ?2", $_val_a, $_val_b);
                break;
              case "inSet":
...
//    $ex_object->loadObject($where_ref2, 'ex_object_id DESC');
  ...
  static function getLatestCompletenessByEvent(CMbObject $object, $event_name) {
    ...
    foreach ($ex_classes_filtered as $_ex_class_id => $_ex_class) {
      $completeness[$_ex_class_id] = array(
  ...
  function loadRefsAllFields() {
    $cache = new Cache(__METHOD__, $this->_id, Cache::INNER);
    if (!$cache->exists()) {
      ...
      $cache->put(array($groups, $fields));

      return $fields;
    }

    list($groups, $fields) = $cache->get();
  ...
  function duplicate() {
    ...
    foreach ($this->loadRefsGroups() as $_group) {
      ...
      foreach ($_group->loadRefsFields() as $_field) {
        ...
        if ($msg = $this->duplicateExObject($_field, "ex_group_id", $_new_group->_id, $_new_field, $_exclude_fields)) {
          continue;
        }
      ...
      $where = array("subgroup_id" => "IS NULL");
      // class_messages
      $this->duplicateExBackRefs($_group, "class_messages", $fwd_field, $fwd_value, array("predicate_id"), $where);

      // host_fields
      $this->duplicateExBackRefs($_group, "host_fields", $fwd_field, $fwd_value, array(), $where);

      // subgroups
      $this->duplicateExBackRefs($_group, "subgroups", "parent_id", $fwd_value, array("predicate_id", "subgroup_id"));

      /** @var CExClassFieldSubgroup[] $_subgroups */
      $_subgroups = $_group->loadBackRefs("subgroups");
      foreach ($_subgroups as $_subgroup) {
        list($_new_class, $_new_id) = explode('-', $this->_duplication_mapping[$_subgroup->_guid]);
    ...
    foreach ($this->loadRefsGroups() as $_group) {
      ...
      foreach ($_group->loadRefsFields() as $_field) {
        if ($_field->predicate_id) {
          ...
          list($_precidate_class, $_predicate_id) = explode("-", $_predicate_guid);

          /** @var CExClassField $_new_field */
          $_new_field                 = CStoredObject::loadFromGuid($this->_duplication_mapping[$_field->_guid]);
        ...
        if ($_field->subgroup_id) {
          ...
          list($_subgroup_class, $_subgroup_id) = explode("-", $_subgroup_guid);
      ...
      foreach ($_group->loadRefsMessages() as $_message) {
        if ($_message->predicate_id) {
          ...
          $new_message                 = CStoredObject::loadFromGuid($this->_duplication_mapping[$_message->_guid]);
        ...
        if ($_message->subgroup_id) {
          ...
          $_new_message                 = CStoredObject::loadFromGuid($this->_duplication_mapping[$_message->_guid]);
  ...
  function duplicateSubgroups(CExClassFieldSubgroup $object, $fwd_value, $parent_value) {
    $this->duplicateExBackRefs($object, "subgroups", "parent_id", $fwd_value, array("predicate_id"));

    $fields = array("subgroup_id" => $fwd_value, "ex_group_id" => $parent_value);
    $this->duplicateExBackRefs($object, "children_messages", $fields, null, array("predicate_id"));
    $this->duplicateExBackRefs($object, "children_host_field", $fields);

    $_subgroups = $object->loadBackRefs("subgrou^s");
   ...
   * @return null|string
   */
  function duplicateExObject(CMbObject $object, $fwd_field, $fwd_value = null, &$new = null, $exclude_fields = array()) {
  ...
  function duplicateExBackRefs(
    CMbObject $object, $backname, $fwd_field, $fwd_value = null, $exclude_fields = array(), $where = array()
  ) {
    $new = null;
  ...
  function getPermissions($fill = false) {
    ...
    $permissions = $this->permissions ? json_decode($this->permissions, true) : array();
  ...
  function checkFields() {
    ...
    foreach ($this->_ref_groups as $_group) {
      if (!array_key_exists($_group->name, $fields_check)) {
        $fields_check[$_group->name] = array();
      }

      foreach ($_group->_ref_fields as $_field) {
        if ($ds->hasTable($ex_object->getTableName())) {
          $fields_check[$_group->name][$_field->name] = ($ds->hasField($ex_object->getTableName(), $_field->name));
        }
      }
    }
