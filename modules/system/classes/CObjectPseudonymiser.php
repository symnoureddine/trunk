...
class CObjectPseudonymiser implements IShortNameAutoloadable {
  static $classes_handled = array(
    ...
    'CFunctions' => array(
      'initials', 'soustitre', 'adresse', 'tel', 'fax', 'email', 'finess', 'siret', 'ean', 'ean_xml_factu', 'rcc',
    ),
    'CMedecin' => array(
  ...
  protected function getRandomNames($use_table) {
    ...
    if ($use_table) {
      ...
      $first_names = array();
      for ($i = 0; $i < $count; $i++) {
        $first_names[] = CMbSecurity::getRandomString(6);
      }
  ...
  protected function pseudonymiseObjects() {
    ...
    foreach ($this->objects as $_object) {
      ...
      switch ($_object->_class) {
        ...
        case 'CUser':
          $_object->user_username = CMbSecurity::getRandomString(12);
