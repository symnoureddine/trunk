...
class CViewSender extends CMbObject {
  ...
  function getProps() {
    ...
    $props["day"]                 = "enum list|" . implode('|', range(1, 28));
    $props["active"]              = "bool notNull default|0";
    $props["max_archives"]        = "num min|1 notNull default|10";
    $props["last_duration"]       = "float loggable|0";
    $props["last_size"]           = "num loggable|0";
    $props["multipart"]           = "bool notNull default|0";
    $props["last_datetime"]       = "dateTime loggable|0";
    $props["last_status"]         = "enum list|triggered|producted loggable|0";
    $props["last_http_code"]      = "num loggable|0";
    $props["last_error_datetime"] = "dateTime loggable|0";
   ...
   * @return bool
   */
  function getActive($minute, $hour = null, $day = null) {
  ...
  function makeFile() {
    ...
    if ($this->last_http_code >= 400 || file_put_contents($file, $contents) === false) {
      ...
      if ($this->last_http_code >= 400) {
        trigger_error(CAppUI::tr('CViewSender-error-http return code', $this->name, $this->last_http_code), E_USER_WARNING);
      }
      else {
  ...
  function sendFile() {
    ...
    foreach ($this->loadRefSendersSource() as $_source_to_view_sender) {
      ...
      if ($source->_id && $source->active && $_sender_source->actif) {
        try {
          ...
          switch (get_class($source)) {
            ...
            default:
              throw new CMbException("CViewSenderSource-msg-View sender source incompatible %s with the instance role %s");
          }
        }
  ...
  function send(CSourceToViewSender $source_to_view_sender) {
    ...
    foreach ($this->_files_list as $_i => &$_file) {
      $basename = $this->name;
      if ($this->multipart) {
        $destination_basename = $source->fileprefix . $basename . "/" . $this->getDateTime() . "/" . $_file["title"];
      }
      else {
   ...
   *
   * @param CExchangeSource $source     Exchange source
  ...
  function archiveFile(CExchangeSource $source, $basename, $compressed) {
    try {
      ...
      $files = $source->getListFiles($directory);
      rsort($files);
      ...
      if ($source instanceof CSourceFileSystem) {
        ...
        $rm_dir    =
          rtrim($full_path, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . rtrim($rm_dir, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;
      }

      $list_files = array_slice($files, $this->max_archives);
      foreach ($list_files as $_file) {
        $source->delFile(basename($_file), $rm_dir);
      }
    }
    catch (CMbException $e) {
      $e->stepAjax();
    ...
    return count($source->getListFiles($directory));
  }
