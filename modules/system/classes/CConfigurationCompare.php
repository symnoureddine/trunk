...
class CConfigurationCompare implements IShortNameAutoloadable {
  protected $result_instance = array();
  protected $result_groups = array();
  ...
  protected static $xpath_queries = array(
    'check_file_type'  => '/configurations-export',
    'configs_instance' => array(
      'root'   => 'instance-configs',
      'config' => 'instance-config',
    ),
    'configs_groups'   => array(
      'root'    => 'groups-configs',
  ...
  /**
   * Check if a file id valid for comparison or not (if it contains the element get by $xpath_queries['check_file_type'])
   *
   * @return bool
   */
  ...
  protected function getInstanceConfigs() {
    ...
    $instance_configs = $this->xpath->query(static::$xpath_queries['configs_instance']['config'], $instance_root->item(0));
    ...
    foreach ($instance_configs as $_config) {
      ...
      if (count($parts) == 1 || !CModule::exists($parts[0])) {
        ...
        if (!array_key_exists($feature, $this->result['none']['instance'])) {
          $this->result['none']['instance'][$feature] = array();
        }
        ...
        if (!array_key_exists($feature, $this->result[$parts[0]]['instance'])) {
          $this->result[$parts[0]]['instance'][$feature] = array();
        }
    ...
    foreach ($modules as $_module) {
      ...
      foreach ($configs as $_conf) {
        ...
        if (!array_key_exists($feature, $this->result[$mod_name]['groups'])) {
          $this->result[$mod_name]['groups'][$feature] = array();
        }
