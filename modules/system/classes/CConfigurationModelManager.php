...
abstract class CConfigurationModelManager implements IShortNameAutoloadable {
  ...
  static protected $roots = array();

  /** @var array Leaf nodes */
  static protected $leaves = array();

  /** @var array Raw model */
  static protected $raw_model = array();

  /** @var array Inherited raw model */
  static protected $inherited_raw_model = array();

  /** @var array Model */
  static protected $model = array();

  /** @var array Cache of object ids */
  static protected $context_ids = array();
  ...
  static protected function getRawModel() {
    return (isset(static::$raw_model[static::getModule()])) ? static::$raw_model[static::getModule()] : array();
  }
  ...
  static protected function getInheritedRawModel() {
    return (isset(static::$inherited_raw_model[static::getModule()])) ? static::$inherited_raw_model[static::getModule()] : array();
  }
  ...
  static protected function countRootNodes($context_class = null) {
    ...
    return (isset(static::$roots[static::getModule()][$context_class]) && static::$roots[static::getModule()][$context_class]);
  }
  ...
  static protected function registerModelNodes() {
    foreach (static::getSortedRawModel() as $_inherit => $_model) {
      ...
      // Leaf
      list ($_parent_class, $fwd) = explode('.', $_classes[1]);
  ...
  static protected function buildModel() {
    ...
    foreach (static::getInheritedRawModel() as $_inherit => $_model) {
      ...
      static::_buildModelLeaf($list, array(), $_model);

      if (!isset($model[$_inherit])) {
        $model[$_inherit] = array();
      }
  ...
  static protected function getModel() {
    return (isset(static::$model[static::getModule()])) ? static::$model[static::getModule()] : array();
  }
   ...
   * @return array
   */
  static protected function getDefaultValues($keys = array()) {
    $values = array();
   ...
   * @return array
   * @throws CMbException
   * @todo Each configuration set in a leaf is deported to other leaves values because of inheritance which doesn't check inherit keys
   *
   */
  ...
  static protected function buildValues($context) {
    ...
    foreach (static::getRootNodes() as $_root) {
      ...
      foreach ($_root->getObjectIDs() as $_id) {
        ...
        if ($_cache->exists()) {
          ...
          $values["{$_root->getContextClass()}-{$_id}"] =
            array_merge($values['global'], static::getStoredConfigurations($_root->getContextClass(), $_id));

          static::putInStore($_cache, $values["{$_root->getContextClass()}-{$_id}"], "{$_root->getContextClass()}-{$_id}");
        }
      }

      $child_inherits = array();
      ...
      foreach ($_root->getChildren() as $_child) {
        ...
        foreach ($_child->getObjectIDs() as $_child_id) {
          // Check if context configuration in cache
          $_cache = static::getValuesCache(static::getModule(), "{$_child->getContextClass()}-{$_child_id['id']}");
    ...
    return isset($values[$context]) ? CMbArray::unflatten($values[$context]) : array();
  }
  ...
  static protected function checkInherit($context) {
    ...
    list($object_class,) = explode('-', $context);
   ...
   */
  static protected function getStoredConfigurations($object_class = null, $object_id = null, $keys = array(), IConfigurationStrategy $strategy = null) {
    if (!isset(static::$cache[static::getModule()]) || !static::$cache[static::getModule()]) {
      ...
      static::$cache[static::getModule()] = $strategy->getStoredConfigurations($module, $spec, false);
    }
  ...
  static public function clearValuesCache($module) {
    ...
    SHM::remKeys("config-values-{$module}-*");
  }
  ...
  static public function getConfigSpec($feature) {
    list ($module, $_feature) = explode(' ', $feature, 2);
   ...
   * @throws CMbException
   */
  static public function _getModel($module, $inherits = array()) {
    static::setModule($module);
    ...
    if ($inherits) {
      $inherits = (is_array($inherits)) ? $inherits : array($inherits);
    }
   ...
   * @throws CMbException
   */
  static public function getAncestorsConfigs($config_inherit, $module, $config_keys = null, CStoredObject $object = null, IConfigurationStrategy $strategy = null) {
    static::setModule($module);
    static::buildModel();
    ...
    if ($object) {
      ...
      foreach ($ancestors as $_ancestor) {
        $_config = static::getStoredConfigurations($_ancestor->_class, $_ancestor->_id, $config_keys, $strategy);
   ...
   *
   * @return string The values can be CConfiguration::STATUS_EMPTY, CConfiguration::STATUS_DIRTY or CConfiguration::STATUS_OK
   */
