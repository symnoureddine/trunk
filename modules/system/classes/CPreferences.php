...
class CPreferences extends CMbObject
{
    ...
    function getProps()
    {
        ...
        $props["key"]        = "str notNull maxLength|255";
        $props["value"]      = "str";
     ...
     * @return array
     */
    static function getAllPrefs($user_id)
    {
        // Default
        $preferences = CPreferences::get();

        // Profile
        $user = new CUser();
        $user->load($user_id);
        if ($user->profile_id) {
            $preferences = array_merge($preferences, CPreferences::get($user->profile_id));
        }

        // User
        $preferences = array_merge($preferences, CPreferences::get($user->_id));

        return $preferences;
    }

    public static function getAllPrefsForList(CUser $user, array $pref_names): array
    {
        // Default
        $preferences = CPreferences::getPrefValuesForList($pref_names);

        // Profile
        if ($user->profile_id) {
            $preferences
                = array_merge($preferences, CPreferences::getPrefValuesForList($pref_names, $user->profile_id));
        }

        // User
        return array_merge($preferences, CPreferences::getPrefValuesForList($pref_names, $user->_id));
    }
    ...
    static function getUserIDs($key, $strict_key = true, $restricted = false)
    {
        ...
        $where = [
            'key'        => ($strict_key) ? $ds->prepare('= ?', $key) : $ds->prepareLike("%{$key}%"),
