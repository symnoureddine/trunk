...
class CUserAuthentication extends CMbObject
{
    ...
    public $_ref_authentication_factor;
    ...
    function getProps()
    {
        ...
        $props["authentication_factor_id"] = "ref class|CAuthenticationFactor back|authentications";
    ...
    static function logAuth(CUser $user, $previous_user_id = null, bool $is_stateless = false)
    {
        ...
        if ($is_stateless) {
            ...
            if ($last_auth
                ...
                && str_starts_with($last_auth->session_id, 'stateless_')
        ...
        if (session_id() === '') {
            // Api mode
            $auth->session_id          = uniqid('stateless_', true);
        ...
        if (CAuthenticationFactor::isReady() && !CAuthenticationFactor::checkLastAuthenticationWithFactor($user)) {
            // Don't get disabled authentication factors
            $authentication_factors = $user->loadRefAuthenticationFactors(false);

            if ($authentication_factors) {
                $authentication_factor = reset($authentication_factors);
    ...
    static function logStatlessAuth()
    {
    }
    ...
    /**
     * Get used authentication factor
     *
     * @return CAuthenticationFactor|null
     */
    function loadRefAuthenticationFactor()
    {
        return $this->_ref_authentication_factor = $this->loadFwdRef('authentication_factor_id', true);
    }
    ...
    static function getLast($session_id)
    {
        ...
        $auth             = new self();
        $auth->session_id = $session_id;
        if ($auth->loadMatchingObject('user_authentication_id DESC')) {
            return $auth;
        }
    ...
    /**
     * Checks if a CAuthenticationFactor have been set
     *
     * @return bool
     */
    function hasFactorSuccess()
    {
        if (!$this->authentication_factor_id) {
            return true;
        }

        if (CAppUI::$instance->_authentication_factor_success) {
            return true;
        }

        $factor = $this->loadRefAuthenticationFactor();

        return CAppUI::$instance->_authentication_factor_success = !($factor->validation_code);
    }
    ...
    /**
     * @param CUserAuthentication $authentication
     *
     * @return void
     * @throws Exception
     */
    public static function disconnectSimilarOnes(self $authentication): void
    {
        if ($authentication->auth_method === self::AUTH_METHOD_SUBSTITUTION) {
            return;
        }

        $ds                  = $authentication->getDS();
        ...
        $auths = $authentication->loadList($where);
