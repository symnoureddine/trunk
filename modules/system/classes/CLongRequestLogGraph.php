...
class CLongRequestLogGraph implements IShortNameAutoloadable {
  static function getDurationSeries(&$logs, &$module_actions, $threshold, $module = null, $from_date = null, $to_date = null) {
    $series = array(
      'title'   => ($module) ? CAppUI::tr("module-{$module}-court") . "<br />" . CAppUI::tr('CLongRequestLog-duration') : CAppUI::tr('CLongRequestLog-duration'),
    ...
    foreach ($logs as $_log) {
      ...
      if (isset($_log['module_action_id'])) {
        if ($module) {
          ...
          $_serie_label = CAppUI::tr("module-{$module_actions[$_log['module_action_id']]->module}-court") . " - {$module_actions[$_log['module_action_id']]->action}";
        }
      }
