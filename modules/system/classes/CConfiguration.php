...
class CConfiguration extends CMbObject {
  ...
  const GET_VALUES_CACHE_STATUS_CACHE = __CLASS__ . '::getValuesCacheStatus';
  ...
  private static $models_infos = array();
  ...
  function getProps() {
    ...
    $props['static']       = 'bool notNull default|0';
    ...
    if (isset(self::$model_raw[$module])) {
      foreach (self::$model_raw[$module] as $_inherit => $_tree) {
        ...
        if (!isset(self::$model[$module][$_inherit])) {
          self::$model[$module][$_inherit] = array();
        }
  ...
  static function getModelCacheStatus($module) {
    if (!isset(self::$models_infos[$module])) {
      self::$models_infos[$module] = SHM::get("config-module-$module");
    }
   ...
   */
  static function getModel($module, $inherits = array()) {
    if (empty(self::$model[$module])) {
      if (($model = SHM::get("config-module-$module")) && !self::_isModelCacheDirty($model["hash"], $module)) {
        ...
        SHM::put(
          ...
          array(
            ...
            "content" => (isset(self::$model[$module])) ? self::$model[$module] : array(),
          )
        );
      }
    }
    ...
    if (!empty($inherits)) {
      ...
      foreach ($inherits as $_inherit) {
        $subset[$_inherit] = isset(self::$model[$module][$_inherit]) ? self::$model[$module][$_inherit] : array();
      }

      return $subset;
    }

    return (isset(self::$model[$module])) ? self::$model[$module] : array();
  }
  ...
  static function buildAllConfig($module) {
    ...
    foreach ($inherits as $_inherit) {
      ...
      foreach ($all as $_object) {
        ...
        $values_flat[$_guid] = array_merge($values_flat[$_guid], self::getConfigs($_inherit, $module, null, $_object));
      }
    }
  ...
  static function refreshDataCache($module) {
    ...
    if (self::getValuesCacheStatus($module) === self::STATUS_OK) {
      ...
      $hosts_shm = SHM::get("config-values-$module-__HOSTS__");
      ...
      foreach ($hosts as $_host) {
        $_host_value    = SHM::get("config-values-$module-$_host");
    ...
    foreach (self::$values[$module] as $_host => $_configs) {
      SHM::put("config-values-$module-$_host", array('content' => $_configs));
    }
    ...
    mbLog(sprintf("'config-values-%s-*' generated in %f ms, written in %f ms", $module, $t1 * 1000, $t2 * 1000));
  ...
  static function clearDataCache($module = null) {
    if (!$module) {
      SHM::remKeys("config-*");
      SHM::remKeys('CConfiguration::getValuesCacheStatus-*');
      self::$values = array();
    }
    else {
      if (is_array($module)) {
  ...
  protected static function clearModuleCache($module) {
    SHM::rem("config-module-$module");
    SHM::remKeys("config-values-$module-*");
    self::$values[$module] = array();
  ...
  static function getValues($object_guid, $module) {
    ...
    if (empty(self::$hosts)) {
      $hosts = SHM::get("config-values-$module-__HOSTS__");
    ...
    if (empty(self::$values[$module][$object_guid])) {
      $_values = SHM::get("config-values-$module-$object_guid");
      ...
      if (empty($_values)) {
        ...
        if (!isset(self::$values[$module][$object_guid])) {
          self::$values[$module][$object_guid] = array();
        }
        self::$values[$module][$object_guid] = $_values['content'];
      }
    }

    return isset(self::$values[$module][$object_guid]) ? self::$values[$module][$object_guid] : array();
  }
  ...
  static function getValue($object_guid, $feature) {
    ...
    $cache = new Cache("config-value-{$feature}", $object_guid, Cache::INNER);
    $value = $cache->get();
  ...
  static function getValuesCacheStatus($module) {
    $cache = new Cache(self::GET_VALUES_CACHE_STATUS_CACHE, func_get_args(), Cache::INNER_OUTER, 60);
    if ($status = $cache->get()) {
      return $status;
    }
   ...
   * @throws Exception
   */
  static function setConfig($feature, $value, CMbObject $object = null, IConfigurationStrategy $strategy = null, bool $static = false) {
   ...
   * @throws Exception
   */
  static function setConfigs($configs, CMbObject $object = null, IConfigurationStrategy $strategy = null, bool $static = false) {
    $messages = [];
