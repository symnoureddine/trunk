...
abstract class CObjectPurger implements IShortNameAutoloadable
{
    ...
    public function purgeObjects($start = 0, $step = 10, $max_id = null): ?array
    {
        ...
        if ($messages) {
            CApp::log(CAppUI::tr("system-purge-error|pl") . ' / ' . $duration . 's', $messages, CLogger::LEVEL_DEBUG);
        }

        if ($purge) {
            CApp::log(CAppUI::tr("system-purged-object|pl") . ' / ' . $duration . 's', $purge, CLogger::LEVEL_DEBUG);
        }
