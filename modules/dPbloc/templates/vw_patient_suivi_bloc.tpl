  ...
  {{if $type_cell}}
    <div class="suivi_evolve" onclick="{{if $type_cell == 'interv'}}reloadTimingsSuivi('{{$operation->_id}}');{{elseif $type_cell == 'sspi' || $type_cell == "sspi_attente"}}reloadTimingsSSPISuivi('{{$operation->_id}}');{{/if}}">
      {{if $type_cell == "brancardage"}}
        {{assign var=item_brancardage value=$operation->_ref_brancardage->_ref_first_item}}
        <span class="suivi_case brancard {{if $item_brancardage->patient_pret}}suivi_green{{else}}suivi_white{{/if}}" title="{{tr}}CBrancardageItem-patient_pret{{/tr}}"></span>
        <span class="suivi_case brancard {{if $item_brancardage->demande_brancard}}suivi_green{{else}}suivi_white{{/if}}" title="{{tr}}CBrancardageItem-demande_brancard{{/tr}}"></span>
        <span class="suivi_case brancard {{if $item_brancardage->prise_en_charge}}suivi_green{{else}}suivi_white{{/if}}" title="{{tr}}CBrancardageItem-prise_en_charge{{/tr}}"></span>
        <span class="suivi_case brancard {{if $item_brancardage->arrivee}}suivi_green{{else}}suivi_white{{/if}}" title="{{tr}}CBrancardageItem-arrivee{{/tr}}"></span>
