...
class CPlageOp extends CPlageHoraire {
  /** @var string */
  public const RESOURCE_NAME = 'plageop';
  ...
  public function getApiLink(): string {
    return CRouter::getInstance()
      ->generate('bloc_plageop', ["plageop_id" => $this->_id]);
  }
  ...
  function loadRefOriginalOwner() {
    return $this->_ref_original_owner = $this->original_owner_id ? $this->loadRefOriginalChir() : $this->loadRefOriginalSpec();
  }
  ...
  function check() {
    ...
    if ($this->fieldAltered('status') && count($old->_ref_operations) && $this->countOperationsAnnulees() != count($old->_ref_operations)) {
  ...
  function updateFormFields() {
    ...
    if (in_array($this->status, array("free", "deleted"))) {
  ...
  function loadForDays($prat_id, $date_min, $date_max = null) {
    ...
    $where[] = "chir_id $wherePrat OR anesth_id $wherePrat OR
                ...
                (chir_id IS NULL AND spec_id IS NULL AND urgence = '1' AND salle_id " . CSQLDataSource::prepareIn($salle_ids) . ")";
  ...
  static function findPrevOp(&$op, $plage, $seconde_plage, &$new_time, $add_time = true) {
    ...
    if ($prev_op_seconde_plage->loadObject($where, "time_operation DESC")) {
      if ($add_time) {
        $op->time_operation = CMbDT::addTime($prev_op_seconde_plage->temp_operation, $prev_op_seconde_plage->time_operation);
   ...
   */
  public function getResourceSalle():? CItem {
    if (!$salle = $this->loadRefSalle()) {
      return null;
    }

    return new CItem($salle);
  }
   ...
   * @throws \Ox\Core\Api\Exceptions\CApiException
   */
  public function getResourceBloc():? CItem {
    if (!$salle = $this->loadRefSalle()) {
      return null;
    }
