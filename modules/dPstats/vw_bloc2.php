...
if ($operation_ids) {
  ...
  $operations = $operation->loadGroupList(array('operation_id' => CSQLDataSource::prepareIn($operation_ids)));
  ...
  $blocs = CGroups::loadCurrent()->loadBlocs(PERM_READ, true, "nom", array("actif" => "= '1'"), array("actif" => "= '1'"));
...
if ($mode == "csv") {
  ...
  $csv     = new CCSVFile();
  $title   = array("Date", "Salle prévue", "Salle réelle",
    "Date début vacation", "Heure début vacation",
    "Date fin vacation", "Heure fin vacation",
    "N° d'ordre prévu", CAppUI::tr('COperation-time_operation'),
    "N° d'ordre réel", CAppUI::tr('COperation-entree_salle'),
    "IPP", "Patient",
    "Âge", "Poids", "Taille", "NDA", "Prise en charge",
    "Date entrée prévue", "Heure entrée prévue",
    "Date entrée réelle", "Heure entrée réelle",
    "Date sortie prévue", "Heure sortie prévue",
    "Date sortie réelle", "Heure sortie réelle",
    "Durée séjour", "Chirurgien", "Anesthésiste", "Libellé", "DP",
    "CCAM", "Type d'anesthésie", "Côté opéré", "Code ASA",
    "Date placement programme", "Heure placement programme",
    "Date entrée bloc", "Heure entrée bloc",
    "Date entrée salle", "Heure entrée salle",
    "Date début d'induction", "Heure début d'induction",
    "Date fin d'induction", "Heure fin d'induction",
    "Date début d'intervention", "Heure début d'intervention",
    "Date fin d'intervention", "Heure fin d'intervention",
    "Date sortie salle", "Heure sortie salle",
    "patient suivant",
    "Date entrée reveil", "Heure entrée reveil",
    "Date sortie reveil", "Heure sortie reveil");
  ...
  foreach ($operations as $_operation) {
    ...
    $line_op = array(CMbDT::date($_operation->_datetime), $_operation->_ref_salle_prevue, $_operation->_ref_salle_reelle,
      $_operation->_ref_plageop->date, $_operation->_deb_plage,
      $_operation->_ref_plageop->date, $_operation->_fin_plage,
      $_operation->rank,$_operation->time_operation, $_operation->_rank_reel,$_operation->entree_salle,
      $_patient->_IPP, $_patient->_view,
      $_patient->_annees, $_patient->_poids, $_patient->_taille, $_sejour->_NDA, $_sejour->type,
      ...
      $_sejour->_duree_reelle, $_operation->_ref_chir->_view, $_operation->_ref_anesth->_view, $_operation->libelle,
      $_sejour->DP, $_operation->codes_ccam, $_operation->_lu_type_anesth,
      CAppUI::tr("COperation.cote.$_operation->cote"), $_operation->ASA,
