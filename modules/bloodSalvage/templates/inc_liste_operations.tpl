...
{{foreach from=$operations item=_operation}}
    ...
    {{else}}
      ...
      {{if !$vueReduite}}
        <td>
          ...
          <a href="?m=planningOp&tab={{$direction}}&operation_id={{$_operation->_id}}" title="Modifier l'intervention">
  ...
  {{if $conf.dPsalleOp.COperation.modif_salle && !$_operation->_deplacee}}
    <tr>
      <td colspan="5">
