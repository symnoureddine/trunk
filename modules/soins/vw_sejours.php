...
if (!isset($sejours)) {
  ...
  if ($service_id != "NP" || count($services_ids)) {
    ...
    if ($praticien_id && $only_non_checked) {
      ...
      $sejours = isset($sejours) ? $sejours : array();
      if ($service_id || count($services_ids) || $praticien_id || $function_id || $discipline_id) {
        $affectation  = new CAffectation();
        $affectations = $affectation->loadList($where, $order, null, "affectation.sejour_id", $ljoin, null, null, false);
...
$where_affectation = array();

if ($validation_pharm) {
  $count_validation_pharma = array();
}
...
foreach ($sejours as $sejour) {
  ...
  if ($validation_pharm) {
    ...
    $count_validation_pharma[$prescription->_id] = array(
      "valide" => 0,
      "count"  => $nb_lines
    );
    foreach (array($prescription->_ref_prescription_lines, $prescription->_ref_prescription_line_mixes) as $_lines) {
...
if ($count_my_patient && $my_patient && ($userCourant->isSageFemme() || $userCourant->isAideSoignant() || $userCourant->isInfirmiere() || $userCourant->isKine() || $userCourant->isPraticien())) {
