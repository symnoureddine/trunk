...
<script>
  ...
  loadPrescription = function() {
    ...
    Prescription.reloadPrescSejour('{{$prescription_id}}','{{$sejour->_id}}', null, null, null, null, null, null, null, null, '', '{{$type_prescription}}');
  };
  ...
  loadGrossesse = function() {
    {{if $sejour->_ref_grossesse || $sejour->_ref_naissance}}
      ...
      url.addParam('is_tdb_maternite', '0');
      ...
      afterChange: function(container) {
        switch (container.id) {
          ...
          case 'dossier_traitement{{if $vue_condensee_dossier_soins}}_compact{{/if}}':
            Soins.loadSuiviSoins('{{$sejour->_id}}', '{{$date}}');
...
<div id="header-dossier-soins" style="position: relative;" class="me-no-align">
  ...
  <ul id="tab-sejour" class="control_tabs">
    ...
    {{/if}}
    <li>
      <a href="#constantes-medicales" {{if $sejour->_empty_anqs|@count}}class="wrong"{{/if}}>
    ...
    <li style="float: right" class="me-tabs-buttons">
      ...
      {{/if}}
      {{if "synedra"|module_active}}
        {{mb_include module=synedra template=inc_button_synedra _sejour=$sejour}}
      {{/if}}
