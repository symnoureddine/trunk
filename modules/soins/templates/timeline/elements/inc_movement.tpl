      ...
      <tr>
        <td style="width: 50%;">
          ...
          {{if $item->_ref_origine->_id}}
            <strong>{{tr}}CBrancardageItem-_origine{{/tr}} :</strong> <span onmouseover="ObjectTooltip.createEx(this, '{{$item->_ref_origine->_guid}}');">{{$item->_ref_origine}}</span>
          {{/if}}
          {{if $item->_ref_destination->_id}}
            <strong>{{tr}}CBrancardageItem-_destination{{/tr}} :</strong> <span onmouseover="ObjectTooltip.createEx(this, '{{$item->_ref_destination->_guid}}');">{{$item->_ref_destination}}</span>
          {{/if}}
        </td>
        <td>
          {{if $item->prise_en_charge}}
            <span class="timeline_description"><strong>{{tr}}CBrancardageItem-prise_en_charge{{/tr}} :</strong> {{mb_value object=$item field=prise_en_charge}}</span>
          {{/if}}
          {{if $item->depart}}
            <span class="timeline_description"><strong>{{tr}}CBrancardageItem-depart{{/tr}} :</strong> {{mb_value object=$item field=depart}}</span>
          {{/if}}
          {{if $item->arrivee}}
            <span class="timeline_description"><strong>{{tr}}CBrancardageItem-arrivee{{/tr}} :</strong> {{mb_value object=$item field=arrivee}}</span>
          {{/if}}
        </td>
      </tr>
