  ...
  <form name="filtresSoignants" method="get" action="?">
    <table class="main tbl form">
      <tr>
        <th colspan="19" class="button title">
          ...
          <span style="width: 50%; display: block">
            ...
            <button type="button" class="left notext me-tertiary me-dark" onclick="$V(this.form.date, '{{$date_before}}');"></button>
            ...
            <button type="button" class="right notext me-tertiary me-dark" onclick="$V(this.form.date, '{{$date_after}}');"></button>
            </span>
          </span>
      ...
      <tr>
        ...
        {{if $conf.ref_pays == 2}}
          <th class="category narrow">{{tr}}CSejour-back-prestations{{/tr}}</th>
        {{/if}}
        ...
        <th class="category narrow">{{tr}}CAntecedent.court{{/tr}}/{{tr}}CAntecedent.appareil.alle|pl{{/tr}}</th>
        {{if $isImedsInstalled}}
          <th class="category narrow">{{tr}}Labo{{/tr}}</th>
        {{/if}}
        <th class="category narrow"><label title="{{tr}}CPrescription-desc{{/tr}}">{{tr}}CPrescription-court{{/tr}}</label></th>
        <th class="category narrow"><label title="{{tr}}CPrescription-urgence-desc{{/tr}}">{{tr}}CPoseDispositifVasculaire-urgence-court{{/tr}}</label></th>
        {{if "soins Observations manual_alerts"|gconf}}
          <th class="category narrow"><label  title="{{tr}}CObservationMedicale{{/tr}}">{{tr}}CObservationMedicale._show_obs{{/tr}}</label></th>
        {{/if}}
          ...
          <tbody class="sejours_by_service">
            <tr>
              <th class="section" style="width:38px;">
                <input name="check_all_sejours" id="check_all_sejours" type="checkbox" onchange="PersonnelSejour.selectAllSejoursByService(this);"/>
