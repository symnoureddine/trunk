...
{{if ($service_id && $service_id != "NP") || $show_affectation || $function->_id || $discipline->_id || $praticien->_id}}
  <td class="text {{if $sejour->isolement}}isolement{{/if}} {{if !$affectation->_id}}compact{{/if}}">
    {{if !$board && (@$modules.dPplanningOp->_can->admin || ("soins UserSejour can_edit_user_sejour"|gconf && @$modules.dPplanningOp->_can->edit))}}
      <button class="mediuser_black notext" onclick="Soins.paramUserSejour('{{$affectation->sejour_id}}', '{{if $service_id != "NP" }}{{$service_id}}{{/if}}', null, '{{$date}}');"
    ...
    {{if $sejour->_ref_curr_brancardage && $sejour->_ref_curr_brancardage->_id && "brancardage General see_demande_ecap"|gconf}}
      <span id="brancard_{{$sejour->_id}}">
        <a href="#" style="display: inline" onclick="Brancardage.jumelles('{{$sejour->_guid}}');">
          <img src="images/icons/brancard_cligno.gif" title="{{tr}}CBrancardageItem.brancard_cligno{{/tr}}" />
        </a>
      </span>
    {{/if}}
...
<td class="text">
  ...
  {{/if}}
  <button class="lookup notext me-tertiary me-btn-small me-dark" style="float:right;" onclick="Soins.popEtatSejour('{{$sejour->_id}}');">
    {{tr}}CSejour-_etat{{/tr}}
  </button>
    ...
    <span class="compact" style="float:right">
      ...
      {{if $grossesse && $grossesse->_id && $grossesse->active}}
        <img onmouseover="ObjectTooltip.createEx(this, '{{$patient->_ref_last_grossesse->_guid}}')"
             src="style/mediboard_ext/images/icons/grossesse.png" style="background-color: rgb(255, 215, 247);"/>
      {{/if}}
  ...
  {{mb_include module=ssr template=inc_view_patient onclick="showDossierSoins('`$sejour->_id`','$date', '$default_tab');" _sejour=$sejour bebe_indicator=true}}
...
{{if $conf.ref_pays == 2}}
<td>
  {{mb_include module=admissions template=inc_form_prestations sejour=$sejour edit='Ox\Core\Module\CModule::getCanDo'|static_call:"dPadmissions"->edit}}
</td>
{{/if}}
