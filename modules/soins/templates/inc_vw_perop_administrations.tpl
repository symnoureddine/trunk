...
<table class="tbl">
  <tr>
    <td colspan="6">
      <button type="button" class="print" onclick="SalleOp.printFicheBloc($V('print_fiche_bloc'));">
          {{tr}}COperation-action-Block sheet{{/tr}}
      </button>
      <button class="print not-printable me-tertiary me-dark"
              onclick="SurveillancePerop.printSurveillance($V('print_fiche_bloc'));">
          {{tr}}CSupervisionGraphToPack-action-Print monitoring{{/tr}}
      </button>
      <select id="print_fiche_bloc" {{if 1 === $operations|@count}}class="me-no-display"{{/if}}>
        {{foreach from=$operations name=sejour_operations item=operation}}
          <option {{if $smarty.foreach.sejour_operations.last}}selected{{/if}} value="{{$operation->_id}}">
            {{$operation}}
          </option>
        {{/foreach}}
      </select>
        ...
        <form name="editPrefGeste" method="post">
          <label>
            <input type="checkbox" {{if $show_administrations}}checked{{/if}}
                   onclick="PlanSoins.showPeropAdministrations('{{$prescription_id}}', '{{$last_operation->sejour_id}}', this.checked ? 1 : 0);"/>
