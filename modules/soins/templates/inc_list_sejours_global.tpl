...
<table class="main tbl me-no-align me-small">
    ...
    <tr class="not-printable">
      <td style="vertical-align: middle;" class="button">
        <form name="addInputArea" method="get" style="clear:both;float: left;">
          <label><input type="checkbox" name="show_textarea" onclick="$$('tbody.input_area').invoke('toggle')" /></label>
  ...
  {{foreach from=$sejours item=sejour}}
    ...
    {{else}}
      <tr id="line_sejour_{{$sejour->_id}}" {{if $sejour->_ref_curr_affectation->_in_permission}}class="opacity-50"{{/if}}>
  ...
  <tr>
    <th class="title" colspan="16" {{if $print}}onclick="window.print();"{{/if}}>
      ...
      {{if !"soins Sejour select_services_ids"|gconf && $service_id && $service_id != "NP" && (@$modules.dPplanningOp->_can->admin || ("soins UserSejour can_edit_user_sejour"|gconf && @$modules.dPplanningOp->_can->edit))}}
        ...
        <button type="button" class="mediuser_black notext" onclick="Soins.reponsableJour('{{$date}}', '{{$service_id}}', 'TypeHospi');"
  ...
  {{if !$print}}
    <tr>
      ...
      <th colspan="2" rowspan="2" style="width: 20%">
        <input type="text" size="3" onkeyup="Soins.filterFullName(this);" id="filter-patient-name" style="float: right;" />
      ...
      {{/if}}
      <th colspan="{{if $board}}2{{elseif "dPprescription"|module_active}}5{{else}}3{{/if}}" class="narrow">Alertes</th>
      ...
      {{if $conf.ref_pays == 2}}
        <th rowspan="2">{{tr}}CGroups-back-prestations{{/tr}}</th>
      {{/if}}
      {{if !$board}}
    ...
    <tr>
      {{if !$board}}
        {{if "dPprescription"|module_active}}
          ...
          <th><label title="Prescriptions urgentes">{{tr}}CPoseDispositifVasculaire-urgence-court{{/tr}}</label></th>
        {{/if}}
