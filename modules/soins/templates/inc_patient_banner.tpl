...
{{/if}}
{{assign var=isPSL value=0}}
{{if $prescription}}
  {{foreach from=$prescription->_ref_prescription_lines_element item=_prescription}}
    {{if $_prescription->info_psl_id}}
      {{assign var=isPSL value=1}}
    {{/if}}
  {{/foreach}}
{{/if}}
...
<table class="tbl me-no-align me-no-border-radius-bottom me-patient-banner me-variante">
  <tr>
    ...
    <th class="title text" style="border: none; {{$background_color}}">
      ...
      <form name="actionPat" action="?" method="get">
        ...
        <h2 class="title">
          ...
          {{if $is_sejour && $sejour->_covid_diag}}
            <span class="texticon texticon-stup" title="{{$sejour->_covid_diag->libelle}}" style="font-size: 10pt;">
          ...
          {{if $is_sejour}}
            {{if "maternite"|module_active}}
              {{if $object->grossesse_id}}
                {{if $object->_ref_patient->_ref_last_grossesse && $object->_ref_patient->_ref_last_grossesse->_id && !$object->_ref_patient->_ref_last_grossesse->datetime_cloture && $object->_ref_patient->_ref_last_grossesse->active}}
                  <span class="texticon texticon-grossesse" onmouseover="ObjectTooltip.createEx(this, '{{$object->_ref_patient->_ref_last_grossesse->_guid}}')">
                    {{tr}}CGrossesse-in_progress{{/tr}}
                  </span>
                {{/if}}
              {{/if}}

              {{if $object->_ref_patient->civilite == "enf" && $object->_ref_naissance->_id}}
                {{assign var=grossesse_maman value=$object->_ref_naissance->_ref_grossesse}}

                {{if $grossesse_maman && $grossesse_maman->_id}}
                  <span style="font-size: 0.7em;">
                   - {{mb_value object=$grossesse_maman field=_semaine_grossesse}} <span title="{{tr}}CConsultation-Term of mom s pregnancy{{/tr}}">{{tr}}CGrossesse-_semaine_grossesse-court{{/tr}}</span>
              ...
              {{if $pregnancy && $pregnancy->_id}}
                <span style="font-size: 0.8em;">
                  -
                  {{mb_value object=$pregnancy field=_semaine_grossesse}} <span title="{{tr}}CGrossesse-_semaine_grossesse-desc{{/tr}}">{{tr}}CGrossesse-_semaine_grossesse-court{{/tr}}
            ...
            {{if $object->_class == "CSejour"}}
              <span id="sejour-ATNC" class="texticon-atnc dhe_flag_important" title="{{tr}}CSejour-ATNC-desc{{/tr}}" style={{if $object->ATNC != '1'}}"display: none;"{{else}}"font-size: 0.5em;"{{/if}}>
              {{tr}}CSejour-ATNC-court{{/tr}}</span>

              <span id="sejour-ATNC" class="texticon-atnc dhe_flag_RRAC" title="{{tr}}CSejour-RRAC-desc{{/tr}}" style={{if $object->RRAC != '1'}}"display: none;"{{else}}"font-size: 0.5em;"{{/if}}>
              {{tr}}CSejour-RRAC-court{{/tr}}</span>

              {{if isset($late_objectifs|smarty:nodefaults) && $late_objectifs|@count}}
                <span id="sejour-late_objectif" class="texticon texticon-lateObjectifSoin text" onmouseover="ObjectTooltip.createDOM(this, 'tooltip-ObjectifSoin');">{{tr}}CObjectifSoin{{/tr}}</span>
                {{*Permet d'afficher la tooltip contenant la liste des late_objectifs*}}
                <div id="tooltip-ObjectifSoin" style="display:none">{{mb_include template=inc_vw_list_objectifs_soins listObjectifsSoins=$late_objectifs}}</div>
              {{/if}}
            ...
            {{if "nouveal"|module_active && "nouveal general active_prm"|gconf}}
              {{mb_include module=nouveal template=inc_vw_etat_patient sejour=$object}}
            {{/if}}
            <br/>
            {{if $object->rques }}
              <i class="fa fa-exclamation-triangle inc_banner_patient_rques" title="{{$object->rques}}"></i>
            {{/if}}
            ...
            <span style="font-size: 0.6em;">
              ...
              {{if $conf.ref_pays == "2" && $object->canEdit()}}
                {{mb_script module=planningOp script=sejour ajax=true}}
                <button type="button" class="edit notext me-tertiary me-dark" onclick="Sejour.editMotif('{{$object->_id}}');">
                  {{tr}}CSejour-libelle-edit{{/tr}}
                </button>
              {{/if}}
              ...
              {{foreach from=$object->_jour_op item=_info_jour_op}}
                {{if $nb_days_hide_op == 0 || $nb_days_hide_op > $_info_jour_op.jour_op}}
                  {{if $_info_jour_op.rques }}
                    <i class="fa fa-exclamation-triangle inc_banner_patient_rques" title="{{$_info_jour_op.rques}}"></i>
                  {{/if}}
          ...
          {{if $nda_view == true}}
            <span class="me-color-white-medium-emphasis me-no-convert-dark" style="font-size: 0.7em; margin-left: 10px;">[{{$sejour->_NDA_view}}]</span>
          {{/if}}

          {{if $is_sejour && $object->circuit_ambu}}
            <span id="sejour_circuit_ambu" class="texticon-atnc dhe_flag_circuit_ambu" title="{{tr}}CSejour-circuit_ambu-desc{{/tr}}" style="font-size: 0.6em;">
      ...
      {{if $isPSL}}
        <div style="text-align: right" class="me-infos">
          <span style="margin-right: 1em">
            {{if $dossier_medical->groupe_sanguin !="?" && $dossier_medical->groupe_sanguin}}
              <strong>{{mb_label object=$dossier_medical field=groupe_sanguin}} : </strong>
              {{mb_value object=$dossier_medical field=groupe_sanguin}}
              {{if $dossier_medical->rhesus == "NEG"}}
                -
              {{elseif $dossier_medical->rhesus == "POS"}}
                +
              {{/if}}
            {{/if}}
          </span>
          {{if $dossier_medical->phenotype}}
            <strong>{{mb_label object=$dossier_medical field=phenotype}} : </strong>
            {{mb_value object=$dossier_medical field=phenotype}}
          {{/if}}
        </div>
      {{/if}}
    </th>
    <th class="title text me-patient-banner-buttons" style="{{if !$see_modif_patient}}display:none;{{/if}}text-align: right; border: none; width: 10%; {{$background_color}}">
      <div style="height: 20px;">
        ...
        <a href="#print-{{$patient->_guid}}" onclick="Patient.print('{{$patient->_id}}')" class="not-printable" style="float: right;">
      ...
      <div style="height: 20px;">
        {{if "forms"|module_active}}
          <a class="not-printable fas fa-list me-color-white me-no-convert-dark" style="float: right; color: black; cursor: pointer;"
                  title="{{tr}}CExClass|pl{{/tr}}"
                  onclick="ExObject.loadExObjects('{{$sejour->_class}}', '{{$sejour->_id}}', '{{$unique_id_widget_forms}}_modal', 0);
                    Modal.open('{{$unique_id_widget_forms}}_modal', {showClose: true});">
          </a>
        {{/if}}

        {{if $can->edit}}
          <a href="#edit-{{$patient->_guid}}" onclick="Patient.edit('{{$patient->_id}}')" class="not-printable" style="float: right;">
      ...
      {{if $app->user_prefs.vCardExport}}
        <a href="#export-{{$patient->_guid}}" onclick="Patient.exportVcard('{{$patient->_id}}')" class="not-printable">
      ...
      {{if $object|instanceof:'Ox\Mediboard\PlanningOp\CSejour' && "trajectoire"|module_active}}
          <script>
            Main.add(function() {
              ...
              url.requestUpdate('trajectoire_button', function() {$('trajectoire_button').show();});
            });
          </script>
