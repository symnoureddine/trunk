...
{{foreach from=$sejours item=_sejour}}
  ...
  <tr style="height: 38px;">
    ...
    <td>
      <span style="position: relative;">
        ...
        {{if $_sejour->_ref_users_sejour|@count}}
          <span class="countertip" style="margin-top: -4px;margin-left: -9px;">{{$_sejour->_ref_users_sejour|@count}}</span>
        {{/if}}
      </span>
    </td>
    <td>
              ...
              <div style="display: none" id="horaires_user_sejour_{{$_user_sejour->_id}}">
                ...
                <ul>
                  {{foreach from=$_user_sejour->_affectations item=_affectation}}
                    <li>
                      Du {{$_affectation->debut|date_format:$conf.datetime}} au {{$_affectation->fin|date_format:$conf.datetime}}
                    </li>
                  {{/foreach}}
                </ul>
              </div>
    ...
    <td {{if $_sejour->sortie_reelle}}class="hatching"{{/if}}>
      <a href="#{{$patient->_guid}}" style="display:inline-block" onclick="showDossierSoins('{{$_sejour->_id}}','{{$date}}');">
        <span onmouseover="ObjectTooltip.createEx(this, '{{$patient->_guid}}');">
          {{$patient->_view}}
        </span>
      </a>
      <button type="button" class="lookup notext me-tertiary" style="display:inline-block;float:right" onclick="popEtatSejour('{{$_sejour->_id}}');">
    ...
    {{if $conf.ref_pays == 2}}
      <td style="text-align: center;">
        {{mb_include module=admissions template=inc_form_prestations
          sejour=$_sejour edit='Ox\Core\Module\CModule::getCanDo'|static_call:"dPadmissions"->edit view="compact"}}
      </td>
    {{/if}}
    <td class="text">
    ...
    <td style="text-align: center;">
      <span onmouseover="ObjectTooltip.createEx(this, '{{$_sejour->_guid}}');">
        <strong style="font-size: 1.1em;">{{mb_value object=$_sejour field=entree_reelle format=$conf.time}}</strong><br/>
    ...
    <td style="text-align: center;">
      {{if $_sejour->_count_tasks}}
        ...
        <div id="tooltip-content-tasks-{{$_sejour->_id}}" style="display: none; height: 400px; width: 400px:"></div>
      {{/if}}
      ...
      {{if $_sejour->_count_tasks_not_created}}
        ...
        <div id="tooltip-content-tasks-not-created-{{$_sejour->_id}}" style="display: none; height: 400px; width: 400px:"></div>
      {{/if}}
    </td>
    <td>
