...
if ($offline && CModule::getActive("forms")) {
  ...
  try {
    ...
    $formulaires = null;
    CApp::log($e->getMessage(), $e, CLogger::LEVEL_WARNING);
  }
}
...
// Preop
    if ($operation->graph_pack_preop_id) {
      [$perop_datetime_min, $perop_datetime_max] = CObservationResultSet::getTimingsByType($operation, "preop");
...
// Perop
    if ($operation->graph_pack_id) {
      [$perop_datetime_min, $perop_datetime_max] = CObservationResultSet::getTimingsByType($operation, "perop");
...
// SSPI
    if ($operation->graph_pack_sspi_id) {
      [$sspi_datetime_min, $sspi_datetime_max] = CObservationResultSet::getTimingsByType($operation, "sspi");
