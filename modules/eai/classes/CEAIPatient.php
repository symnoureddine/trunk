...
class CEAIPatient extends CEAIMbObject {
  private const MODE_OBTENTION = 'interop';
  ...
  static function storeIPP(CIdSante400 $IPP, CPatient $patient, CInteropSender $sender) {
    ...
    if ($sender->_configs && !$group->_configs["sip_idex_generator"]) {
      ...
      if (!$IPP->id400) {
        ...
        if ($incrementer && $incrementer->manage_range && !CMbRange::in($IPP->id400, $incrementer->range_min, $incrementer->range_max)) {
   ...
   * @param CInteropSender $sender    Sender
   *
   * @return null|string null if successful otbug: création d'un idex sans tag si on a pas de domaine associé au senderherwise returns and error message
   */
   ...
   * @return void
   */
  static function storeOtherIdentifiers($identifiers, CPatient $patient) {
   ...
   * @return null|string null if successful otherwise returns and error message
   */ 
  static function storePatient(CPatient $newPatient, CInteropSender $sender, $generateIPP = false, $data = array()) {
    // Notifier les autres destinataires autre que le sender
    $newPatient->_eai_sender_guid = $sender->_guid;
    $newPatient->_generate_IPP    = $generateIPP;

    // Mode d'obtention
    $newPatient->_mode_obtention = self::MODE_OBTENTION;
