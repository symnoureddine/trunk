...
ExchangeDataFormat = window.ExchangeDataFormat || {
  ...
  refreshExchangesList : function(form) {
    new Url("eai", "ajax_refresh_echanges_data_format_list")
      .addFormData(form)
  ...
  getInfoMessageHL7 : function (exchange_guid) {
    new Url("eai", "ajax_get_info_message_hl7")
      .addParam("exchange_guid", exchange_guid)
      .requestUpdate("msg-message-edit-message");
    return false;
  },
  ...
  refreshActiveMessageSupported : function(message_supported_id, family_name, category_name, uid) {
  ...
  fillMessageSupportedID : function(uid, message_supported_id) {
    var form = getForm('editActorMessageSupported-'+uid);
    $V(form.message_supported_id, message_supported_id);

    ExchangeDataFormat.refreshActiveMessageSupported(message_supported_id, $V(form.profil), $V(form.transaction), uid);
  },
