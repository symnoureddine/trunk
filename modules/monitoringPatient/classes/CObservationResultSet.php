...
class CObservationResultSet extends CMbObject
{
    ...
    function getProps()
    {
        ...
        $props["patient_id"]    = "ref notNull class|CPatient back|patient_observation_result_sets";
        $props["datetime"]      = "dateTime notNull";
        $props["context_class"] = "str notNull";
        $props["context_id"]    = "ref class|CMbObject meta|context_class back|observation_result_sets";
        $props["sender_id"]     = "ref class|CInteropActor meta|sender_class back|observation_result_set";
        $props["sender_class"]  = "str";
     ...
     * @param bool $cache Utilisation du cache
     *
     * @return CStoredObject|CInteropSender
     * @throws Exception
     */
    function loadRefSender($cache = true)
    {
        return $this->_ref_sender = $this->loadFwdRef("sender_id", $cache);
    }
     ...
     * Load results
     *
     * @return CObservationResult[]|CStoredObject[]
     * @throws Exception
     */
     ...
     * @param dateTime  $datetime_min Datetime min
     * @param dateTime  $datetime_max Datetime max
    ...
    static function getResultsFor(CMbObject $object, $datetime_min = null, $datetime_max = null, $print = false)
    {
        ...
        foreach ($results as $_result) {
            ...
            $data[$_result["value_type_id"]][$unit_id][] = [
                0           => $_time,
                1           => $float_value,
                "ts"        => $_time,
                "value"     => $_result["value"],
        ...
        if ($sejour->grossesse_id) {
            ...
            if ($type === "sspi") {
                ...
                if ($grossesse->datetime_accouchement) {
                    $time_fin_op_iso = !$print ? $grossesse->datetime_accouchement : CMbDT::dateTime("+6 HOUR", $grossesse->datetime_accouchement);
                } else {
                    $time_fin_op_iso = !$print ? CMbDT::dateTime() : CMbDT::dateTime("+6 HOUR", $time_debut_op_iso);
                }
            }
    ...
    static function buildGraphs(
        ...
        foreach ($graph_links as $key_gl => $_gl) {
            ...
            if ($_go instanceof CSupervisionGraph) {
                ...
                $_go->buildTimeline($times, $results, $time_debut_op_iso, $time_fin_op_iso, $interv, $type, $_gl->pack_id);
                $graphs[] = $_go;
            }
        }
    ...
    static function getChronological(
        ...
        /** @var self[] $list */
        $list = (new self())->loadList($where, $order);
        $grid = [];
            ...
            foreach ($graphs as $_graph) {
                if ($_graph instanceof CSupervisionGraph) {
                    ...
                    foreach ($_axes as $_axis) {
                        ...
                        foreach ($_series as $_serie) {
                            foreach ($results as $_result) {
                                if ($_result->value_type_id == $_serie->value_type_id
                    ...
                    foreach ($results as $_result) {
                        if ($_result->value_type_id == $_graph->value_type_id
    ...
    static function buildEventsGrid(
        ...
        if (!$element_main || $element_main == "supervision-timeline-geste") {
            ...
            foreach ($anesths_perop as $_perop) {
                ...
                $_content = sprintf(
                    '<span onmouseover="ObjectTooltip.createEx(this, \'%s\');">%s</span>',
                    $_perop->_guid,
                ...
                if ($_perop->commentaire) {
                    $comment = sprintf('<div class="empty text">%s</div>', $_perop->commentaire);
                }
        ...
        if (CModule::getActive("dPprescription") && CPrescription::isPlanSoinsActive() &&
            ...
            $owner_crc    = CMedicament::getHash(CGroups::loadCurrent()->_guid);

            if ($prescription->_id) {
                $lines = $prescription->loadPeropLines($interv->_id, false, $datetime_min, $datetime_max, $print);
                ...
                foreach ($lines as $_line_array) {
                    ...
                    if ($_line instanceof CPrescriptionLineMix) {
                        ...
                        if ($_line->continuite == "discontinue") {
                            ...
                            foreach ($_line->_variations as $_datetime => $_variations_line) {
                                ...
                                foreach ($_variations_line as $_time => $_variation) {
                                    $date_time = CMbDT::date($_datetime) . " " . ($_save_hour ?: $_time);

                                    $datetime_fin = CMbDT::dateTime(
                                        "+" . (intval($_variation['pourcentage'] * 60 / 100)) . " minutes",
                                        $date_time
                                    );

                                    if ($_save_date_time) {
                                        $_variations[1][$_save_date_time]["fin"] = $date_time;
                                    }
                                    ...
                                    $_save_hour      = CMbDT::time($datetime_fin);
                        ...
                        foreach ($_line->_ref_lines as $_line_mix_item) {
                            ...
                            $ratio = null;
                            if (($produit->_ratio_microg || $produit->_ratio_mg || $produit->_ratio_UI) && !$is_B05) {
                                if ($produit->_ratio_microg) {
                            ...
                            $first_adm = reset($_line_mix_item->_ref_administrations);
                            ...
                            foreach ($_line_mix_item->_ref_administrations as $_adm) {
                                if ($_adm->dateTime >= $datetime_min && $_adm->dateTime <= $datetime_max) {
                                    $period_ok = true;
                                } else {
                                    $period_ok = false;
                                }
                                ...
                                if ($ratio) {
                                    $_adm->_quantite_massique = CMbMath::roundSig($quantite / $ratio, 2);
                                }
                                ...
                                $unit = $_adm->_ref_object->_unite_reference_libelle;

                                $cumul_massique = "";
                                if ($_adm->_quantite_massique && $unit != "mg") {
                                    $cumul_massique = CAppUI::tr(
                                            "CPrescriptionLineMix-whether"
                                        ) . " " . $quantity_total["cumul_massique"][$_line->_id] . " " . $_line_mix_item->_unite_massique;
                                }

                                $counter_adm_mix++;

                                $total_cumul['mix'][$_line->_id]                = $quantity_total["cumul"][$_line->_id] . " " . $unit . " " . $cumul_massique;
                                ...
                                // Show in the charts
                                if ($readonly) {
                                    $_content = "<strong>$quantite $unit</strong>";
                                } else {
                                    $_content = sprintf(
                                        '<strong><span onmouseover="ObjectTooltip.createEx(this, \'%s\');">%s</span></strong>',
                                        $_adm->_guid,
                                        ($_adm->_cumul_massique && !$qte_lt && !$unite_lt) ?
                                ...
                                $_item["period_datetime_max"] = $datetime_max;
                                $_item["line_id"]             = $_line->_id;
                            ...
                            if (!$print) {
                                ...
                                foreach ($_planifications_mix as $_planification) {
                                    ...
                                    if (isset($orig_dates_mix[$_planification->dateTime])) {
                                    ...
                                    $store_administration = true;
                                    $period_ok            = true;

                                    if ($_line->conditionnel) {
                                        if (!count($_line->_ref_segments)) {
                                            $store_administration = false;
                                            $period_ok            = false;
                                        }

                                        foreach ($_line->_ref_segments as $_segment) {
                                            $fin = $_segment->fin ?: $sejour->sortie;
                                            if ($_planification->dateTime < $_segment->debut || $_planification->dateTime > $fin) {
                                                $store_administration = false;
                                                $period_ok            = false;
                                            }
                                        }
                    ...
                    if ($_line->conditionnel) {
                        $class_bg_conditionnelle = "bg_conditionnel";
                    ...
                    if (!isset($groups[$_subkey])) {
                        ...
                        if ($_line instanceof CPrescriptionLineMix) {
                            if ($readonly) {
                                ...
                                $_content = $smarty->fetch(
                                    "perop/inc_timeline_group_CPrescriptionLineMix.tpl",
                                    '',
                                    '',
                                    0
                                );
                            ...
                            if ($pack->planif_display_mode === "token" &&
                                ($_line instanceof CPrescriptionLineMedicament ||
                                    $_line instanceof CPrescriptionLineElement)
                                && ($_line->signee || (!CAppUI::gconf(
                                            "dPprescription general signature_perop_obligatoire"
                                        ) && $current_user->isIADE()))
                                && !$print
                            ) {
                            ...
                            if ($pack->planif_display_mode === "in_place"
                                ...
                                foreach ($_planifications as $_planification) {
                                    ...
                                    if ($_planification->dateTime >= $datetime_min && $_planification->dateTime <= $datetime_max) {
                                        $period_ok = true;
                                    } else {
                                        $period_ok = false;
                                    }
                            ...
                            $smarty->assign("can_adm", CPrescription::canAdmPerop());
                            $smarty->assign("readonly", $readonly);
                            $_content = $smarty->fetch("perop/inc_timeline_group_{$_line->_class}.tpl", '', '', 0);
                        }
                        ...
                        $classname_sup = "$class_bg_conditionnelle $class_bg_chevron $status_line";
        ...
        if ($sejour->grossesse_id) {
            ...
            if ($type === "sspi") {
                ...
                if ($grossesse->datetime_debut_travail) {
                    ...
                    $datetime_min = self::getMinTiming($interv, $min_fields, CMbDT::dateTime(!$print ? "-1 HOUR" : null, $interv->_datetime), $type);
                }

                // Fin
                if ($grossesse->datetime_accouchement) {
                    $datetime_max = $print ? CMbDT::dateTime("+1 HOUR", $grossesse->datetime_accouchement) : $grossesse->datetime_accouchement;
                } else {
                    $datetime_max = max(CMbDT::dateTime("+4 HOUR", $interv->_datetime), CMbDT::dateTime("+4 HOUR", $datetime_min));
                }
            }
        }
