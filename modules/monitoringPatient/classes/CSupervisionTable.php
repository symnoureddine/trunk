...
class CSupervisionTable extends CSupervisionTimedEntity implements ISupervisionTimelineItem {
   ...
   * @return void
   */
  public function buildTimeline($times, $results, $start, $end, $operation = null, $type = 'perop', $pack_id) {
    if (is_string($start)) {
      ...
      if ($row->active) {
        ...
        $this->_timeline_groups[] = $group;
          ...
          foreach ($row->_data as $time => $values) {
              $content_data = '';
              if ($values['value']) {
                  $content_data = "<span onclick=\"SurveillancePerop.editObservationResultSet('{$values['set_id']}', '{$pack_id}', {$values['result_id']}, null, true, '{$type}');\" 
