...
<script>
  ...
  formTimedDataCallback = function (origin, memo, id, obj) {
    if (obj._formula_result) {
      var input = origin.up("form").elements.value;
                ...
                <td style="width: 12em;">
                  {{if $_axis->_labels|@count}}
                    ...
                    <select name="label_id"
                            onchange="$V(this.form.elements.value, this.selectedIndex ? this.options[this.selectedIndex].get('value') : ''); if (this.selectedIndex) {$V(this.form.del, 0)}">
            ...
            <td>
              ...
              {{if $_graph->type == "enum"}}
                <select name="value">
                  ...
                  {{if !$has_found_value}}
                    <option value="{{$_result->value}}" selected>{{$_result->value}}</option>
                  {{/if}}
                </select>
              {{elseif $_graph->type == "set"}}
                <script>
                  Main.add(function () {
                    ...
                    var tf = new TokenField(form.elements.value, {separator: "\n"});
                    document.on("click", ".values-{{$uid_form}}", function (e) {
                      var elt = Event.element(e);
                      tf.toggle(elt.value, elt.checked);
                      form.elements.value.onchange();
                    });
                  });
                </script>
                <input type="hidden" name="value" value="{{$_result->value}}"
              ...
              {{if $_result->value == "Oui" || $_result->value == "Non" || $_result->value == ""}}
              <input type="hidden" name="value" value="{{$_result->value}}" />

              <input type="checkbox" onclick="$V(this.form.elements.value,this.checked?'Oui':'Non')"
                     name="_value" {{if $_result->value == "Oui"}} checked {{/if}} />
              {{else}}
              <input type="text" name="value" value="{{$_result->value}}" />
              {{/if}}
              {{/if}}
            </td>
          </tr>
