...
SurveillancePerop = {
  ...
  editPeropAdministration: function (operation_id, container, line_guid, planification_systeme_id, type, datetime, administration_guid) {
    ...
    new Url('salleOp', 'ajax_vw_surveillance_perop_administration')
      ...
      .requestModal("95%", "90%", {onClose:  container ? (function (container) {
  ...
  modeGrillePerop: function (operation_id, mode, datetime, code_cis, line_guid, quantite, container, element_main) {
    ...
    new Url("prescription", "vw_mode_grille_perop")
      ...
      .requestModal("100%", "100%", {
        onClose: element_main ? (function (container) {
          container.retrieve("timeline").updateChildrenSelected(null, element_main);
  ...
  tabModeGrillePerop: function (operation_id, mode, datetime, code_cis, quantite, atc_level, letter, page, show_all_products) {
    new Url("prescription", "ajax_mode_grille_perop")
      .addParam("operation_id", operation_id)
      .addParam("quantite", quantite)
      .addParam("mode", mode)
  ...
  submitProtocoleSurvPerop: function (container) {
    var modal_perop = Modal.open('modal_apply_protocole_surv_perop', {width: 1200, height: 800, title: "Application d'un protocole"});
    modal_perop.observe("afterClose", function () {
   ...
   * @param type
   */
  editObservationResultSet: function (result_set_id, pack_id, result_id, container, element_main, type) {
    container = !container ? $$('.timeline-container-' + type)[0] : container;
    type = type == 'partogramme' ? 'perop' : type;
  ...
  addAdministration: function (line_id, datetime, line_class, quantite, unit, container, element_main) {
    new Url("planSoins", "httpreq_add_administration")
  ...
  getDateTimeElement: function (event, element, date, timeline) {
    if ($('show_timings_perop')) {
      $('show_timings_perop').remove();
    }

    var observer = function (div, action) {
      div.stopObserving(action, observer);
      div.remove();
    };

    var div_elt = DOM.div({id: "show_timings_perop", className: "tooltip tooltip-perop"}, "Date : " + date).setStyle({
    ...
    div_elt.observe('mouseover', observer.curry(div_elt, 'mouseout'));
    div_elt.observe('click', observer.curry(div_elt, 'click'));

    var element_width = div_elt.getWidth();
    var timeline_size = timeline.props.top.width;
    var pos_cursor = event.layerX;
    var pos_container = event.layerX + element_width + 3;

    if (pos_container >= timeline_size) {
      var pos_left = pos_cursor - element_width - 25;
  ...
  showPartogramme: function (operation_id, show_cormack, dossier_perinatal) {
    new Url("salleOp", "ajax_vw_surveillance_perop")
      ...
      .requestUpdate("surveillance_parto");
  },
  /**
   ...
   * @param oField
   */
  updateQuantite: function(ratio_massique, oField){
    if(!ratio_massique){
      return;
    }
    ...
    if(oField.name == "quantite"){
      var quantite_massique = ($V(oField) / ratio_massique).toFixed(2);
      $V(oForm._quantite_massique, quantite_massique, false);
    }
    if(oField.name == "_quantite_massique"){
      var quantite = ($V(oField) * ratio_massique).toFixed(5);
