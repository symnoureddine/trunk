...
SurveillanceTimelineItem = Class.create({
  ...
  initialize:   function (vis, stl, key, container, group_data, item_data, options) {
    ...
    options.template = function (item) {
      ...
      if (item.template === "perf") {
        ...
        item.data.each(function (d) {
          ...
          if (!d.ponctual) {
            content += '<div class="perf" onmousemove="if ($(\'show_timings_perop\')) { $(\'show_timings_perop\').remove();}" ' +
    ...
    timeline.on("doubleClick", function (e) {
      ...
      if (element.hasClassName('bg_conditionnel')) {
        return;
      }
      ...
      if (group_perop !== 'CAnesthPerop' && container.get("readonly") !== "1") {
        ...
        if (date.toDATETIME(true) < current_date.toDATETIME(true)) {
  ...
  eventHandler: function (item, callback, params) {
    ...
    // Period line
    if ((datetime_main < item.period_datetime_min) || (datetime_main > item.period_datetime_max)) {
      alert($T('CPrescriptionLine-msg-You must be between %s and %s to move this element', (new Date(item.period_datetime_min)).toLocaleDateTime(), (new Date(item.period_datetime_max)).toLocaleDateTime()));
