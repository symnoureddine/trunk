  ...
  initialize: function (container, readonly, type, defaultOptions, timings, display_current_time, print, frequency_automatic_graph) {
    ...
    if (!window._timelineMouseEvents) {
      ...
      document.on("mousemove", ".vis-group", function (event) {
        var element = Event.element(event);
        var timeline = SurveillanceTimelineItem.timeline;
        ...
        if (timeline) {
          ...
          var start = timeline.timeAxis.body.range.start;
          var end = timeline.timeAxis.body.range.end;
      ...
      document.on("dblclick", ".vis-group", function (event) {
        ...
        if (item) {
          ...
          if ((current_user_id == user_id) && (item.class_group === 'CSupervisionTimedData' || item.class_group === 'CSupervisionTimedPicture')) {
            SurveillancePerop.editObservationResultSet(item.set_id, item.pack_id, item.result_id, container, element_main, type);
          }
          ...
          if ((current_user_id == user_id) && target && (target.object_class === "CPrescriptionLineMedicament") && period_ok) {
