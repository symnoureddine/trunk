...
<script>
  Main.add(function() {
    InseeFields.initCPVille("group", "cp", "ville", null, "tel");
...
<form name="group" method="post" onsubmit="return checkForm(this);">
  ...
  <ul id="tabs_groups" class="control_tabs">
    ...
    <li>
      <a href="#iconographie">Iconographie</a>
    </li>
  </ul>
  ...
  <table id="identification" class="form" style="display: none;">
    ...
    {{if $conf.ref_pays == 2}}
      ...
      <tr>
        <th>{{mb_label object=$group field="ean_xml_factu"}}</th>
        <td>{{mb_field object=$group field="ean_xml_factu"}}</td>
      </tr>
      <tr>
