...
class CGroups extends CEntity implements IGeocodable {
  ...
  private static $fields_etiq = array("ETAB NOM");
   ...
   */
  function loadBlocs($permType = PERM_READ, $load_salles = true, $order = "nom", $where = array(), $whereSalle = array()) {
  ...
  static function get($group_id = null) {
    global $g;
    $group_id = $group_id ? $group_id : $g;

    $cache = new Cache(__CLASS__, '::all', Cache::INNER_OUTER, 60);
    if ($cache->exists()) {
      $groups = $cache->get();
    }
    else {
    ...
    return isset($groups[$group_id]) ? $groups[$group_id] : reset($groups);
  }
  ...
  function fillLimitedTemplate(&$template, $prefixe = null) {
    ...
    $template->addProperty($prefixe . "$etablissement_section - " . CAppUI::tr('CFunctions-adresse'), "$this->adresse \n $this->cp $this->ville");
    $template->addProperty($prefixe . "$etablissement_section - " . CAppUI::tr('CFunctions-ville'), $this->ville);
    $template->addProperty($prefixe . "$etablissement_section - " . CAppUI::tr('CFunctions-tel'), $this->getFormattedValue("tel"));
    $template->addProperty($prefixe . "$etablissement_section - " . CAppUI::tr('CGroups-fax-court'), $this->getFormattedValue("fax"));
    $template->addProperty($prefixe . "$etablissement_section - " . CAppUI::tr('CGroups-mail'), $this->getFormattedValue("mail"));
    $template->addProperty($prefixe . "$etablissement_section - " . CAppUI::tr('CGroups-mail_apicrypt'), $this->getFormattedValue("mail_apicrypt"));
    $template->addProperty($prefixe . "$etablissement_section - " . CAppUI::tr('CGroups-domiciliation'), $this->domiciliation);
    $template->addProperty($prefixe . "$etablissement_section - " . CAppUI::tr('CMbFieldSpec.siret'), $this->siret);
    $template->addProperty($prefixe . "$etablissement_section - " . CAppUI::tr('CGroups-finess-court'), $this->finess);
    ...
    $template->addBarCode($prefixe . "$etablissement_section - " . CAppUI::tr('CGroups-FINESS Bar Code'), $this->finess, $barcode);
   ...
   * @throws Exception
   */
  public function getResourceHyperTextLinks(): ?CCollection {
    if (!$hyperlinks = $this->loadBackRefs('hypertext_links')) {
      return null;
    }

    $collection = new CCollection($hyperlinks);
    $collection->setName(CHyperTextLink::RESOURCE_NAME);
