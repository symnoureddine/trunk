...
Echeance = {
  ...
  /**
   * Impression du BVR pour une échéance donnée
   *
   * @param factureClass  Classe de la facture associée
   * @param factureId     Identifiant de la facture associée
   * @param echeanceId    Identifiant de l'échéance
   */
  printBVR: function(factureClass, factureId, echeanceId) {
    var callback = Echeance.refresh
    new Url('facturation', 'ajax_edit_bvr')
      .addParam('facture_class', factureClass)
      .addParam('facture_id'   , factureId)
      .addParam('echeance_id'  , echeanceId)
      .addParam('type_pdf'     , 'bvr')
      .addParam('suppressHeaders', '1')
      .popup(1000, 600);
  },
  /**
   * Impression d'un ensemble d'échéance
   *
   * @param factureClass
   * @param factureId
   * @param echeanceId
   */
  printAllBVR: function(factureClass, factureId, echeances) {
    var callback = Echeance.refresh
    new Url('facturation', 'ajax_edit_bvr')
      .addParam('facture_class', factureClass)
      .addParam('facture_id'   , factureId)
      .addParam('echeances[]'    , echeances, true)
      .addParam('type_pdf'     , 'bvr')
      .addParam('suppressHeaders', '1')
      .popup(1000, 600);
  },
