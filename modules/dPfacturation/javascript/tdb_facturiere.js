...
TdbFacturiere = {
  ...
  /**
   * Envoi de facture multiple
   */
  multiSend: function() {
    this.getListContainer().down('.tdb-facturiere-multi-actions').down('button.send').disable = true;
    $('tdb_facturiere_' + this.selectedFactureClass + '_multi_result').update($T('Loading in progress'));
    var factures_id = [];
    this.getCheckboxes().each(
      function(input) {
        input.hide();
        if (input.checked) {
          this.getIndicatorByInput(input).show().addClassName('ajax-loading');
          factures_id.push(input.get('facture-id'));
        }
      }.bind(this)
    );
    this.sendAll(factures_id, this.refreshList());
  },

  /**
   * Envoi d'une liste de facture en XML
   *
   * @param factures_id Id des factures
   * @param callback    Script à exécuter à la fin du traitement
   */
  sendAll: function(factures_id, callback) {
    new Url('tarmed', 'ajax_send_file_http')
      .addParam('factures_id[]', factures_id)
      .addParam('facture_class', this.selectedFactureClass)
      .requestUpdate('tdb_facturiere_' + this.selectedFactureClass + '_multi_result', callback);
  },
