...
<div id="reload-{{$facture->_guid}}">
  ...
  {{if "dPfacturation CRelance use_relances"|gconf || "dPfacturation CReglement use_echeancier"|gconf || $conf.ref_pays == 2}}
    <script>
    ...
    <ul id="tabs-configure-{{$facture->_guid}}" class="control_tabs">
      ...
      {{if $conf.ref_pays == 2}}
        ...
        <li onmousedown="Facture.showFiles('{{$facture->patient_id}}', '{{$facture->_guid}}');">
          <a href="#files_facture-{{$facture->_guid}}">{{tr}}CFacture-files{{/tr}}</a>
        </li>
      {{/if}}
      <li><a href="#reglements_facture-{{$facture->_guid}}">{{tr}}CFactureEtablissement-_reglements_total{{/tr}}</a></li>
      {{if "dPfacturation CRelance use_relances"|gconf}}
      ...
      {{/if}}
      {{if $conf.ref_pays == 2 && $facture->cloture}}
        <li><a href="#avoir-{{$facture->_guid}}">{{tr}}CFactureAvoir{{/tr}}</a></li>
      {{/if}}
    </ul>
  {{/if}}
      ...
      {{if $facture && $facture->_id}}
        <legend>
          ...
          <span onmouseover="ObjectTooltip.createEx(this, '{{$facture->_guid}}')">
            {{$facture}} {{if $facture->_current_fse}}({{tr}}CConsultation-back-fses{{/tr}}: {{if $facture->_current_fse->_class == 'CPyxvitalFSE'}}{{$facture->_current_fse->facture_numero}}{{else}}{{$facture->_current_fse->numero}}{{/if}}){{/if}}
          </span>
        </legend>
              ...
              <td colspan="7">
                {{if $facture->extourne_id}}
                  <div class="small-warning">
                    ...
                    <span onmouseover="ObjectTooltip.createEx(this, '{{$facture->_ref_extourne->_guid}}')">
                    ...
                    <strong>
                      {{if $facture->extourne}}
                        ...
                        {{if $facture->_ref_new_facture}}
                          ...
                          <span onmouseover="ObjectTooltip.createEx(this, '{{$facture->_ref_new_facture->_guid}}')">
                ...
                {{foreach from=$facture->_ref_consults item=_consult}}
                  <li>
                    <span onmouseover="ObjectTooltip.createEx(this, '{{$_consult->_guid}}')">
                      <button type="button" class="edit notext me-tertiary" onclick="Consultation.editModal('{{$_consult->_id}}', 'reglement', null, function() {Facture.reloadFactureModal('{{$facture->_id}}', '{{$facture->_class}}');});">
                        {{tr}}CConsultation{{/tr}}
                      </button>
                    ...
                    {{/if}}
                    <i class="fas fa-{{$cons_class}}" title="{{tr}}CFacture-consultation-{{$cons_title}}{{/tr}}"></i>
                    {{if !$facture->cloture &&
  ...
  <!-- Documents -->
  {{if $conf.ref_pays == 2}}
    <div id="files_facture-{{$facture->_guid}}"></div>
  {{/if}}
  ...
  {{/if}}
  {{if $conf.ref_pays == 2 && $facture->cloture}}
    <div id="avoir-{{$facture->_guid}}">
      {{mb_include module=facturation template=inc_vw_avoirs object=$facture}}
    </div>
  {{/if}}
</div>
