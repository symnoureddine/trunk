                ...
                <tr>
                  <td>
                    <strong onmouseover="ObjectTooltip.createEx(this, '{{$facture->_guid}}')">
                      {{$facture}}
                      {{if $facture->_current_fse}}({{tr}}CConsultation-back-fses{{/tr}}: {{if $facture->_current_fse->_class == 'CPyxvitalFSE'}}{{$facture->_current_fse->facture_numero}}{{else}}{{$facture->_current_fse->numero}}{{/if}}){{/if}}
                    </strong>
