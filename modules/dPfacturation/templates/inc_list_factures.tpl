...
<table class="tbl">
  ...
  <tr>
    <th colspan="15" class="title">
      ...
      {{else}}
        ...
        {{if !$type_relance}}
            {{if !$print}}
              <button type="button" class="print me-tertiary" style="float: right;" onclick="Facture.refreshList(1);">{{tr}}Print{{/tr}}</button>
            {{/if}}
            <a class="button download me-tertiary" href="?m=facturation&raw=ajax_list_factures&export_csv=1" target="_blank" style="float: right;">
              {{tr}}Export-CSV{{/tr}}
            </a>
        {{/if}}
      {{/if}}
  ...
  <tr>
    ...
    {{if $use_bill_ch}}
      ...
      <th>
        <input type="text" size="3" onkeyup="Facture.filterFullName(this);" id="filter-patient-name" style="float: right;" />
  ...
  {{foreach from=$factures item=_facture}}
    ...
    <tr>
      ...
      {{if $use_bill_ch}}
        ...
        <td class="{{$line_class}} _assurance_patient_view" rowspan="{{$rowspan}}">{{$_facture->_assurance_patient_view}}</td>
      ...
      {{/if}}
      <td class="{{$line_class}}" {{if $rowspan == 2}}rowspan="{{$rowspan}}"{{else}}colspan="2"{{/if}}rowspan="{{$rowspan}}">
        {{mb_value object=$_facture field=_statut_view}}
      </td>
    {{if $rowspan == 2}}
      <td class="{{$line_class}}" style="color:{{if $_facture->bill_date_printed}}green{{else}}maroon{{/if}}; cursor: help;">
      ...
      <tr>
      <td class="{{$line_class}}" style="color:{{if $_facture->justif_date_printed}}green{{else}}maroon{{/if}}; cursor: help;">
        {{mb_include module=facturation template=inc_printed_bill
