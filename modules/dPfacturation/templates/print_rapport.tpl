      ...
      <table class="tbl">
        ...
        <tr id="line_facture_{{$_facture->_guid}}">
          ...
          <td>
            ...
            {{if $conf.ref_pays == 2 && $_facture->_montant_avoir}}
              ({{tr}}CFactureAvoir{{/tr}} : {{mb_value object=$_facture field=_montant_avoir}})
            {{/if}}
          </td>
              ...
              {{if abs($_facture->_du_restant_patient) > 0.01 || ("dPfacturation CReglement use_lock_acquittement"|gconf && $_facture->du_patient)}}
                <tr>
                  <td colspan="4" class="button">
                    ...
                    <button class="add" type="button" onclick="Rapport.addReglement('{{$_facture->_guid}}', '{{$new_reglement.emetteur}}', '{{$new_reglement.montant}}', '{{$new_reglement.mode}}', null, '{{$_plage.plage->_guid}}');">
            ...
            <td class="button">
              ...
              {{elseif $_facture->_is_relancable}}
                <form name="relance_{{$_facture->_guid}}" method="post" action="" onsubmit="return onSubmitFormAjax(this, {onComplete : function() {location.reload();}});">
                  ...
                  <button class="add notext" type="submit">{{tr}}CFacture-action-create-relance{{/tr}}</button>
                </form>
              {{/if}}
            </td>
          {{/if}}
        ...
        <tr id="{{$_plage.plage->_guid}}_total">
          <td colspan="{{if $filter->_etat_accident_travail != 'no'}}5{{else}}4{{/if}}" style="text-align: right" >
