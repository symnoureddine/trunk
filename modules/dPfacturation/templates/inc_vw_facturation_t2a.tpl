...
<tr>
  ...
  <td>
    <form name="facture_date-{{$facture->_guid}}" method="post" action="" onsubmit="return Facture.editDateFacture(this);">
  ...
  <td colspan="3">
    ...
    {{/if}}
    {{if $facture->cloture}}
      <button type="button" class="pdf" onclick="Facture.printFactureFR('{{$facture->_id}}', '{{$facture->_class}}');" style="float:left;">{{tr}}CFacture-action-pdf-invoice{{/tr}}</button>
    {{/if}}
  </td>
</tr>
  ...
  <tr>
    <td colspan="3">
      {{if $facture->numero == 1 && $facture->cloture && !$facture->_is_urg}}
        <button class="edit notext" style="float:right;" onclick="Facture.editRepartition('{{$facture->_id}}', '{{$facture->_class}}');">{{tr}}CFactureCabinet-action-modify-du-patient-tiers{{/tr}}</button>
      {{/if}}
    </td>
    ...
    <tr>
      <td colspan="7">
        <form name="change_type_facture" method="post">
          ...
          {{if !$facture->cloture}}
            <button class="submit" type="button" onclick="Facture.modifCloture(this.form);" >{{tr}}CFactureCabinet-action-close-invoice{{/tr}}</button>
          {{else}}
            <button class="submit" type="button" onclick="Facture.modifCloture(this.form);" >{{tr}}CFactureCabinet-action-reopen-invoice{{/tr}}</button> Cloturée le {{$facture->cloture|date_format:$conf.date}}
          {{/if}}
        </form>
      </td>
    </tr>
