  ...
  <thead>
    ...
    <tr>
      <th colspan="9" class="narrow tdb-facturiere-multi-actions" id="tdb_facturiere_{{$facture_switch}}_multi_result"
        ...
        {{if $conf.ref_pays === "2"}}
          <button class="print" onclick="TdbFacturiere.multiPrint()" disabled
                  onmousemove="ObjectTooltip.createDOM(this, this.down('.small-info'));">
            {{tr}}CFacture.action print selected{{/tr}}
            <div class="small-info" style="display: none;">
              {{tr}}CFacture.action print selected info{{/tr}}
            </div>
          </button>
          <button class="send" onclick="TdbFacturiere.multiSend()" disabled
                  onmousemove="ObjectTooltip.createDOM(this, this.down('.small-info'));">
            {{tr}}CFacture.action send selected{{/tr}}
            <div class="small-info" style="display: none;">
              {{tr}}CFacture.action send selected info{{/tr}}
            </div>
          </button>
        {{/if}}
      </th>
      ...
      <tr>
        <td>
          <input type="checkbox" class="tdb-facturiere-checkbox" data-facture-guid="{{$_facture->_guid}}"
                 ...
                 data-praticien-id="{{$_facture->praticien_id}}" data-facture-cloture="{{if $_facture->cloture}}1{{else}}0{{/if}}"
