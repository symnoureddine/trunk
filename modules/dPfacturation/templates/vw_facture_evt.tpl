        ...
        <form name="tarifFrm" action="?m={{$m}}" method="post" onsubmit="return checkForm(this)">
          ...
          <table style="width: 100%">
            ...
            {{if @$modules.tarmed->_can->read && 'tarmed CCodeTarmed use_cotation_tarmed'|gconf}}
              <tr>
                <th>{{tr}}CTarmed{{/tr}}</th>
                <td colspan="3">{{mb_field object=$evenement field="_tokens_tarmed" readonly="readonly" hidden=1}}
                  {{foreach from=$evenement->_ref_actes_tarmed item=acte_tarmed}}
                    <span onmouseover="ObjectTooltip.createEx(this, '{{$acte_tarmed->_guid}}');">{{$acte_tarmed->_shortview}}</span>
                  {{/foreach}}
                </td>
              </tr>
              <tr>
                <th>{{tr}}CCaisseMaladie{{/tr}}</th>
                <td colspan="3">{{mb_field object=$evenement field="_tokens_caisse" readonly="readonly" hidden=1}}
                  {{foreach from=$evenement->_ref_actes_caisse item=acte_caisse}}
