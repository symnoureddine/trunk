...
<ul id="tab-evt-reglement" class="control_tabs small">
  ...
  {{if $app->user_prefs.ccam_consultation == 1}}
    ...
    {{if @$modules.tarmed->_can->read && 'tarmed CCodeTarmed use_cotation_tarmed'|gconf}}
      ...
      <li><a href="#caisse_tab">{{tr}}CCaisseMaladie{{/tr}}</a></li>
    {{/if}}
  {{/if}}
</ul>
...
{{if $app->user_prefs.ccam_consultation == 1}}
  ...
  {{if @$modules.tarmed->_can->read && 'tarmed CCodeTarmed use_cotation_tarmed'|gconf}}
    ...
    <script>
      Main.add(function() {
        ActesTarmed.loadList('{{$object->_id}}', '{{$object->_class}}', '{{$object->praticien_id}}');
        ActesCaisse.loadList('{{$object->_id}}', '{{$object->_class}}', '{{$object->praticien_id}}');
      });
    </script>
