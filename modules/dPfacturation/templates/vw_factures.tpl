...
{{if 'ref_pays'|conf === '2'}}
  {{assign var=use_bill_ch value=1}}
{{/if}}
    ...
    <table class="form" name="choix_type_facture">
      ...
      <tr>
        ...
        {{me_form_field nb_cells=2 mb_object=$facture mb_field=statut_envoi class="me-w33"}}
          <select name="xml_etat">
            <option value="" {{if !$xml_etat}} selected="selected" {{/if}}>-- {{tr}}common-all|f|pl{{/tr}}</option>
        ...
        {{me_form_field nb_cells=2 label=CFactureCabinet-statut-invoice class="me-w33"}}
          <select name="search_easy" multiple>
            <option value="0" {{if $search_easy == "0"}} selected="selected" {{/if}}>-- {{tr}}common-all|f|pl{{/tr}}</option>
      ...
      <tr>
        ...
        {{me_form_field nb_cells=2 mb_object=$facture mb_field=numero}}
          <select name="numero">
            <option value="0" {{if $numero == "0"}} selected="selected" {{/if}}>-- {{tr}}common-all|f|pl{{/tr}}</option>
      ...
      <tr>
        {{me_form_field nb_cells=2 label=CMediusers-praticien layout=true field_class="me-padding-0 me-no-border" animated=false}}
          ...
          <select name="activeChirSel" style="width: 15em;" onchange="$V(this.form.chirSel, $V(this));">
            <option value="0" {{if !$chirSel}} selected="selected" {{/if}}>&mdash; {{tr}}CMediusers-select-professionnel{{/tr}}</option>
            {{if $facture->_class == "CFactureEtablissement"}}
              <option value="-1" {{if $chirSel == "-1"}} selected="selected" {{/if}}><b>&mdash; {{tr}}All{{/tr}}</b></option>
            {{/if}}
        ...
        {{/me_form_field}}
        {{if $conf.ref_pays == 2}}
          {{me_form_field nb_cells=2 label=CFactureCabinet-type_facture}}
            {{mb_field object=$facture field=type_facture emptyLabel="All"}}
          {{/me_form_field}}
        {{else}}
          <td colspan="2"></td>
        {{/if}}
      ...
      {{if $conf.ref_pays == 2}}
        <tr>
          {{me_form_field nb_cells=6 mb_object=$facture mb_field=statut_pro}}
            {{mb_field object=$facture field=statut_pro emptyLabel="All"}}
          {{/me_form_field}}
        </tr>
        <tr>
          {{me_form_field nb_cells=2 label="CFacture.Bill printed"}}
            <select name="bill_printed" >
              <option value="-1">{{tr}}All{{/tr}}</option>
              <option value="1">{{tr}}CFacture.given_to_the_patient{{/tr}}</option>
              <option value="0">{{tr}}CFacture.not_given_to_the_patient{{/tr}}</option>
            </select>
          {{/me_form_field}}
          {{me_form_field nb_cells=2 label="CFacture.Justificatif printed"}}
            <select name="justif_printed">
              <option value="-1">{{tr}}All{{/tr}}</option>
              <option value="1">{{tr}}CFacture.given_to_the_patient{{/tr}}</option>
              <option value="0">{{tr}}CFacture.not_given_to_the_patient{{/tr}}</option>
            </select>
          {{/me_form_field}}
          {{me_form_field nb_cells=2 mb_class=CFactureCabinet mb_field=envoi_xml}}
            <select name="envoi_xml">
              <option value="-1">{{tr}}All{{/tr}}</option>
      ...
      <tr>
        <td class="button" colspan="6">
          <button type="button" onclick="$V(this.form.page, 0);Facture.refreshList();" class="search me-primary">
            {{tr}}Filter{{/tr}}
          </button>
          <button type="button" onclick="Facture.showLegend('{{$facture->_class}}');" class="search me-tertiary" style="float:right;">
