...
<fieldset class="me-no-align me-no-box-shadow">
  ...
  {{if @$modules.tarmed->_can->read && 'tarmed CCodeTarmed use_cotation_tarmed'|gconf}}
    <div id="check_bill_relance_id" style="display:none;" data-count="" data-source_envoi=""></div>
  {{/if}}
    ...
    {{foreach from=$facture->_ref_relances item=relance}}
      <tr>
        ...
        <td>
          ...
          {{/if}}
          {{if $relance->statut_envoi != 'envoye' && $facture->_ref_last_relance->_id == $relance->_id}}
            <form name="XML_{{$relance->_guid}}" method="post" action="" onsubmit="return false;">
              ...
              <button type="button" class="send notext" onclick="Relance.checkRelance(this.form);">{{tr}}CRelance-action-Generate the relaunch XML{{/tr}}</button>
            </form>
          {{/if}}
        </td>
      </tr>
