...
class CFactureTest extends UnitTestMediboard {
   ...
   * @config ref_pays 2
   */
  public function testRoundValueSuisse() {
    foreach ($this->getAllRoundPossibilities() as $attente => $valeurs_tests) {
      foreach ($valeurs_tests as $_valeur_test) {
        $result = CFacture::roundValue($_valeur_test);
        $this->assertEquals($attente, $result);
      }
    }
  }
  ...
  /**
   * Test de chargement du compte de facturation suisse
   */
  public function testloadRefCompteCh() {
    $facture = new CFacture();
    $this->assertInstanceOf(CMediusersCompteCh::class, $facture->loadRefCompteCh());
  }
...
//  }
  ...
  public function testGetTraitementPeriodeSejours() {
    ...
    $sejour1->entree_reelle = $sejour1->entree = $sejour1->_entree = $sejour1->entree_prevue;
    $sejour1->sortie_prevue = $dates_sejour1["sortie"];
    $sejour1->sortie_reelle = $sejour1->sortie = $sejour1->_sortie = $sejour1->sortie_prevue;
    ...
    $sejour2->entree_reelle = $sejour2->entree = $sejour2->_entree = $sejour2->entree_prevue;
    $sejour2->sortie_prevue = $dates_sejour2["sortie"];
    $sejour2->sortie_reelle = $sejour2->sortie = $sejour2->_sortie = $sejour2->sortie_prevue;
  ...
  public function testSaveConsult() {
    ...
    $consultation = $this->getRandomObjects(CConsultation::class, 1);
  ...
  public function testSaveEvt() {
    //Création de la facture
    $evt = $this->getRandomObjects(CEvenementPatient::class, 1);
