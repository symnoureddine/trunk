...
// Création du template
  ...
  //Titres
  $titles = array(
    CAppUI::tr("CFactureCabinet"), CAppUI::tr("$type_object-".($type_view == "consult" ? "_prat_id" : "praticien_id")),
  ...
  foreach ($listReglements as $key_date => $_date) {
    foreach ($_date["reglements"] as $_reglement) {
      ...
      switch ($type_view) {
        case "consult":
          $tarifs = $facture->_ref_consults; break;
        case "evt":
          $tarifs = $facture->_ref_evts; break;
        case "etab":
          $tarifs = $facture->_ref_sejours; break;
        default:
      }
      $line_tarif = "";
      foreach ($tarifs as $_tarif) {
        ...
        switch ($type_view) {
          case "consult":
            $line_tarif .= CMbDT::format($_tarif->_date, CAppUI::conf("date"))." ".$_tarif->tarif; break;
          case "evt":
            $line_tarif .= $_tarif->tarif; break;
          case "etab":
            $line_tarif .= $_tarif->_view; break;
          default:
        }
      }
