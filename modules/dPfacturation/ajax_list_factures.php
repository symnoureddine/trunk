...
$envoi_xml        = CView::get("envoi_xml", "enum list|-1|0|1 default|-1", true);
CView::checkin();
CView::enforceSlave();
...
foreach ($factures as $key => $_facture) {
  ...
  if (!$_facture->cloture && !$use_auto_cloture && $search_easy == 4 && ($nb_tarmed != 0 || $nb_caisse != 0 || $nb_ngap != 0 || $nb_ccam != 0)) {
    unset($factures[$key]);
...
if (!$export_csv) {
  ...
  $use_bill_ch = CAppUI::conf("ref_pays") == 2 ? 1 : 0;
  $columns = array(
    CAppUI::tr("Date"),
