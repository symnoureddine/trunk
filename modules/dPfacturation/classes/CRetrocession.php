  ...
  function updateMontant ($code = "") {
    ...
    if ($code == $this->code || !$code) {
      ...
      elseif (($this->code_class == "CActeTarmed" || $this->code_class == "CActeCaisse") &&
            CModule::getActive("tarmed") && CAppUI::gconf("tarmed CCodeTarmed use_cotation_tarmed")) {
        $_code = $this->loadRefCode();
        $tarmed = $_code->_ref_tarmed;
        $pm = $this->code_class == "CActeTarmed" ? $tarmed->tp_al * $tarmed->f_al : $_code->_ref_prestation_caisse->tarif;
        $pt = $this->code_class == "CActeTarmed" ? $tarmed->tp_tl * $tarmed->f_tl : 0;
