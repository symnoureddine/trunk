...
class CEditPdf implements IShortNameAutoloadable {
  ...
  function editFacture($create = true, $executant_id = null) {
    ...
    foreach ($this->factures as $the_facture) {
      foreach (count($this->echeances) > 0 ? $this->echeances : array(false) as $_echeance) {
        ...
        if ($executant_id) {
          ...
          $this->facture->_montant_factures_caisse = array();
        ...
        $adherent = $this->facture->loadNumAdherent($this->facture->_ref_compte_ch->adherent);
  ...
  function editRelancePied($use_bvr = true) {
    ...
    elseif ($use_bvr && CAppUI::conf("ref_pays") == 2) {
  ...
  function editCenterJustificatif($cle_facture, $montant_facture) {
    ...
    if (CAppUI::gconf("dPfacturation CEditPdf separate_by_prat")) {
      foreach ($this->facture->_ref_actes_tarmed as $key_acte_tarmed => $acte_tarmed) {
        if ($acte_tarmed->executant_id != $this->facture->_ref_praticien->_id) {
          unset($this->facture->_ref_actes_tarmed[$key_acte_tarmed]);
        }
      }
      foreach ($this->facture->_ref_actes_caisse as $key_acte_caisse => $acte_caisse) {
        if ($acte_caisse->executant_id != $this->facture->_ref_praticien->_id) {
          unset($this->facture->_ref_actes_caisse[$key_acte_caisse]);
        }
      }
    }
    $tab_actes = array (
    ...
    foreach ($tab_actes as $keytab => $tab_acte) {
      ...
      foreach ($tab_acte as $acte) {
        ...
        if ($tab_tarmed || ($keytab == "caisse" && $tab_caisse)) {
          $ligne++;
          ...
          if ($this->pdf->getY() >= 265) {
            ...
            $this->editCell($this->pdf->getX(), $debut_lignes + $ligne*3, 130, CAppUI::tr("CEditPdf.Total_intermediaire"), "R");
            $this->pdf->Cell(28, null, round($montant_intermediaire, 2) , null, null, "R");
            $this->pdf->setFont($this->font, '', 8);
            $this->pdf->AddPage();
            $nb_pages++;
            $this->ajoutEntete2($nb_pages);
            $this->editCell(10, $this->pdf->getY()+4, $this->colonnes[0]+$this->colonnes[1], CAppUI::tr("CPatient"));
            $this->pdf->Cell($this->colonnes[2], null, $this->patient->nom." ".$this->patient->prenom." ".$this->patient->naissance);
          ...
          // Traitement pour les libelles multi-lignes
          $lib_array = explode(" ", $acte->libelle);
          $libelle = array(	);
          foreach ($lib_array as $_lib) {
            if (count($libelle) === 0) {
              $libelle[] = $_lib;
              continue;
            }
            $str = $libelle[count($libelle) - 1] . " " . $_lib;
            if (strlen($str) < 90) {
              $libelle[count($libelle) - 1] .= " " . $_lib;
            }
            else {
              $libelle[] = $_lib;
            }
          ...
          // Affichage des lignes de libelle
          foreach ($libelle as $_key => $_libelle) {
            if ($_key > 0) {
              $this->pdf->setXY(37, $debut_lignes + $ligne*3);
            }
            $this->pdf->Write(0, $_libelle);
            $ligne++;
          }
          $x = 0;
          $this->pdf->setX(15);
          $this->pdf->setFont($this->font, '', 8);
          $acte_caisse = $acte->type === "CActeCaisse";
          foreach ($tailles_colonnes as $key => $largeur) {
            ...
            switch ($key) {
              ...
              case "Pt PM/Prix":
                ...
                break;
              case "fPM":
                $valeur = round($acte->coeff_pm,2) ;
                break;
              case "VPtPM":
                $valeur = round($acte->coeff, 2);
                break;
              case "VPtPT":
                $valeur = $acte_caisse ? "" : round($acte->coeff, 2);
                break;
              case "Pt PT":
                $valeur = $acte->pt;
                $cote = "R";
                break;
              case "fPT":
                $valeur = $acte_caisse ? "" : round($acte->coeff_pt, 2);
                break;
              case "Montant":
          ...
          elseif ($acte->code_caisse) {
            $caisse = new CActeCaisse();
            $caisse->code = $acte->code;
            $caisse->loadRefPrestationCaisse();
            if ($caisse->_ref_caisse_maladie->_id && $caisse->_ref_caisse_maladie->loadRefCodeXML()->service == "drug") {
              $acte_m = $acte->montant_base * $acte->quantite;
              $acte_m *= $this->facture->type_facture === "accident" ?
                $caisse->_ref_caisse_maladie->coeff_accident : $caisse->_ref_caisse_maladie->coeff_maladie;
              $medicaments += $acte_m;

              // Le coefficient est déjà appliqué pour cette ligne
              $acte->coeff = "1";
            }
            else {
              $autre_temp = $acte->montant_base * $acte->coeff * $acte->quantite;
            }
          }
  ...
  function loadTotaux($executant_id = null){
    ...
    foreach ($this->facture->_ref_actes_tarmed as $acte) {
      if ($executant_id && $acte->executant_id != $executant_id) {
        unset($this->facture->_ref_actes_tarmed[$acte->_id]);
        continue;
      }
    ...
    foreach ($this->facture->_ref_actes_caisse as $acte) {
      if ($executant_id && $acte->executant_id != $executant_id) {
        unset($this->facture->_ref_actes_caisse[$acte->_id]);
        continue;
      }
      $add = true;
      if ($acte->code_caisse) {
  ...
  function editRelanceListCorps($relances, $num_frais) {
    ...
    foreach ($relances as $_relance) {
      ...
      $this->editCell(60, $y+4+$y_to_add, $col2, CAppUI::tr("date.From")." ".CMbDT::format($facture->cloture, "%d %B %Y"), null, "R", 5);
      $this->pdf->setFont($this->font, '', 8);
  ...
  function editHautFacture($montant_facture, $relance = false, $acompte = 0) {
    ...
    if ($this->facture->_class == "CFactureCabinet" && count($this->facture->_ref_consults) == 1 && $use_date_consult) {
      ...
      list($first_date, $last_date) = $this->facture->getTraitementPeriode();
    ...
    foreach ($tarif as $key => $value) {
      ...
      else if ($key === "Montant_echeance" && $value) {
        $name_cell = CAppUI::tr(
          "CEditPdf.recap.$key",
  ...
  function editBVR($montant_facture) {
    ...
    $num_adherent = $this->facture->loadNumAdherent($this->facture->_ref_compte_ch->adherent);
    ...
    for ($i = 0; $i<=1; $i++) {
      ...
      $this->pdf->setFont($this->font, '', 8);
      $banque = $this->facture->loadRefCompteCh()->loadRefBanque();
      $banque = $banque->_id ? $banque : $this->praticien->loadRefBanque();
      $etab_adresse1 = $banque->_id ? $banque->nom : $default_etab_adresse1;
      $etab_adresse2 = $banque->_id ? $banque->cp." ".$banque->ville : $default_etab_adresse2;
  ...
  function ajoutEntete1(){
    ...
    $conf_cab = CAppUI::conf("tarmed coefficient pt_maladie", $this->facture->_host_config);
    ...
    list($first, $last) = $this->facture->getTraitementPeriode();
    if ($last > $first) {
    ...
    $line_mandataire = array(CAppUI::tr("CEditPdf.mandataire"), CAppUI::tr("CEditPdf.ean_rcc"));
    if (CAppUI::gconf("dPfacturation CEditPdf see_mandataire_justif")) {
      $line_mandataire[] = $this->praticien->ean." - ".($this->facture->_ref_compte_ch->rcc ? $this->facture->_ref_compte_ch->rcc : "");
    ...
    if (CAppUI::gconf("dPfacturation Other select_diagnostic_ambu")) {
      $diagnostic = $this->facture->loadRefDiagnostic();
      $msg_info .= $diagnostic ? $diagnostic->_view : "";
    }
    ...
    $lignes = array(
      array(CAppUI::tr("CPatient"), CAppUI::tr("CPatient-_p_last_name"), $this->patient->nom     ,null, CAppUI::tr("CFacture-assurance"), $assur_nom),
      array("", CAppUI::tr("CPatient-_p_first_name")  , $this->patient->prenom),
      array("", CAppUI::tr("CEditPdf.rue")            , str_replace(array("\r\n", "\n", "\r"), " ", $this->patient->adresse)),
      array("", CAppUI::tr("CEditPdf.NPA")            , $this->patient->cp      , null, $assur["civilite"]),
      array("", CAppUI::tr("CEditPdf.localite")       , $this->patient->ville   , null, $assur["nom"]),
      array("", CAppUI::tr("CPatient-naissance"), $naissance, null, str_replace(array("\r\n", "\n", "\r"), " ", $assur["adresse"])),
      array("", CAppUI::tr("CPatient-sexe")           , strtoupper($this->patient->sexe) , null, $assur["cp"]),
      array("", CAppUI::tr("CEditPdf.date_cas")       , $date_cas),
      array("", CAppUI::tr("CEditPdf.num_cas")        , $ref_accident),
      array("", CAppUI::tr("CEditPdf.num_avs")        , $this->patient->avs),
      array("", CAppUI::tr("CEditPdf.num_cada")       , $num_cada),
      array("", CAppUI::tr("CEditPdf.num_assure")     , $_ref_assurance),
      array("", CAppUI::tr("CEditPdf.name_entreprise"), $nom_entreprise),
      array("", CAppUI::tr("CEditPdf.canton")         , CAppUI::gconf("dPfacturation CEditPdf canton")),
      array("", CAppUI::tr("CEditPdf.copie")          , "Non"),
      array("", CAppUI::tr("CEditPdf.type_rbt")       , $this->type_rbt),
      array("", CAppUI::tr("CTarmed.loi")             , $loi),
      array("", CAppUI::tr("CEditPdf.num_contrat")    , ""),
      array("", CAppUI::tr("CEditPdf.motif_ttt"), $motif  , null, CAppUI::tr("CEditPdf.num_facture"), $this->facture->_id),
      array("", CAppUI::tr("CEditPdf.ttt"), $traitement, null, CAppUI::tr("CEditPdf.date_facture"), CMbDT::format($this->facture->cloture, "%d.%m.%Y")),
      array("", CAppUI::tr("CEditPdf.role_localite") , "-", null, $relance, $num_relance),
      $line_mandataire,
      array(CAppUI::tr("CEditPdf.diagnostic"), null, $msg_info),
      array(CAppUI::tr("CEditPdf.list_ean") , "", "1/".$this->praticien->ean." 2/".$ean2),
      array(CAppUI::tr("CEditPdf.commentaire"), substr(str_replace("\r\n", ' ', $this->facture->remarque)." ".$remarque, 0, 110))
    );
  ...
  function ajoutEntete2($nb){
    ...
    $lignes = array(
      array(CAppUI::tr("CEditPdf.document"), CAppUI::tr("CEditPdf.identification"),
                $this->facture->_id." ".CMbDT::format(null, "%d.%m.%Y %H:%M:%S"), "", "Page $nb"),
      array(CAppUI::tr("CEditPdf.auteur")  , CAppUI::tr("CEditPdf.num_ean")."(B)",
                $this->auteur["EAN"], $this->auteur["nom_dr"], CAppUI::tr("CMedecin-tel")." : ".$this->auteur["tel"]),
      array(CAppUI::tr("CFacture")         , CAppUI::tr("CEditPdf.num_rcc")."(B)",
                $this->auteur["RCC"], $auteur, CAppUI::tr("CMedecin-fax").": ".$this->auteur["fax"]),
      array(CAppUI::tr("CEditPdf.fourn_of"), CAppUI::tr("CEditPdf.num_ean")."(P)",
                $presta["EAN"], $presta["nom_dr"], CAppUI::tr("CMedecin-tel").":  ".$presta["0"]->tel),
      array(CAppUI::tr("CEditPdf.prest")   , CAppUI::tr("CEditPdf.num_rcc")."(B)",
                $presta["RCC"], $presta_adresse, CAppUI::tr("CMedecin-fax").": ".$presta["0"]->fax)
    );
  ...
  static function loadAllElementBill($facture, $praticien, $type_pdf = null) {
    ...
    elseif ($facture->assurance_accident && !$facture->send_assur_compl && $facture->type_facture == "accident") {
      if ($facture->_coeff == CAppUI::conf("tarmed coefficient pt_maladie", $facture->_host_config)) {
        $type_rbt = $facture->_ref_assurance_accident->type_pec;
      }
      else {
        $type_rbt = "TP";
      }
    ...
    $acteur["etablissement"] = array(
      ...
      "ville"    =>  $group->ville,
      "EAN"      =>  $group->ean,
      "EAN_XML"  =>  $group->ean_xml_factu ? : $group->ean,
      "RCC"      =>  $group->rcc,
      "tel"      =>  $group->tel,
      "fax"      =>  $group->fax,
    );
    $acteur["praticien"] = array(
      "0"        =>  $function_prat,
      ...
      "EAN"      =>  $praticien->ean,
      "EAN_XML"  =>  $praticien->ean_xml_factu ? : $praticien->ean,
      "RCC"      =>  $facture->_ref_compte_ch ? $facture->_ref_compte_ch->rcc : "",
      "cp"       =>  $function_prat->cp,
    ...
    $acteur["cabinet"]["EAN_XML"] = $function_prat->ean_xml_factu ? : $function_prat->ean;
    ...
    if (CAppUI::conf("ref_pays") == 2) {
      $use_conf_bvr = $type_pdf && stristr($type_pdf, "bvr") ? "_bvr" : "";
      $auteur = $acteur[CAppUI::gconf("dPfacturation CEditPdf auteur_facture$use_conf_bvr")];
      $fourn_presta = $acteur[CAppUI::gconf("dPfacturation CEditPdf fournisseur_presta$use_conf_bvr")];
    }

    if (CAppUI::conf("ref_pays") == 2 && CAppUI::gconf("dPfacturation CEditPdf see_fct_bvr")) {
      $auteur["fct"] = $fourn_presta["fct"] = $function_prat->_view;
    }

    return array($type_rbt, $destinataire, $auteur, $fourn_presta, $assurance_patient, $assurance_select);
  }
  ...
  function loadAllElements(){
    list($this->type_rbt, $this->destinataire, $this->auteur, $this->fourn_presta) =
