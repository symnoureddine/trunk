...
class CModuleBuilder
{
    ...
    private function buildImage(): bool
    {
        $image_data  = str_replace(' ', '+', $this->mod_image);
        $image_data  = base64_decode(substr($image_data, strpos($image_data, ",") + 1));
        $file_handle = fopen($this->root_dir . "modules/{$this->module_name}/images/iconographie/fr/icon.png", "w+");
        if (!$file_handle) {
            return false;
        }
