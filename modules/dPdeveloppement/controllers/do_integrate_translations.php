...
foreach ($translations as $_translation) {
  ...
  /*$src_split = explode('-', $_translation->source);
  $module    = 'common';
  if (isset($src_split[1])) {
    if (CModule::exists($src_split[1])) {
      $module = $src_split[1];
    }
    elseif (CModule::exists("dP$src_split[1]")) {
      $module = "dP$src_split[1]";
    }
  }*/
...
SHM::remKeys("locales-$_lang-*"); // Remove the keys from SHM
