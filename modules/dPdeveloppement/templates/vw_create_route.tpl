...
<form name="create-route" method="get" onsubmit="return onSubmitFormAjax(this, null, 'result-create-route');">
  ...
  <table class="main form">
    ...
    <tr>
      <th>{{tr}}dPdeveloppement-Api-security{{/tr}}</th>
      <td class="narrow">security</td>
      <td>
        {{foreach from=$allowed_security item=_secu}}
          <label>
            <input type="checkbox" name="security[{{$_secu}}]" checked value="{{$_secu}}"/>
            {{$_secu}}
          </label>
        {{/foreach}}
      </td>
