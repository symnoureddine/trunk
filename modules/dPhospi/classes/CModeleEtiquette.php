...
class CModeleEtiquette extends CMbObject {
  ...
  static $text_fields = array("texte", "texte_2", "texte_3", "texte_4");
  ...
  function printEtiquettes($printer_guid = null, $stream = 1) {
    ...
    for ($i = 0; $i < $nb_etiqs; $i++) {
      ...
      if (round($pdf->GetX()) >= round($this->largeur_page - 2 * $this->marge_horiz - ($this->nb_colonnes - 1) * $this->marge_horiz_etiq)) {
        $pdf->SetX(0);
      ...
      if (preg_match("/@BARCODE_(.*)@/", $fragment, $matches) == 1) {
        ...
        switch ($pays) {
          case "2":
            ...
            $pdf->setX($save_x);
            $pdf->setFont("C39HrP24DhTt", "", $size * 3);
  ...
  public static function getFields() {
    return array(
      ...
      "General"                    => array("DATE COURANTE", "HEURE COURANTE", "UTILISATEUR CONNECTE"),
