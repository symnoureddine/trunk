...
<table class="tbl print_sejour me-no-align me-small">
  ...
  <tr>
    <th colspan="9" class="title">
      {{if !$readonly}}
        <button type="button" class="search me-tertiary me-dark me-small" onclick="Modal.open('legend_suivi')" style="float: right;">Légende</button>
      {{/if}}
      ...
      {{if !$readonly}}
        <div class="me-inline-block me-margin-left-12">
          <select name="other_sejour_id" class="me-small" onchange="Soins.loadSuivi('{{$sejour->_id}}', null, null, null, null, null, this.value);">
  ...
  {{if !$readonly}}
    <tr>
      <td colspan="9" class="me-bg-transparent" style="background-color: #fff; vertical-align: top;">
          ...
          <div class="text">
            ...
            {{if $isPraticien}}
              {{if (in_array($sejour->type, 'Ox\Mediboard\PlanningOp\CSejour::getTypesSejoursUrgence'|static_call:$sejour->praticien_id) && "dPprescription CPrescription prescription_suivi_soins"|gconf && "dPprescription"|module_active)}}
                <button class="add me-tertiary" onclick="addPrescription('{{$sejour->_id}}', '{{$user->_id}}')"
                        style="display: inline !important;">Ajouter une prescription
                </button>
              {{/if}}
              {{if ("dPcabinet"|module_active && $dtnow >= $sejour->_entree && $dtnow <= $sejour->_sortie)}}
                {{if "planSoins"|module_active}}
                  <button type="button" class="new me-tertiary" id="newConsult" style="display: inline !important;"
                          onclick="validateAdministration('{{$sejour->_id}}');">Nouvelle consultation
                  </button>
                {{/if}}
                <button type="button" class="new oneclick me-secondary" id="newConsultEntree" {{if $has_obs_entree}}disabled{{/if}}
            ...
            {{if $cancelled_nb}}
              {{if $show_cancelled}}
                <button type="button" class="search me-tertiary me-dark"
                        onclick="Soins.loadSuivi('{{$sejour->_id}}', null, null, null, null, '0');">Masquer les {{$cancelled_nb}}
                  annulée(s)
                </button>
              {{else}}
                <button type="button" class="search me-tertiary me-dark"
                        onclick="Soins.loadSuivi('{{$sejour->_id}}', null, null, null, null, '1');">Afficher les {{$cancelled_nb}}
                  annulée(s)
                </button>
              {{/if}}
            {{/if}}
        ...
        <fieldset style="width: 68%; display: inline-block; float: right;" class="me-ws-nowrap me-small me-small-fields">
          <legend>
            {{tr}}filters{{/tr}}
          </legend>
          <input name="_show_obs_view" id="_show_obs_view" type="checkbox"
                 {{if $_show_obs}}checked{{elseif $cible != ""}}disabled{{/if}} onclick="Soins.loadSuivi('{{$sejour->_id}}', null, null, null, null, null, '{{$other_sejour_id}}');" />
          ...
          <select name="_degre_obs" id="_degre_obs" {{if !$_show_obs}}disabled{{/if}}
            ...
            <option value="high" {{if $_degre_obs === "high"}}selected{{/if}}>{{tr}}CObservationMedicale.degre.high{{/tr}}</option>
            <option value="info" {{if $_degre_obs === "info"}}selected{{/if}}>{{tr}}CObservationMedicale.degre.info{{/tr}}</option>
            <option value="low" {{if $_degre_obs === "low"}}selected{{/if}}>{{tr}}CObservationMedicale.degre.low{{/tr}}</option>
          </select>
          {{/me_form_field}}
          ...
          <select name="_type_obs" id="_type_obs" {{if !$_show_obs}}disabled{{/if}}
            ...
            <option value="" {{if $_type_obs === ""}}selected{{/if}}>{{tr}}CObservationMedicale.type.{{/tr}}</option>
          ...
          {{me_form_field animated=false field_class="me-form-group-inline"}}
            <select name="_etiquette_obs" id="_etiquette_obs" {{if !$_show_obs}}disabled{{/if}}
              ...
              {{foreach from=$observation_med->_specs.etiquette->_locales key=key_etiquette item=name_etiquette}}
                <option value="{{$key_etiquette}}" {{if $key_etiquette == $_etiquette_obs}}selected="selected"{{/if}}>{{$name_etiquette}}</option>
              {{/foreach}}
            </select>
          {{/me_form_field}}

          <input class="me-margin-left-8" name="_show_trans_view" id="_show_trans_view" type="checkbox" {{if $_show_trans}}checked{{/if}}
          ...
          {{if $count_macrocibles}}
            <input class="me-margin-left-8" name="_only_macrocible" id="_only_macrocible_view" type="checkbox" {{if $only_macrocible}}checked{{/if}}
                   onclick="Soins.loadSuivi('{{$sejour->_id}}', null, null, null, null, null, '{{$other_sejour_id}}');" />
            <label for="_only_macrocible_view" title="{{tr}}CTransmissionMedicale-Only macrocible{{/tr}}">{{tr}}CTransmissionMedicale-Only macrocible-court{{/tr}}</label>
          {{/if}}
          ...
          <input class="me-margin-left-8" name="_show_adm_cancelled_view" id="_show_adm_cancelled_view" type="checkbox" {{if $show_adm_cancelled}}checked{{/if}}
                 onclick="Soins.loadSuivi('{{$sejour->_id}}', null, null, null, null, null, '{{$other_sejour_id}}');" />
          <label for="_show_adm_cancelled_view" title="{{tr}}CTransmissionMedicale-Show adm cancelled{{/tr}}">{{tr}}CTransmissionMedicale-Show adm cancelled-court{{/tr}}</label>

          <input class="me-margin-left-8" name="_show_rdv_externe_view" id="_show_rdv_externe_view" type="checkbox" {{if $show_rdv_externe}}checked{{/if}}
                 onclick="Soins.loadSuivi('{{$sejour->_id}}', null, null, null, null, null, '{{$other_sejour_id}}');" />
          <label for="_show_rdv_externe_view" title="{{tr}}CRDVExterne-action-Show external events{{/tr}}">{{tr}}CRDVExterne-court{{/tr}}</label>

          <input class="me-margin-left-8" name="_show_call_view" id="_show_call_view" type="checkbox" {{if $show_call}}checked{{/if}}
                 onclick="Soins.loadSuivi('{{$sejour->_id}}', null, null, null, null, null, '{{$other_sejour_id}}');" />
          <label for="_show_call_view" title="{{tr}}CAppelSejour-action-Show calls{{/tr}}">{{tr}}CAppelSejour-event-appel{{/tr}}</label>

          {{if "soins Other see_volet_diet"|gconf}}
            <input class="me-margin-left-8" name="_show_diet_view" id="_show_diet_view" type="checkbox" {{if $_show_diet}}checked{{/if}}
                   onclick="Soins.loadSuivi('{{$sejour->_id}}', null, null, null, null, null, '{{$other_sejour_id}}');" />
            <label for="_show_diet_view" title="Afficher les transmissions et observations de type diététique">Diet.</label>
          {{/if}}

          <br />

          {{me_form_field animated=false field_class="me-form-group-inline"}}
          <select name="selCible" style="width: 150px" onchange="Soins.loadSuivi('{{$sejour->_id}}', '', '', this.value, null, null, '{{$other_sejour_id}}')">
            ...
            {{foreach from=$cibles item=cibles_by_state key=state}}
              {{if $cibles_by_state|@count}}
                ...
                {{foreach from=$cibles_by_state item=_cible}}
                  <option {{if $_cible == $cible}}selected{{/if}} value="{{$_cible}}">{{$_cible}}</option>
                {{/foreach}}
              {{/if}}
            {{/foreach}}
          </select>
          ...
          {{if $users}}
            {{me_form_field animated=false field_class="me-form-group-inline"}}
            <select name="user_id" style="width: 150px" onchange="Soins.loadSuivi('{{$sejour->_id}}',this.value, null, null, null, null, '{{$other_sejour_id}}')">
              <option value="">&mdash; {{tr}}CUser.all{{/tr}}</option>
              {{foreach from=$users item=_user}}
                <option value="{{$_user->_id}}" {{if $user_id == $_user->_id}}selected{{/if}}>{{$_user->_view}}</option>
              {{/foreach}}
            </select>
            {{/me_form_field}}
          {{/if}}
          ...
          {{if $functions}}
            {{me_form_field animated=false field_class="me-form-group-inline"}}
            <select name="function_id" style="width: 150px" onchange="Soins.loadSuivi('{{$sejour->_id}}', null, this.value, null, null, null, '{{$other_sejour_id}}')">
              <option value="">&mdash; {{tr}}CFunctions.all{{/tr}}</option>
              {{foreach from=$functions item=_function}}
                <option value="{{$_function->_id}}" {{if $function_id == $_function->_id}}selected{{/if}}>{{$_function->_view}}</option>
              {{/foreach}}
            </select>
            {{/me_form_field}}
          {{/if}}
