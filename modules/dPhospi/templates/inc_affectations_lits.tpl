...
<tr class="me-lit {{if !"dPhospi CLit show_in_tableau"|gconf}}lit{{/if}}" id="lit-{{$curr_lit->_id}}">
  <td>
    ...
    {{if "hotellerie"|module_active && $curr_lit->_ref_last_cleanup->_id}}
      <span onmouseover="ObjectTooltip.createEx(this, '{{$curr_lit->_ref_last_cleanup->_guid}}')" style="float: right;">
      ...
      <table id="affectation_{{$curr_affectation->_id}}"
        ...
        <tr class="patient">
          {{if $curr_affectation->sejour_id}}
            ...
            -->
              ...
              {{/if}}
              {{if $conf.ref_pays != "2" && $sejour->_couvert_ald}}
          ...
          {{/if}}
            ...
            <span {{if $app->touch_device}}onclick{{else}}onmouseover{{/if}}="ObjectTooltip.createEx(this, '{{$sejour->_guid}}')"
        ...
        {{if !$curr_affectation->sejour_id}}
          {{if $curr_affectation->sortie < $dtnow && $mode == 0}}
            <tr class="litdispo">
              <td colspan="2">{{tr}}Fin-blocage{{/tr}} {{$curr_affectation->sortie|date_format:"%A %d %B %Hh%M"}}</td>
            </tr>
          {{/if}}
          <tr class="dates">
              ...
              <form name="SeptieSejour{{$sejour->_id}}" method="post" class="prepared">
                ...
                {{if $can->edit}}
                  <label title="Séjour propre" class="me-small-fields">
                    <input type="radio" name="septique" value="0" {{if $sejour->septique == 0}}checked{{/if}}
                           onclick="this.form.submit()" />
                    Propre
                  </label>
                  <label title="Séjour septique" class="me-small-fields">
                    <input type="radio" name="septique" value="1" {{if $sejour->septique == 1}}checked{{/if}}
                           onclick="this.form.submit()" />
                    Septique
                  </label>
                {{else}}
