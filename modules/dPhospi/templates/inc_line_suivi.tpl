  ...
  <td class="narrow text">
    {{if $_suivi->object_id}}
      <span onmouseover="ObjectTooltip.createEx(this, '{{$_suivi->_ref_object->_guid}}');">
       {{if 'CPrescription::isMPMActive'|static_call:null && $_suivi->_ref_object|instanceof:'Ox\Mediboard\Mpm\CPrescriptionLineMedicament'}}
  ...
  {{if !$readonly}}
    <td class="button">
      ...
      {{if $_suivi->_canEdit || !$period_modif || 'Ox\Core\CMbDT::timeRelative'|static_call:$_suivi->date:$dtnow:"%02d" < $period_modif}}
        ...
        {{if $_suivi->_canEdit}}
          <button type="button" class="edit notext" onclick="Soins.addObservation(null, null, '{{$_suivi->_id}}');">
...
{{if $_suivi|@instanceof:'Ox\Mediboard\Prescription\CPrescriptionLineElement' || $_suivi|@instanceof:'Ox\Mediboard\Prescription\CPrescriptionLineComment'}}
  ...
  <td colspan="4" {{if $_suivi->_count.transmissions}} class="arretee" {{/if}}>
    {{if !$readonly}}
      <button type="button" class="tick" onclick="addTransmissionAdm('{{$_suivi->_id}}','{{$_suivi->_class}}');" style="float: right;">
...
{{if $_suivi|@instanceof:'Ox\Mediboard\Cabinet\CConsultation'}}
  ...
  <td class="text" colspan="3">
    {{if $_suivi->_refs_dossiers_anesth|@count}}
      ...
      {{/if}}
    {{else}}
      {{if $_suivi->motif}}<u>{{tr}}CConsultation-motif{{/tr}}</u> : {{mb_value object=$_suivi field=motif}}<br />{{/if}}
      {{if $_suivi->histoire_maladie}}
        <u>{{tr}}CConsultation-histoire_maladie{{/tr}}</u> : {{mb_value object=$_suivi field=histoire_maladie}}
        <br />
      {{/if}}
      {{if $_suivi->examen}}<u>{{tr}}CConsultation-examen{{/tr}}</u> : {{mb_value object=$_suivi field=examen}}<br />{{/if}}
      {{if $_suivi->traitement}}<u>{{tr}}CConsultation-traitement{{/tr}}</u> : {{mb_value object=$_suivi field=traitement}}<br />{{/if}}
      {{if $_suivi->rques}}<u>{{tr}}CConsultation-rques{{/tr}}</u> : {{mb_value object=$_suivi field=rques}}<br />{{/if}}
      {{if $_suivi->conclusion}}<u>{{tr}}CConsultation-conclusion{{/tr}}</u> : {{mb_value object=$_suivi field=conclusion}}
        <br />
      {{/if}}
    {{/if}}
...
{{if $_suivi|@instanceof:'Ox\Mediboard\Soins\CRDVExterne'}}
  ...
  <td colspan="4" class="text">
    <span title="{{$_suivi->description}}">{{$_suivi->libelle}}</span> ({{mb_value object=$_suivi field=duree}} min - {{mb_value object=$_suivi field=statut}})<br />
    {{if $_suivi->commentaire}}
      <strong>{{mb_label object=$_suivi field=commentaire}}</strong>: {{mb_value object=$_suivi field=commentaire}} <br />
    {{/if}}
  </td>
  <td></td>
{{/if}}
      ...
      {{if $type_trans != "0"}}
        <td style="width: 18%; page-break-inside: avoid;">
          {{if is_array($_trans_by_type)}}
            ...
            {{if $_trans_by_type|@count > 1}}
              ...
              {{foreach from=$_trans_by_type item=_trans name=_trans}}
                ...
                {{/if}}
              {{/foreach}}
              <button type="button" class="merge notext" style="float: right;" onclick="Soins.mergeTrans('{{$transmissions_ids}}')"></button>
            {{/if}}
          ...
          {{if !$readonly && !$_trans_by_type|@count && (!$_suivi[0]->_ref_object|instanceof:'Ox\Mediboard\Prescription\CCategoryPrescription' || !$_suivi[0]->_ref_object->cible_importante)}}
            <button type="button" class="add notext transmission_add" style="float: right;" title="Ajouter une transmission"
      ...
      <td class="nowrap button">
        {{if $_suivi.data|@count <= 1 && $_suivi.action|@count <= 1 && $_suivi.result|@count <= 1}}
          ...
          {{if $_suivi[0]->_canEdit}}
            ...
            {{else}}
              <button type="button" class="edit notext" onclick="Soins.addTransmission('{{$_suivi[0]->sejour_id}}', null, {
