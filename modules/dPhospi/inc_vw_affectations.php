...
function loadSejourNonAffectes($where, $order = null, $praticien_id = null, $prestation_id = null) {
  ...
  foreach ($sejourNonAffectes as $sejour) {
    ...
    $sejour->_ref_patient->loadRefDossierMedical(false);
    $sejour->_ref_patient->updateBMRBHReStatus();
  ...
  if ($order === null && count($sejourNonAffectes)) {
    ...
    array_multisort(
      CMbArray::pluck($sejourNonAffectes, "_ref_praticien", "function_id"), SORT_ASC,
      CMbArray::pluck($sejourNonAffectes, "entree_prevue"), SORT_ASC,
      CMbArray::pluck($sejourNonAffectes, "_ref_patient", "nom"), SORT_ASC,
      CMbArray::pluck($sejourNonAffectes, "_ref_patient", "prenom"), SORT_ASC,
      $sejourNonAffectes,
      $keys
    );
...
function loadVueTempo(&$objects = array(), $suivi_affectation, $lits = array(), &$operations = array(), $date_min, $date_max, $period, $prestation_id, &$functions_filter = null, $filter_function = null, &$sejours_non_affectes = null) {
  ...
  foreach ($objects as $_object) {
    ...
    if ($prestation_id) {
      if ($pays == 2) {
        $sejour->_liaisons_sejour = implode("|", CMbArray::pluck($sejour->loadAllLiaisonsForDay($date_max, $prestation_id), "nom"));
        if ($_object->_class == "CAffectation") {
          $_object->_liaisons_sejour = $sejour->_liaisons_sejour;
        }
      }
    }
    ...
    foreach ($operations[$sejour->_id] as $_operation) {
      ...
      if (($_operation->_datetime_best > $date_max)) {
        ...
        $_operation->_width_uscpo[$_object->_id] = CMbDT::position(CMbDT::dateTime("+$fin_uscpo hours + $min_operation minutes", $_operation->_datetime_best), max($date_min, $_object->entree), $period) - $_operation->_fin_offset[$_object->_id];
      }
    }
