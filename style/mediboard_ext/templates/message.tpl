...
{{if $messagerie}}
  <script>
    Main.add(function () {
      App.initMessageBackgroundTask({{$messagerie.counters|@json}});
      {{if $porteDocuments}}
      App.initPorteDocBackgroundTask({{$porteDocuments.counters|@json}});
      {{/if}}
    });
  </script>
{{/if}}
...
{{if $remaining_days}}
  <div class="info" style="border-bottom: 1px solid #ccc; background-color: #f6f6f6; white-space: normal;">
    ...
    <a href="#1" class="button" onclick="popChgPwd()" title="">
      <img src="style/phenx/images/icons/passwd.png" alt="{{tr}}common-action-Change your password{{/tr}}">
