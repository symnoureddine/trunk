...
/**
 * Wrapper des chapitres (pages) de l'Install
 */
@Component
export default class Chapitre extends INVue{
  ...
  protected applyFilter(search: string, elements: object[], labelFields: string|string[]): object[] {
  ...
  protected extractData(collection: object[]): object[] {
    if (!collection) {
      return collection
    }
  ...
  public scroll(event: {target: any}, scrollTop?: number): void {
    ...
    let atTop = target.scrollTop < 10
    let toCompact = !atTop && (target.scrollHeight > (window.innerHeight - 70))
