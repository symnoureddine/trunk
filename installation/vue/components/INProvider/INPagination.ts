...
export default class INPagination {
  ...
  constructor(provider?: INProvider) {
    this.provider = provider ? provider : new INProvider()
    return this
  }
  ...
  private extractCursorFromLink(link: string): number {
    let limitPos = link.match(/[\?\&]limit=[0-9]*/g)
    let offsetPos = link.match(/[\?\&]offset=[0-9]*/g)
    if (!limitPos || !offsetPos) {
      return 0
    }
    let offset = offsetPos[0].substr(offsetPos[0].indexOf("=") + 1)
    let limit = limitPos[0].substr(limitPos[0].indexOf("=") + 1)
    if (offset === "0" || limit === "0") {
      return 1
    }
