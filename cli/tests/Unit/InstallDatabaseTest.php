 ...
 */
class InstallDatabaseTest extends UnitTestMediboard {

  const DATABASE_NAME = 'mediboard_phpunit_fresh_install';
  ...
  private function makeCommand(): Command {
    ...
    return $application->find('install:database');
  }
   ...
   * @group schedules
   */
  public function testExecuteWrongPassword() {
    $command        = $this->makeCommand();
    $command_tester = new CommandTester($command);
   ...
   * @group schedules
   */
  public function testExecuteBddAlreadyExists() {
    $command        = $this->makeCommand();
    $command_tester = new CommandTester($command);
   ...
   * @group schedules
   */
  public function testExecuteInvalidDatabase() {
    $inputs = [
      'no',
