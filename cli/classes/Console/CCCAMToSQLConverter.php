...
class CCCAMToSQLConverter extends Command {
   ...
   *  - Start est la valeur à laquelle on commence à découper la ligne (les caractères avant sont les données sur le type de ligne)
   ...
   *
   * Dans les tailles de champs si à la place d'une taille on a "skip|x" alors on ne traitera pas ce champs et on sautera x caractères
   *
   * Si un type utilise plusieurs tables (ou qu'un champ d'une ligne doit être importé dans une autre table), on ajoute une clé
   * {typerubrique} et une fonction pour gérer ce cas de figure.
   */
  static public $complex_types = array(
    // Tables principales
    // - Niveau ACTE
    "101"   => array(
      "table"     => "p_acte",
  ...
  // Dernier code "simple"
  const CODE_MAX_SIMPLE = '099';
  // Taille du champs "Type"
  const TYPE_LENGTH = 3;
  // Taille du champs "Rubrique"
  const RUBRIQUE_LENGTH = 2;
  ...
  protected function execute(InputInterface $input, OutputInterface $output) {
    ...
    if (!$first_line || substr($first_line, 0, self::TYPE_LENGTH) !== '000' || strpos(substr($first_line, 41, 6), 'CCAM') === false) {
  ...
  protected function sanitizeValues($values) {
    return trim(str_replace(array("\\", "'"), array("\\\\", "\'"), $values));
  }
   ...
   * @return void
   */
  protected function extractSimpleLines() {
    $old_type  = null;
    $type      = null;
    $to_insert = array();
    while ($this->line = $this->readLine()) {
      ...
      $values = $this->convertCode(
        $type, self::$simple_types[$type]['start'], self::$simple_types[$type]['occu'], $rubrique, $sequence, $long_field
      );
   ...
   * @return array
   */
  protected function convertCode($type, $start, $nb_occu, $rubrique = null, $sequence = null, $long_field = null) {
    ...
    for ($i = 0; $i < $nb_occu; $i++) {
      ...
      foreach (self::$simple_types[$type]['fields'] as $_field_name => $_size) {
        ...
        $line[$_field_name] = "'" . trim($this->sanitizeValues($content));
        if ($long_field === null || $long_field != $_field_name) {
          $line[$_field_name] .= "'";
        }
        $start += $_size;
      }
   ...
   * @return void
   */
  protected function writeInsert($type, $values) {
   ...
   * @return array
   */
  protected function getNextSequence($values, $type, $rubrique, $sequence, $long_field) {
    $this->file_pos = ftell($this->fp);
    while ($this->line = $this->readLine()) {
      ...
      if ($new_type != $type || $new_rubrique != $rubrique || $new_sequence == $sequence || $new_sequence == '01') {
        break;
      }
   ...
   */
  protected function extractComplexLines() {
    $i              = 0;
    $this->lines    = array();
    $this->file_pos = ftell($this->fp);
        ...
        switch ($type) {
          ...
          case '110':
            $date_effet_acte = $this->sanitizeValues(
              substr(
                $this->line, self::$complex_types[$type]['start'],
          ...
          case '201':
            if ($rubrique == '01') {
              ...
              $this->lines[$type][] = array('CODEACTE' => $code_acte, 'ACTIVITE' => $code_activite);
              break;
            }
          ...
          case '220':
            $this->extractCodeActivite220($code_acte, $code_activite, $date_effet_activite, $type, $rubrique);
            break;
          ...
          case '320':
            $this->extractCodePhase320($code_acte, $code_activite, $code_phase, $date_effet_phase, $type, $this->line);
            break;
   ...
   * @return array
   */
  protected function extractCode101() {
    $acte = array();
    do {
      $type     = substr($this->line, 0, self::TYPE_LENGTH);
      $rubrique = substr($this->line, self::TYPE_LENGTH, self::RUBRIQUE_LENGTH);
      ...
      if (!array_key_exists($type, self::$complex_types) || !array_key_exists($rubrique, self::$complex_types[$type]['rubriques'])) {
        $this->file_pos = ftell($this->fp);
        continue;
      }
   ...
   * @return void
   */
  protected function extractCode10151($code_acte) {
    if (!array_key_exists('10151', $this->lines)) {
      $this->lines['10151'] = array();
    }
   ...
   * @return void
   */
  protected function extractCode10152($code_acte) {
    if (!array_key_exists('10152', $this->lines)) {
      $this->lines['10152'] = array();
    }
   ...
   * @return void
   */
  protected function extractCode10180($code_acte) {
    if (!array_key_exists('10180', $this->lines)) {
      $this->lines['10180'] = array();
    }
   ...
   * @return void
   */
  protected function extractCodeActe($code_acte, $date_effet, $type, $rubrique) {
    $start = self::$complex_types[$type]['start'];
   ...
   * @return void
   */
  protected function extractCodeActe140($code_acte, $date_effet, $type, $rubrique) {
    if (!array_key_exists($type, $this->lines)) {
      $this->lines[$type] = array();
    }
   ...
   * @return void
   */
  protected function extractCodeTypeForfait($code_acte, $date_effet) {
    if (!array_key_exists('11001', $this->lines)) {
      $this->lines['11001'] = array();
    }
   ...
   * @return void
   */
  protected function extractCodeActivite($code_acte, $code_activite, $type, $rubrique) {
    $start = self::$complex_types[$type]['start'];
    for ($i = 0; $i < self::$complex_types[$type]['occu']; $i++) {
      $import_line = array(
        'CODEACTE' => $code_acte,
   ...
   * @return void
   */
  protected function extractCodeActivite20150($code_acte, $code_activite) {
    if (!array_key_exists('20150', $this->lines)) {
      $this->lines['20150'] = array();
    }
    ...
    while ($line = $this->readLine()) {
      ...
      $import_line['TEXTE'] .= $this->sanitizeValues(
        substr(
          $line, self::$complex_types['20150']['start'] + self::$complex_types['20150']['fields']['NUMERO'],
   ...
   * @return void
   */
  protected function extractCodeModifActivite($code_acte, $code_activite, $date_effet) {
    if (!array_key_exists('21001', $this->lines)) {
      $this->lines['21001'] = array();
    }
   ...
   * @return void
   */
  protected function extractCodeActivite220($code_acte, $code_activite, $date_effet, $type, $rubrique) {
    $start = self::$complex_types[$type]['start'];
    for ($i = 0; $i < self::$complex_types[$type]['occu']; $i++) {
      $occurence_size = 0;
      ...
      $start_regle = $occurence_size * self::$complex_types[$type]['occu'] + self::$complex_types[$type]['start'] + $i;
   ...
   * @return string
   */
  protected function extractPhase($code_acte, $code_activite, $type, $rubrique) {
    $start = self::$complex_types[$type]['start'];
   ...
   * @return void
   */
  protected function extractCodePhase30102($code_acte, $code_activite, $code_phase, $line) {
    $start = self::$complex_types['30102']['start'];
    if (!array_key_exists('30102', $this->lines)) {
      $this->lines['30102'] = array();
    }
   ...
   * @return void
   */
  protected function extractCodePhase310($code_acte, $code_activite, $code_phase, $type, $rubrique) {
    $start = self::$complex_types[$type]['start'];
    ...
    while ($line = $this->readLine()) {
      ...
      $this->extractCodePhase320($code_acte, $code_activite, $code_phase, $import_line['DATEEFFET'], $new_type, $line);
      $this->file_pos = ftell($this->fp);
    }
   ...
   * @return void
   */
  protected function extractCodePhase320($code_acte, $code_activite, $code_phase, $date_effet, $type, $line) {
    if (!array_key_exists($type, $this->lines)) {
      $this->lines[$type] = array();
    }
   ...
   * @return void
   */
  protected function extractCodePhase350($code_acte, $code_activite, $code_phase, $type, $rubrique) {
    $start = self::$complex_types[$type]['start'];
   ...
   * @return array
   */
  protected function getNextComplexSequence($fields, $type, $rubrique, $sequence, $long_libelle = null) {
    if (!$long_libelle) {
      return $fields;
    }
   ...
   * @return void
   */
  protected function writeComplexInsert() {
   ...
   * @return string
   */
  protected function createQueryHeader($headers) {
   ...
   * @return void
   */
  protected function storeToArchive() {
