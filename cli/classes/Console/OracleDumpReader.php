...
class OracleDumpReader extends MediboardCommand {
  ...
  protected function execute(InputInterface $input, OutputInterface $output) {
    ...
    elseif ($header === self::EXPDP_HEADER) {
      ...
      throw new Exception("Wrong file format, expected '".self::EXPDP_HEADER . "' or '". self::EXPDP_HEADER . "' in header");
    }
  ...
  function readTableInfo(&$previous_table, $table_name) {
    ...
    if ($table_def && $columns) {
      ...
      $this->tablesDefs[$table_name] = array(
        "position" => $this->pos(),
        "end"      => null,
        "columns"  => $table_def,
        "create"   => $create_table,
    ...
    for ($i = 0; $i < $count; $i++) {
      ...
      switch ($t) {
        case self::TYPE_NUMBER:
          $type = "NUMBER";
          $this->skip(1);
          $params = array(dechex($this->readUByte()), $this->readUByte());
          break;

        case self::TYPE_VARCHAR2:
          $type = "VARCHAR2";
          $this->skip(1);
          $params = array($this->readUByte());
          $this->skip(1);
