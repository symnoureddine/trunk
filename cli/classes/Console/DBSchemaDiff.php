...
class DBSchemaDiff extends MediboardCommand
{
    ...
    /** @var int */
    const RETURN_FAILED = 1;

    /** @var int */
    const NB_STEP_PER_LINE = 50;
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!empty($this->diff_table)) {
            $this->output->writeln('<error>There was ' . count($this->diff_table) . ' tables with differences:</error>');
    ...
    private function step(string $msg): void
    {
        ...
        if ($this->current_line <= self::NB_STEP_PER_LINE) {
            $this->current_line++;
            $this->output->write($msg);
        }

        if (($this->current_line === self::NB_STEP_PER_LINE) || ($this->count_echo === $datas_count)) {
            if ($this->current_line < self::NB_STEP_PER_LINE) {
                $this->output->write(str_repeat(' ', self::NB_STEP_PER_LINE - $this->current_line));
            }

            $this->current_line = 0;
            $percent            = round(($this->count_echo * 100) / $datas_count);

            $stat = ' ' . str_pad($this->count_echo, 4);
            $stat .= ' / ';
            $stat .= $datas_count . ' (' . str_pad($percent, 2) . '%)';
