...
EOT
  ...
  private function askQuestions() {
    ...
    if ($context === 'commit') {
      ...
      $hash           = $this->ask(new Question("Enter the revision number (default last commit) ?", $last_commit_id));
   ...
   *
   * @return string Command to display confirmation
   */
  private function buildCommand($tool, $context, $path, $hash, $options): string {
    ...
    switch ($context) {
      ...
      case self::CONTEXT_COMMIT:
        exec("git diff-tree --no-commit-id --name-only -r {$hash}", $paths);
        break;
    ...
    switch ($tool) {
      ...
      case self::TOOL_PHPCS:
        $command = "vendor/bin/phpcs -p {$paths_placeholder} --report=full --standard=dev/CodeSniffer/Standard/ruleset.xml";
        break;
      case self::TOOL_PHPCBF:
        $command = "vendor/bin/phpcbf -p {$paths_placeholder} -p --standard=dev/CodeSniffer/Standard/ruleset.xml";
        break;
      case self::TOOL_PHPCOMPAT:
        $command = "vendor/bin/phpcs -p {$paths_placeholder} --report=full --standard=dev/CodeSniffer/PHPC/ruleset.xml --extensions=php";
        break;
    ...
    return $this->command = str_replace($paths_placeholder, implode(' ', $paths), $command);
  }
